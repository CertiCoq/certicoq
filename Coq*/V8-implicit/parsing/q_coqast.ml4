...
(* $Id: q_coqast.ml4 10007 2007-07-16 09:18:44Z corbinea $ *)
...
let mlexpr_of_impl = function
  | Term.Impl -> <:expr< Term.Impl>>
...
let rec mlexpr_of_constr = function
  ...
  | Topconstr.CArrow (loc,imp,a,b) ->
  <:expr< Topconstr.CArrow $dloc$ $mlexpr_of_impl imp$
     $mlexpr_of_constr a$ $mlexpr_of_constr b$ >>
  | Topconstr.CProdN (loc,l,a) ->
      <:expr< Topconstr.CProdN $dloc$
        $mlexpr_of_list
        (mlexpr_of_pair
          (mlexpr_of_list
            (mlexpr_of_pair (mlexpr_of_pair (fun _ -> dloc) mlexpr_of_name)
              mlexpr_of_impl))
          mlexpr_of_constr) l$
        $mlexpr_of_constr a$ >>
  | Topconstr.CLambdaN (loc,l,a) ->
      <:expr< Topconstr.CLambdaN $dloc$
        $mlexpr_of_list
        (mlexpr_of_pair
          (mlexpr_of_list
            (mlexpr_of_pair (mlexpr_of_pair (fun _ -> dloc) mlexpr_of_name)
          ...
          mlexpr_of_constr) l$
        $mlexpr_of_constr a$ >>
  | Topconstr.CLetIn (loc,_,_,_,_) -> failwith "mlexpr_of_constr: TODO"
  | Topconstr.CAppExpl (loc,a,l) ->
      <:expr< Topconstr.CAppExpl $dloc$
        $mlexpr_of_pair
          (mlexpr_of_option mlexpr_of_int) mlexpr_of_reference a$
        $mlexpr_of_list (mlexpr_of_pair mlexpr_of_constr mlexpr_of_impl) l$ >>
  | Topconstr.CApp (loc,a,l) ->
      <:expr< Topconstr.CApp $dloc$
        $mlexpr_of_pair
          (mlexpr_of_option mlexpr_of_int) mlexpr_of_constr a$
        $mlexpr_of_list
        (mlexpr_of_pair (mlexpr_of_pair mlexpr_of_constr mlexpr_of_impl)
          (mlexpr_of_option (mlexpr_of_located mlexpr_of_explicitation))) l$ >>
...
let rec mlexpr_of_atomic_tactic = function
  ...
  | Tacexpr.TacCut(imp,c) ->
      <:expr< Tacexpr.TacCut $mlexpr_of_impl imp$ $mlexpr_of_constr c$ >>
  | Tacexpr.TacAssert (t,ipat,c) ->
      ...
      <:expr< Tacexpr.TacAssert $mlexpr_of_option mlexpr_of_tactic t$ $ipat$ 
	      $mlexpr_of_constr c$ >>
  | Tacexpr.TacGeneralize(imps,cl) ->
      <:expr< Tacexpr.TacGeneralize
        $mlexpr_of_list mlexpr_of_impl imps$
        $mlexpr_of_list mlexpr_of_constr cl$ >>
  | Tacexpr.TacGeneralizeDep(imp,c) ->
      <:expr< Tacexpr.TacGeneralizeDep
        $mlexpr_of_impl imp$ $mlexpr_of_constr c$ >>
  | Tacexpr.TacLetTac (na,imp,c,cl) ->
      let na = mlexpr_of_name na in
      let cl = mlexpr_of_clause_pattern cl in
      <:expr< Tacexpr.TacLetTac $na$ $mlexpr_of_impl imp$
                $mlexpr_of_constr c$ $cl$ >>
