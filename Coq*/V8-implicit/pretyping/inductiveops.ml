...
(* $Id: inductiveops.ml 9707 2007-03-15 16:36:15Z herbelin $ *)
...
type inductive_family = inductive * constr list * impl list

let make_ind_family (mis, params, imps) = (mis,params,imps)
let dest_ind_family (mis,params,imps) = (mis,params,imps)

let map_ind_family f (mis,params, imps) = (mis, List.map f params, imps)
...
type inductive_type = IndType of inductive_family * constr list * impl list

let make_ind_type (indf, realargs,imps) = IndType (indf,realargs,imps)
let dest_ind_type (IndType (indf,realargs,imps)) = (indf,realargs,imps)

let map_inductive_type f (IndType (indf, realargs,imps)) =
  IndType (map_ind_family f indf, List.map f realargs, imps)
...
let mkAppliedInd (IndType ((ind,params,pimps), realargs,rimps)) =
  applist (mkInd ind,params@realargs,pimps@rimps)
...
let get_constructor (ind,mib,mip,params) j =
  ...
  let (_,allargs,imps) = decompose_app ccl in
  let vargs = list_skipn (List.length params) allargs in
  let vimps = list_skipn (List.length params) imps in
  { cs_cstr = ith_constructor_of_inductive ind j;
    cs_params = params;
    cs_nargs = rel_context_length args;
    cs_args = args;
    cs_concl_realargs = Array.of_list vargs;
    cs_concl_imps = Array.of_list vimps }

let get_constructors env (ind,params,pimps) =
  let (mib,mip) = Inductive.lookup_mind_specif env ind in
  Array.init (Array.length mip.mind_consnames)
    (fun j -> get_constructor (ind,mib,mip,params) (j+1))

let rec instantiate args c = match kind_of_term c, args with
  | Prod (_,_,_,c), a::args -> instantiate args (subst1 a c)
  | LetIn (_,_,b,_,c), args -> instantiate args (subst1 b c)
  | _, [] -> c
...
let rec instantiate_context sign args =
  ...
  | (_,_,None,_)::sign, a::args -> aux (a::subst) (sign,args)
  | (_,_,Some b,_)::sign, args -> aux (substl subst b::subst) (sign,args)
  | [], [] -> subst
  | _ -> anomaly "Signature/instance mismatch in inductive family"
  in aux [] (List.rev sign,args)

let get_arity env (ind,params,pimps) =
  let (mib,mip) = Inductive.lookup_mind_specif env ind in
  let parsign =
...
let build_dependent_constructor cs =
  ...
  applist
    (mkConstruct cs.cs_cstr,
     List.map (lift cs.cs_nargs) cs.cs_params @ inst,
     List.map (fun _ -> Impl) cs.cs_params @ imps)

let build_dependent_inductive env ((ind, params,pimps) as indf) =
  let arsign,_ = get_arity env indf in
  let (inst,aimps) = extended_rel_list 0 arsign in
  let nrealargs = List.length arsign in
  applist 
    (mkInd ind,
     List.map (lift nrealargs) params @ inst,
     pimps @ aimps)
...
let make_arity_signature env dep indf =
  ...
  if dep then
    ...
    name_context env
      ((Anonymous,Expl,None,build_dependent_inductive env indf)::arsign)
...
let build_branch_type env dep p cs =
  let base =
    appvect (lift cs.cs_nargs p, cs.cs_concl_realargs,cs.cs_concl_imps) in
  if dep then
    it_mkProd_or_LetIn_name env
      (applist (base,[build_dependent_constructor cs],[Expl]))
      cs.cs_args
  else
...
let extract_mrectype t =
  let (t, l,imps) = decompose_app t in
  match kind_of_term t with
    | Ind ind -> (ind, l,imps)
    | _ -> raise Not_found

let find_mrectype env sigma c =
  let (t, l,imps) = decompose_app (whd_betadeltaiota env sigma c) in
  match kind_of_term t with
    | Ind ind -> (ind, l,imps)
    | _ -> raise Not_found
...
let find_rectype env sigma c =
  ...
  match kind_of_term t with
    | Ind ind ->
        ...
        let (pimps,rimps) = list_chop mib.mind_nparams imps in
        IndType((ind, par,pimps),rargs,rimps)
    | _ -> raise Not_found
...
let is_predicate_explicitly_dep env pred arsign = 
  let rec srec env pval arsign =
    ...
    match kind_of_term pv', arsign with
      | Lambda (na,imp,t,b), (_,_,None,_)::arsign ->
	  srec (push_rel_assum (na,imp,t) env) b arsign
      | Lambda (na,_,_,_), _ ->
...
let type_case_branches_with_names env indspec pj c =
  let (ind,args,imps) = indspec in
  let (lbrty,conclty,_) = Inductive.type_case_branches env indspec pj c in
  let (mib,mip) = Inductive.lookup_mind_specif env ind in
  let params = list_firstn mib.mind_nparams args in
  let pimps = list_firstn mib.mind_nparams imps in
  if is_elim_predicate_explicitly_dependent
    env pj.uj_val (ind,params,pimps) then
    (set_pattern_names env ind lbrty, conclty)
  else (lbrty, conclty)

(* Type of Case predicates *)
let arity_of_case_predicate env (ind,params,pimps) dep k =
  let arsign,_ = get_arity env (ind,params,pimps) in
  let mind = build_dependent_inductive env (ind,params,pimps) in
  let concl = if dep then mkArrow Expl mind (mkSort k) else mkSort k in
...
let control_only_guard env = 
  let rec control_rec c = match kind_of_term c with
    ...
    | App (c,cl,_)         -> control_rec c; Array.iter control_rec cl
    | Cast (c1,_, c2)  -> control_rec c1; control_rec c2
    | Prod (_,_,c1,c2)     -> control_rec c1; control_rec c2
    | Lambda (_,_,c1,c2)   -> control_rec c1; control_rec c2
    | LetIn (_,_,c1,c2,c3) -> control_rec c1; control_rec c2; control_rec c3
  in 
  control_rec 
