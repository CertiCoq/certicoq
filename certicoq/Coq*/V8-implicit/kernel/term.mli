...
(*i $Id: term.mli 9851 2007-05-23 10:29:01Z herbelin $ i*)
...
val impl_merge : impl list -> impl
val impl_mergev : impl array -> impl
...
val mkProd : name * impl * types * types -> types
val mkNamedProd : identifier -> impl -> types -> types -> types
...
val mkArrow : impl -> types -> types -> constr

(* Constructs the abstraction $[x:t_1]t_2$ *)
val mkLambda : name * impl * types * constr -> constr
val mkNamedLambda : identifier -> impl -> types -> constr -> constr

(* Constructs the product [let x = t1 : t2 in t3] *)
val mkLetIn : name * impl * constr * types * constr -> constr
val mkNamedLetIn : identifier -> impl -> constr -> types -> constr -> constr

(* [mkApp (f,[| t_1; ...; t_n |]] constructs the application
   $(f~t_1~\dots~t_n)$. *)
val mkApp : constr * constr array * impl array -> constr
...
val destProd : types -> name * impl * types * types

(* Destructs the abstraction $[x:t_1]t_2$ *)
val destLambda : constr -> name * impl * types * constr

(* Destructs the let $[x:=b:t_1]t_2$ *)
val destLetIn : constr -> name * impl * constr * types * constr

(* Destructs an application *)
val destApp : constr -> constr * constr array * impl array

(* Obsolete synonym of destApp *)
val destApplication : constr -> constr * constr array * impl array

(* Decompose any term as an applicative term; the list of args can be empty *)
val decompose_app : constr -> constr * constr list * impargs
...
val applist : constr * constr list * impargs -> constr
val applistc : constr -> constr list -> impargs -> constr
val appvect : constr * constr array * impl array -> constr
val appvectc : constr -> constr array -> impl array -> constr

(* [prodn n l b] = $(x_1:T_1)..(x_n:T_n)b$ 
   where $l = [(x_n,T_n);\dots;(x_1,T_1);Gamma]$ *)
val prodn : int -> (name * impl * constr) list -> constr -> constr

(* [compose_prod l b] = $(x_1:T_1)..(x_n:T_n)b$
   where $l = [(x_n,T_n);\dots;(x_1,T_1)]$.
   Inverse of [decompose_prod]. *)
val compose_prod : (name * impl * constr) list -> constr -> constr

(* [lamn n l b] = $[x_1:T_1]..[x_n:T_n]b$
   where $l = [(x_n,T_n);\dots;(x_1,T_1);Gamma]$ *)
val lamn : int -> (name * impl * constr) list -> constr -> constr

(* [compose_lam l b] = $[x_1:T_1]..[x_n:T_n]b$
   where $l = [(x_n,T_n);\dots;(x_1,T_1)]$.
   Inverse of [decompose_lam] *)
val compose_lam : (name * impl * constr) list -> constr -> constr
...
val decompose_prod : constr -> ((name*impl*constr) list) * constr

(* Transforms a lambda term $[x_1:T_1]..[x_n:T_n]T$ into the pair
   $([(x_n,T_n);...;(x_1,T_1)],T)$, where $T$ is not a lambda. *)
val decompose_lam : constr -> ((name*impl*constr) list) * constr

(* Given a positive integer n, transforms a product term 
   $(x_1:T_1)..(x_n:T_n)T$
   into the pair $([(xn,Tn);...;(x1,T1)],T)$. *)
val decompose_prod_n :
  int -> constr -> ((name * impl * constr) list) * constr
...
val kind_of_econstr : econstr -> econstr extracted_term
val fold_econstr : econstr extracted_term -> econstr

val expl_filter : 'a array -> impl array -> 'a array
val extract_closed : constr -> bool
val extract_closed_gen :
  (int -> bool) -> (identifier -> bool) -> constr -> bool

val extract : constr -> econstr

(* Syntactic check of terms reducible to the identity *)
val is_coercion : econstr -> bool
