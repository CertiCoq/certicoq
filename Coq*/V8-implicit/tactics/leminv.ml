...
(* $Id: leminv.ml 9850 2007-05-22 21:37:55Z herbelin $ *)
...
let rec add_prods_sign env sigma t =
  match kind_of_term (whd_betadeltaiota env sigma t) with
	...
	let b'= subst1 (mkVar id) b in
        add_prods_sign (push_named (id,imp,None,c1) env) sigma b'
    | LetIn (na,imp,c1,t1,b) ->
	let id = id_of_name_using_hdchar env t na in
	let b'= subst1 (mkVar id) b in
        add_prods_sign (push_named (id,imp,Some c1,t1) env) sigma b'
    | _ -> (env,t)
...
let compute_first_inversion_scheme env sigma ind sort dep_option =
  let indf,realargs,raimps = dest_ind_type ind in
  ...
  let pty,goal =
    if dep_option  then
      ...
      let goal = 
	mkProd
	  (Anonymous, Expl, mkAppliedInd ind,
           applist(mkVar p,realargs@[mkRel 1],raimps@[Expl]))
      in
      pty,goal
    else
      let i = mkAppliedInd ind in
      let ivars = global_vars env i in
      let revargs,revimps,ownsign =
	fold_named_context
	  (fun env (id,imp,_,_ as d) (revargs,imps,hyps) ->
             if List.mem id ivars then 
	       ((mkVar id)::revargs,imp::imps,add_named_decl d hyps)
	     else 
	       (revargs,imps,hyps))
          env ~init:([],[],[]) 
      in
      ...
      let goal =
        mkArrow Expl i
          (applist(mkVar p, List.rev revargs, List.rev revimps)) in
      (pty,goal)
  in
  let npty = nf_betadeltaiota env sigma pty in
  let extenv = push_named (p,Expl,None,npty) env in
  extenv, goal
  ...
  *)
  ...
  let (_,ownSign,mvb) =
    List.fold_left
	 ...
	 (h::avoid, add_named_decl (h,Expl,None,mvty) sign, (mv,mkVar h)::mvb))
      (ids_of_context invEnv, ownSign, [])
      meta_types 
  in
  let invProof = 
    it_mkNamedLambda_or_LetIn (local_strong (whd_meta mvb) pfterm)
      ownSign
  in
  invProof
