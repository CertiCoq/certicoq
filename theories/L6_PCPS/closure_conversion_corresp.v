...
Section CC_correct.
  ...
  Lemma get_var_project_var_sound Scope Funs Γ FVs FVmap S x :
    ...
    {{ fun _ t s' =>
         exists C S', 
           fresh S' (fst s') /\
           let '(x', _, f) := t in
           project_var Scope Funs (subst FVmap) Γ FVs S x x' C S' /\
           is_exp_ctx f C }}.
  Proof.
  ...
  Lemma get_vars_project_vars_sound Scope Funs Γ FVs FVmap S xs :
    ...
    {{ fun _ t s' =>
         exists C S', 
           fresh S' (fst s') /\
           let '(xs', f) := t in
           project_vars Scope Funs (subst FVmap) Γ FVs S xs xs' C S' /\
           is_exp_ctx f C }}.
  Proof.
  ...
  Lemma subst_BoundVar_f_eq_subdomain FVmap x t S :
    f_eq_subdomain S ((subst FVmap) {x ~> x}) (subst (M.set x (BoundVar t) FVmap)).
  Proof.  
  ...
  Lemma make_env_spec fv FVmap_o Γ_n Γ_o Scope Funs FVs S :
    ...
    {{ fun s t  s' =>
         ...
         exists C S' FVs' tau t,
           fresh S' (fst s') /\
           project_vars Scope Funs (subst FVmap_o) Γ_o FVs S (PS.elements fv) FVs' C S' /\
  ...
  Lemma make_full_closure_spec B FVmap_n FVmap_o Γ Scope Funs FVs FVs' typ S S1 S2 S1' S2' :
    ...
    {{ fun s t s' =>
         ...
         exists C S',
           fresh S' (fst s') /\
           make_closures B S Γ C (subst FVmap_n') S' /\
           is_exp_ctx f C /\
  ...
  Proof.
    revert FVmap_n FVmap_o Γ Scope Funs FVs FVs' typ S S1 S2 S1' S2'.
    induction B;
      intros FVmap_n FVmap_o Γ Scope Funs FVs FVs' typ S
  ...
  Instance project_vars_Proper Scope Funs :
    Proper
      (f_eq_subdomain (Setminus _ Funs Scope) ==> eq ==> eq ==> eq ==> eq ==> eq ==> eq ==> eq ==> iff)
      (project_vars Scope Funs).
  Proof. 
  ...
  Lemma Closure_conversion_f_eq_subdomain_mut :
    (forall e Scope Funs σ σ' FVs Γ e' C
       (Hcc : Closure_conversion Scope Funs σ Γ FVs e e' C)
       (Hfeq : f_eq_subdomain (Setminus _ Funs Scope) σ σ'),
       Closure_conversion Scope Funs σ' Γ FVs e e' C) /\
    (forall B Funs σ σ' FVs B'
       (Hinc : Included _ (name_in_fundefs B) Funs)
       (Hcc : Closure_conversion_fundefs Funs σ FVs B B')
       (Hfeq : f_eq_subdomain Funs σ σ'),
       Closure_conversion_fundefs Funs σ' FVs B B').
  Proof. 
    ...
    - econstructor; try reflexivity. rewrite <- image_f_eq_subdomain; [| eassumption ].
      ...
      eapply project_var_Proper; [ symmetry; eassumption | | | | | | | | ];
      try reflexivity. eassumption.
      inv H12. inv H4. simpl in H1; subst. destruct H2 as [C' [e' [Heq Hcc]]].
      constructor. now split; eauto.
      assert (Hcc' : Closure_conversion Scope Funs σ' Γ FVs (Ecase v l)  (Ecase x' l') C).
  ...
  Lemma subst_add_params_f_eq_subdomain S l ts FVmap :
    ...
    f_eq_subdomain S
     (subst (add_params l ts FVmap)) (subst FVmap).
  Proof.
    intros Hd.
    revert ts; induction l; intros ts.
    - simpl. reflexivity.
    - simpl. destruct ts.
  ...
  Lemma exp_closure_conv_Closure_conv_sound :
    (forall e Scope Funs Γ FVs FVmap S
       ...
       {{ fun s ef s' =>
            ...
            exists C, is_exp_ctx (snd ef) C /\
                 Closure_conversion Scope Funs (subst FVmap) Γ FVs e (fst ef) C /\
                 fresh S (fst s')
       }}) /\
    (forall B FVmap Funs FVs S
       ...
       {{ fun s B' s' =>
            fresh S (fst s) ->       
            Closure_conversion_fundefs Funs (subst FVmap) FVs B B' /\
            fresh S (fst s')
       }}).
  Proof.
    ...
    - eapply pre_post_mp_r. eapply bind_triple; [ now eapply get_name_fresh |].
      ...
      + eapply make_env_spec. eapply binding_in_map_antimon; [| eassumption ].
        ...
        * eapply binding_not_in_map_antimon; [| eassumption ].
          ...
          now apply Included_refl.
        * eassumption.
      + intros [[tau FVmap'] f] _. eapply pre_existential; intros C1.
        eapply pre_existential; intros S2. eapply pre_existential; intros FVs'.
        eapply pre_existential; intros tau'. eapply pre_existential; intros t.
        ...
        * intros [e' f''] s3. eapply pre_post_mp_l.
          ...
          intros x Hin. eapply Hin. zify. omega. eapply Disjoint_Included_r; [| eassumption ]. 
    ...
    - eapply pre_post_mp_r. eapply bind_triple.
      ...
      + intros [[x tau] f] s1. eapply pre_existential. intros C1.
        ...
        * intros [xs f'] s2. eapply pre_existential. intros C2.
          ...
          intros tinf s4. eapply return_triple.
