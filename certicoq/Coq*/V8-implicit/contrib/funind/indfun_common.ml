...
let chop_rlambda_n  =
  let rec chop_lambda_n acc n rt =
	...
	match rt with
	  | Rawterm.RLambda(_,name,imp,t,b) ->
              chop_lambda_n ((name,imp,t,false)::acc) (n-1) b
	  | Rawterm.RLetIn(_,name,imp,v,b) ->
              chop_lambda_n ((name,imp,v,true)::acc) (n-1) b
	  | _ -> 
...
let chop_rprod_n  =
  let rec chop_prod_n acc n rt =
	...
	match rt with
	  | Rawterm.RProd(_,name,imp,t,b) ->
              chop_prod_n ((name,imp,t)::acc) (n-1) b
