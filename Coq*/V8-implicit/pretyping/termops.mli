...
(*i $Id: termops.mli 9917 2007-06-30 09:55:33Z herbelin $ i*)
...
(* iterators on terms *)
val prod_it : init:types -> (name * impl * types) list -> types
val lam_it : init:constr -> (name * impl * types) list -> constr
val rel_vect : int -> int -> constr array
val rel_list : int -> int -> constr list
val extended_rel_list : int -> rel_context -> constr list * impl list
val extended_rel_vect : int -> rel_context -> constr array * impl array
val push_rel_assum : name * impl * types -> env -> env
val push_rels_assum : (name * impl * types) list -> env -> env
...
val it_named_context_quantifier :
  (named_declaration -> 'a -> 'a) -> init:'a ->
    named_context -> 'a
val it_mkNamedProd_or_LetIn   :
  init:types -> named_context -> types
val it_mkNamedLambda_or_LetIn :
  init:constr -> named_context -> constr
...
val occur_var_in_decl :
  env ->
  identifier -> 'a * impl * constr option * types -> bool
...
val mkProd_name : env -> name * impl * types * types -> types
val mkLambda_name : env -> name * impl * types * constr -> constr

(* Deprecated synonyms of [mkProd_name] and [mkLambda_name] *)
val prod_name : env -> name * impl * types * types -> types
val lambda_name : env -> name * impl * types * constr -> constr

val prod_create : env -> impl * types * types -> constr
val lambda_create : env -> impl * types * constr -> constr
...
val mkProd_or_LetIn_name :
  env -> types -> rel_declaration -> types
val mkLambda_or_LetIn_name :
  env -> constr -> rel_declaration -> constr
val it_mkProd_or_LetIn_name   :
  env -> types -> rel_context -> types
val it_mkLambda_or_LetIn_name :
  env -> constr -> rel_context -> constr
...
val assums_of_rel_context : rel_context -> (name * impl * constr) list
