...
let mkRApp(rt,rtl,imps) = RApp(dummy_loc,rt,rtl,imps)
let mkRLambda(n,imp,t,b) = RLambda(dummy_loc,n,imp,t,b)
let mkRProd(n,imp,t,b) = RProd(dummy_loc,n,imp,t,b)
let mkRLetIn(n,imp,t,b) = RLetIn(dummy_loc,n,imp,t,b)
...
let raw_decompose_prod = 
  ...
  | RProd(_,n,imp,t,b) -> 
      raw_decompose_prod ((n,imp,t)::args) b 
  | rt -> args,rt
  in
  raw_decompose_prod []
...
let raw_decompose_prod_or_letin = 
  let rec raw_decompose_prod args = function 
  | RProd(_,n,imp,t,b) -> 
      raw_decompose_prod ((n,imp,None,Some t)::args) b 
  | RLetIn(_,n,imp,t,b) -> 
      raw_decompose_prod ((n,imp,Some t,None)::args) b 
  | rt -> args,rt
  in
  raw_decompose_prod []

let raw_compose_prod = 
  List.fold_left (fun b (n,imp,t) -> mkRProd(n,imp,t,b))

let raw_compose_prod_or_letin = 
  List.fold_left (
      fun concl decl -> 
	match decl with 
	  | (n,imp,None,Some t) -> mkRProd(n,imp,t,concl)
	  | (n,imp,Some bdy,None) -> mkRLetIn(n,imp,bdy,concl)
	  | _ -> assert false)
...
let raw_decompose_prod_n n = 
  let rec raw_decompose_prod i args c = 
    if i<=0 then args,c
      ...
      match c with
	| RProd(_,n,imp,t,b) -> 
	    raw_decompose_prod (i-1) ((n,imp,t)::args) b 
	| rt -> args,rt
  in
...
let raw_decompose_prod_or_letin_n n = 
  let rec raw_decompose_prod i args c = 
    if i<=0 then args,c
      ...
      match c with
	| RProd(_,n,imp,t,b) -> 
	    raw_decompose_prod (i-1) ((n,imp,None,Some t)::args) b 
	| RLetIn(_,n,imp,t,b) -> 
	    raw_decompose_prod (i-1) ((n,imp,Some t,None)::args) b 
	| rt -> args,rt
  in
  raw_decompose_prod n []
...
let raw_decompose_app = 
  let rec decompose_rapp acc acci rt =
(*     msgnl (str "raw_decompose_app on : "++ Printer.pr_rawconstr rt); *)
    match rt with 
    | RApp(_,rt,rtl,imps) -> 
	decompose_rapp 
          (List.fold_left (fun y x -> x::y) acc rtl)
          (List.fold_left (fun y x -> x::y) acci imps) rt
    | rt -> rt,List.rev acc, List.rev acci
  in
  decompose_rapp [] []
...
let raw_make_eq ?(typ= mkRHole ()) t1 t2  = 
  mkRApp(mkRRef (Lazy.force Coqlib.coq_eq_ref),
         [typ;t2;t1],[Term.Expl;Term.Expl;Term.Expl])

(* [raw_make_neq t1 t2] build the rawconstr corresponding to [t1 <> t2] *) 
let raw_make_neq t1 t2 = 
  mkRApp(mkRRef (Lazy.force Coqlib.coq_not_ref),
         [raw_make_eq t1 t2],[Term.Expl])

(* [raw_make_or P1 P2] build the rawconstr corresponding to [P1 \/ P2] *) 
let raw_make_or t1 t2 =
  mkRApp (mkRRef(Lazy.force Coqlib.coq_or_ref),[t1;t2],[Term.Expl;Term.Expl])
...
let change_vars = 
  let rec change_vars mapping rt = 
    match rt with 
      ...
      | REvar _ -> rt 
      | RPatVar _ -> rt 
      | RApp(loc,rt',rtl,imps) -> 
	  RApp(loc,
	       change_vars mapping rt',
	       List.map (change_vars mapping) rtl,
	       imps)
      | RLambda(loc,name,imp,t,b) -> 
	  RLambda(loc,
		  name,imp,
		  change_vars mapping t,
		  change_vars (remove_name_from_mapping mapping name) b
		 )
      | RProd(loc,name,imp,t,b) -> 
	  RProd(loc,
		  name,imp,
		  change_vars mapping t,
		  change_vars (remove_name_from_mapping mapping name) b
		 )
      | RLetIn(loc,name,imp,def,b) -> 
	  RLetIn(loc,
		 name,imp,
	  ...
	  let new_mapping =
            List.fold_left
              (fun m (na,_) -> remove_name_from_mapping m na)
              mapping nal in 
	  RLetTuple(loc,
		    nal,
...
let rec alpha_pat excluded pat = 
  match pat with 
    | PatVar(loc,Anonymous,imp) -> 
	let new_id = Indfun_common.fresh_id excluded "_x" in 
	PatVar(loc,Name new_id,imp),(new_id::excluded),Idmap.empty
    | PatVar(loc,Name id,imp) -> 
	if List.mem id excluded 
	then 
	  let new_id = Nameops.next_ident_away id excluded in 
	  PatVar(loc,Name new_id,imp),(new_id::excluded),
...
let rec alpha_rt excluded rt = 
  let new_rt = 
    match rt with   
      ...
      | RLambda(loc,Anonymous,imp,t,b) -> 
	  let new_id = Nameops.next_ident_away (id_of_string "_x") excluded in 
	  let new_excluded = new_id :: excluded in 
	  let new_t = alpha_rt new_excluded t in 
	  let new_b = alpha_rt new_excluded b in 
	  RLambda(loc,Name new_id,imp,new_t,new_b)
      | RProd(loc,Anonymous,imp,t,b) -> 
	let new_t = alpha_rt excluded t in 
	let new_b = alpha_rt excluded b in 
	RProd(loc,Anonymous,imp,new_t,new_b)
    | RLetIn(loc,Anonymous,imp,t,b) -> 
	let new_t = alpha_rt excluded t in 
	let new_b = alpha_rt excluded b in 
	RLetIn(loc,Anonymous,imp,new_t,new_b)
    | RLambda(loc,Name id,imp,t,b) -> 
	let new_id = Nameops.next_ident_away id excluded in 
	let t,b = 
	  if new_id = id 
	  then t,b
	...
	let new_t = alpha_rt new_excluded t in 
	let new_b = alpha_rt new_excluded b in 
	RProd(loc,Name new_id,imp,new_t,new_b)
    | RLetIn(loc,Name id,imp,t,b) -> 
	let new_id = Nameops.next_ident_away id excluded in 
	let t,b = 
	  if new_id = id 
	  then t,b
	...
	let new_t = alpha_rt new_excluded t in 
	let new_b = alpha_rt new_excluded b in 
	RLetIn(loc,Name new_id,imp,new_t,new_b)


    | RLetTuple(loc,nal,(na,rto),t,b) -> 
	let rev_new_nal,new_excluded,mapping = 
	  List.fold_left 
	    (fun (nal,excluded,mapping) (na,imp) -> 
	       match na with 
		 | Anonymous -> ((na,imp)::nal,excluded,mapping)
		 | Name id ->  
		     let new_id = Nameops.next_ident_away id excluded in 
		     if new_id = id 
		     then 
		       (na,imp)::nal,id::excluded,mapping 
		     else 
		       (Name new_id,imp)::nal,id::excluded,
                       (Idmap.add id new_id mapping)
	    )
	    ([],excluded,Idmap.empty)
	    nal
	in
...
let is_free_in id =
  let rec is_free_in = function
    ...
    | REvar _ -> false
    | RPatVar _ -> false
    | RApp(_,rt,rtl,_) -> List.exists is_free_in (rt::rtl)
    | RLambda(_,n,_,t,b) | RProd(_,n,_,t,b) | RLetIn(_,n,_,t,b) ->
	let check_in_b =
	  match n with
	...
	let check_in_nal = 
	  not (List.exists
                (function (Name id',_) -> id'= id | _ -> false) nal) 
	in 
	...
	List.map pattern_to_term patternl
      ...
      mkRApp(mkRRef(Libnames.ConstructRef constr),
	     implicit_args@patl_as_term,
             List.map(fun _->Term.Impl)implicit_args@
             List.map(fun _->Term.Expl)patl_as_term
	    )
...
let replace_var_by_term x_id term = 
  let rec replace_var_by_pattern rt = 
    match rt with 
      ...
      | RLetIn(_,Name id,_,_,_) when id_ord id x_id == 0 -> rt
      | RLetIn(loc,name,imp,def,b) -> 
	  RLetIn(loc,
		 name,imp,
	  ...
	  when List.exists (function (Name id,_) -> id = x_id | _ -> false) nal  ->  
	  rt
...
let ids_of_pat = 
  let rec ids_of_pat ids = function 
    | PatVar(_,Anonymous,_) -> ids 
    | PatVar(_,Name id,_) -> Idset.add id ids 
...
let ids_of_rawterm c = 
  let rec ids_of_rawterm acc c = 
    ...
    match c with
      ...
      | RLambda (loc,na,_,ty,c) -> idof na :: ids_of_rawterm [] ty @ ids_of_rawterm [] c @ acc
      | RProd (loc,na,_,ty,c) -> idof na :: ids_of_rawterm [] ty @ ids_of_rawterm [] c @ acc
      | RLetIn (loc,na,_,b,c) -> idof na :: ids_of_rawterm [] b @ ids_of_rawterm [] c @ acc
      ...
      | RLetTuple (_,nal,(na,po),b,c) -> 
          List.map (fun (na,_) -> idof na) nal @
          ids_of_rawterm [] b @ ids_of_rawterm [] c @ acc
...
let zeta_normalize = 
  let rec zeta_normalize_term rt = 
    match rt with 
      ...
      | RLetIn(_,Name id,imp,def,b) -> 
	  zeta_normalize_term (replace_var_by_term id def b)
      | RLetIn(loc,Anonymous,imp,def,b) -> zeta_normalize_term b
      | RLetTuple(loc,nal,(na,rto),def,b) -> 
	  RLetTuple(loc,
		    nal,
...
let expand_as = 
  ...
  let rec expand_as map rt = 
    match rt with 
      ...
      | RApp(loc,f,args,imps) ->
          RApp(loc,expand_as map f,List.map (expand_as map) args,imps)
      | RLambda(loc,na,imp,t,b) ->
          RLambda(loc,na,imp,expand_as map t, expand_as map b)
      | RProd(loc,na,imp,t,b) ->
          RProd(loc,na,imp,expand_as map t, expand_as map b)
      | RLetIn(loc,na,imp,v,b) ->
          RLetIn(loc,na, imp, expand_as map v,expand_as map b)
      ...
      | RRec _ ->  error "Not handled RRec"
      | RDynamic _ -> error "Not handled RDynamic"
      | RCast(loc,b,CastConv(kind,t)) ->
          RCast(loc,expand_as map b,CastConv(kind,expand_as map t))
      | RCast(loc,b,CastCoerce) -> RCast(loc,expand_as map b,CastCoerce)
      | RCases(loc,po,el,brl) ->
	  RCases(loc, option_map (expand_as map) po,
