...
(* $Id: g_vernac.ml4 10007 2007-07-16 09:18:44Z corbinea $ *)
...
*)
  ...
  rec_definition:
	...
	":="; def = lconstr; ntn = decl_notation ->
          let names =
            List.map (fun ((_,na),_) -> na) (names_of_local_assums bl) in
          let ni =
...
*)
  (* ... with coercions *)
  record_field:
    [ [ id = name -> (false,AssumExpr(id,Expl,CHole loc))
      | id = name; oc = of_type_with_opt_coercion; t = lconstr ->
         (oc,AssumExpr (id,Expl,t))
      | "["; id = name; "]" -> (false,AssumExpr(id,Impl,CHole loc))
      | "["; id = name; oc = of_type_with_opt_coercion; t = lconstr; "]" ->
         (oc,AssumExpr (id,Impl,t))
      | id = name; oc = of_type_with_opt_coercion;
             t = lconstr; ":="; b = lconstr -> (oc,DefExpr (id,Expl,b,Some t))
      | id = name; ":="; b = lconstr ->
         (match b with
             CCast(_,b,Rawterm.CastConv (_,t)) ->
               (false,DefExpr(id,Expl,b,Some t))
           | _ -> (false,DefExpr(id,Expl,b,None)))
      | "["; id = name; oc = of_type_with_opt_coercion;
             t = lconstr; ":="; b = lconstr; "]" ->
               (oc,DefExpr (id,Impl,b,Some t))
      | "["; id = name; ":="; b = lconstr; "]" ->
         (match b with
             CCast(_,b,Rawterm.CastConv(_,t)) ->
               (false,DefExpr(id,Impl,b,Some t))
           | _ -> (false,DefExpr(id,Impl,b,None))) ] ]
  ;
  assum_list:
    [ [ bl = LIST1 assum_coe -> bl
      | b = simple_assum_coe -> [b Expl] ] ]
  ;
  assum_coe:
    [ [ "("; a = simple_assum_coe; ")" -> a Expl
      | "["; a = simple_assum_coe; "]" -> a Impl ] ]
  ;
  simple_assum_coe:
    [ [ idl = LIST1 identref; oc = of_type_with_opt_coercion; c = lconstr -> 
        fun imp -> (oc,((idl,c),imp)) ] ]
  ;
  constructor:
...
GEXTEND Gram
  ...
  printable:
    [ [ IDENT "Term"; qid = global -> PrintName qid
      | IDENT "Extraction"; c = constr -> PrintExtr c
