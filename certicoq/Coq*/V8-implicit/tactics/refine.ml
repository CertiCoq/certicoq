...
let replace_by_meta env sigma = function
  ...
  | (TH (c,mm,_)) as th ->
	...
	| Lambda (Name id,imp,c1,c2) when isCast c2 ->
	    let _,_,t = destCast c2 in mkNamedProd id imp c1 t
	| Lambda (Anonymous,imp,c1,c2) when isCast c2 ->
	    let _,_,t = destCast c2 in mkArrow imp c1 t
	| _ -> (* (App _ | Case _) -> *)
...
let rec compute_metamap env sigma c = match kind_of_term c with
  ...
  | Lambda (name,imp,c1,c2) ->
      ...
      let env' = push_named (v,imp,None,c1) env in
	...
	| th ->
	    ...
	    TH (mkLambda (Name v,imp,c1,m), mm, sgp)
      end

  | LetIn (name, imp, c1, t1, c2) ->
      let v = fresh env name in
      let th1 = compute_metamap env sigma c1 in
      let env' = push_named (v,imp,Some c1,t1) env in
	...
	| TH (c1,mm1,sgp1), TH (c2,mm2,sgp2) ->
	    ...
	    TH (mkNamedLetIn v imp m1 t1 m2, mm1@mm2, sgp1@sgp2)
      end
...
let rec tcc_aux subst (TH (c,mm,sgp) as _th) gl =
  ...
  match (kind_of_term c,sgp) with
    ...
    | LetIn (Name id,imp,c1,t1,c2), _ when not (isMeta (strip_outer_cast c1)) ->
	let c = pf_concl gl in
	let newc = mkNamedLetIn id imp c1 t1 c in
	tclTHEN 
    ...
    | LetIn (Name id,imp,c1,t1,c2), _ ->
	tclTHENS
          (assert_tac true (Name id) imp t1) 
