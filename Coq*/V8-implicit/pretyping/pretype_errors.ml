...
(* $Id: pretype_errors.ml 9668 2007-02-21 18:12:24Z herbelin $ *)
...
let env_ise sigma env =
  ...
  Sign.fold_rel_context
    (fun (na,imp,b,ty) e ->
      push_rel
        (na, imp, option_map (nf_evar sigma) b, nf_evar sigma ty)
        e)
    ctxt
    ~init:env0
...
let contract env lc =
  let l = ref [] in
  let contract_context (na,imp,c,t) env =
    match c with
      | Some c' when isRel c' ->
	  l := (substl !l c') :: !l;
	  env
