...
and fterm =
  ...
  | FApp of fconstr * fconstr array * impl array
  | FFix of fixpoint * fconstr subs
  | FCoFix of cofixpoint * fconstr subs
  | FCases of case_info * fconstr * fconstr * fconstr array
  | FLambda of int * (name * impl * constr) list * constr * fconstr subs
  | FProd of name * impl * fconstr * fconstr
  | FLetIn of name * impl * fconstr * fconstr * constr * fconstr subs
...
type stack_member =
  | Zapp of fconstr array * impl array
...
let empty_stack = []
let append_stack v imps2 s =
  if Array.length v = 0 then s else
  match s with
  | Zapp(l, imps) :: s ->
      Zapp (Array.append v l, Array.append imps2 imps) :: s
  | _ -> Zapp(v,imps2) :: s
...
let rec stack_args_size = function
  | Zapp (v, _) :: s -> Array.length v + stack_args_size s
  ...
  | Zapp (v, imps) :: s ->
      (match Array.length v with
        ...
        | _ ->
            ...
            Some (v.(0), (Zapp (Array.sub v 1 n, Array.sub imps 1 n):: s)))
  | _ -> None
let rec decomp_stackn = function
  | Zapp (v, imps) :: s ->
      if Array.length v = 0 then decomp_stackn s else (v, imps, s)
  | _ -> assert false
let array_of_stack s =
  let rec stackrec = function
  | [] -> []
  | Zapp (args, imps) :: s -> (args,imps) :: (stackrec s)
  | _ -> assert false
  in
  let (args,imps) = List.split (stackrec s) in
  Array.concat args, Array.concat imps
let rec stack_assign s p c = match s with
  | Zapp (args, imps):: s ->
      let q = Array.length args in 
      if p >= q then
	Zapp (args, imps) :: stack_assign s (p-q) c
      else
...
let rec stack_tail p s = 
  if p = 0 then s else
    match s with
	  ...
	  else Zapp (Array.sub args p (q-p), Array.sub imps p (q-p)) :: s
...
let rec lft_fconstr n ft =
  match ft.term with
    ...
    | FLambda(k,tys,f,e) ->
        {norm=Cstr; term=FLambda(k,tys,f,subs_shft(n,e))}
...
let rec compact_constr (lg, subs as s) c k =
  match kind_of_term c with
    ...
    | App(f,v,imps) ->
        ...
        let (v',s) = compact_vect s v k in
        if f==f' && v==v' then c,s else mkApp(f',v',imps), s
    | Lambda(n,imp,a,b) ->
        let (a',s) = compact_constr s a k in
        let (b',s) = compact_constr s b (k+1) in
        if a==a' && b==b' then c,s else mkLambda(n,imp,a',b'), s
    | Prod(n,imp,a,b) ->
        let (a',s) = compact_constr s a k in
        let (b',s) = compact_constr s b (k+1) in
        if a==a' && b==b' then c,s else mkProd(n,imp,a',b'), s
    | LetIn(n,imp,a,ty,b) ->
        let (a',s) = compact_constr s a k in
        let (ty',s) = compact_constr s ty k in
        let (b',s) = compact_constr s b (k+1) in
        if a==a' && ty==ty' && b==b' then c,s else mkLetIn(n,imp,a',ty',b'), s
    | Fix(fi,(na,ty,bd)) ->
...
let destFLambda clos_fun t =
  match t.term with
      FLambda(_,[(na,imp,ty)],b,e) ->
        (na,imp,clos_fun e ty,clos_fun (subs_lift e) b)
    | FLambda(n,(na,imp,ty)::tys,b,e) ->
        (na,imp,clos_fun e ty,
         {norm=Cstr;term=FLambda(n-1,tys,b,subs_lift e)})
    | _ -> assert false
...
let mk_clos_deep clos_fun env t =
  match kind_of_term t with
    ...
    | App (f,v,imps) ->
        { norm = Red;
	  term = FApp (clos_fun env f, Array.map (clos_fun env) v, imps) }
    | Case (ci,p,c,v) ->
        { norm = Red;
    ...
    | Prod (n,imp,t,c)   ->
        { norm = Whnf;
	  term = FProd (n, imp, clos_fun env t, clos_fun (subs_lift env) c) }
    | LetIn (n,imp,b,t,c) ->
        { norm = Red;
	  term = FLetIn (n, imp, clos_fun env b, clos_fun env t, c, env) }
    | Evar(ev,args) ->
...
let rec to_constr constr_fun lfts v =
  match v.term with
	...
	mkApp (constr_fun lfts f,
	       Array.map (constr_fun lfts) ve, imps)
    | FLambda _ ->
        let (na,imp,ty,bd) = destFLambda mk_clos2 v in
	mkLambda (na, imp, constr_fun lfts ty,
	                   constr_fun (el_lift lfts) bd)
    | FProd (n,imp,t,c)   ->
	mkProd (n, imp, constr_fun lfts t,
	                constr_fun (el_lift lfts) c)
    | FLetIn (n,imp,b,t,f,e) ->
        let fc = mk_clos2 (subs_lift e) f in
	mkLetIn (n, imp, constr_fun lfts b,
	            constr_fun lfts t,
...
let rec zip m stk =
  match stk with
    | [] -> m
    | Zapp (args, imps) :: s ->
        zip {norm=neutr m.norm; term=FApp(m, args, imps)} s
    | Zcase(ci,p,br)::s ->
        let t = FCases(ci, p, m, br) in
        zip {norm=neutr m.norm; term=t} s
    | Zfix(fx,par)::s ->
        zip fx (par @ append_stack [|m|] [|Expl|] s)
    | Zshift(n)::s ->
...
let strip_update_shift_app head stk =
  ...
  let rec strip_rec rstk h depth = function
    ...
    | (Zapp (args, imps) :: s) ->
        strip_rec (Zapp (args,imps) :: rstk)
          {norm=h.norm;term=FApp(h,args,imps)} depth s
    | Zupdate(m)::s ->
...
let get_nth_arg head n stk =
  ...
  let rec strip_rec rstk h depth n = function
    ...
    | Zapp (args, imps)::s' ->
        ...
        then
          strip_rec (Zapp (args, imps)::rstk)
            {norm=h.norm;term=FApp(h,args,imps)} depth (n-q) s'
        else
          let bef = Array.sub args 0 n in
          let aft = Array.sub args (n+1) (q-n-1) in
          let impbef = Array.sub imps 0 n in
          let impaft = Array.sub imps (n+1) (q-n-1) in
          let stk' =
            List.rev (if n = 0 then rstk else (Zapp (bef,impbef):: rstk)) in
          (Some (stk', args.(n), imps.(n)), append_stack aft impaft s')
    | Zupdate(m)::s ->
...
let rec get_args n tys f e stk =
  match stk with
    ...
    | Zapp (l,aimps) :: s ->
        ...
        if n == na then
          if aimps<>Array.of_list(List.map(fun(_,imp,_)->imp)tys) then
            get_impls [] tys f e (Array.to_list l) (Array.to_list aimps) s
          else (Inl (subs_cons(l,e)),s)
        else if n < na then (* more arguments *)
          let args = Array.sub l 0 n in
          let eargs = Array.sub l n (na-n) in
          let eimps = Array.sub aimps n (na-n) in
          if Array.sub aimps 0 n<>
            Array.of_list (List.map(fun(_,imp,_)->imp)tys) then
            get_impls [] tys f e (Array.to_list l) (Array.to_list aimps) s
          else (Inl (subs_cons(args,e)), Zapp(eargs,eimps) :: s)
        else (* more lambdas *)
          let etys = list_skipn na tys in
          if aimps<>Array.sub
  (Array.of_list(List.map(fun(_,imp,_)->imp)tys)) 0 na then
            get_impls [] tys f e (Array.to_list l) (Array.to_list aimps) s
          else get_args (n-na) etys f (subs_cons(l,e)) s
...
let rec reloc_rargs_rec depth stk =
  match stk with
      Zapp (args,imps) :: s ->
        Zapp (lift_fconstr_vect depth args,imps) :: reloc_rargs_rec depth s
  ...
  match stk with
      Zapp (args,imps)::s ->
        ...
        if n > q then drop_parameters depth (n-q) s
          ...
          let impaft = Array.sub imps n (q-n) in
          reloc_rargs depth (append_stack aft impaft s)
...
let rec knh m stk =
  match m.term with
    ...
    | FLOCKED -> assert false
    | FApp(a,b,imps) -> knh a (append_stack b imps (zupdate m stk))
    | FCases(ci,p,t,br) -> knh t (Zcase(ci,p,br)::zupdate m stk)
    | FFix(((ri,n),(_,_,_)),_) ->
        (match get_nth_arg m ri.(n) stk with
             (Some(pars,arg,Expl),stk') -> knh arg (Zfix(m,pars)::stk')
           | (None, stk') -> (m,stk')
           | _ -> assert false)
...
and knht e t stk =
  match kind_of_term t with
    | App(a,b,imps) ->
        knht e a (append_stack (mk_clos_vect e b) imps stk)
    | Case(ci,p,t,br) ->
  ...
  | FConstruct(ind,c) when red_set info.i_flags fIOTA ->
      (match strip_update_shift_app m stk with
        ...
        | (_, cargs, Zfix(fx,par)::s) ->
            ...
            let stk' = par @ append_stack [|rarg|] [|Expl|] s in
...
let rec zip_term zfun m stk =
  match stk with
    | [] -> m
    | Zapp (args,imps) :: s ->
        zip_term zfun (mkApp(m, Array.map zfun args, imps)) s
    | Zcase(ci,p,br)::s ->
        let t = mkCase(ci, zfun p, m, Array.map zfun br) in
        zip_term zfun t s
    | Zfix(fx,par)::s ->
        let h = mkApp(zip_term zfun (zfun fx) par,[|m|],[|Expl|]) in
        zip_term zfun h s
    | Zshift(n)::s ->
...
and norm_head info m =
  if is_val m then (incr prune; term_of_fconstr m) else
    match m.term with
      | FLambda(n,tys,f,e) ->
          let (e',rvtys) =
            List.fold_left (fun (e,ctxt) (na,imp,ty) ->
              (subs_lift e, (na,imp,kl info (mk_clos e ty))::ctxt))
              (e,[]) tys in
          let bd = kl info (mk_clos e' f) in
          List.fold_left (fun b (na,imp,ty) -> mkLambda(na,imp,ty,b)) bd rvtys
      | FLetIn(na,imp,a,b,f,e) ->
          let c = mk_clos (subs_lift e) f in
          mkLetIn(na, imp, kl info a, kl info b, kl info c)
      | FProd(na,imp,dom,rng) ->
          mkProd(na, imp, kl info dom, kl info rng)
