...
let make_existential loc ?(opaque = true) env isevars imp c =
  let evar =
    Evarutil.e_new_evar isevars env ~src:(loc, QuestionMark opaque) imp c in
  ...
  let rec aux names conttac conttype = function
	...
	let hyptype = substl names t in
	  ...
	  (try trace (str " assert: " ++ my_print_constr (Global.env ()) hyptype)
	   with _ -> ());
	let tac = assert_tac true (Name n) Expl hyptype in
	let conttac = 
	  (fun cont -> 
	     conttac
	     (tclTHENS tac
	...
	let conttype = 
	  (fun typ -> 
	     let tex = mkLambda (Name n, Expl, t, typ) in
	       conttype
		 (mkApp (Lazy.force ex_ind, [| t; tex |],[|Expl;Expl|])))
	in
...
let mkProj1 a b c = 
  mkApp (Lazy.force proj1, [| a; b; c |],[|Impl;Impl;Expl|])

let mkProj2 a b c = 
  mkApp (Lazy.force proj2, [| a; b; c |],[|Impl;Impl;Expl|])

let mk_ex_pi1 a b c =
  mkApp (Lazy.force ex_pi1, [| a; b; c |],[|Impl;Impl;Expl|])

let mk_ex_pi2 a b c =
  mkApp (Lazy.force ex_pi2, [| a; b; c |],[|Impl;Impl;Expl|])
    
let mkSubset name typ prop = 
  mkApp ((Lazy.force sig_).typ,
	 [| typ; mkLambda (name, Expl, typ, prop) |],[|Expl;Expl|])

let mk_eq typ x y =
  mkApp (Lazy.force eq_ind, [| typ; x ; y |],[|Expl;Expl;Expl|])
let mk_eq_refl typ x =
  mkApp (Lazy.force eq_refl, [| typ; x |],[|Impl;Impl|])
let mk_JMeq typ x typ' y =
  mkApp (Lazy.force jmeq_ind, [| typ; x ; typ'; y |],[|Expl;Expl;Expl;Expl|])
let mk_JMeq_refl typ x =
  mkApp (Lazy.force jmeq_refl, [| typ; x |],[|Impl;Impl|])
...
let mk_conj l =
  let conj_typ = Lazy.force and_typ in
    unsafe_fold_right
      (fun c conj ->
	 mkApp (conj_typ, [| c ; conj |],[|Expl;Expl|]))
      l
...
let and_tac l hook =
  ...
  let rec aux ((accid, goal, tac, extract) as acc) = function
	...
	let extract = List.map (fun (id, x, y, f) -> (id, x, y, (fun c -> f (mkProj1 goal elgoal c)))) extract in
	  aux ((string_of_id id) ^ "_" ^ accid, mkApp (andc, [| goal; elgoal |],[|Expl;Expl|]), tac', 
...
let pr_meta_map evd =
  ...
  let pr_meta_binding = function
    | (mv,Cltyp (na,imp,b)) ->
      	hov 0 
	  (pr_meta mv ++ pr_impl imp (pr_name na) ++ str " : " ++
