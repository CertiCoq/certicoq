...
let r_of_posint dloc n =
  ...
  let rec r_of_pos n =
    if less_than n four then small_r dloc n
      ...
      let b =
        RApp(dloc,RRef(dloc,glob_Rmult),
             [r2;r_of_pos q],[Term.Expl;Term.Expl]) in
      if r then RApp(dloc,RRef(dloc,glob_Rplus),
                     [r1;b],[Term.Expl;Term.Expl]) else b in
  if n <> zero then r_of_pos n else RRef(dloc,glob_R0)

let r_of_int dloc z =
  if is_strictly_neg z then
    RApp (dloc, RRef(dloc,glob_Ropp), [r_of_posint dloc (neg z)],[Term.Expl]) 
  else
    r_of_posint dloc z
...
let bigint_of_r = function
  | RApp (_,RRef (_,o), [a],_) when o = glob_Ropp ->
