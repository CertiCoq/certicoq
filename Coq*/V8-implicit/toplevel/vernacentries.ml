...
(*i $Id: vernacentries.ml 9821 2007-05-11 17:00:58Z aspiwack $ i*)
...
let vernac_assumption kind l nl=
  List.iter (fun (is_coe,((idl,c),imp)) ->
    declare_assumption idl is_coe kind [] c nl) l
...
let vernac_print = function
  ...
  | PrintExtr c ->
      let sigma = Evd.empty in
      let env = Global.env() in
      let c = interp_constr sigma env c in
      msgnl (print_extr c)
