...
(* $Id: eauto.ml4 9969 2007-07-11 21:48:04Z letouzey $ *)
...
and e_trivial_resolve db_list local_db gl = 
  try 
    let (hd,_,_) = head_constr_bound gl in
    Auto.priority 
      (e_my_find_search db_list local_db hd gl)
  with Bound | Not_found -> []

let e_possible_resolve db_list local_db gl =
  try 
    let (hd,_,_) = head_constr_bound gl in
    List.map snd (e_my_find_search db_list local_db hd gl)
  with Bound | Not_found -> []
