...
let mk_evd metalist gls =
  ...
  let add_one (meta,typ) evd = 
    meta_declare meta Expl typ evd in
  ...
  let add_aux c gls=
	...
	Var id -> 
	  ...
	  let id=pf_get_new_id local_hyp_prefix gls in 
	    keep:=Idset.add id !keep; 
	    letin_tac false (Names.Name id) Expl c Tacexpr.nowhere gls in 
...
let enstack_subsubgoals env se stack gls=
  let hd,params,imps = decompose_app (special_whd gls se.se_type) in
	...
	Ind ind when is_good_inductive env ind ->
	  ...
	  let process i gentyp = 
	    ...
	    let appterm = applist (constructor,params,imps) in
	    ...
	    let refiner =
              applist (appterm,List.rev holes,
                       List.map(fun _ -> Expl)holes) in
	    ...
	    let evd0 = List.fold_left 
	      (fun evd (m,typ) -> meta_declare m Expl typ evd) evd nmetas in  
	      List.iter (fun (m,typ) -> 
			   Stack.push 
			     {se_meta=m;
			      se_type=typ;
...
let concl_refiner metas body gls =
  ...
  let rec aux env avoid subst = function
	...
	let nenv = Environ.push_named (_x,Expl,None,_A) env in
	let asort = family_of_sort (Typing.sort_of nenv evd _A) in
	let nsubst = (n,mkVar _x)::subst in
	  if rest = [] then 
	    asort,_A,mkNamedLambda _x Expl _A (subst_meta nsubst body)
	  else
	    let bsort,_B,nbody = 
	      aux nenv (_x::avoid) ((n,mkVar _x)::subst) rest in
	    let body = mkNamedLambda _x Expl _A nbody in
	      if occur_term (mkVar _x) _B then
	        begin
		  let _P = mkNamedLambda _x Expl _A _B in
		    match bsort,sort with
			InProp,InProp ->
			  let _AxB =
                            mkApp(Lazy.force _ex,[|_A;_P|],[|Expl;Expl|]) in
			    InProp,_AxB,
			  mkApp(Lazy.force _ex_ind,[|_A;_P;concl;body|],
                          [|Impl;Impl;Impl;Expl|])
		    | InProp,_ ->
			let _AxB =
                          mkApp(Lazy.force _sig,[|_A;_P|],[|Expl;Expl|]) in
			let _P0 = mkLambda(Anonymous,Expl,_AxB,concl) in
			  InType,_AxB,
			mkApp(Lazy.force _sig_rect,[|_A;_P;_P0;body|],
                          [|Impl;Impl;Impl;Expl|])
		    | _,_ -> 
			let _AxB =
                          mkApp(Lazy.force _sigT,[|_A;_P|],[|Expl;Expl|]) in
			let _P0 = mkLambda(Anonymous,Expl,_AxB,concl) in
			  InType,_AxB,
		...
		begin
		  match asort,bsort with
		      InProp,InProp ->
			let _AxB =
                          mkApp(Lazy.force _and,[|_A;_B|],[|Expl;Expl|]) in
			  InProp,_AxB,
		     mkApp(Lazy.force _and_rect,[|_A;_B;concl;body|],
                          [|Impl;Impl;Impl;Expl|])
		    |_,_ ->
		       let _AxB =
                         mkApp(Lazy.force _prod,[|_A;_B|],[|Expl;Expl|]) in
		       let _P0 = mkLambda(Anonymous,Expl,_AxB,concl) in
			 InType,_AxB,
...
let instr_cut mkstat _thus _then cut gls0 = 
  ...
  let thus_tac gls= 
    ...
    tclTHENS (internal_cut c_id Expl c_stat) 
...
let instr_rew _thus rew_side cut gls0 = 
  ...
  let thus_tac new_eq gls= 
	...
	Lhs ->
	  let new_eq =
            mkApp(_eq,[|typ;cut.cut_stat.st_it;rhs|],[|Expl;Expl;Expl|]) in
	    tclTHENS (internal_cut c_id Expl new_eq) 
      ...
      | Rhs ->
	  let new_eq =
            mkApp(_eq,[|typ;lhs;cut.cut_stat.st_it|],[|Expl;Expl;Expl|]) in
	    tclTHENS (internal_cut c_id Expl new_eq) 
	      [tclTHEN tcl_erase_info 
...
let instr_claim _thus st gls0 = 
  ...
  let ninfo1 = {pm_stack=
    ...
    tclTHENS (internal_cut id Expl st.st_it) 
...
let assume_tac hyps gls = 
  List.fold_right 
    (fun (Hvar st | Hprop st) -> 
	 ...
	 (push_intro_tac 
	    (fun id -> 
	       convert_hyp (id,Expl,None,st.st_it)) st.st_label))
	 hyps tclIDTAC gls 
    ...
    (function 
	 (Hvar {st_label=nam;st_it=c} | Hprop {st_label=nam;st_it=This c}) -> 
	   tclTHEN 
	     (push_intro_tac 
		(fun id -> 
		   convert_hyp (id,Expl,None,c)) nam)
...
let assume_st hyps gls = 
  List.fold_right 
    (fun st -> 
       tclTHEN 
	 (push_intro_tac 
	    (fun id -> convert_hyp (id,Expl,None,st.st_it)) st.st_label))
	 hyps tclIDTAC gls 
  ...
  List.fold_right 
    (fun st -> 
	 ...
	 (push_intro_tac 
	    (fun id -> 
	       convert_hyp
                 (id,Expl,Some (fst st.st_it),snd st.st_it)) st.st_label))
	 hyps tclIDTAC gls 
...
let rec build_product args body =
  match args with 
	...
	let lbody =
	  ...
	  mkProd (st.st_label, Expl, st.st_it, lbody)
    | [] -> body 

let rec build_applist prod = function
    [] -> [],prod
  | n::q -> 
      let (_,imp,typ,_) = destProd prod in
...
let instr_suffices _then cut gls0 = 
  ...
  let mimps = List.map (fun _ -> Expl) ctx in
  let c_ctx,c_head = build_applist c_stat metas in
  let c_term = applist (mkVar c_id,List.map mkMeta metas,mimps) in  
  let thus_tac gls= 
...
let conjunction_arity id gls =
  let typ = pf_get_hyp_typ gls id  in
  let hd,params,imps = decompose_app (special_whd gls typ) in
  let env =pf_env gls in 
    match kind_of_term hd with
	Ind ind when is_good_inductive env ind ->
	  let mib,oib=
...
let rec consider_match may_intro introduced available expected gls =
  match available,expected with 
	...
	tclIFTHENELSE (convert_hyp (id,Expl,None,st.st_it))
	  begin
	    match st.st_label with 
		Anonymous -> 
...
let rec build_function args body =
  match args with 
	...
	let id = match st.st_label with
	  ...
	  | Name id -> id in
	  mkLambda (Name id, Expl ,st.st_it, subst_term (mkVar id) pfun)
    | [] -> body 

let define_tac id args body gls =
  let t = build_function args body in
    letin_tac true (Name id) Expl t Tacexpr.nowhere gls
...
let cast_tac id_or_thesis typ gls = 
  match id_or_thesis with
      This id ->
	let (_,imp,body,_) = pf_get_hyp gls id in
	  convert_hyp (id,imp,body,typ) gls
    | Thesis (For _ ) -> 
	error "\"thesis for ...\" is not applicable here" 
    | Thesis Plain ->    
...
let build_per_info etype casee gls = 
  ...
  let hd,args,imps = decompose_app (special_whd gls ctyp) in
  let ind = 
    try
      destInd hd 
    with _ -> 
  ...
  let pimps,raimps = list_chop nparams imps in
  let abstract_obj c imp body = 
    let typ=pf_type_of gls c in 
      lambda_create env (imp,typ,subst_term c body) in
  let pred= List.fold_right2 abstract_obj    
    real_args raimps (lambda_create env (Expl,ctyp,subst_term casee concl)) in
    is_dep,
  {per_casee=casee;
   per_ctype=ctyp;
   per_ind=ind;
...
let suppose_tac hyps gls0 = 
  ...
  let ninfo2 = {pm_stack=stack} in
    tclTHENS (internal_cut id Expl clause) 
...
let thesis_for obj typ per_info env=
  ...
  let cind,all_args,aimps=decompose_app typ in
  ...
  let _,rimps = list_chop per_info.per_nparams aimps in
  let _ = if not (List.for_all2 eq_constr params per_info.per_params) then
    errorlabstrm "thesis_for" 
      ((Printer.pr_constr_env env obj) ++ spc () ++ 
	 str "cannot give an induction hypothesis (wrong parameters)") in
  let hd2 =
    applist ((lift (List.length rc) per_info.per_pred),args@[obj],
             rimps@[Expl]) in
...
let rec build_product_dep pat_info per_info args body gls =
  match args with 
	...
	let ptyp =
	  ...
	  mkProd (st.st_label,Expl,ptyp,lbody)
    | [] -> body 
...
let build_dep_clause params pat_info per_info hyps gls =
  ...
  let prod_one st body =
    match st.st_label with
	Anonymous -> mkProd(Anonymous,Expl,st.st_it,lift 1 body)
      | Name id -> mkNamedProd id Expl st.st_it (lift 1 body) in
  let let_one_in st body =
    match st.st_label with
	Anonymous ->
          mkLetIn(Anonymous,Expl,fst st.st_it,snd st.st_it,lift 1 body)
      | Name id -> 
	  mkNamedLetIn id Expl (fst st.st_it) (snd st.st_it) (lift 1 body) in
  let aliased_clause = 
...
type instance_stack =
    (constr option*(constr list * impl list) list) list

let initial_instance_stack ids =
  List.map (fun id -> id,[None,[],[]]) ids

let push_one_arg arg (imp:impl) = function 
    [] -> anomaly "impossible"
  | (head,args,imps) :: ctx -> 
      ((head,(arg::args),imp::imps) :: ctx)

let push_arg arg imp stacks =
  List.map (fun (id,stack) -> (id,push_one_arg arg imp stack)) stacks
...
let pop_one (id,stack) =  
  let nstack=
	...
	[] -> anomaly "impossible"
      | [c] as l -> l
      | (Some head,args,imps)::(head0,args0,imps0)::ctx ->
	  let arg = applist (head,(List.rev args),List.rev imps) in
	    (head0,(arg::args0),Expl::imps0)::ctx (* TODO: fix Expl *)
     | (None,args,imps)::(head0,args0,imps0)::ctx ->
	 (head0,(args@args0),(imps@imps0))::ctx
  in id,nstack
...
let hrec_for fix_id per_info gls obj_id =
  ...
  let rc,hd1=decompose_prod typ in
  let cind,all_args,imps=decompose_app typ in
  let ind = destInd cind in assert (ind=per_info.per_ind);
  let params,args= list_chop per_info.per_nparams all_args in 
  let _,aimps = list_chop per_info.per_nparams imps in 
  assert begin
    try List.for_all2 eq_constr params per_info.per_params with 
        Invalid_argument _ -> false end;
  let hd2 = applist (mkVar fix_id,args@[obj],aimps@[Expl]) in 
...
let rec execute_cases fix_name per_info tacnext args objs nhrec tree gls =
  match tree, objs  with
	...
	let args0 = push_arg skipped Expl args in
	  execute_cases fix_name per_info tacnext args0 next_objs nhrec t gls
    | End_patt (id,nhyps),[] -> 
	begin
	  match List.assoc id args with
	      [None,br_args,br_imps] -> 
		let metas = 
		  list_tabulate (fun n -> mkMeta (succ n)) nhyps in
		  tclTHEN
		    (tclDO nhrec introf)
		    (tacnext 
		       (applist (mkVar id,List.rev_append br_args  metas,
                                 List.rev_append br_imps
                                   (List.map(fun _->Expl)metas)))) gls
	...
	let env=pf_env gls in
	let ctyp=pf_type_of gls casee in
	let hd,all_args,aimps = decompose_app (special_whd gls ctyp) in
	let _ = assert (destInd hd = ind) in (* just in case *)
	let params,real_args = list_chop nparams all_args in
	let _,raimps = list_chop nparams aimps in
	let abstract_obj imp c body = 
	  let typ=pf_type_of gls c in 
	    lambda_create env (imp,typ,subst_term c body) in
	let elim_pred = List.fold_right2 abstract_obj    
	  raimps real_args
          (lambda_create env (Expl,ctyp,subst_term casee concl)) in
	  ...
	  let objs,recs,nhrec = aux 0 args_ids in
	    tclTHENLIST
		   ...
		   let hrecs = 
		     List.map 
			  ...
			  hrec_for (out_name fix_name) per_info gls1 id) 
		       recs in
		     generalize hrecs (List.map(fun _->Expl) hrecs) gls1
	       end;
	       match bro with
		   None -> 
		 ...
		 | Some (sub_ids,tree) ->
		     ...
		     let construct = 
		       applist (mkConstruct(ind,succ i),params,
                                List.map(fun _ -> Impl) params) in
		     let p_args = 
	...
	match et,ek with
	  ...
	  | ET_Induction,EK_nodep ->
		...
		[generalize (pi.per_args@[pi.per_casee])
                  (List.map(fun _->Expl)pi.per_args@[Expl]); 
...
let rec abstract_metas n avoid head = function
  ...
  | (meta,typ)::rest -> 
      ...
      let p,term,args,imps =
        abstract_metas (succ n) (id::avoid) head rest in 
      succ p,mkLambda(Name id,Expl,typ,subst_meta [meta,mkRel p] term),
      (mkMeta n)::args, Expl::imps
