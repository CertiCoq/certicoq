...
(*i $Id: declarations.ml 9821 2007-05-11 17:00:58Z aspiwack $ i*)
...
let subst_rel_declaration sub (id,imp,copt,t as x) =
  let copt' = option_smartmap (subst_mps sub) copt in
  let t' = subst_mps sub t in
  if copt == copt' & t == t' then x else (id,imp,copt',t')
...
| PolymorphicArity (ctx,s) ->
    PolymorphicArity (subst_rel_context sub ctx,s)
