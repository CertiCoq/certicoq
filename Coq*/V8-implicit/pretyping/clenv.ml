...
(* $Id: clenv.ml 9878 2007-06-06 17:00:01Z herbelin $ *)
...
let clenv_imp clenv =
  if noccur_implicit (named_context_val clenv.env) (clenv_value clenv)
  then Expl
  else Impl
...
let clenv_push_prod cl =
  ...
  let rec clrec typ = match kind_of_term typ with
	...
	let e' = meta_declare mv imp t ~name:na' cl.evd in
	let concl = if dep then subst1 (mkMeta mv) u else u in
	let def = applist (cl.templval.rebus,[mkMeta mv],[imp]) in
...
let clenv_environments evd bound t =
  let rec clrec (e,metas,imps) n t =
    match n, kind_of_term t with
      | (Some 0, _) -> (e, List.rev metas, List.rev imps, t)
      | (n, Cast (t,_,_)) -> clrec (e,metas,imps) n t
      | (n, Prod (na,imp,t1,t2)) ->
	  let mv = new_meta () in
	  let dep = dependent (mkRel 1) t2 in
	  let na' = if dep then na else Anonymous in
	  let e' = meta_declare mv imp t1 ~name:na' e in
	  clrec (e', (mkMeta mv)::metas, imp::imps) (option_map ((+) (-1)) n)
	    (if dep then (subst1 (mkMeta mv) t2) else t2)
      | (n, LetIn (na,_,b,_,t)) -> clrec (e,metas,imps) n (subst1 b t)
      | (n, _) -> (e, List.rev metas, List.rev imps, t)
  in 
  clrec (evd,[],[]) bound t
...
let clenv_environments_evars env evd bound t =
  let rec clrec (e,ts,imps) n t =
    match n, kind_of_term t with
      | (Some 0, _) -> (e, List.rev ts, List.rev imps, t)
      | (n, Cast (t,_,_)) -> clrec (e,ts,imps) n t
      | (n, Prod (na,imp,t1,t2)) ->
          let e',constr = Evarutil.new_evar e env imp t1 in
	  let dep = dependent (mkRel 1) t2 in
	  clrec (e', constr::ts, imp::imps) (option_map ((+) (-1)) n)
	    (if dep then (subst1 constr t2) else t2)
      | (n, LetIn (na,_,b,_,t)) -> clrec (e,ts,imps) n (subst1 b t)
      | (n, _) -> (e, List.rev ts, List.rev imps, t)
  in 
  clrec (evd,[],[]) bound t
...
let clenv_conv_leq env sigma t c bound =
  ...
  let evars,args,imps,_ = clenv_environments_evars env evd (Some bound) ty in
  let evars = Evarconv.the_conv_x_leq env t (applist (c,args,imps)) evars in
  let evars,_ = Evarconv.consider_remaining_unif_problems env evars in
  let args = List.map (whd_evar (Evd.evars_of evars)) args in
  check_evars env sigma evars (applist (c,args,imps));
  args,imps

let mk_clenv_from_n gls n (c,cty) =
  let evd = create_goal_evar_defs gls.sigma in
  let (env,args,imps,concl) = clenv_environments evd n cty in
  { templval =
      mk_freelisted (applist (c,args,imps));
    templtyp = mk_freelisted concl;
    evd = env;
...
let clenv_unique_resolver allow_K clenv gl =
  let (hd,_,_) = whd_stack clenv.templtyp.rebus in
  if isMeta hd then
...
let clenv_pose_dependent_evars clenv =
  ...
  List.fold_left
    (fun clenv mv ->
      ...
      let imp = meta_impl clenv.evd mv in
      let (evd,evar) = 
	new_evar clenv.evd (cl_env clenv) ~src:(dummy_loc,GoalEvar) imp ty in
...
let rec similar_type_shapes t u =
  let t,_,_ = decompose_app t and u,_,_ = decompose_app u in
  match kind_of_term t, kind_of_term u with
  | Prod (_,imp1,t1,t2), Prod (_,imp2,u1,u2) ->
      imp1=imp2 && similar_type_shapes t2 u2
