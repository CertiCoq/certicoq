...
(* $Id: cases.ml 9707 2007-03-15 16:36:15Z herbelin $ *)
...
module type S = sig
  val compile_cases :
    loc ->
    (impl -> type_constraint -> env -> rawconstr -> unsafe_judgment) *
      Evd.evar_defs ref ->
    impl -> type_constraint -> 
...
let make_anonymous_patvars =
  list_tabulate (fun _ -> PatVar (dummy_loc,Anonymous,Expl)) 

(* Environment management *)
let push_rels vars env = List.fold_right push_rel vars env

let push_rel_defs =
  List.fold_right (fun (x,imp,d,t) e -> push_rel (x,imp,Some d,t) e)
...
let mkSpecialLetInJudge j (na,(imp,deppat,nondeppat,d,t)) =
  { uj_val =
      (match d with
	| DepAlias -> mkLetIn (na,imp,deppat,t,j.uj_val)
	| NonDepAlias ->
	    ...
	    then 
	      ...
	      mkLetIn (na,imp,deppat,t,j.uj_val));
...
type tomatch_status =
  | Pushed of ((impl * constr * tomatch_type) * int list)
  | Alias of (impl * constr * constr * alias_constr * constr)
...
and build_rawpattern args = function
  ...
  | MakeAlias (AliasLeaf, rh) ->
      assert (args = []);
      rawpattern_of_partial_history [PatVar (dummy_loc, Anonymous,Expl)] rh
...
let rec simplify_history = function
  ...
  | Continuation (0, l, MakeAlias (f, rh)) ->
	...
	| AliasLeaf -> 
	    assert (l = []);
	    PatVar (dummy_loc, Anonymous, Expl) in
      feed_history pat rh
  | h -> h
...
let inductive_template isevars env tmloc ind =
  ...
  let hole_source = match tmloc with 
   ...
   let (_,evarl,_,aimps) =
    List.fold_right
      (fun (na,imp,b,ty) (subst,evarl,n,imps) ->
	match b with
        | None ->
	    let ty' = substl subst ty in
	    let e = e_new_evar isevars env ~src:(hole_source n) imp ty' in
	    (e::subst,e::evarl,n+1,imp::imps) 
	| Some b ->
	    (b::subst,evarl,n+1,imps))
      arsign ([],[],1,[]) in
   applist (mkInd ind,List.rev evarl, List.rev aimps)
...
let mkExistential env ?(src=(dummy_loc,Evd.InternalHole)) isevars =
  e_new_evar isevars env ~src:src Impl (new_Type ())
...
let adjust_tomatch_to_pattern pb ((imp,current,typ),deps) =
  ...
  let tomatch = ((imp,current,typ),deps) in
  match typ with
  | NotInd (None,typ) ->
...
let mkDeclTomatch na imp = function
  | IsInd (t,_) -> (na,imp,None,t)
  | NotInd (c,t) -> (na,imp,c,t)
...
let alias_of_pat = function
  | PatVar (_,name,_) -> name
  | PatCstr(_,_,_,name) -> name

let unalias_pat = function
  | PatVar (c,name,imp) as p ->
      if name = Anonymous then p else PatVar (c,Anonymous,imp)
...
let rec adjust_local_defs loc = function
  | (pat :: pats, (_,_,None,_) :: decls) ->
      pat :: adjust_local_defs loc (pats,decls)
  | (pats, (_,imp,Some _,_) :: decls) ->
      PatVar (loc, Anonymous, imp) :: adjust_local_defs loc (pats,decls)
  | [], [] -> []
...
let is_dep_patt eqn = function
  | PatVar (_,name,_) -> occur_in_rhs name eqn.rhs
  | PatCstr _ -> true
...
let dependent_decl a = function
  | (na,_,None,t) -> dependent a t
  | (na,_,Some c,t) -> dependent a t || dependent a c
...
let regeneralize_index_tomatch n =
  ...
  | Pushed ((imp,c,tm),l)::rest ->
      ...
      Pushed ((imp,c,tm),l)::(genrec depth rest)
  | Alias (imp,c1,c2,d,t)::rest ->
      Alias (imp,regeneralize_index n depth c1,c2,d,t)::(genrec depth rest)
  | Abstract d::rest ->
...
let replace_tomatch n c =
  ...
  | [] -> []
  | Pushed ((imp,b,tm),l)::rest ->
      let b = replace_term n c depth b in
      let tm = map_tomatch_type (replace_term n c depth) tm in
      List.iter (fun i -> if i=n+depth then anomaly "replace_tomatch") l;
      Pushed ((imp,b,tm),l)::(replrec depth rest)
  | Alias (imp,c1,c2,d,t)::rest ->
      Alias (imp,replace_term n c depth c1,c2,d,t)::(replrec depth rest)
  | Abstract d::rest ->
...
let rec liftn_tomatch_stack n depth = function
  ...
  | Pushed ((imp,c,tm),l)::rest ->
      ...
      Pushed ((imp,c,tm),l)::(liftn_tomatch_stack n depth rest)
  | Alias (imp,c1,c2,d,t)::rest ->
      Alias (imp,liftn n depth c1,liftn n depth c2,d,liftn n depth t)
  ...
  let names4,_ =
    List.fold_left2
	 ...
	 let na =
	   merge_name
	     (fun (na,_,_,t) ->
               Name (next_name_away (named_hd env t na) avoid))
	     d na 
	 in
...
let recover_alias_names get_name =
  List.map2 (fun x (_,imp,c,t) ->(get_name x,imp,c,t))
...
let build_aliases_context env sigma names allpats pats =
  ...
  let rec insert env sign1 sign2 n newallpats oldallpats = function
    | (_,deppat,_,_,_)::pats, Anonymous::names when not (isRel deppat) ->
    ...
    | (imp,deppat,nondeppat,d,t)::pats, na::names ->
	...
	let decl = (na,imp,Some deppat,t) in
	let a = (imp,deppat,nondeppat,d,t) in
...
let insert_aliases_eqn sign eqnnames alias_rest eqn =
  let thissign =
    List.map2 (fun na (_,imp,c,t) -> (na,imp,c,t)) eqnnames sign in
  { eqn with
...
let prepare_unif_pb typ cs =
  ...
  let (args,imps) = extended_rel_list (-n) cs.cs_args in
  let ci =
    applist (mkConstruct cs.cs_cstr, cs.cs_params@args,
             List.map(fun _ ->Impl)cs.cs_params@imps) in
...
*)
    ...
    then
      ...
      let sign =
        (Anonymous,Expl,None,build_dependent_inductive env indf)::sign in
      let pred =
        it_mkLambda_or_LetIn (lift (List.length sign) mtyp) sign in
...
(*
      ...
      let pred =
        it_mkLambda_or_LetIn (lift (List.length sign) mtyp) sign imps in
*)
      (* "TODO4-2" *)
      (* We skip parameters *)
      let cis = 
	Array.map
	  (fun cs -> 
            let (inst,imps) = extended_rel_list 0 cs.cs_args in
	    applist (mkConstruct cs.cs_cstr,inst,imps))
	  cstrs in
...
let rec extract_predicate l = function
  | pred, Alias (imp,deppat,nondeppat,_,_)::tms ->
  ...
  | PrLetIn (([],dep),pred), Pushed ((_,cur,_),_)::tms ->
      extract_predicate (if dep<>Anonymous then cur::l else l) (pred,tms)
  | PrLetIn ((_,dep),pred), Pushed ((_,cur,IsInd (_,(IndType(_,realargs,_)))),_)::tms ->
      ...
      let sign,k =
        if names = [] & n <> 1 then
          ...
          (if dep<>Anonymous then
            ((let (_,imp,c,t) = List.hd sign in
              (dep,imp,c,t)::List.tl sign),n-1)
          else
            (sign,n))
        else
          (* Real args are OK *)
          (List.map2 (fun na (_,imp,c,t) -> (na,imp,c,t)) (dep::names) sign,
...
let expand_arg n alreadydep (na,imp,t) deps (k,pred) =
...
let find_predicate loc env isevars p typs cstrs current
  (IndType (indf,realargs,raimps)) tms =
  let (dep,pred) =
    match p with
  ...
  let typ = whd_beta (applist (pred, realargs,raimps)) in
  if dep then
    (pred, whd_beta (applist (typ, [current],[Expl])), new_Type ())
  else
    (pred, typ, new_Type ())
...
let build_leaf pb =
  ...
  pb.typing_function Expl tycon rhs.rhs_env rhs.it

(* Building the sub-problem when all patterns are variables *)
let shift_problem (imp,current,t) pb =
  {pb with
     tomatch =
       Alias (imp,current,current,NonDepAlias,type_of_tomatch t)::pb.tomatch;
...
let build_branch current deps pb eqns const_info =
  ...
  let typs = List.map2 (fun (_,imp,c,t) na -> (na,imp,c,t)) cs_args names in
  let _,typs',_ =
    List.fold_right
      (fun (na,imp,c,t as d) (env,typs,tms) ->
	 let tm1 = List.map List.hd tms in
	 let tms = List.map List.tl tms in
 	 (push_rel d env, (na,imp,to_mutind env pb.isevars tm1 c t)::typs,tms))
  ...
  let currents =
    list_map2_i
      (fun i (na,imp,t) deps ->
        Pushed ((imp,mkRel i, lift_tomatch_type i t), deps))
      1 typs' (List.rev dep_sign) in

  let sign = List.map (fun (na,imp,t) -> mkDeclTomatch na imp t) typs' in

  let ind = (* bug: ill-formed ind (implicit of params needed) *)
    appvect (
      applist (mkInd (inductive_of_constructor const_info.cs_cstr),
                      List.map (lift const_info.cs_nargs)
                        const_info.cs_params,
                      List.map(fun _->Expl) const_info.cs_params),
      const_info.cs_concl_realargs,
      const_info.cs_concl_imps) in

  let cur_alias = lift (List.length sign) current in
  let currents = Alias (Expl,ci,cur_alias,alias_type,ind) :: currents in
...
and match_current pb tomatch =
  let ((imp,current,typ as ct),deps) = adjust_tomatch_to_pattern pb tomatch in
  match typ with
    | NotInd (_,typ) ->
	check_all_variables typ pb.mat;
	compile (shift_problem ct pb)
    | IsInd (_,(IndType(indf,realargs,raimps) as indt)) ->
	let mind,_,_ = dest_ind_family indf in
	let cstrs = get_constructors pb.env indf in
	let eqns,onlydflt = group_equations pb mind current cstrs pb.mat in
	if (Array.length cstrs <> 0 or pb.mat <> []) & onlydflt  then
	  compile (shift_problem ct pb)
	else
          (if imp = Impl then error "match on an implicit subterm forbidden";
	  ...
	  { uj_val = applist (case, inst,List.map(fun _->Expl)inst);
...
and compile_alias pb (imp,deppat,nondeppat,d,t as alias) rest =
  let history = simplify_history pb.history in
  let sign, newenv, mat =
    insert_aliases pb.env (Evd.evars_of !(pb.isevars)) alias pb.mat in
  let n = List.length sign in
...
let build_expected_arity env isevars isdep tomatchl =
  let cook n = function
    | _,IsInd (_,IndType(indf,_,_)) ->
        ...
        let sign,_ = get_arity env indf' in
	Some (build_dependent_inductive env indf', sign)
    | _,NotInd _ -> None
  in
  let rec buildrec n env = function
	...
	match cook n tm with
	  ...
	  | Some (ty1,aritysign) ->
		...
		| [] ->
		    if isdep then
		      mkProd (Anonymous, Expl, ty1,
			      buildrec (n+1)
				(push_rel_assum (Anonymous, Expl, ty1) env)
				ltm)
...
let extract_predicate_conclusion isdep tomatchl pred =
  ...
  let rec decomp_lam_force n l p =
    ...
    match kind_of_term p with
      | Lambda (na,_,_,c) -> decomp_lam_force (n-1) (na::l) c
      | _ -> (* eta-expansion *)
          let na = Name (id_of_string "x") in
          decomp_lam_force (n-1) (na::l)
            (applist (lift 1 p, [mkRel 1],[Expl])) in
...
let set_arity_signature dep n arsign tomatchl pred x =
  ...
  let rec decomp_lam_force n avoid l p =
    ...
    match p with
      ...
      | RLambda (_,(Anonymous as na),_,_,c) ->
      ...
      | _ ->
          ...
          (* eta-expansion *)
            (let a = RVar (dummy_loc,x) in
             match p with
               | RApp (loc,p,l,imps) -> RApp (loc,p,l@[a],imps@[Expl])
               | _ -> (RApp (dummy_loc,p,[a],[Expl]))) in
  let rec decomp_block avoid p = function
    | ([], _) -> x := Some p
    | ((_,IsInd (_,IndType(indf,realargs,raimps)))::l),(y::l')  ->
...
let prepare_predicate_from_tycon loc dep env isevars tomatchs
  (sign:predicate_pattern list) c =
  let cook (n, l, env, signs) = function
    | imp,c,IsInd (_,IndType(indf,realargs,raimps)) ->
	let indf' = lift_inductive_family n indf in
  ...
  let names = List.rev (List.map (List.map (fun(na,_,_,_)->na)) signs) in
  let allargs =
 ...
 (* let c = whd_betadeltaiota env (evars_of isevars) c in *)
    ...
    if isEvar c or List.exists (eq_constr c) allargs then
      e_new_evar isevars env  ~src:(loc, Evd.CasesType) Impl
...
let extract_arity_signature env0 tomatchl tmsign =
  let get_one_sign n tm (na,t) =
    match tm with
      | NotInd (bo,typ) -> 
	  (match t with
	    | None -> [na,Expl,option_map (lift n) bo,lift n typ]
	    | Some (loc,_,_,_) -> 
 	    user_err_loc (loc,"",
	    str "Unexpected type annotation for a term of non inductive type"))
      | IsInd (_,IndType(indf,realargs,raimps)) ->
          let indf' = lift_inductive_family n indf in
	  ...
	  let realnal =
	    match t with
		  ...
		  List.rev realnal
	      | None ->
                  list_tabulate (fun _ -> (Anonymous,Expl)) nrealargs in
	  (na,Expl,None,build_dependent_inductive env0 indf')
	  ::(List.map2 (fun (x,_) (_,imp,c,t) ->(x,imp,c,t)) realnal arsign) in
...
let prepare_predicate loc typing_fun isevars env tomatchs sign tycon = function
  ...
  | Some rtntyp ->
      ...
      let allnames =
        List.rev (List.map (List.map (fun(na,_,_,_)->na)) arsign) in
...
let compile_cases loc (typing_fun, isevars) imp (tycon : Evarutil.type_constraint) env (predopt, tomatchl, eqns)=
  ...
  let tomatchs =
    coerce_to_indtype (typing_fun Impl) isevars env matx tomatchl in
    
  (* We build the elimination predicate if any and check its consistency *)
  (* with the type of arguments to match *)
  let tmsign = List.map snd tomatchl in
  let pred =
    prepare_predicate loc (typing_fun Impl)
      isevars env tomatchs tmsign tycon predopt in
  ...
  let pb =
    { env      = env;
      ...
      caseloc  = loc;
      typing_function =
        fun imp' -> typing_fun (impl_merge[imp;imp']) } in
