...
(*i $Id: Compare_dec.v 9698 2007-03-12 17:11:32Z letouzey $ i*)
...
Definition le_gt_dec n m : {n <= m} + {n > m}.
  exact le_lt_dec.
Defined.
Definition le_gt_or n m := sumbool_or (le_gt_dec n m).

Definition le_lt_eq_dec n m : n <= m -> {n < m} + {n = m}.
  intros; elim (lt_eq_lt_dec n m); auto with arith.
  intros; elim impl_neg with (m < n); auto with arith.
Defined.
...
Proof.
  intros x y H; elim (le_lt_dec' x y);
...
Proof.
  ...
  unfold nat_compare; destruct (lt_eq_lt_dec 0 (S m)) as [[H|H]|H]; 
    apply impl_PI; auto; intros; try discriminate.
...
Proof.
  ...
  split; [inversion 1 |].
  unfold nat_compare; destruct (lt_eq_lt_dec (S n) 0) as [[H|H]|H]; 
    apply impl_PI in H; auto; intros; try discriminate.
...
Proof.
 ...
 split; auto with arith.
 unfold nat_compare; destruct (lt_eq_lt_dec 0 (S m)) as [[H|H]|H]; 
  apply impl_PI in H; intuition; try discriminate.
 inversion H.
