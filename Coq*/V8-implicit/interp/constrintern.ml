...
(* $Id: constrintern.ml 9969 2007-07-11 21:48:04Z letouzey $ *)
...
type var_internalisation_data =
    identifier list * Term.impl list *
    Impargs.implicits_list * scope_name option list
...
let intern_var (env,_,_ as genv) (ltacvars,vars2,vars3,(_,impls)) loc id =
  ...
  try
    let l,imps,impl,argsc = List.assoc id impls in
    let l = List.map2 
      (fun id imp -> (CRef (Ident (loc,id)),imp), Some (loc,ExplByName id))
      l imps in
...
let find_appl_head_data (_,_,_,(_,impls)) = function
  | RRef (_,ref) as x ->
      x,implicits_of_global ref,find_arguments_scope ref,[]
  | x -> x,[],[],[]
...
let intern_inductive r =
  ...
  | SyntacticDef sp ->
	...
	| RApp (_,RRef(_,IndRef ind),l,imps)
...
let interp_reference vars r =
  let r,_,_,_ =
    intern_reference (Idset.empty,None,[]) (vars,[],[],([],[])) r 
  in r
...
let subst_cases_pattern loc alias intern subst scopes a =
  ...
  | AVar id ->
	...
	with Not_found -> 
	  if id = ldots_var then [], [[], PatVar (loc,Name id,Term.Expl)] else
...
let rec patt_of_rawterm loc cstr =
  match cstr with
	...
	if !dump then add_glob loc x; 
 	(c,[])
    | RApp (_,RApp(_,h,l1,imps1),l2,imps2) ->
        patt_of_rawterm loc (RApp(loc,h,l1@l2,imps1@imps2))
    | RApp (_,RRef(_,(ConstructRef c as x)),pl,imps) ->
        if !dump then add_glob loc x;
...
let rec intern_cases_pattern genv scopes (ids,subst as aliases) tmp_scope = 
  ...
  | CPatDelimiters (loc, key, e) ->
      intern_cases_pattern genv (find_delimiters_scope loc key::scopes)
        aliases None e
  | CPatAtom (loc, Some head, imp) ->
      (match maybe_constructor head with
	 | ConstrPat (c,args) ->
             if imp=Term.Impl then
               user_err_loc(loc, "intern_cases_pattern",
                 str "cannot do pattern-matching on an implicit arg");
	 ...
	 | VarPat id ->
	     ...
	     (ids,[subst, PatVar (loc,alias_of (ids,subst),imp)]))
  | CPatAtom (loc, None,imp) ->
      (ids,[subst, PatVar (loc,alias_of aliases,imp)])
  | CPatOr (loc, pl) ->
      assert (pl <> []);
      let pl' = 
...
let get_implicit_name n imps =
  Some (fst(Impargs.name_of_implicit (List.nth imps (n-1))))
...
let exists_implicit_name id =
  List.exists
    (fun imp -> is_status_implicit imp & id = fst(name_of_implicit imp))
...
let internalise sigma globalenv env allow_patvar lvar c =
  let rec intern (ids,tmp_scope,scopes as env) = function
	...
	(match intern_impargs c env imp subscopes l with
          | [],_ -> c
          | l,imps -> RApp (constr_loc x, c, l,imps))
    ...
    | CArrow (loc,imp,c1,c2) ->
        RProd (loc, Anonymous, imp, intern_type env c1, intern_type env c2)
    | CProdN (loc,[],c2) ->
        intern_type env c2
    ...
    | CLetIn (loc,(_,na),imp,c1,c2) ->
	RLetIn (loc, na, imp, intern (reset_tmp_scope env) c1,
	...
	check_projection isproj (List.length args) f;
        let (args,imps) = intern_args env args_scopes args in
	...
	let args,imps = 
	  intern_impargs
            c env impargs args_scopes (merge_impargs l args) in
	check_projection isproj (List.length args) c;
	(match c with 
          (* Now compact "(f args') args" *)
	  | RApp (loc', f', args',imps') ->
              RApp (join_loc loc' loc, f',args'@args,imps'@imps)
	  | _ -> RApp (loc, c, args, imps))
	...
	let env' = reset_tmp_scope env in
        ...
        let nal = List.map(fun((_,na),imp) -> (na,imp)) nal in
        let ids = List.map fst nal in
        RLetTuple (loc, nal, (na', p'), b',
                   intern (List.fold_left (push_name_env lvar) env ids) c)
  ...
  and intern_local_binder ((ids,ts,sc as env),bl) = function
	...
	let ((loc,na),_) = List.hd nal in
	(* TODO: fail if several names with different implicit types *)
	let ty = locate_if_isevar loc na (intern_type env ty) in
        List.fold_left
          (fun ((ids,ts,sc),bl) ((_,na),imp) ->
            ((name_fold Idset.add na ids,ts,sc), (na,imp,None,ty)::bl))
          (env,bl) nal
    | LocalRawDef((loc,na),imp,def) ->
        ((name_fold Idset.add na ids,ts,sc),
         (na,imp,Some(intern env def),RHole(loc,Evd.BinderType na))::bl)
  ...
  and intern_case_item (vars,_,scopes as env) (tm,(na,t)) =
	...
	let loc,ind,l,imps = match t with
	    | RRef (loc,IndRef ind) -> (loc,ind,[],[])
	    | RApp (loc,RRef (_,IndRef ind),l,imps) -> (loc,ind,l,imps)
	...
	let parnal,realnal = list_chop nparams nal in
        let _, rimps = list_chop nparams imps in
        let realnal' = List.combine realnal rimps in
  ...
  and iterate_prod loc2 env ty body = function
	...
	RProd (join_loc loc1 loc2, na, imp, ty, body)
  ...
  and iterate_lam loc2 env ty body = function
	...
	let ty = locate_if_isevar loc1 na (intern_type env ty) in
	RLambda (join_loc loc1 loc2, na, imp, ty, body)
    | [] -> intern env body
  ...
  and intern_impargs c env l subscopes args =
    ...
    let rec aux n impl subscopes eargs rargs =
	  ...
	  begin try 
	    let id,_ = name_of_implicit imp in
	    let (_,(a,impa)) = List.assoc id eargs in
	    let eargs' = List.remove_assoc id eargs in
            let (args,imps) = aux (n+1) impl' subscopes' eargs' rargs in
	    intern enva a :: args, impa::imps
	  with Not_found ->
	  if rargs=[] & eargs=[] & not (maximal_insertion_of imp) then
	    ...
	    [],[]
            ...
            let (args,imps) = aux (n+1) impl' subscopes' eargs rargs in
	    RHole (set_hole_implicit (n,get_implicit_name n l) c) :: args,
            Term.Expl::imps
	     
	  end
      | (imp::impl', (a,impa)::rargs') ->
          let (args,imps) = aux (n+1) impl' subscopes' eargs rargs' in
	  (intern enva a :: args, impa::imps)
      | (imp::impl', []) -> 
	  if eargs <> [] then 
  ...
  and intern_args env subscopes = function
    ...
    | (a,imp)::args ->
        let (enva,subscopes) = apply_scope_env env subscopes in
        let (args,imps) = intern_args env subscopes args in
        (intern enva a :: args, imp:: imps)
...
let interp_context sigma env params = 
  List.fold_left
    (fun (env,params) d -> match d with
      | LocalRawAssum ([(_,na),imp],(CHole _ as t)) ->
	  let t = interp_binder sigma env na t in
	  let d = (na,imp,None,t) in
	  (push_rel d env, d::params)
      | LocalRawAssum (nal,t) ->
	  let t = interp_type sigma env t in
	  let ctx =
            list_map_i
              (fun i ((_,na),imp) -> (na,imp,None,lift i t)) 0 nal in
	  let ctx = List.rev ctx in
	  (push_rel_context ctx env, ctx@params)
      | LocalRawDef ((_,na),imp,c) ->
	  let c = interp_constr_judgment sigma env c in
	  let d = (na, imp,Some c.uj_val, c.uj_type) in
	  (push_rel d env,d::params))
    (env,[]) params
