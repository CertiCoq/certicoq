...
type raw_context = (binder_type*Term.impl*rawconstr) list
...
let compose_raw_context = 
  let compose_binder  (bt,imp,t) acc =
    match bt with 
      | Lambda n -> mkRLambda(n,imp,t,acc)
      | Prod n -> mkRProd(n,imp,t,acc)
      | LetIn n -> mkRLetIn(n,imp,t,acc)
  in
...
let combine_args imp arg args = 
  let (argsl,imps) = args.value in
  {
    ...
    *)
    value = (arg.value::argsl,imp::imps);
  }
...
let rec replace_var_by_term_in_binder x_id term = function
  | [] -> []
  | (bt,imp,t)::l -> 
      (bt,imp,replace_var_by_term x_id term t)::
...
let apply_args ctxt body args imps = 
  ...
  in
  let rec do_apply avoid ctxt body args imps = 
    match ctxt,args,imps with  
      | (_,[],_::_)|(_,_::_,[]) -> assert false
      | _,[],[] -> (* No more args *)
	  (ctxt,body)
      | [],_,_ -> (* no more fun *)
	  let f,args',imps' = raw_decompose_app body in
	  (ctxt,mkRApp(f,args'@args,imps'@imps))
      | (Lambda Anonymous,imp,t)::ctxt',arg::args',_::imps' -> 
	  do_apply avoid ctxt' body args' imps'
      | (Lambda (Name id),imp,t)::ctxt',arg::args',_::imps' -> 
	  ...
	  do_apply avoid new_ctxt' new_body args' imps'
      | (bt,imp,t)::ctxt',_,_ -> 
	  ...
	  let new_ctxt',new_body = 
	    do_apply new_avoid new_ctxt' new_body args imps
	  in
	  (new_bt,imp,t)::new_ctxt',new_body
  in 
  do_apply []  ctxt body args imps


let combine_app f args = 
  let (argsl,imps) = args.value in
  let new_ctxt,new_value = apply_args f.context f.value argsl imps in 
  { 
...
let combine_lam n imp t b = 
  {
    context = []; 
    value = mkRLambda(n, imp, compose_raw_context t.context t.value, 
		      compose_raw_context b.context b.value )
  }



let combine_prod n imp t b = 
  { context = t.context@((Prod n,imp,t.value)::b.context); value = b.value}

let combine_letin n imp t b = 
  { context = t.context@((LetIn n,imp,t.value)::b.context); value = b.value}
...
let build_constructors_of_type ind' argl = 
  ...
  Array.mapi (fun i _ ->
		...
		in
                let imps =
                  let (cargs,_) = Sign.decompose_prod_assum
                    (ind.Declarations.mind_nf_lc.(i)) in
                  list_skipn npar (Sign.imps_from_rel_context cargs) in
		let pat_as_term = 
		  mkRApp(mkRRef (ConstructRef(ind',i+1)),argl,imps)
		in
...
let rec find_type_of nb b = 
  let f,_,_ = raw_decompose_app b in 
  match f with 
    | RRef(_,ref) -> 
	begin 
	  let ind_type  = 
	    match ref with 
		  ...
		  let ret_type,_,_ = decompose_app ret_type in 
...
let raw_push_named (na,imp,raw_value,raw_typ) env = 
  match na with 
    | Anonymous -> env 
    | Name id -> 
	...
	Environ.push_named (id,imp,value,typ) env
...
let add_pat_variables pat typ env : Environ.env = 
  let rec add_pat_variables env pat typ  : Environ.env = 
    ...
    match pat with 
      | PatVar(_,na,imp) -> Environ.push_rel (na,imp,None,typ) env 
      | PatCstr(_,c,patl,na) -> 
	  let Inductiveops.IndType(indf,indargs,imps) = 
	  ...
	  let cs_args_types :types list = List.map (fun (_,_,_,t) -> t) constructor.Inductiveops.cs_args in 
  ...
  let res =
	...
	(fun (na,imp,v,t) (env,ctxt) ->
	 match na with
	   | Anonymous -> assert false
	   | Name id ->
	       let new_t =  substl ctxt t in
	       let new_v = option_map (substl ctxt) v in
	       observe (str "for variable " ++ Ppconstr.pr_impl imp (Ppconstr.pr_id id) ++  fnl () ++
	       ...
	       (Environ.push_named (id,imp,new_v,new_t) env,mkVar id::ctxt)
      )
...
let rec pattern_to_term_and_type env typ  = function
  | PatVar(loc,Anonymous,_) -> assert false
  | PatVar(loc,Name id,_) ->
	mkRVar id
  | PatCstr(loc,constr,patternl,_) ->
	...
	with Not_found -> assert false 
      ...
      let cs_args_types :types list = List.map (fun (_,_,_,t) -> t) constructor.Inductiveops.cs_args in 
      let _,cstl,_ = Inductiveops.dest_ind_family indf in 
	...
	List.map2 (pattern_to_term_and_type env)  (List.rev cs_args_types)  patternl
      ...
      mkRApp(mkRRef(Libnames.ConstructRef constr),
	     implicit_args@patl_as_term,
             List.map(fun _->Impl)implicit_args@
             Sign.imps_from_rel_context constructor.Inductiveops.cs_args
	    )
...
let rec build_entry_lc env funnames avoid rt  : rawconstr build_entry_return = 
  ...
  match rt with 
	...
	mk_result [] rt avoid 
    | RApp(_,_,_,_) ->
	let f,args,imps = raw_decompose_app rt in
	let args_res : (rawconstr list * Term.impl list) build_entry_return =
	  List.fold_right2 (* create the arguments lists of constructors and combine them *)
	    (fun imp arg ctxt_argsl ->
	       let arg_res = build_entry_lc env funnames ctxt_argsl.to_avoid arg in
	       combine_results (combine_args imp) arg_res ctxt_argsl
	    )
	    imps args
	    (mk_result [] ([],[]) avoid)
	in
	  ...
	  match f with
		...
		*)
		...
		let new_result = 
		  List.map 
		    (fun arg_res -> 
		       let new_hyps = 
			 [Prod (Name res),Expl,res_raw_type;
			  Prod Anonymous,Expl,
                          mkRApp(res_rt,(mkRVar id)::fst arg_res.value,
                                 Expl::snd arg_res.value)]
		       in
		...
		*)
		{
		  args_res with 
		    ...
		    List.map 
		      (fun args_res -> 
			 {args_res with value =
                          mkRApp(f,fst args_res.value,snd args_res.value)})
		      args_res.result
		}
		...
		*)
		...
		build_entry_lc 
		  ...
		  funnames 
		  avoid
		  (mkRLetIn(new_n,imp,t,mkRApp(new_b,args,imps)))
		...
		*)
		build_entry_lc env funnames avoid (mkRApp(b,args,imps))
	...
	*)
	...
	in
	let new_env = raw_push_named (new_n,imp,None,t) env in
	let b_res = build_entry_lc new_env funnames avoid b in
	combine_results (combine_lam new_n imp) t_res b_res
    | RProd(_,n,imp,t,b) ->
	(* we first compute the list of constructor 
	   corresponding to the body of the function, 
	   then the one corresponding to the type 
	...
	*)
	let t_res = build_entry_lc env funnames avoid t in
	let new_env = raw_push_named (n,imp,None,t) env in
	let b_res = build_entry_lc new_env funnames avoid b in
	combine_results (combine_prod n imp) t_res b_res
    | RLetIn(_,n,imp,v,b) ->
	(* we first compute the list of constructor 
	   corresponding to the body of the function, 
	...
	*)
	...
	let new_env = 
	  match n with
	      Anonymous -> env
	    | Name id -> Environ.push_named (id,imp,Some v_as_constr,v_type) env 
	in
	let b_res = build_entry_lc new_env funnames avoid b in
	combine_results (combine_letin n imp) v_res b_res
    | RCases(_,_,el,brl) -> 
	...
	*)
	...
	begin
          let (nal,imps) = List.split nal in
	...
	*)
	let case_resl = 
	    ...
	    (fun (case_arg,_) ctxt_argsl ->
	       ...
	       combine_results (combine_args Expl) arg_res ctxt_argsl
	    )
	    el
	      (mk_result [] ([],[]) avoid)
	in
		 ...
		 let env_with_pat_ids = add_pat_variables pat typ new_env in 
		   ...
		   (fun id acc -> 		 
			...
			Detyping.detype false [] (Termops.names_of_rel_context env_with_pat_ids) typ_of_id
		      in
		      mkRProd (Name id,Expl,raw_typ_of_id,acc))
		   pat_ids
	...
	*)
	  ...
	  (List.flatten
	    (
		 ...
		 List.fold_right 
		   ...
		   idl
		   [(Prod Anonymous,Expl,raw_make_eq ~typ pat_as_term e)]
	      )
	      patl
	      (fst matched_expr.value)
	      types
	  )
	  )
	  @
	  (if List.exists (function (unifl,_) ->
	   ...
	   then 
	     ...
	     [(Prod Anonymous,Expl,make_discr pats_as_constr i  )]
	   else 
	     []
	  )
	in
...
*)
let rec rebuild_cons nb_args relname args imps crossed_types depth rt = 
  match rt with 
    | RProd(_,n,imp,t,b) -> 
	...
	begin
	  match t with 
	    | RApp(_,(RVar(_,res_id) as res_rt),args',imps') when is_res res_id ->
		begin
		  match args',imps' with 
		    | (RVar(_,this_relname))::args',imp::imps' -> 
			let new_b,id_to_exclude =  
			  rebuild_cons 
			    nb_args relname
			    args imps new_crossed_types
			    (depth + 1) b
			in 
			...
			let new_t = 
			  mkRApp(mkRVar(mk_rel_id this_relname),args'@[res_rt],imps'@[Expl]) 
			in mkRProd(n,imp,new_t,new_b),
			Idset.filter not_free_in_t id_to_exclude
		    | _ -> (* the first args is the name of the function! *)
			assert false 
		end
	    | RApp(_,RRef(_,eq_as_ref),[_;RVar(_,id);rt],_) 
		...
		let new_b,id_to_exclude =  
		  rebuild_cons 
		    nb_args relname
		    new_args imps new_crossed_types
		    (depth + 1) subst_b
		in 
		mkRProd(n,imp,t,new_b),id_to_exclude
		...
		match n with
		  | Name id when Idset.mem id id_to_exclude && depth >= nb_args ->
			...
			(Idset.filter not_free_in_t id_to_exclude)
		  | _ -> mkRProd(n,imp,t,new_b),Idset.filter not_free_in_t id_to_exclude
	end
    | RLambda(_,n,imp,t,b) ->
	begin
	  let not_free_in_t id = not (is_free_in id t) in
	  ...
	  match n with
		...
		let new_b,id_to_exclude = 
		  rebuild_cons 
		    nb_args relname
		    (args@[mkRVar id]) (imps@[Expl]) new_crossed_types
		    (depth + 1 ) b 
		in
		...
		then 
		  ...
		  RProd(dummy_loc,n,imp,t,new_b),Idset.filter not_free_in_t id_to_exclude
	...
	begin
	  ...
	  let new_b,id_to_exclude = 
	    rebuild_cons 
	      nb_args relname
	      args imps (t::crossed_types)
	      (depth + 1 ) b in
	  match n with 
	    | Name id when Idset.mem id id_to_exclude && depth >= nb_args  -> 
		new_b,Idset.remove id (Idset.filter not_free_in_t id_to_exclude)
	    | _ -> RLetIn(dummy_loc,n,imp,t,new_b),
		Idset.filter not_free_in_t id_to_exclude
	end
...
(* 	    | _ -> *)
    ...
    | _ -> mkRApp(mkRVar  relname,args@[rt],imps@[Expl]),Idset.empty


(* debuging wrapper *)
let rebuild_cons nb_args relname args imps crossed_types rt = 
(*   observennl  (str "rebuild_cons : rt := "++ pr_rawconstr rt ++  *)
(* 		 str "nb_args := " ++ str (string_of_int nb_args)); *)
  let res = 
    rebuild_cons nb_args relname args imps crossed_types 0 rt 
  in
...
let rec compute_cst_params relnames params = function 
  ...
  | RApp(_,RVar(_,relname'),rtl,_) when Idset.mem relname' relnames ->
      compute_cst_params_from_app [] (params,rtl)
  | RApp(_,f,args,_) -> 
      List.fold_left (compute_cst_params relnames) params (f::args)
  | RLambda(_,_,_,t,b) | RProd(_,_,_,t,b) | RLetIn(_,_,_,t,b) | RLetTuple(_,_,_,t,b) -> 
...
and compute_cst_params_from_app acc (params,rtl) = 
  match params,rtl with 
    ...
    | ((Name id,_,_,is_defined) as param)::params',(RVar(_,id'))::rtl' 
	when id_ord id id' == 0 && not is_defined -> 
	compute_cst_params_from_app (param::acc) (params',rtl')
    | _  -> List.rev acc 
   
let compute_params_name relnames (args : (Names.name * impl * Rawterm.rawconstr * bool) list array) csts =  
  let rels_params = 
    Array.mapi 
      (fun i args -> 
	 List.fold_left 
	   (fun params (_,_,cst) -> compute_cst_params relnames params cst) 
	   args
	   csts.(i)
      )
      args
  ...
  let _ = 
    try 
      list_iter_i
	(fun i ((n,imp,nt,is_defined) as param) -> 
	   if array_for_all 
	     (fun l -> 
		let (n',imp',nt',is_defined') = List.nth l i in 
		n = n' && imp=imp' && Topconstr.eq_rawconstr nt nt' && is_defined = is_defined')
	     rels_params
	   then 
	     l := param::!l
	) 
...
let rec rebuild_return_type rt = 
  match rt with 
    ...
    | Topconstr.CArrow(loc,imp,t,t') -> 
	Topconstr.CArrow(loc,imp,t,rebuild_return_type t')
    | Topconstr.CLetIn(loc,na,imp,t,t') -> 
	Topconstr.CLetIn(loc,na,imp,t,rebuild_return_type t') 
    | _ -> Topconstr.CArrow(dummy_loc,Expl,rt,Topconstr.CSort(dummy_loc,RType None))


let do_build_inductive 
    funnames (funsargs: (Names.name * impl * rawconstr * bool) list list)  
    returned_types 
    (rtl:rawconstr list) =
...
(*   Pp.msgnl (prlist_with_sep fnl Printer.pr_rawconstr rtl); *)
  ...
  let env = 
    Array.fold_right 
      (fun id env -> 
	 Environ.push_named (id,Expl,None,Typing.type_of env Evd.empty (Tacinterp.constr_of_id env id))  env
      )
      funnames 
      (Global.env ())
  in      
 ...
 let next_constructor_id = ref (-1) in 
  ...
  in
  let rel_constructors i rt : (identifier*impl*rawconstr) list = 
    next_constructor_id := (-1);
    List.map (fun constr -> (mk_constructor_id i),Expl,constr) (constr i rt)
  in
  ...
  let rel_constructors = (* Taking into account the parameters in constructors *)
    Array.map (List.map 
    (fun (id,imp,rt) -> (id,imp,snd (chop_rprod_n nrel_params rt))))
    rel_constructors
  in
  let rel_arity i funargs =  (* Reduilding arities (with parameters) *)
    let rel_first_args :(Names.name * impl * Rawterm.rawconstr * bool ) list  = 
      (snd (list_chop nrel_params funargs))
    in 
    List.fold_right
      (fun (n,imp,t,is_defined) acc -> 
	 if is_defined
	 then 
	   Topconstr.CLetIn(dummy_loc,(dummy_loc, n),imp,Constrextern.extern_rawconstr Idset.empty t,
			    acc)
	 else
	   Topconstr.CProdN
	   (dummy_loc,
	    [[((dummy_loc,n),imp)],Constrextern.extern_rawconstr Idset.empty t],
	    acc
	   )
      )
      rel_first_args
  ...
  *)
  ...
  let rel_params =  
    List.map 
      (fun (n,imp,t,is_defined) -> 
	 if is_defined 
	 then
	   Topconstr.LocalRawDef((dummy_loc,n), imp, Constrextern.extern_rawconstr Idset.empty t)
	 else
