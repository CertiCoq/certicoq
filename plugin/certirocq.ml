...
module CompileFunctor (CI : CompilerInterface) = struct
  ...
  let template_ocaml id filename name =
    Printf.sprintf "external %s : unit -> Obj.t = \"%s\"\nlet _ = Certirocq_plugin.Certirocq.register_certirocq_run \"%s\" \"%s\" (Obj.magic %s)" name name id filename name
  ...
  let certirocq_eval_named ~opaque_access opts env sigma c global_id imports =
    ...
    let dontlink = "str,unix,dynlink,threads,zarith,rocq-runtime,rocq-runtime.plugins.ltac,rocq-runtime.interp,rocq-certirocq.plugin" in
