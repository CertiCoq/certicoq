...
let coq_rename_vars env vars =
  ...
  List.fold_right
    (fun (na,imp,t) (newvars, newenv) -> 
       let id = next_name_away na !avoid in
       avoid := id :: !avoid;
       id :: newvars, Environ.push_named (id, imp, None, t) newenv)
    vars ([],env)
...
let decomp_type_lambdas env t =
  let rec loop vars t = match kind_of_term t with
    | Lambda (n, imp, a, t) when is_Set a -> 
	loop ((n,imp, a) :: vars) t
    | _ -> 
	let vars, env = coq_rename_vars env vars in
	let t = substl (List.map mkVar vars) t in
...
let decompose_arrows = 
  let rec arrows_rec l c = match kind_of_term c with
    | Prod (_,Expl,t,c) when not (dependent (mkRel 1) c) ->
        arrows_rec (t :: l) c
...
let rec eta_expanse t vars env i =
  ...
  if i = 0 then
    ...
    match kind_of_term (Typing.type_of env Evd.empty t) with
      | Prod (n, Expl, a, b) when not (dependent (mkRel 1) b) ->
	  let avoid = ids_of_named_context (Environ.named_context env) in
	  let id = next_name_away n avoid in
	  let env' = Environ.push_named (id, Expl,None, a) env in
	  let t' = mkApp (t, [| mkVar id |],[|Expl|]) in
...
let rec tr_positive p = match kind_of_term p with
  | Term.Construct _ when p = Lazy.force coq_xH ->
      Cst 1
  | Term.App (f, [|a|],_) when f = Lazy.force coq_xI ->
      Plus (Mult (Cst 2, tr_positive a), Cst 1)
  | Term.App (f, [|a|],_) when f = Lazy.force coq_xO ->
...
let rec tr_arith_constant t = match kind_of_term t with
  | Term.Construct _ when t = Lazy.force coq_Z0 ->
      Cst 0
  | Term.App (f, [|a|],_) when f = Lazy.force coq_Zpos ->
      tr_positive a
  | Term.App (f, [|a|],_) when f = Lazy.force coq_Zneg ->
...
and tr_term tv bv env t = match kind_of_term t with
  | Term.App (f, [|a;b|],_) when f = Lazy.force coq_Zplus -> 
      Plus (tr_term tv bv env a, tr_term tv bv env b)
  | Term.App (f, [|a;b|],_) when f = Lazy.force coq_Zminus -> 
      Moins (tr_term tv bv env a, tr_term tv bv env b)
  | Term.App (f, [|a;b|],_) when f = Lazy.force coq_Zmult -> 
      Mult (tr_term tv bv env a, tr_term tv bv env b)
  | Term.App (f, [|a;b|],_) when f = Lazy.force coq_Zdiv -> 
	...
	with 
	  ...
	  | NotFO -> (* we need to abstract some part of (f cl) *)
		...
		| x :: l as args ->
		    ...
		    with NotFO ->
		      abstract (applist (app, [x],[Expl])) l
		    end
	      in
	      let app,l = match cl with 
		| x :: l -> applist (f, [x],[Expl]), l | [] -> raise NotFO
	      in
	      abstract app l
	end

and quantifiers n a b tv bv env =
  let vars, env = coq_rename_vars env [n,Expl,a] in
...
and tr_formula tv bv env f =
  let c, args,_ = decompose_app f in
