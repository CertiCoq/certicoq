...
let judge_of_relative env n = 
  try
    let (_,_,_,typ) = lookup_rel n env in
    { uj_val  = mkRel n;
...
let extract_context_levels env =
  List.fold_left
    (fun l (_,_,b,p) -> if b=None then extract_level env p::l else l) []
...
let judge_of_abstraction env name imp var j =
  let t = mkLambda (name, imp, var.utj_val, j.uj_val) in
  if imp=Impl && not(extract_closed j.uj_val) then
    error_implicit_lambda env t;
  { uj_val = t;
    uj_type = mkProd (name, imp, var.utj_val, j.uj_type) }

(* Type of let-in. *)

let judge_of_letin env name imp defj typj j =
  let t = mkLetIn (name, imp, defj.uj_val, typj.utj_val, j.uj_val) in
  if imp=Impl && not(extract_closed j.uj_val) then
    error_implicit_lambda env t;
  { uj_val = t ;
...
let judge_of_apply env funj argjv imps0 =
  let imps = Array.map (fun _ -> Expl) argjv in
  let rec apply_rec n typ cst = function
    | [] -> 
        assert (imps=imps0);
	{ uj_val  = mkApp (j_val funj, Array.map j_val argjv,imps);
          uj_type = typ },
	cst
    | hj::restjl ->
...
*)
let judge_of_product env name imp t1 t2 =
  let s = sort_of_product env t1.utj_type t2.utj_type in
  { uj_val = mkProd (name, imp, t1.utj_val, t2.utj_val);
    uj_type = mkSort s }
...
let rec execute env cstr cu =
  match kind_of_term cstr with
    ...
    | App (f,args,imps) ->
        (* TODO: check infered imps is the same as in the input *)
	...
	let (j,cu2) =
	  match kind_of_term f with
		...
		execute env f cu1
	in
        let (appj,_ as j) = judge_of_apply env j jl imps in
	univ_combinator cu2 j
	    
    | Lambda (name,imp,c1,c2) -> 
        let (varj,cu1) = execute_type env c1 cu in
	let env1 = push_rel (name,imp,None,varj.utj_val) env in
        let (j',cu2) = execute env1 c2 cu1 in 
        (judge_of_abstraction env name imp varj j', cu2)
	  
    | Prod (name,imp,c1,c2) ->
        let (varj,cu1) = execute_type env c1 cu in
	let env1 = push_rel (name,Expl,None,varj.utj_val) env in
        let (varj',cu2) = execute_type env1 c2 cu1 in
	(judge_of_product env name imp varj varj', cu2)

    | LetIn (name,imp,c1,c2,c3) ->
	  ...
	  univ_combinator cu2 (judge_of_cast env j1 DEFAULTcast j2) in
        let env1 = push_rel (name,imp,Some j1.uj_val,j2.utj_val) env in
        let (j',cu4) = execute env1 c3 cu3 in
        (judge_of_letin env name imp j1 j2 j', cu4)
...
let infer_local_decl env id = function
  | LocalDef (c,imp) -> 
      ...
      if imp = Expl && not (noccur_implicit_env env j.uj_val) then
        error "explicit local declaration uses an implicit variable";
      (Name id, imp, Some j.uj_val, j.uj_type), cst
  | LocalAssum (c,imp) ->
      let (j,cst) = infer env c in
      (Name id, imp, None, assumption_of_judgment env j), cst
