...
(* $Id: ppconstr.ml 9918 2007-06-30 11:01:05Z herbelin $ *)      
...
let decode_constrlist_value = function
  | CAppExpl (_,_,l) -> List.map fst l
  | CApp (_,_,l) -> List.map(fun((a,_),_)->a) l
...
let pr_implc pr sep inherited (a,imp) =
  if imp=Impl then str"["++pr mt ltop a++str"]"
  else pr sep inherited a

let pr_expl_args pr (a,expl) =
  match expl with
  | None -> pr_implc (fun _ -> pr) mt (lapp,L) a
  | Some (_,ExplByPos n) ->
      anomaly("Explicitation by position not implemented")
  | Some (_,ExplByName id) -> 
      str "(" ++ pr_id id ++ str ":=" ++
      pr_implc (fun _ -> pr) mt ltop a ++ str ")"
...
let green() = stras(0,"\027[1;32m")
(*
    SETCOLOR_FAILURE="echo -en \\033[1;31m"
    SETCOLOR_WARNING="echo -en \\033[1;33m"
*)
let black() = stras(0,"\027[0;39m")


let pr_impl imp pp =
  if imp=Impl then str"["++pp++str"]" else pp

let pr_name_impl (na,imp) = pr_impl imp (pr_lname na)
...
let rec pr_patt sep inh p =
  ...
  | CPatAtom (_,None,imp) -> pr_impl imp (str "_"), latom
  | CPatAtom (_,Some r,imp) -> pr_impl imp (pr_reference r), latom
  | CPatOr (_,pl) ->
...
let pr_binder many pr (nal,t) =
  match t with
  | CHole _ -> prlist_with_sep spc pr_name_impl nal
  | _ ->
      if List.for_all(fun (_,imp)->imp=Impl) nal then
        hov 1 (str"["++prlist_with_sep spc pr_name
                         (List.map (fun((_,na),_)->na) nal) ++
               str" : " ++ pr t ++ str"]")
      else
        let s = prlist_with_sep spc pr_name_impl nal ++ str" : " ++ pr t in
        hov 1 (if many then surround s else s)

let pr_binder_among_many pr_c = function
  | LocalRawAssum (nal,t) ->
      pr_binder true pr_c (nal,t)
  | LocalRawDef (na,imp,c) ->
      let c,topt = match c with
        | CCast(_,c, CastConv (_,t)) -> c, t
        | _ -> c, CHole dummy_loc in
      hov 1 (surround
        (pr_name_impl (na,imp) ++ pr_opt_type pr_c topt ++
         str":=" ++ cut() ++ pr_c c))
...
let rename na (na',imp) t c =
  match (na,na') with
    | (_,Name id), (_,Name id') -> ((na',imp),t,replace_vars_constr_expr [id,id'] c)
    | (_,Name id), (_,Anonymous) -> ((na,imp),t,c)
    | _ -> ((na',imp),t,c)
  
let split_product na' = function
  | CArrow (loc,_,t,c) -> (na',t,c)
  | CProdN (loc,[[(na,_)],t],c) -> rename na na' t c
  | CProdN (loc,([(na,_)],t)::bl,c) -> rename na na' t (CProdN(loc,bl,c))
  | CProdN (loc,((na,_)::nal,t)::bl,c) ->
      rename na na' t (CProdN(loc,(nal,t)::bl,c))
  | _ -> anomaly "ill-formed fixpoint body"

let merge_binders ((na1,imp),ty1) cofun ((na2,_),ty2) codom =
  let na =
  ...
  let ty =
    match ty1, ty2 with
      ...
      | _ ->
          ...
          ty2 in
  (LocalRawAssum ([na,imp],ty), codom)
            
let rec strip_domain bvar cofun c =
  match c with
    | CArrow(loc,imp,a,b) ->
        merge_binders bvar cofun (((dummy_loc,Anonymous),imp),a) b
    | CProdN(loc,[([na],ty)],c') ->
...
let rec split_fix n typ def =
  if n = 0 then ([],typ,def)
    ...
    let ((na,imp),t,typ) = split_product na typ in
    let (bl,typ,def) = split_fix (n-1) typ def in
    (LocalRawAssum ([na,imp],t)::bl,typ,def)
    ...
    let ids = names_of_local_assums bl in
	  ...
	  CStructRec ->
	    if List.length ids > 1 && n <> None then 
	      spc() ++ str "{struct " ++ pr_name (snd (fst (List.nth ids (out_some n)))) ++ str"}"
	    else mt() 
	| CWfRec c ->
	    spc () ++ str "{wf " ++ pr lsimple c ++ pr_name (snd (fst (List.nth ids (out_some n)))) ++ str"}"
	| CMeasureRec c ->
	    spc () ++ str "{measure " ++ pr lsimple c ++ pr_name (snd (fst (List.nth ids (out_some n)))) ++ str"}"
  in
...
let tm_clash = function
  | ((CRef (Ident (_,id)),_), Some (CApp (_,_,nal))) 
      when List.exists
        (function (CRef (Ident (_,id')),_),_ -> id=id' | _ -> false)
	nal
      -> Some id
  | ((CRef (Ident (_,id)),_), Some (CAppExpl (_,_,nal))) 
      when List.exists
        (function (CRef (Ident (_,id')),_) -> id=id' | _ -> false)
	nal
      -> Some id
  | _ -> None
...
let pr_appexpl pr f l =
	...
	prlist (fun (c,imp) ->
          pr_sep_com spc 
            (fun c -> pr_implc (fun _->pr) mt (lapp,L) (c,imp)) c) l)
...
let rec pr sep inherited a =
  ...
  | CCoFix (_,id,cofix) ->
      ...
      lfix
  | CArrow (_,imp,a,b) ->
      let dom = pr_implc pr mt (larrow,L) (a,imp) in
      hov 0 (dom ++ str " ->" ++ pr (fun () ->brk(1,0)) (-larrow,E) b),
      larrow
  | CProdN _ ->
  ...
  | CLambdaN _ ->
      ...
      hov 0 (
        ...
        str " =>" ++ pr spc ltop a),
      llambda
  | CLetIn (_,(_,Name x),Expl,(CFix(_,(_,x'),[_])|CCoFix(_,(_,x'),[_]) as fx), b)
      when x=x' ->
      hv 0 (
  ...
  | CLetTuple (_,nal,(na,po),c,b) ->
      hv 0 (
        str "let " ++
	hov 0 (str "(" ++
               prlist_with_sep sep_v pr_name_impl nal ++
               str ")" ++
...
let rec strip_context n iscast t =
  if n = 0 then
	...
	LocalRawAssum ([(loc,Anonymous),imp],t) :: bl', c 
    | CCast (_,c,_) -> strip_context n false c
    | CLetIn (_,na,imp,b,c) -> 
	let bl', c = strip_context (n-1) iscast c in
	LocalRawDef (na,imp,b) :: bl', c
