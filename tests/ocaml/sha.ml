...
let rec zlength_aux acc = function
| Nil -> acc
| Cons (_, l0) -> zlength_aux (Z.succ acc) l0
...
let rec bytelist_to_intlist = function
| Nil -> Nil
| Cons (h1, l) ->
  (match l with
   | Nil -> Nil
     ...
     (match l0 with
      ...
      | Cons (h3, l1) ->
        (match l1 with
         | Nil -> Nil
         | Cons (h4, t) ->
           Cons ((bytes_to_Int h1 h2 h3 h4), (bytelist_to_intlist t)))))
...
let generate_and_pad msg =
  let n0 = zlength msg in
  app
    (bytelist_to_intlist
      (app msg
        (app (Cons ((Byte.repr (Zpos (XO (XO (XO (XO (XO (XO (XO XH))))))))),
          Nil))
          (repeat Byte.zero
            (Z.to_nat
              (Z.modulo (Z.opp (Z.add n0 (Zpos (XI (XO (XO XH)))))) (Zpos (XO
                (XO (XO (XO (XO (XO XH))))))))))))) (Cons
...
let rec w m t =
  match zlt t (Zpos (XO (XO (XO (XO XH))))) with
  | Left -> m t
  | Right ->
    Int.add
      (Int.add (sigma_3 (w m (Z.sub t (Zpos (XO XH)))))
        (w m (Z.sub t (Zpos (XI (XI XH))))))
      (Int.add (sigma_2 (w m (Z.sub t (Zpos (XI (XI (XI XH)))))))
        (w m (Z.sub t (Zpos (XO (XO (XO (XO XH))))))))
...
let rec round regs m t =
  match zlt t Z0 with
  | Left -> regs
  | Right ->
    rnd_function (round regs m (Z.sub t (Zpos XH))) (nthi k256 t) (w m t)

(** val hash_block : registers -> Int.int list -> registers **)

let hash_block r block =
  map2 Int.add r (round r (nthi block) (Zpos (XI (XI (XI (XI (XI XH)))))))

(** val hash_blocks : registers -> Int.int list -> registers **)

let rec hash_blocks r = function
| Nil -> r
| Cons (i, l) ->
  hash_blocks
    (hash_block r
      (firstn (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S
        O)))))))))))))))) (Cons (i, l))))
    (skipn (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S (S O))))))))))))))))
      (Cons (i, l)))
