...
(* $Id: inv.ml 9950 2007-07-06 14:00:59Z glondu $ *)
...
let make_inv_predicate env sigma indf realargs id status concl =
  ...
  let rec build_concl eqns n = function
	...
	in
        ...
        let eqn = applist (eq_term ,[eqnty;lhs;rhs],[Expl;Expl;Expl]) in 
	build_concl ((Anonymous,Expl,lift n eqn)::eqns) (n+1) restlist
  in
...
let projectAndApply thin id eqname names (depids:named_context) gls =
    ...
    match othin with
      | Some thin ->
		 ...
		 (onLastHyp (fun id ->
	       ...
	       names;
	     tclMAP (fun (id,_,_,_) gl ->
	       intro_move None (if thin then None else !first_eq) gl)
	       nodepids;
	     tclMAP (fun (id,_,_,_) -> tclTRY (clear [id])) depids]
      | None -> tclIDTAC
  in
  (tclTHENSEQ
    [tclDO neqns intro;
...
let raw_inversion inv_kind id status names gl =
  ...
  let (cut_concl,case_tac) =
    if status <> NoDep & (dependent c (pf_concl gl)) then 
      Reduction.beta_appvect elim_predicate
        (Array.of_list (realargs@[c])) (Array.of_list(raimps@[Expl])),
      case_then_using 
    else 
      Reduction.beta_appvect elim_predicate (Array.of_list realargs)
        (Array.of_list raimps),
      case_nodep_then_using 
  in
  (tclTHENS
     (true_cut Anonymous Expl cut_concl)
     [case_tac names 
        ...
        (fun id ->
           (tclTHEN
              (apply_term (mkVar id)
                ...
                (list_tabulate (fun _ -> Expl) neqns))
              reflexivity))])
  gl
