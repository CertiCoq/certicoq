...
let make_refl_eq type_of_t t  =
  ...
  mkApp(refl_equal_term,[|type_of_t;t|],[|Impl;Impl|])
...
let cut_replacing id imp t tac :tactic= 
  tclTHENS (cut imp t)
...
let is_trivial_eq t = 
  match kind_of_term t with 
    | App(f,[|_;t1;t2|],_) when eq_constr f (Lazy.force eq) -> 
	eq_constr t1 t2
    | _ -> false 


let rec incompatible_constructor_terms t1 t2 = 
  let c1,arg1,_ = decompose_app t1 
  and c2,arg2,_ = decompose_app t2 
  in 
  (not (eq_constr t1 t2)) &&
...
let prove_trivial_eq h_id context (type_of_term,term) = 
  ...
  tclTHENLIST
    [
	 ...
	 in
	 let context_hyps' = 
	   (mkApp(Lazy.force refl_equal,[|type_of_term;term|],[|Impl;Impl|]))::
	     (List.map mkVar context_hyps)
	 in
	 let to_refine = applist(mkVar h_id,List.rev context_hyps',
          List.map(fun _->Expl) context_hyps') in 
	 refine to_refine g
      )
    ]
...
let isAppConstruct t = 
  if isApp t 
  then let(f,_,_)=destApp t in isConstruct f
  else false 
...
(*       observe (str "compute_substitution : " ++ pr_lconstr t1 ++ str " === " ++ pr_lconstr t2); *)
	...
	begin 
      ...
      then 
	begin
	  let c1,args1,_ = destApp t1 
	  and c2,args2,_ = destApp t2 
	  in 
    ...
    let witness_fun = 
      mkLetIn(Anonymous,Expl,make_refl_eq t1_typ t1,t,
	       mkApp(mkVar hyp_id,Array.init old_context_length (fun i -> mkRel (old_context_length - i)),
                     Array.init old_context_length (fun _ -> Expl))
	      )
    in
    let new_type_of_hyp,ctxt_size,witness_fun = 
      list_fold_left_i 
	(fun i (end_of_type,ctxt_size,witness_fun) ((x',imp',b',t') as decl) -> 
	   try 
	     let witness = Intmap.find i sub in 
	     if b' <> None then anomaly "can not redefine a rel!";
	     (pop end_of_type,ctxt_size,mkLetIn(x',imp',witness,t',witness_fun))
	   with Not_found  -> 
	...
	(fun g ->
	   ...
	   let to_refine =
             applist(witness_fun,List.rev_map mkVar new_ids,List.map(fun _ -> Expl) new_ids) in 
	   refine to_refine g
	)
    in
    let simpl_eq_tac = 
...
let rewrite_until_var arg_num eq_ids : tactic =
  ...
  *)
  let test_var g = 
    let _,args,imps = destApp (pf_concl g) in 
...
let clean_hyp_with_heq ptes_infos eq_hyps hyp_id env sigma = 
  ...
  let rec scan_type  context type_of_hyp : tactic = 
    ...
    then 
      begin 
	let (x,imp,t_x,t') = destProd type_of_hyp in	
	...
	if is_property ptes_infos t_x actual_real_type_of_hyp then
	  begin
	    let pte,pte_args,_ =  (destApp t_x) in 
