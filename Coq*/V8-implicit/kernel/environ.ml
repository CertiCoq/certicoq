...
(* $Id: environ.ml 9911 2007-06-26 14:19:41Z letouzey $ *)
...
let map_named_val f (ctxt,ctxtv) = 
  let ctxt =
    List.map (fun (id,imp,body,typ) ->
      (id, imp, option_map f body, f typ)) ctxt in
  (ctxt,ctxtv)

let set_impl_named_val imp (ctxt,ctxtv) =
  (set_impl_named_context imp ctxt, ctxtv)
...
let named_type id env =
  let (_,_,_,t) = lookup_named id env in t

let named_body id env =
  let (_,_,b,_) = lookup_named id env in b
...
let noccur_implicit hyps body =
  let hyps = named_context_of_val hyps in
  extract_closed_gen (fun _ -> true) (is_expl_named hyps) body

let noccur_implicit_env env body =
  let rels = rel_context env in
  let hyps = named_context env in
  extract_closed_gen (is_expl_rel rels) (is_expl_named hyps) body
...
let remove_hyps ids check_context check_value (ctxt, vals) =
  let ctxt,vals,rmv = 
    List.fold_right2 (fun (id,_,_,_ as d) (id',v) (ctxt,vals,rmv) ->
...
let register =
  ...
  let constr_of_int31 =
	...
	in
	  mkApp(mkConstruct(ind, 1), array_of_int tag, Array.make 31 Expl)
  in
...
let rec needed_assumptions t env =   
 (* goes recursively into the terms to see if it depends on assumptions
  ...
  match kind_of_term t with
    ...
    | Cast (e1,_,e2) | Prod (_,_,e1,e2) | Lambda (_,_,e1,e2) -> 
                   (needed_assumptions e1 env)**(needed_assumptions e2 env)
    | LetIn (_,_,e1,e2,e3) ->(needed_assumptions e1 env)**
                           (needed_assumptions e2 env)**
                           (needed_assumptions e3 env)
    | App (e1, e_array,_) -> (needed_assumptions e1 env)**
