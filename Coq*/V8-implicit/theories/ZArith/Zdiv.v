...
(*i $Id: Zdiv.v 9983 2007-07-12 17:15:22Z barras $ i*)
...
Proof.
  ...
  rewrite Zabs_eq; 
   [ apply Zdiv_eucl_exist; apply impl_PI; assumption
   | apply impl_PI; assumption ].
  cut [(- b > 0)]; [ intro Hb'' | omega ].
  elim (Zdiv_eucl_exist (impl_PI Hb'') a); intros qr.
...
Proof.
  ...
  intros.
  elim (Z_ge_lt_dec (a / c) (b / c)); intro; 
   apply impl_PI; trivial.
  absurd (b - a >= 1).
  omega.
  rewrite H0.
  rewrite H2.
  assert
    (c * (b / c) + b mod c - (c * (a / c) + a mod c) =
      c * (b / c - a / c) + b mod c - a mod c) by ring.
  rewrite H3.
