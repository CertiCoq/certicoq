...
Lemma ln_exists1 : forall y:R, [0 < y] -> [1 <= y] -> { z:R | y = exp z}.
Proof.
  intros; set (f := fun x:R => exp x - y); cut (f 0 <= 0).
  intro; cut (continuity f).
  intro; cut (0 <= f y).
  intro; cut (f 0 * f y <= 0).
  intro; assert (X := IVT_cor f 0 y H2 (Rlt_le _ _ (impl_PI H)) H4); elim X; intros t H5;
    exists t; elim H5; intros; unfold f in H7;
  ...
  replace (y + (exp y - y)) with (exp y); [ apply (exp_ineq1 y (impl_PI H)) | ring ].
...
Proof.
  ...
  cut [1 <= / y]. intro H0.
  (*  assert (H0 : 1 <= / y). bug of assert *)
2:  apply Rmult_le_reg_l with y.
2:  apply H.
2:  rewrite <- Rinv_r_sym.
2:  rewrite Rmult_1_r; left; apply (Rnot_le_lt _ _ n).
2:  red in |- *; intro; rewrite H0 in H; elim (Rlt_irrefl _ H).
  cut [0 < / y]. intro H1.
  (* assert (H1 : 0 < / y). bug of assert *)
2:  apply Rinv_0_lt_compat; apply H.
  assert (H2 := ln_exists1 _ H1 H0); elim H2; intros; exists (- x);
...
Proof.
  intros; unfold ln in |- *; case (Rlt_dec 0 x); intro; apply impl_PI.
  unfold Rln in |- *;
...
Proof.
  ...
  apply H3; split.
  unfold D_x in |- *; split.
  case (Rcase_abs h); intro; apply impl_PI.
