...
(** val compareSpec2Type :
    comparison -> compareSpecT **)
...
module Coq_Pos =
 struct
  ...
  (** val double_pred_mask :
      positive -> mask **)

  let double_pred_mask = function
  | XI p -> IsPos (XO (XO p))
  | XO p ->
    IsPos (XO (pred_double p))
  | XH -> IsNul
  ...
  | XI p0 ->
    (match p0 with
     | XI p1 ->
       sqrtrem_step (fun x0 -> XI
         x0) (fun x0 -> XI x0)
         (sqrtrem p1)
     | XO p1 ->
       sqrtrem_step (fun x0 -> XO
         x0) (fun x0 -> XI x0)
         (sqrtrem p1)
     | XH ->
       Pair (XH, (IsPos (XO XH))))
  ...
  | D2 l ->
    Npos (of_uint_acc l (XO XH))
  | D3 l ->
    Npos (of_uint_acc l (XI XH))
  | D4 l ->
    Npos (of_uint_acc l (XO (XO XH)))
  | D5 l ->
    Npos (of_uint_acc l (XI (XO XH)))
  | D6 l ->
    Npos (of_uint_acc l (XO (XI XH)))
  | D7 l ->
    Npos (of_uint_acc l (XI (XI XH)))
  | D8 l ->
    Npos
      (of_uint_acc l (XO (XO (XO
        XH))))
  | D9 l ->
    Npos
      (of_uint_acc l (XI (XO (XO
        XH))))
  ...
  | XI p0 ->
    Little.succ_double
      (to_little_uint p0)
  | XO p0 ->
    Little.double (to_little_uint p0)
  | XH -> D1 Nil0
  ...
  | PeanoOne ->
    PeanoSucc (XH, PeanoOne)
  | PeanoSucc (p0, q0) ->
    PeanoSucc ((succ (XO p0)),
      (PeanoSucc ((XO p0),
      (peanoView_xO p0 q0))))
  ...
  | PeanoOne ->
    PeanoSucc ((succ XH), (PeanoSucc
      (XH, PeanoOne)))
  | PeanoSucc (p0, q0) ->
    PeanoSucc ((succ (XI p0)),
      (PeanoSucc ((XI p0),
      (peanoView_xI p0 q0))))
  ...
  | PeanoSucc (p0, q0) ->
    f p0
      (coq_PeanoView_iter a f p0 q0)

  (** val eqb_spec :
      positive -> positive -> reflect **)
  ...
  module Private_Dec =
   struct
    ...
    let max_case_strong n0 m compat hl hr =
      ...
      (match c with
       | CompGtT ->
         compat n0 (max n0 m) __
           (hl __)
       | _ ->
         compat m (max n0 m) __
           (hr __))
    ...
    let min_case_strong n0 m compat hl hr =
      ...
      (match c with
       | CompGtT ->
         compat m (min n0 m) __
           (hr __)
       | _ ->
         compat n0 (min n0 m) __
           (hl __))
  ...
  let max_case_strong n0 m x0 x1 =
    Private_Dec.max_case_strong n0 m
      (fun _ _ _ x2 -> x2) x0 x1
  ...
  let min_case_strong n0 m x0 x1 =
    Private_Dec.min_case_strong n0 m
      (fun _ _ _ x2 -> x2) x0 x1
...
| Nil -> a0
| Cons (b, t0) ->
  f b (fold_right f a0 t0)

(** val existsb :
    ('a1 -> bool) -> 'a1 list -> bool **)
 ...
 struct
  module Raw =
   struct
    ...
    (** val elements_aux :
        X.t list -> tree -> X.t list **)

    let rec elements_aux acc = function
    | Leaf -> acc
    | Node (_, l, x0, r) ->
      elements_aux (Cons (x0,
        (elements_aux acc r))) l
    ...
    | Leaf -> acc
    | Node (_, l, x0, r) ->
      rev_elements_aux (Cons (x0,
        (rev_elements_aux acc l))) r
    ...
    let treeify_cont f g acc =
      ...
      (match l0 with
       | Nil -> bogus
       | Cons (x0, acc0) ->
         let Pair (r, acc1) = g acc0
         in
         Pair ((Node (Black, l, x0,
         r)), acc1))

    (** val treeify_aux :
        bool -> positive -> elt list
        -> (tree, elt list) prod **)

    let rec treeify_aux pred0 = function
    | XI n1 ->
      treeify_cont
        (treeify_aux False n1)
        (treeify_aux pred0 n1)
    | XO n1 ->
      treeify_cont
        (treeify_aux pred0 n1)
        (treeify_aux True n1)
    ...
    let rec filter_aux f s acc =
      ...
      | Leaf -> acc
      | Node (_, l, k, r) ->
        let acc0 = filter_aux f r acc
        in
        (match f k with
         | True ->
           filter_aux f l (Cons (k,
             acc0))
         | False ->
           filter_aux f l acc0)
    ...
    let rec partition_aux f s acc1 acc2 =
      ...
      | Node (_, sl, k, sr) ->
        let Pair (acc3, acc4) =
          partition_aux f sr acc1
            acc2
        in
        (match f k with
         | True ->
           partition_aux f sl (Cons
             (k, acc3)) acc4
         | False ->
           partition_aux f sl acc3
             (Cons (k, acc4)))
    ...
    let partition f s =
      let Pair (ok, ko) =
        partition_aux f s Nil Nil
      in
      Pair ((treeify ok),
      (treeify ko))

    (** val union_list :
        elt list -> elt list -> elt
        list -> elt list **)
    ...
    let inter t1 t2 =
      ...
      | Lt ->
        filter (fun k -> mem k t2) t1
      | Gt ->
        filter (fun k -> mem k t1) t2
  ...
  (** val remove_min :
      t_ -> (elt, t) prod option **)

  let remove_min s =
    mk_opt_t
      (Raw.remove_min (this s))
 end
...
| Next (t1, t2) ->
  Next ((subst_expr i t0 t1),
    (subst_expr i t0 t2))
| Lseg (t1, t2) ->
  Lseg ((subst_expr i t0 t1),
    (subst_expr i t0 t2))

(** val subst_spaces :
    var -> expr -> space_atom list
    -> space_atom list **)
...
| Nil -> Cons (a, Nil)
| Cons (h, t0) ->
  (match isGeq (cmp a h) with
   | True -> Cons (a, l)
   | False ->
     Cons (h, (insert cmp a t0)))
...
(** val prio :
    pure_atom list -> pure_atom list
    -> var **)

let prio gamma delta =
  list_prio var2 gamma
    (list_prio var1 delta var0)

type clause =
| PureClause of pure_atom list
   * pure_atom list * var
| PosSpaceClause of pure_atom list
   * pure_atom list * space_atom list
| NegSpaceClause of pure_atom list
   * space_atom list * pure_atom list
...
(** val order_eqv_pure_atom :
    pure_atom -> pure_atom **)

let order_eqv_pure_atom = function
| Eqv (i, j) ->
  (match expr_cmp i j with
   | Lt -> Eqv (j, i)
   | _ -> Eqv (i, j))
...
let normalize_atoms pa =
  rsort_uniq pure_atom_cmp
    (map order_eqv_pure_atom pa)
...
let mkPureClause gamma delta =
  PureClause (gamma, delta,
    (prio gamma delta))

(** val order_eqv_clause :
    clause -> clause **)
...
| Equ (x0, y) ->
  mkPureClause Nil (Cons
    ((order_eqv_pure_atom (Eqv (x0,
       y))), Nil))
| Nequ (x0, y) ->
  mkPureClause (Cons
    ((order_eqv_pure_atom (Eqv (x0,
...
(** val cnf :
    entailment -> clause list **)

let cnf = function
| Entailment (a, a0) ->
  let Assertion (pureL, spaceL) = a
  in
  let Assertion (pureR, spaceR) = a0
  in
  let Pair (p, n0) = mk_pureR pureR
  in
  app (map mk_pureL pureL)
    (app (Cons ((PosSpaceClause
      (Nil, Nil, spaceL)), Nil))
      (match spaceL with
       | Nil ->
         (match spaceR with
          ...
          | Cons (_, _) ->
            Cons ((NegSpaceClause
              (p, spaceR, n0)), Nil))
       | Cons (_, _) ->
         Cons ((NegSpaceClause (p,
           spaceR, n0)), Nil)))

(** val pure_atom_gt :
    pure_atom -> pure_atom -> bool **)
...
| Eqv (t1, t2) ->
  Eqv ((subst_expr i t0 t1),
    (subst_expr i t0 t2))

(** val subst_pures :
    var -> expr -> pure_atom list ->
    pure_atom list **)

let subst_pures i t0 pa =
  map (subst_pure i t0) pa

(** val compare_space_atom :
    space_atom -> space_atom ->
    comparison **)
...
let compare_clause cl1 cl2 =
    ...
    (match cl2 with
     | PureClause (neg', pos', _) ->
       (match compare_list
                ...
                neg' with
        | Eq ->
          compare_list pure_atom_cmp
            pos pos'
        | x0 -> x0)
     | _ -> Lt)
  | PosSpaceClause (gamma, delta,
                    sigma) ->
    (match cl2 with
     | PureClause (_, _, _) -> Gt
     | PosSpaceClause (gamma',
                       delta', sigma') ->
       (match compare_list
                pure_atom_cmp gamma
                gamma' with
  ...
  | NegSpaceClause (gamma, sigma,
                    delta) ->
    (match cl2 with
     | NegSpaceClause (gamma',
                       sigma', delta') ->
       (match compare_list
                pure_atom_cmp gamma
                gamma' with
...
let prio1000 =
  z2id (Zpos (XO (XO (XO (XI (XO (XI
    (XI (XI (XI XH))))))))))

(** val prio1001 : Ident.t **)

let prio1001 =
  z2id (Zpos (XI (XO (XO (XI (XO (XI
    (XI (XI (XI XH))))))))))
...
| PosSpaceClause (_, _, _) ->
  prio1000
| NegSpaceClause (_, _, _) ->
  prio1001
...
(** val eq_space_atomlist :
    space_atom list -> space_atom
    list -> bool **)

let eq_space_atomlist a b =
  isEq
    (compare_list compare_space_atom
      a b)
...
(** val greater_than_expr :
    var -> expr -> bool **)

let greater_than_expr i = function
| Nil1 -> True
| Var j ->
  (match Ident.compare i j with
   | Gt -> True
   | _ -> False)

(** val greater_than_atom :
    pure_atom -> pure_atom -> bool **)
...
(** val greater_than_atoms :
    pure_atom -> pure_atom list ->
    bool **)

let greater_than_atoms s delta =
  forallb (fun u ->
    greater_than_atom s u) delta

(** val greater_than_all :
    var -> pure_atom list -> bool **)

let greater_than_all i =
  forallb (fun a ->
    let Eqv (x0, y) = a in
    (match greater_than_expr i x0 with
...
(** val pure_atom2pn_atom :
    bool -> pure_atom -> pn_atom **)
...
let pn_atom_cmp a1 a2 =
  ...
  | Equ (e1, e2) ->
    (match a2 with
     | Equ (e1', e2') ->
       pure_atom_cmp (Eqv (e1, e2))
         (Eqv (e1', e2'))
     | Nequ (e1', e2') ->
       (match expr_eq e1 e1' with
        | True -> Lt
        | False ->
          pure_atom_cmp (Eqv (e1,
            e2)) (Eqv (e1', e2'))))
  | Nequ (e1, e2) ->
    (match a2 with
     | Equ (e1', e2') ->
...
| PureClause (gamma, delta, _) ->
  rsort pn_atom_cmp
    (app
      (map (pure_atom2pn_atom False)
        gamma)
      (map (pure_atom2pn_atom True)
        delta))
| _ -> Nil
...
let compare_clause2 cl1 cl2 =
  ...
  | PureClause (_, _, _) ->
    (match cl2 with
     | PureClause (_, _, _) ->
       compare_list pn_atom_cmp
         (pure_clause2pn_list cl1)
...
module DebuggingHooks =
 struct
  ...
  (** val print_spatial_model :
      clause -> (var, expr) prod
      list -> clause **)
...
| Cons (p, cs') ->
  let Pair (a1, b1') = p in
  let new_cs =
    rewriteC b_cmp b1 b2 cs'
  in
  (match isEq (b_cmp b1 b1') with
   | True ->
     Cons ((Pair (a1, b2)), new_cs)
   | False ->
     Cons ((Pair (a1, b1')), new_cs))
...
let mergeC a_cmp b_cmp fAB a1 a2 cs =
  match isEq
          (b_cmp
            (lookC a_cmp fAB a1 cs)
            (lookC a_cmp fAB a2 cs)) with
  | True -> cs
  | False ->
    Cons ((Pair (a1,
      ...
      (rewriteC b_cmp
        (lookC a_cmp fAB a1 cs)
        (lookC a_cmp fAB a2 cs) cs))))

(** val cclose_aux :
    clause list -> (expr, expr) prod
    list **)
...
module Superposition =
 struct
  ...
  (** val ef_aux :
      pure_atom list -> var -> expr
      -> expr -> pure_atom list ->
      pure_atom list -> clause list
      -> clause list **)
      ...
      (match match expr_eq s u with
       ...
       | True ->
         Cons
           ...
           (ef_aux neg u0 u v
             (insert_uniq
               pure_atom_cmp a2 pos0)
             pos' l0))
       | False -> l0)

  (** val ef :
      ce_type -> clause -> clause
      list -> clause list **)
  ...
  | Eqv (u, v) ->
    (match expr_eq s u with
     ...
     | False ->
       (match expr_eq s v with
        | True ->
          norm_pure_atom (Eqv (u,
            t0))
        | False -> atm))

  (** val rewrite_in_space :
      expr -> expr -> space_atom ->
      space_atom **)

  let rewrite_in_space s t0 = function
  | Next (u, v) ->
    Next ((rewrite_expr s t0 u),
      (rewrite_expr s t0 v))
  | Lseg (u, v) ->
    Lseg ((rewrite_expr s t0 u),
      (rewrite_expr s t0 v))

  (** val rewrite_clause_in_space :
      clause -> space_atom ->
      space_atom **)
  ...
  (** val simplify_atoms :
      clause list -> space_atom list
      -> space_atom list **)

  let simplify_atoms l atms =
    fold_left (fun atms0 d ->
      map
        (rewrite_clause_in_space d)
        atms0) l atms
  ...
  (** val partial_mod :
      model -> clause list -> clause
      list -> ((model, clause list)
      prod, ((model, clause) prod,
      ce_type) prod) sum **)
       ...
       (match is_model_of (rev0 r)
        ...
        | False ->
          (match clause_generate r cl with
           ...
           | Inr cty ->
             Inr
               (DebuggingHooks.print_ce_clause
                 r cl cty)))
     | _ -> Inl (Pair (r, acc)))
  ...
  (** val main_terminate :
      positive -> clause list ->
      clause list ->
      (((superposition_result,
      clause list) prod, M.t) prod,
      M.t) prod **)

  let rec main_terminate n0 units l =
    match Coq_Pos.eqb n0 XH with
    | True ->
      Pair ((Pair ((Pair ((Aborted
        l), units)), M.empty)),
        M.empty)
    | False ->
      (match existsb is_empty_clause
               (map delete_resolved
                 l) with
       | True ->
         Pair ((Pair ((Pair (Valid,
           units)), M.empty)),
           M.empty)
       | False ->
         let Pair (us, rs) =
           partition is_unit_clause l
         in
         (match partial_mod Nil Nil
                  (filter not_taut
                    (map
                      (simplify
                        (DebuggingHooks.print_eqs_list
                          (cclose us)))
                      rs)) with
          | Inl p ->
            let Pair (r, selected) =
  ...
  let check_clauseset s =
    main (XO (XO (XO (XO (XO (XO (XO
      (XO (XO (XI (XO (XI (XO (XO
      (XI (XI (XO (XI (XO (XI (XI
      (XO (XO (XI (XI (XI (XO (XI
      (XI
      XH)))))))))))))))))))))))))))))
      Nil
      (DebuggingHooks.print_pures_list
        (rsort
          (rev_cmp compare_clause2)
          (M.elements
            (M.filter not_taut s))))
 end
...
module HeapResolve =
 struct
  ...
  (** val unfolding1' :
      space_atom list -> space_atom
      list -> space_atom list ->
      (pure_atom, space_atom list)
      prod list **)
    ...
    app (unfolding1 pc nc)
      (app (unfolding2 pc nc)
        (app (unfolding3 pc nc)
          (app (unfolding4 pc nc)
            (app (unfolding6 pc nc)
              l))))
  ...
  let unfolding pc nc =
    M.fold (fun c ->
      M.union
        (spatial_resolution pc c))
      (do_unfold (S (S (S (S (S (S
        (S (S (S (S (S (S (S (S (S
        (S (S (S (S (S (S (S (S (S
        (S (S (S (S (S (S (S (S (S
        (S (S (S (S (S (S (S (S (S
...
module VeriStar =
 struct
  ...
  let rec the_loop_terminate n0 sigma nc s cl =
    match Coq_Pos.eqb n0 XH with
    | True ->
      Aborted ((M.elements s), cl)
    | False ->
      let Pair (p, _) =
        Superposition.check_clauseset
          s
      in
                      ...
                      (DebuggingHooks.print_wf_set
                        (HeapResolve.do_wellformed
                          (DebuggingHooks.print_spatial_model
                           ...
                           (Nil,
                           Nil,
                           (Superposition.simplify_atoms
                           units
                           sigma))))
                           r)))
                      s_star) s_star) with
          | True ->
            (match Superposition.is_model_of_PI
                     (rev0 r)
                     (DebuggingHooks.print_spatial_model
                       (Superposition.simplify
                         units nc) r) with
             | True ->
               (match isEq
                        (M.compare
  ...
  (** val check_entailment :
      entailment -> veristar_result **)

  let check_entailment ent =
    let s =
      clause_list2set
        (pure_clauses
          (map order_eqv_clause
            (cnf ent)))
    in
    let Entailment (a, a0) = ent in
    let Assertion (_, sigma) = a in
    let Assertion (pi', sigma') = a0
    in
    let Pair (pi'plus, pi'minus) =
      mk_pureR pi'
    in
    the_loop (XO (XO (XO (XO (XO (XO
      (XO (XO (XO (XI (XO (XI (XO
      (XO (XI (XI (XO (XI (XO (XI
      (XI (XO (XO (XI (XI (XI (XO
      (XI (XI
      XH)))))))))))))))))))))))))))))
      sigma (NegSpaceClause
      (pi'plus, sigma', pi'minus))
      (DebuggingHooks.print_new_pures_set
        s) empty_clause
 end
...
let harder_ent () =
  Entailment ((Assertion (Nil, (Cons
    ((Next ((x (XI (XO (XO XH)))),
    (x (XI XH)))), (Cons ((Lseg
    ((x (XI (XI XH))),
    (x (XO (XO (XO XH)))))), (Cons
    ((Lseg ((x XH),
    (x (XO (XI XH))))), (Cons ((Next
    ((x (XO (XI (XO XH)))),
    (x (XO XH)))), (Cons ((Next
    ((x (XI XH)),
    (x (XI (XI XH))))), (Cons ((Lseg
    ((x (XI (XO XH))),
    (x (XO (XI (XO XH)))))), (Cons
    ((Next ((x (XO (XO XH))),
    (x (XI (XO (XO XH)))))), (Cons
    ((Next ((x (XI (XO (XI XH)))),
    (x (XI (XO XH))))), (Cons ((Lseg
    ((x (XO (XO (XO XH)))),
    (x (XI (XO XH))))), (Cons ((Next
    ((x (XI (XI (XO XH)))),
    (x (XO (XO (XI XH)))))), (Cons
    ((Next ((x (XO (XO (XI XH)))),
    (x (XI (XI (XO XH)))))), (Cons
    ((Next ((x (XO XH)),
    (x (XO (XO XH))))), (Cons ((Lseg
    ((x (XO (XI XH))),
    (x (XI XH)))),
    Nil)))))))))))))))))))))))))))),
    (Assertion (Nil, (Cons ((Lseg
    ((x (XO XH)),
    (x (XO (XI (XO XH)))))), (Cons
    ((Lseg ((x (XO (XO (XI XH)))),
    (x (XI (XI (XO XH)))))), (Cons
...
(** val main_h :
    VeriStar.veristar_result **)

let main_h () =
  VeriStar.check_entailment
    (harder_ent ())
