...
(* $Id: termops.ml 9917 2007-06-30 09:55:33Z herbelin $ *)
...
let pr_impl imp t =  
  if imp=Impl then str"["++t++str"]" else t
...
let rec pr_constr c = match kind_of_term c with
  ...
  | Cast (c,_, t) -> hov 1 
      (str"(" ++ pr_constr c ++ cut() ++
       str":" ++ pr_constr t ++ str")")
  | Prod (Name(id),imp,t,c) -> hov 1
      (str"forall " ++ pr_impl imp (pr_id id) ++ str":" ++
       pr_constr t ++ str"," ++
       spc() ++ pr_constr c)
  | Prod (Anonymous,imp,t,c) -> hov 0
      (str"(" ++ pr_impl imp (pr_constr t) ++ str " ->" ++ spc() ++
       pr_constr c ++ str")")
  | Lambda (na,imp,t,c) -> hov 1
      (str"fun " ++ pr_impl imp (pr_name na) ++ str":" ++
       pr_constr t ++ str" =>" ++ spc() ++ pr_constr c)
  | LetIn (na,imp,b,t,c) -> hov 0
      (str"let " ++ pr_impl imp (pr_name na) ++ str":=" ++ pr_constr b ++
       str":" ++ brk(1,2) ++ pr_constr t ++ cut() ++
       pr_constr c)
  | App (c,l,imps) ->
      let args = List.combine(Array.to_list  l) (Array.to_list imps) in
      let pr_arg (a,imp) = pr_impl imp (pr_constr a) in
      hov 1
        (str"(" ++ pr_constr c ++ spc() ++
        prlist_with_sep spc pr_arg args ++ str")")
  | Evar (e,l) -> hov 1
      (str"Evar#" ++ int e ++ str"{" ++
...
let pr_var_decl env (id,imp,c,typ) =
  let pid = if imp=Impl then str"["++pr_id id ++str"]" else pr_id id in
  ...
  let ptyp = (str" : " ++ pt) in
    hv 2 (pid ++ cut() ++ pbody ++ ptyp)

let pr_rel_decl env (na,imp,c,typ) =
  let pbody = match c with
    | None -> mt ()
    | Some c ->
	(* Force evaluation *) 
...
let refresh_universes t =
  ...
  let rec refresh t = match kind_of_term t with
    ...
    | Prod (na,imp,u,v) -> mkProd (na,imp,u,refresh v)
    | _ -> t in
  let t' = refresh t in
...
let prod_it ~init =
  List.fold_left (fun c (n,imp,t) -> mkProd (n, imp, t, c)) init

(* lam_it b [xn:Tn;..;x1:T1] = [x1:T1]..[xn:Tn]b *)
let lam_it ~init =
  List.fold_left (fun c (n,imp,t) -> mkLambda (n, imp, t, c)) init
...
let extended_rel_list n hyps =
  let rec reln l imps p = function
    | (_,imp,None,_) :: hyps -> reln (mkRel (n+p) :: l) (imp::imps) (p+1) hyps
    | (_,imp,Some _,_) :: hyps -> reln l imps (p+1) hyps
    | [] -> l,imps
  in 
  reln [] [] 1 hyps

let extended_rel_vect n hyps =
  let (inst,imps) = extended_rel_list n hyps in
  Array.of_list inst, Array.of_list imps



let push_rel_assum (x,imp,t) env = push_rel (x,imp,None,t) env

let push_rels_assum assums =
  push_rel_context (List.map (fun (x,imp,t) -> (x,imp,None,t)) assums)
...
let push_named_rec_types (lna,typarray,_) env =
  let ctxt =
    array_map2_i
      (fun i na t ->
	 match na with
	   | Name id -> (id, Expl, None, type_app (lift i) t)
...
let rec lookup_rel_id id sign = 
  let rec lookrec = function
    | (n, (Anonymous,_,_,_)::l) -> lookrec (n+1,l)
    | (n, (Name id',_,_,t)::l)  -> if id' = id then (n,t) else lookrec (n+1,l)
    | (_, [])                 -> raise Not_found
  in 
  lookrec (1,sign)
...
let map_constr_with_named_binders g f l c = match kind_of_term c with
  ...
  | Prod (na,imp,t,c) -> mkProd (na, imp, f l t, f (g na l) c)
  | Lambda (na,imp,t,c) -> mkLambda (na, imp, f l t, f (g na l) c)
  | LetIn (na,imp,b,t,c) -> mkLetIn (na, imp, f l b, f l t, f (g na l) c)
...
let fold_rec_types g (lna,typarray,_) e =
  let ctxt =
    array_map2_i
      (fun i na t -> (na, Expl, None, type_app (lift i) t)) lna typarray in
  Array.fold_left
...
let map_constr_with_binders_left_to_right g f l c = match kind_of_term c with
  ...
  | Prod (na,imp,t,c) ->
      let t' = f l t in
      mkProd (na, imp, t', f (g (na,imp,None,t) l) c)
  | Lambda (na,imp,t,c) ->
      let t' = f l t in
      mkLambda (na, imp, t', f (g (na,imp,None,t) l) c)
  | LetIn (na,imp,b,t,c) ->
      let b' = f l b in
      let t' = f l t in
      let c' = f (g (na,imp,Some b,t) l) c in
      mkLetIn (na, imp, b', t', c')
  | App (c,[||],_) -> assert false
  | App (c,al,imps) ->
      (*Special treatment to be able to recognize partially applied subterms*)
      let i = Array.length al - 1 in
      let hd = f l (mkApp (c, Array.sub al 0 i, Array.sub imps 0 i)) in
      mkApp (hd, [| f l al.(i) |], [|imps.(i)|])
...
let map_constr_with_full_binders g f l cstr = match kind_of_term cstr with
  ...
  | Prod (na,imp,t,c) ->
      let t' = f l t in
      let c' = f (g (na,imp,None,t) l) c in
      if t==t' && c==c' then cstr else mkProd (na, imp, t', c')
  | Lambda (na,imp,t,c) ->
      let t' = f l t in
      let c' = f (g (na,imp,None,t) l) c in
      if t==t' && c==c' then cstr else  mkLambda (na, imp, t', c')
  | LetIn (na,imp,b,t,c) ->
      let b' = f l b in
      let t' = f l t in
      let c' = f (g (na,imp,Some b,t) l) c in
      if b==b' && t==t' && c==c' then cstr else mkLetIn (na, imp, b', t', c')
  | App (c,al,imps) ->
      let c' = f l c in
      let al' = Array.map (f l) al in
      if c==c' && array_for_all2 (==) al al' then cstr
      else mkApp (c', al', imps)
  | Evar (e,al) ->
      let al' = Array.map (f l) al in
  ...
  | Fix (ln,(lna,tl,bl)) ->
      ...
      let l' =
        array_fold_left2 (fun l na t -> g (na,Expl,None,t) l) l lna tl in
      let bl' = Array.map (f l') bl in
...
let fold_constr_with_binders g f n acc c = match kind_of_term c with
  ...
  | Prod (_,_,t,c) -> f (g n) (f n acc t) c
  | Lambda (_,_,t,c) -> f (g n) (f n acc t) c
  | LetIn (_,_,b,t,c) -> f (g n) (f n (f n acc b) t) c
  | App (c,l,_) -> Array.fold_left (f n) (f n acc c) l
...
let iter_constr_with_full_binders g f l c = match kind_of_term c with
  ...
  | Prod (na,imp,t,c) -> f l t; f (g (na,imp,None,t) l) c
  | Lambda (na,imp,t,c) -> f l t; f (g (na,imp,None,t) l) c
  | LetIn (na,imp,b,t,c) -> f l b; f l t; f (g (na,imp,Some b,t) l) c
  | App (c,args,_) -> f l c; Array.iter (f l) args
...
let occur_var_in_decl env hyp (_,_,c,typ) =
  match c with
...
let dependent m t =
  let rec deprec m t =
	...
	| App (fm,lm,_), App (ft,lt,imps)
            when Array.length lm < Array.length lt ->
            let llm = Array.length lm in
            let llt = Array.length lt in
	    deprec m (mkApp (ft,Array.sub lt 0 llm,Array.sub imps 0 llm));
	    Array.iter (deprec m)
	      (Array.sub lt llm (llt - llm))
...
let prefix_application eq_fun (k,c) (t : constr) = 
  ...
  match kind_of_term c', kind_of_term t' with
    | App (f1,cl1,_), App (f2,cl2,imps2) ->
	let l1 = Array.length cl1
	and l2 = Array.length cl2 in
	if l1 <= l2
	   && eq_fun c' (mkApp (f2, Array.sub cl2 0 l1, Array.sub imps2 0 l1))
        then
	  Some (mkApp (mkRel k, Array.sub cl2 l1 (l2 - l1),
                       Array.sub imps2 l1 (l2-l1)))
	else 
	  None
    | _ -> None
...
let subst_term_occ_decl locs c (id,imp,bodyopt,typ as d) =
  match bodyopt with
    | None -> (id,imp,None,subst_term_occ locs c typ)
    | Some body -> 
	if locs = [] then
	  (id,imp,Some (subst_term c body),type_app (subst_term c) typ)
	else if List.mem 0 locs then 
	  d
	else 
...
let vars_of_env env =
  let s = 
    Sign.fold_named_context (fun (id,_,_,_) s -> Idset.add id s)
      (named_context env) ~init:Idset.empty in
  Sign.fold_rel_context
    (fun (na,_,_,_) s -> match na with Name id -> Idset.add id s | _ -> s)
...
let hdchar env c = 
  let rec hdrec k c =
    match kind_of_term c with
    | Prod (_,_,_,c)       -> hdrec (k+1) c
    | Lambda (_,_,_,c)     -> hdrec (k+1) c
    | LetIn (_,_,_,_,c)    -> hdrec (k+1) c
	...
	(if n<=k then "p" (* the initial term is flexible product/function *)
	   ...
	   try match Environ.lookup_rel (n-k) env with
	     | (Name id,_,_,_)   -> lowercase_first_char id
	     | (Anonymous,_,_,t) -> hdrec 0 (lift (n-k) t)
	   with Not_found -> "y")
    | Fix ((_,i),(lna,_,_)) -> 
...
let mkProd_name   env (n,imp,a,b) = mkProd (named_hd_type env a n, imp,a, b)
let mkLambda_name env (n,imp,a,b) = mkLambda (named_hd_type env a n, imp,a, b)

let lambda_name = mkLambda_name
let prod_name = mkProd_name

let prod_create   env (imp,a,b) =
  mkProd (named_hd_type env a Anonymous, imp, a, b)
let lambda_create env (imp,a,b) =
  mkLambda (named_hd_type env a Anonymous, imp, a, b)

let name_assumption env (na,imp,c,t) =
  match c with
    | None      -> (named_hd_type env t na, imp, None, t)
    | Some body -> (named_hd env body na, imp, c, t)
...
let mkProd_or_LetIn_name env b d =
  mkProd_or_LetIn (name_assumption env d) b 
let mkLambda_or_LetIn_name env b d =
  mkLambda_or_LetIn (name_assumption env d) b
...
let ids_of_rel_context sign =
  Sign.fold_rel_context
    (fun (na,_,_,_) l -> match na with Name id -> id::l | Anonymous -> l)
    sign ~init:[]

let ids_of_named_context sign =
  Sign.fold_named_context (fun (id,_,_,_) idl -> id::idl) sign ~init:[]
...
let names_of_rel_context env =
  List.map (fun (na,_,_,_) -> na) (rel_context env)
...
let rec rename_bound_var env avoid c =
  ...
  let rec rename avoid c =
	...
	mkProd (na', imp, c1, rename avoid' c2)
    | LetIn (na,imp,c1,t,c2) ->
	let na',avoid' = concrete_let_name None avoid env_names na c2 in
	mkLetIn (na',imp,c1,t, rename avoid' c2)
