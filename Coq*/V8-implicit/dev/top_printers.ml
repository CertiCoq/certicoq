...
let constr_display csr =
  ...
  | LetIn (na,Impl,b,t,c) ->
      ...
      ^(term_display t)^","^(term_display c)^")"
  | App (c,l,imps) ->
      "App("^(term_display c)^","^(array_display l)^","^imps_display imps^")\n"
  ...
  and imps_display v =
    "[|"^
    (Array.fold_right (fun x i -> (if x=Impl then "*" else "-")^i) v "")
    ^"|]"
...
let print_pure_constr csr =
  ...
  | Prod (Name(id),imp,t,c) ->
      let lp,rp = if imp=Impl then ("[","]") else ("(",")") in
      open_hovbox 1;
      print_string("forall "^lp); print_string (string_of_id id); 
      print_string ":"; box_display t; 
      print_string rp; print_cut(); 
      box_display c; close_box()
  | Prod (Anonymous,imp,t,c) ->
      let lp,rp = if imp=Impl then ("[","]") else ("","") in
      print_string("("^lp); box_display t; print_string rp;
      print_cut(); print_string "->";
      box_display c; print_string ")"; 
  | Lambda (na,imp,t,c) ->
      let lp,rp = if imp=Impl then ("[","]") else ("(",")") in
      print_string ("fun "^lp); name_display na;
      print_string ":"; box_display t; print_string rp;
      print_cut(); box_display c; 
  | LetIn (na,imp,b,t,c) ->
      let lp,rp = if imp=Impl then ("[","]") else ("","") in
      print_string ("let "^lp); name_display na; print_string (rp^":="); 
  ...
  | App (c,l,imps) -> 
      ...
      box_display c; 
      ignore (array_map2
        (fun x imp -> print_space ();
          if imp=Impl then (print_string"[";box_display x;print_string"]")
          else box_display x) l imps;
      print_string ")")
