...
(* $Id: evar_tactics.ml 9868 2007-05-29 10:01:06Z herbelin $ *)
  ...
  let evl = 
	...
	ConclLocation () -> evar_list sigma gl.it.evar_concl 
	  ...
	  let decl = Environ.lookup_named_val id gl.it.evar_hyps in
		...
		InHyp ->  
		  (match decl with 
		      (_,_,None,typ) -> evar_list sigma typ
		    | _ -> error 
			"please be more specific : in type or value ?")
	      | InHypTypeOnly ->
		  let (_, _, _, typ) = decl in evar_list sigma typ
	      | InHypValueOnly ->
		  (match decl with 
		      (_,_,Some body,_) -> evar_list sigma body
...
let let_evar name typ gls =
  ...
  let evd',evar = Evarutil.new_evar evd (pf_env gls) Expl typ in
  Refiner.tclTHEN (Refiner.tclEVARS (evars_of evd'))
    (Tactics.letin_tac true name Expl evar nowhere) gls
