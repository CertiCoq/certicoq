...
Proof.
  ...
  case (Rle_dec x (m - eps)); intro; apply impl_PI.
  assumption.
...
Proof.
  ...
  case (Rle_dec x a); intro; apply impl_PI.
  case (Rle_dec x0 a); intro; apply impl_PI.
  ...
  intro; unfold h in |- *; case (Rle_dec x a); intro; apply impl_PI.
  case (Rle_dec x1 a); intro; apply impl_PI.
  unfold Rminus in |- *; rewrite Rplus_opp_r; rewrite Rabs_R0; assumption.
  case (Rle_dec x1 b); intro; apply impl_PI.
  ...
  case (Rle_dec x b); intro; apply impl_PI.
  case (Rle_dec x1 a); intro; apply impl_PI.
  ...
  rewrite <- Rabs_Ropp; rewrite Ropp_minus_distr; apply RRle_abs.
  assumption.
  unfold h in |- *; case (Rle_dec x a); intro; apply impl_PI.
  elim (Rlt_irrefl _ (Rle_lt_trans _ _ _ r H4)).
  case (Rle_dec x b); intro; apply impl_PI.
  elim (Rlt_irrefl _ (Rle_lt_trans _ _ _ r H6)).
  case (Rle_dec x0 a); intro; apply impl_PI.
  elim (Rlt_irrefl _ (Rlt_trans _ _ _ H1 (Rlt_le_trans _ _ _ H10 r))).
  case (Rle_dec x0 b); intro; apply impl_PI.
  elim (Rlt_irrefl _ (Rle_lt_trans _ _ _ r H10)).
  unfold Rminus in |- *; rewrite Rplus_opp_r; rewrite Rabs_R0; assumption.
  intros; elim H3; intros; unfold h in |- *; case (Rle_dec c a); intro; apply impl_PI.
  elim r; intro.
...
Proof.
  ...
  intro; case (Rle_dec x (M - eps)); intro; apply impl_PI.
  apply r.
...
(* X possède au moins deux éléments distincts *)
  ...
  elim Hyp; intros; elim H4; intros; decompose [and] H5;
    assert (H10 := H3 _ H6); assert (H11 := H3 _ H8); 
      ...
      destruct (total_order_or x x0) as [[a|b]|r].
  assumption.
