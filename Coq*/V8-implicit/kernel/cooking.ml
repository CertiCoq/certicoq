...
(*i $Id: cooking.ml 9795 2007-04-25 15:13:45Z soubiran $ i*)
...
type work_list =
    (identifier array * impl array) Cmap.t *
    (identifier array * impl array) KNmap.t
...
let share r (cstl,knl) =
  ...
  let f,(l,imps) =
    ...
    | ConstructRef ((kn,i),j) -> 
        let (args,_) = KNmap.find kn knl in
	mkConstruct ((pop_kn kn,i),j),
        (args, Array.create (Array.length args) Impl)
    | ConstRef cst ->
	mkConst (pop_con cst), Cmap.find cst cstl in
  let c = mkApp (f, Array.map mkVar l, imps) in
...
let update_case_info ci modlist =
  try
    let ind, n = 
      ...
      | App (f,l,_) -> (destInd f, Array.length l)
      | Ind ind -> ind, 0
      | _ -> assert false in
...
let cook_constant env r =
  ...
  let typ = match cb.const_type with
	...
	let typ =
          abstract_constant_type (expmod_constr r.d_modlist t) hyps in
