...
Module PTree <: TREE.
  ...
  Fixpoint get (A : Type) (i : elt) (m : t A) {struct i} : option A :=
    match m with
    | Leaf => None
    | Node l o r =>
        match i with
  ...
  Fixpoint set (A : Type) (i : elt) (v : A) (m : t A) {struct i} : t A :=
    match m with
    | Leaf =>
        match i with
  ...
  Fixpoint remove (A : Type) (i : elt) (m : t A) {struct i} : t A :=
    match i with
    | xH =>
        match m with
        | Leaf => Leaf
  ...
  Qed.
    ...
    Lemma gleaf : forall (A : Type) (i : elt), get i (Leaf : t A) = None.
  ...
  Lemma rleaf : forall (A : Type) (i : elt), remove i (Leaf : t A) = Leaf.
  ...
  End BOOLEAN_EQUALITY.
    ...
    Fixpoint append (i j : elt) {struct i} : elt :=
      match i with
      | xH => j
    ...
    Lemma append_assoc_0 : forall (i j : elt),
    ...
    Lemma append_assoc_1 : forall (i j : elt),
    ...
    Lemma append_neutral_r : forall (i : elt), append i xH = i.
    Proof.
      induction i; simpl; congruence.
    Qed.

    Lemma append_neutral_l : forall (i : elt), append xH i = i.
    Proof.
      simpl; auto.
    Qed.

    Fixpoint xmap (A B : Type) (f : elt -> A -> B) (m : t A) (i : elt)
             {struct m} : t B :=
      match m with
      | Leaf => Leaf
  ...
  Definition map (A B : Type) (f : elt -> A -> B) m := xmap f m xH.

    Lemma xgmap:
      forall (A B: Type) (f: elt -> A -> B) (i j : elt) (m: t A),
  ...
  Lemma gnode':
    forall (A: Type) (l r: t A) (x: option A) (i: elt),
  ...
  Qed.
    ...
    Fixpoint xelements (A : Type) (m : t A) (i : elt) 
                       (k: list (elt * A)) {struct m}
                       : list (elt * A) :=
      match m with
      | Leaf => k
      | Node l None r =>
  ...
  Definition elements (A: Type) (m : t A) := xelements m xH nil.
    ...
    Lemma xelements_correct:
      forall (A: Type) (m: t A) (i j : elt) (v: A) k,
  ...
  Qed.
    ...
    Fixpoint xget (A : Type) (i j : elt) (m : t A) {struct j} : option A :=
      match i, j with
      | _, xH => get i m
    ...
    Lemma xget_diag :
      forall (A : Type) (i : elt) (m1 m2 : t A) (o : option A),
    ...
    Lemma xget_left :
      forall (A : Type) (j i : elt) (m1 m2 : t A) (o : option A) (v : A),
    ...
    Lemma get_xget_h :
      forall (A: Type) (m: t A) (i: elt), get i m = xget i xH m.
    Proof.
  ...
  Lemma in_xelements:
    forall (A: Type) (m: t A) (i k: elt) (v: A) l,
  ...
  Definition xkeys (A: Type) (m: t A) (i: elt) (l: list (elt * A)) :=
    List.map (@fst elt A) (xelements m i l).

  Lemma in_xkeys:
    forall (A: Type) (m: t A) (i k: elt) l,
    In k (xkeys m i l) ->
  ...
  Proof.
    ...
    exploit (append_injective i (k~0)%positive xH). rewrite append_neutral_r. auto.
    congruence.
    ...
    exploit (append_injective i (j~1)%positive xH). rewrite append_neutral_r. auto. congruence.
  ...
  Theorem elements_keys_norepet:
    ...
    list_norepet (List.map (@fst elt A) (elements m)).
  Proof.
    intros. change (list_norepet (xkeys m 1%positive nil)). apply xelements_keys_norepet.
  ...
  Lemma xfold_xelements:
    forall (A B: Type) (f: B -> elt -> A -> B) m i v l,
  ...
  Theorem fold1_spec:
    ...
    List.fold_left (fun a p => f a (snd p)) (elements m) v.
  Proof.
    intros. apply fold1_xelements with (l := @nil (elt * A)). 
  Qed.
...
Module PMap <: MAP.
  ...
  Definition get (A : Type) (i : elt) (m : t A) :=
    match PTree.get i (snd m) with
    | Some x => x
    | None => fst m
    end.

  Definition set (A : Type) (i : elt) (x : A) (m : t A) :=
