...
(* $Id: cctac.ml 9857 2007-05-24 14:21:08Z corbinea $ *)
...
let rec decompose_term env t=
	...
	let targs=Array.map (decompose_term env) args in
	  array_fold_left2 (fun s t imp->Appli (s,t,imp)) tf targs imps
    | Construct c->
...
let rec pattern_of_constr env c =
  match kind_of_term (whd env c) with
	...
	let pargs,lrels = List.split 
	  ...
	  PApp (pf,List.rev pargs,List.rev(Array.to_list imps)),
...
let rec proof_tac p gls =
  match p.p_rule with
	...
	let l=constr_of_term p.p_lhs and 
	  ...
	  exact_check
	    (mkApp(Lazy.force _sym_eq,[|typ;r;l;c|],[|Impl;Impl;Impl;Expl|])) gls
    | Refl t ->
	let lr = constr_of_term t in
	let typ = pf_type_of gls lr in 
	exact_check
	   (mkApp(Lazy.force _refl_equal,[|typ;constr_of_term t|],[|Impl;Impl|])) gls
    | Trans (p1,p2)->
	...
	let prf = 
	  mkApp(Lazy.force _trans_eq,[|typ;t1;t2;t3;_M 1;_M 2|],
                [|Impl;Impl;Impl;Impl;Expl;Expl|]) in
	...
	let appx1 =
          mkLambda(Name id,Expl,typf,mkApp(mkRel 1,[|tx1|],[|Expl|])) in
	let lemma1 =
	  mkApp(Lazy.force _f_equal,
		[|typf;typfx;appx1;tf1;tf2;_M 1|],
                [|Impl;Impl;Impl;Impl;Impl;Expl|]) in
	let lemma2=
	  mkApp(Lazy.force _f_equal,[|typx;typfx;tf2;tx1;tx2;_M 1|],
                [|Impl;Impl;Impl;Impl;Impl;Expl|]) in
	let prf = 
  ...
  | Inject (prf,cstr,nargs,argind) ->
	 ...
	 let injt=
	   mkApp (Lazy.force _f_equal,[|intype;outtype;proj;ti;tj;_M 1|], 
                  [|Impl;Impl;Impl;Impl;Impl;Expl|]) in 
...
let refute_tac c t1 t2 p gls =      
  ...
  let neweq=
    mkApp(Lazy.force _eq,
	  [|intype;tt1;tt2|],[|Expl;Expl;Expl|]) in
  let hid=pf_get_new_id (id_of_string "Heq") gls in
  let false_t=mkApp (c,[|mkVar hid|],[|Expl|]) in
    tclTHENS (true_cut (Name hid) Expl neweq)
      [proof_tac p; simplest_elim false_t] gls

let convert_to_goal_tac c t1 t2 p gls =
  let tt1=constr_of_term t1 and tt2=constr_of_term t2 in
  let sort=pf_type_of gls tt2 in
  let neweq=mkApp(Lazy.force _eq,[|sort;tt1;tt2|],[|Expl;Expl;Expl|]) in 
  let e=pf_get_new_id (id_of_string "e") gls in
  let x=pf_get_new_id (id_of_string "X") gls in
  let identity=mkLambda (Name x,Expl,sort,mkRel 1) in 
  let endt=mkApp (Lazy.force _eq_rect,
		  [|sort;tt1;identity;c;tt2;mkVar e|],
                  [|Impl;Impl;Impl;Expl;Impl;Expl|]) in
    tclTHENS (true_cut (Name e) Expl neweq) 
      [proof_tac p;exact_check endt] gls

let convert_to_hyp_tac c1 t1 c2 t2 p gls =
  let tt2=constr_of_term t2 in
  let h=pf_get_new_id (id_of_string "H") gls in
  let false_t=mkApp (c2,[|mkVar h|],[|Expl|]) in
    tclTHENS (true_cut (Name h) Expl tt2)
...
let discriminate_tac cstr p gls =
  ...
  let identity=
    mkLambda(Name xid,Impl,outsort,mkLambda(Name tid,Expl,mkRel 1,mkRel 1)) in
  let trivial=pf_type_of gls identity in
  let outtype=mkType (new_univ ()) in
  let pred=mkLambda(Name xid,Expl,outtype,mkRel 1) in
  let hid=pf_get_new_id (id_of_string "Heq") gls in     
  let proj=build_projection intype outtype cstr trivial concl gls in
  let injt=mkApp (Lazy.force _f_equal,
		  [|intype;outtype;proj;t1;t2;mkVar hid|],
                  [|Impl;Impl;Impl;Impl;Impl;Expl|]) in   
  let endt=mkApp (Lazy.force _eq_rect,
		  [|outtype;trivial;pred;identity;concl;injt|],
                  [|Impl;Impl;Impl;Expl;Impl;Expl|]) in
  let neweq=mkApp(Lazy.force _eq,[|intype;t1;t2|],[|Expl;Expl;Expl|]) in
    tclTHENS (true_cut (Name hid) Expl neweq) 
      [proof_tac p;exact_check endt] gls
...
let build_term_to_complete uf meta pac =
  ...
  let imps = List.map (fun _ -> Expl) all_args in
  applistc (mkConstruct cinfo.ci_constr) all_args imps
