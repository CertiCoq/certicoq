...
(*i $Id: Specif.v 9906 2007-06-22 08:05:11Z herbelin $ i*)
...
(*
  Lemma Choice0 :
   ...
   sig (fun f:S -> S' => forall z:S, smash (R z (f z))).
   intro H.
  ...
  Lemma Choice0' :
   (forall x:S, sig (fun y:S' => R x y)) ->
   sig (fun f:S -> S' => smash(forall z:S, R z (f z))).
   intro H.
   exists (fun z:S => match H z with
                      | exist y _ => y
                      end).
