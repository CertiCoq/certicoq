...
(* $Id: pptactic.ml 9954 2007-07-06 16:35:07Z letouzey $ *)
...
let strip_prod_binders_expr n ty =
  let rec strip_ty acc n ty =
    match ty with
      ...
      | Topconstr.CArrow(_,imp,a,b) ->
          let na = ((dummy_loc,Anonymous),imp) in
          if n=1 then
            (List.rev (([na],a)::acc), b)
          else strip_ty (([na],a)::acc) (n-1) b
...
let pr_pose prlc prc na imp c = match na with
  | Anonymous -> spc() ++ pr_impl imp (prc c)
  | Name id -> spc() ++ 
      if imp = Term.Impl then
        str"[" ++ pr_id id ++ str " :=" ++ spc() ++ prlc c ++ str"]"
      else
        surround (pr_id id ++ str " :=" ++ spc() ++ prlc c)
...
let pr_constr_impl (imp,c) =
   spc () ++
  if imp=Term.Impl then str "[" ++ pr_lconstr c ++ str"]"
  else pr_constr c in
...
(*  match t with
  ...
  | _ ->*)
    let s = prlist_with_sep spc pr_name_impl nal ++ str ":" ++ pr_lconstr t in
      ...
      (nal,ty)::bll ->
        if n <= List.length nal then
          match list_chop (n-1) nal with
              _, ((_,Name id),_) :: _ -> id, (nal,ty)::bll
            | bef, ((loc,Anonymous),imp) :: aft ->
                let id = next_ident_away_from (id_of_string"y") avoid in
                id, ((bef@((loc,Name id),imp)::aft, ty)::bll)
            | _ -> assert false
        else
  ...
  let names =
    List.fold_left
      (fun ln (nal,_) -> List.fold_left
          (fun ln na -> match na with ((_,Name id),_) -> id::ln | _ -> ln)
          ln nal)
      [] bll in
...
and pr_atom1 = function
  ...
  | TacCut (imp,c) -> hov 1 (str "cut" ++ pr_constr_impl (imp,c))
  ...
  | TacGeneralize (imps,l) ->
      hov 1 (str "generalize" ++ spc () ++
             prlist_with_sep spc pr_constr_impl (List.combine imps l))
  | TacGeneralizeDep (imp,c) ->
      hov 1 (str "generalize" ++ spc () ++ str "dependent" ++
             pr_constr_impl (imp,c))
  | TacLetTac (na,imp,c,cl) when cl = nowhere ->
      hov 1 (str "pose" ++ pr_pose pr_lconstr pr_constr na imp c)
  | TacLetTac (na,imp,c,cl) ->
      hov 1 (str "set" ++ pr_pose pr_lconstr pr_constr na imp c ++
...
*)
  ...
  | TacSpecialize (n,c) ->
      hov 1 (str "specialize" ++ spc () ++ pr_opt int n ++ 
             pr_with_bindings c)
  | TacLApply (imp,c) -> 
      hov 1 (str "lapply" ++ pr_constr_impl (imp, c))
...
let strip_prod_binders_rawterm n (ty,_) =
  let rec strip_ty acc n ty =
    if n=0 then (List.rev acc, (ty,None)) else
      match ty with
          Rawterm.RProd(loc,na,imp,a,b) ->
            strip_ty (([(dummy_loc,na),imp],(a,None))::acc) (n-1) b
        | _ -> error "Cannot translate fix tactic: not enough products" in
  strip_ty [] n ty
  ...
  let rec strip_ty acc n ty =
    if n=0 then (List.rev acc, (sigma,ty)) else
      match Term.kind_of_term ty with
          Term.Prod(na,imp,a,b) ->
            strip_ty (([(dummy_loc,na),imp],(sigma,a))::acc) (n-1) b
        | _ -> error "Cannot translate fix tactic: not enough products" in
  strip_ty [] n ty
