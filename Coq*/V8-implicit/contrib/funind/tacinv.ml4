...
let lift1_lvars lvars= List.map 
 (function x,(nme,imp,c) -> lift 1 x, (nme, imp,(*lift 1*) c)) lvars

let pop1_levar levars =
 List.map (function ev,imp,tev -> ev, imp,popn 1 tev) levars


let rec add_n_dummy_prod t n = 
 if n<=0 then t
 else add_n_dummy_prod (mkNamedProd (id_of_string "DUMMY") Expl mkthesort t) (n-1)
...
let mkProdEg teq eql eqr concl =
 mkProd (name_of_string "eg", Expl, mkEq teq eql eqr, lift 1 concl)
...
let rec patternify ltypes c nme =
 match ltypes with
  ...
  | (mv,imp,t)::ltypes' -> 
     ...
     let res = 
      patternify ltypes' (mkLambda (newname_append nme "rec", imp,tlift, c')) nme in
     res
...
let rec npatternify ltypes c =
 match ltypes with
  ...
  | (mv,nme,imp,t)::ltypes' -> 
     ...
     let tlift = lift (List.length ltypes') t in
     let res = 
      npatternify ltypes' (mkLambda (newname_append nme "", imp,tlift, c')) in
     res
...
let rec apply_levars c lmetav =
 match lmetav with
  ...
  | (i,imp,typ) :: lmetav' -> 
     ...
     ((exkey,typ)::levars),  applistc trm [mkEvar exkey] [imp]
...
let rec mkAppRel c imps n =
 match imps with
  | [] -> c
  | imp::imps' -> 
     let newc = mkApp (c,[|(mkRel n)|],[|imp|]) in mkAppRel newc imps' (n-1)

let applFull c typofc =
 let lv,_ = decompose_prod typofc in 
 let imps = List.rev(List.map(fun(_,imp,_)->imp)lv) in
 let res = mkAppRel c imps (List.length imps) in
 res
...
let rec build_rel_map typ type_of_b = 
 match (kind_of_term typ), (kind_of_term type_of_b) with
  ...
  | Prod (name,imp,c1,c2), Prod (nameb,impb,c1b,c2b) when imp=impb -> 
     let map1 = build_rel_map c1 c1b in
     let map2 = build_rel_map (pop c2) (pop c2b) in
     merge_smap map1 map2
  | App (f,args,_), App (fb,argsb,_)
     when Array.length args = Array.length argsb ->
...
type mimickinfo =
    {
     ...
     lst_vars: (constr*(name*impl*constr)) list ; (* Variables rencontrées jusque là *)
...
type 'a funind = (* 'A = CONTR  OU  CONSTR ARRAY *)
    {
     ...
     evarlist: (constr*impl*types) list; (* [(ev1,tev1);(ev2,tev2)...]]
...
let rec proofPrinc mi: constr funind =
 match kind_of_term mi.mimick with
  ...
  | Case (cinfo, pcase, b, arrPt) -> 
     ...
     let _nmeb,_,_ = List.hd prod_pcase in
    ...
    { cases with
     princ = if mi.doeqs then mkApp (trm',[|(mkRefl type_of_b newb)|],[|Expl|]) else trm';
     params = [] (* FIX: fix parms here (fixpt inside a match)*)
    }


  | Lambda(nme, imp, typ, cstr) ->
     let _, _,_, cconcl = destProd mi.concl in
     let d,f=mi.fonc in
     let newenv = push_rel (nme,imp,None,typ) mi.env in
     let newlst_var = (* if this lambda is a param, then don't add it here *)
      if mi.fix then (mkRel 1,(nme,imp,typ)) :: lift1_lvars mi.lst_vars
     ...
     if mi.fix 
     then { resrec with princ = mkLambda (nme,imp,typ,resrec.princ) ; params = [] }
       ...
       { resrec with
        ...
        evarlist = List.map (fun (ev,imp,tev) -> ev, imp,substmeta tev) resrec.evarlist;
