...
(* $Id: inductive.ml 9442 2006-12-12 11:06:05Z barras $ *)
...
let find_rectype env c =
  let (t, l, imps) = decompose_app (whd_betadeltaiota env c) in
...
let find_inductive env c =
  ...
  match kind_of_term t with
    | Ind ind
        when (fst (lookup_mind_specif env ind)).mind_finite ->
        (ind, l, imps)
    | _ -> raise Not_found
...
let find_coinductive env c =
  ...
  match kind_of_term t with
    | Ind ind
        when not (fst (lookup_mind_specif env ind)).mind_finite ->
        (ind, l, imps)
    | _ -> raise Not_found
...
let instantiate_params full t args sign =
  ...
  let (rem_args, subs, ty) =
    Sign.fold_rel_context
      (fun (_,_,copt,_) (largs,subs,ty) ->
        match (copt, largs, kind_of_term ty) with
          | (None, a::args, Prod(_,_,_,t)) ->
              (args, a::subs, t)
          | (Some b,_,LetIn(_,_,_,_,t)) ->
              (largs, (substl subs b)::subs, t)
...
let rec make_subst env = function
  | (_,_,Some _,_ as t)::sign, exp, args ->
  ...
  | (na,imp,None,t as d)::sign, Some u::exp, [] ->
	...
	(na,imp,None,t)::ctx, cons_subst u s subst
      else
	d::ctx, subst
  | sign, [], _ ->
...
let build_dependent_inductive ind (_,mip) params all_imps =
...
let is_correct_arity env c pj ind specif params all_imps = 
  let arsign,_ = get_instantiated_arity specif params in
  let rec srec env pt ar u =
    let pt' = whd_betadeltaiota env pt in
    match kind_of_term pt', ar with
      | Prod (na1,imp1,a1,t), ((_,imp2,None,a1')::ar') when imp1=imp2 ->
          let univ =
            try conv env a1 a1'
            with NotConvertible -> raise (LocalArity None) in
          srec (push_rel (na1,imp1,None,a1) env) t ar'
            (Constraint.union u univ)
      | Prod (_,Expl,a1,a2), [] -> (* whnf of t was not needed here! *)
          let ksort = match kind_of_term (whd_betadeltaiota env a2) with
            | Sort s -> family_of_sort s 
	    | _ -> raise (LocalArity None) in
	  let dep_ind = build_dependent_inductive ind specif params all_imps in
          let univ =
...
let build_branches_type ind (_,mip as specif) params dep p =
  let build_one_branch i cty =
    ...
    let inst_imps = imps_from_rel_context  args in
    let nargs = rel_context_length args in
    let (_,allargs,allimps) = decompose_app ccl in
    let (lparams,vargs) = list_chop (inductive_params specif) allargs in
    let (pimps,aimps) = list_chop (inductive_params specif) allimps in
    let (cargs,cimps) =
      if dep then
        let cstr = ith_constructor_of_inductive ind (i+1) in
        let cpimps = List.map (fun _ -> Impl) pimps in 
        let dep_cstr =
          applist
            (mkConstruct cstr,lparams@(local_rels args),cpimps@inst_imps) in
        (vargs @ [dep_cstr], aimps@[Expl])
      else
        (vargs,aimps) in
    let base =
      beta_appvect (lift nargs p)
        (Array.of_list cargs) (Array.of_list cimps) in
    it_mkProd_or_LetIn base args in
  Array.mapi build_one_branch mip.mind_nf_lc

(* [p] is the predicate, [c] is the match object, [realargs] is the
   list of real args of the inductive type *)
let build_case_type dep p c realargs aimps =
  let (args,imps) =
    if dep then (realargs@[c],aimps@[Expl]) else (realargs,aimps) in
  beta_appvect p (Array.of_list args) (Array.of_list imps)

let type_case_branches env (ind,largs,imps) pj c =
  let specif = lookup_mind_specif env ind in 
  let nparams = inductive_params specif in
  let (params,realargs) = list_chop nparams largs in
  let (_,aimps) = list_chop nparams imps in
  let p = pj.uj_val in
  let (dep,univ) = is_correct_arity env c pj ind specif params imps in
  let lc = build_branches_type ind specif params dep p in
  let ty = build_case_type dep p c realargs aimps in
  (lc, ty, univ)
...
let push_var renv (x,imp,ty,spec) =
  { renv with 
    env = push_rel (x,imp,None,ty) renv.env;
...
let push_var_renv renv (x,imp,ty) =
  push_var renv (x,imp,ty,Not_subterm)
...
let add_subterm renv (x,imp,a,spec) =
  assert (imp=Expl);
  push_var renv (x,imp,a,spec_of_tree spec)
    ...
    match lrec with
        ra::lr ->
          ...
          (match kind_of_term c' with
              Lambda(x,imp,a,b) ->
                let renv' = push_var renv (x,imp,a,ra) in
...
let rec subterm_specif renv t = 
  ...
  let f,l,imps = decompose_app (whd_betadeltaiota renv.env t) in
...
*)
        ...
        let oind =
          ...
          try let (ind,_,_) = find_inductive env' clfix in Some ind
        ...
        (match oind with
          ...
          | Some ind ->
              ...
              let sign,strippedBody =
                decompose_lam_n_assum nbOfAbst theBody in
    ...
    | Lambda (x,imp,a,b) -> 
        assert (l=[]);
        subterm_specif (push_var_renv renv (x,imp,a)) b
...
let check_one_fix renv recpos def =
  ...
  let rec check_rec_call renv t = 
    ...
    if noccur_with_meta renv.rel_min nfi t then ()
      ...
      let (f,l,imps) = decompose_app (whd_betaiotazeta renv.env t) in
      match kind_of_term f with
        | Rel p -> 
        ...
        | Const kn -> 
	        ...
	        check_rec_call renv
                  (applist(constant_value renv.env kn, l, imps))
        ...
        | Lambda (x,imp,a,b) ->
            check_rec_call (push_var_renv renv (x,imp,a)) b;
  ...
  and check_nested_fix_body renv decr recArgsDecrArg body =
	...
	| Lambda (x,imp,a,b) ->
	    check_rec_call renv a;
            let renv' = push_var_renv renv (x,imp,a) in
    ...
    let rec check_occur env n def = 
      match kind_of_term (whd_betadeltaiota env def) with
	    ...
	    if noccur_with_meta n nbfix a then
	      let env' = push_rel (x, imp, None, a) env in
              if n = k+1 then
                if imp=Impl then failwith "fix on an implicit arg" else
...
let rec codomain_is_coind env c =
  ...
  match kind_of_term b with
    | Prod (x,imp,a,b) ->
	codomain_is_coind (push_rel (x, imp, None, a) env) b 
    | _ -> 
...
let check_one_cofix env nbfix def deftype = 
  let rec check_rec_call env alreadygrd n vlra  t =
    if not (noccur_with_meta n nbfix t) then
      let c,args,imps = decompose_app (whd_betadeltaiota env t) in
      match kind_of_term c with 
