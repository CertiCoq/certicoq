...
let destructurate t =
  let c, args,_ = Term.decompose_app t in
  match Term.kind_of_term c, args with
    ...
    | Term.Prod (Names.Anonymous,Term.Expl,typ,body), [] -> Kimp(typ,body)
    | Term.Prod (Names.Name _,_,_,_),[] ->
...
let dest_const_apply t = 
  let f,args,_ = Term.decompose_app t in 
  let ref = 
...
let do_left t = 
  ...
  else Term.mkApp (Lazy.force coq_c_do_left, [|t |],[|Term.Expl|] )

let do_right t = 
  if t = Lazy.force coq_c_nop then Lazy.force coq_c_nop
  else Term.mkApp (Lazy.force coq_c_do_right, [|t |],[|Term.Expl|])

let do_both t1 t2 = 
  if t1 = Lazy.force coq_c_nop then do_right t2
  else if t2 = Lazy.force coq_c_nop then do_left t1
  else
    Term.mkApp (Lazy.force coq_c_do_both , [|t1; t2 |],[|Term.Expl;Term.Expl|])

let do_seq t1 t2 =
  if t1 = Lazy.force coq_c_nop then t2
  else if t2 = Lazy.force coq_c_nop then t1
  else
    Term.mkApp (Lazy.force coq_c_do_seq, [|t1; t2 |],[|Term.Expl;Term.Expl|])
...
let rec mk_nat = function
  | 0 -> Lazy.force coq_O
  | n -> Term.mkApp (Lazy.force coq_S, [| mk_nat (n-1) |],[|Term.Expl|])
...
let mk_list typ l =
  let rec loop = function
    | [] ->
	Term.mkApp (Lazy.force coq_nil, [|typ|],[|Term.Impl|])
    | (step :: l) -> 
	Term.mkApp (Lazy.force coq_cons,
          [|typ; step; loop l |],[|Term.Impl;Term.Expl;Term.Expl|]) in
  loop l
  ...
  if n=Bigint.one then Lazy.force coq_xH 
    ...
    Term.mkApp
      ((if r = Bigint.zero then Lazy.force coq_xO else Lazy.force coq_xI),
       [| mk_positive q |],[|Term.Expl|]) 

let mk_Z n =
  if n = Bigint.zero then Lazy.force coq_Z0 
  else if Bigint.is_strictly_pos n then
    Term.mkApp (Lazy.force coq_Zpos, [| mk_positive n |],[|Term.Expl|])
  else
    Term.mkApp (Lazy.force coq_Zneg,
      [| mk_positive (Bigint.neg n)|],[|Term.Expl|])
