...
let mkDecideEqGoal eqonleft op rectype c1 c2 g = 
  let equality    =
    mkApp(build_coq_eq(), [|rectype; c1; c2|],[|Expl;Expl;Expl|]) in
  let disequality =
    mkApp(build_coq_not (), [|equality|],[|Expl|]) in
  if eqonleft then mkApp(op, [|equality; disequality |],[|Expl;Expl|])
  else mkApp(op, [|disequality; equality |],[|Expl;Expl|])
...
let mkGenDecideEqGoal rectype g = 
  ...
  (mkNamedProd xname Expl rectype 
     (mkNamedProd yname Expl rectype 
        (mkDecideEqGoal true (build_coq_sumbool ())
          rectype (mkVar xname) (mkVar yname) g)))

let eqCase tac eqns = 
  tclTHEN intro (onLastHyp (fun eqn -> tac (eqn::eqns)))
...
let solveArg eqonleft op a1 a2 tac eqns g = 
  let rectype = pf_type_of g a1 in
  let decide  = mkDecideEqGoal eqonleft op rectype a1 a2 g in
  let subtacs = 
    if eqonleft then [eqCase tac eqns ;diseqCase eqonleft;default_auto] 
    else [diseqCase eqonleft;eqCase tac eqns;default_auto] in
  (tclTHENS (h_elim_type decide) subtacs) g

let finishArg eqonleft eqns =
  tclTHEN (choose_eq eqonleft)
    (List.fold_left
      (fun tac eqn -> tclTHEN (Equality.rewriteLR (mkVar eqn)) tac)
      h_reflexivity eqns)
...
let solveEqBranch rectype g =
  try
    ...
    let getargs l =
      let _,args,imps = decompose_app l in
      list_skipn nparams args in
    let rargs   = getargs rhs
    and largs   = getargs lhs in 
    List.fold_right2 (solveArg eqonleft op) largs rargs
      (finishArg eqonleft) [] g
...
let decideEquality c1 c2 g = 
  ...
  (tclTHENS (cut Expl decide) [default_auto;decideEqualityGoal]) g
