...
(* $Id: unification.ml 9879 2007-06-07 07:45:49Z herbelin $ *)
...
let abstract_scheme env c l lname_typ =
  List.fold_left2
    (fun t (locc,a) ((na,imp,_,ta)) ->
       let na = match kind_of_term a with Var id -> Name id | _ -> na in
       if occur_meta ta then error "cannot find a type for the generalisation"
       else if occur_meta a then lambda_name env (na,imp,ta,t)
       else lambda_name env (na,imp,ta,subst_term_occ locc a t))
    c
    (List.rev l)
    lname_typ
...
let abstract_list_all env sigma typ c l =
  ...
  let p =
    abstract_scheme env c (List.map (function a -> [],a) l) ctxt in 
  try 
...
let solve_pattern_eqn_array env f l imps c (metasubst,evarsubst) =
  match kind_of_term f with
    | Meta k -> 
	let pb = (ConvUpToEta (Array.length l),TypeNotProcessed) in
	(k,solve_pattern_eqn env (Array.to_list l) (Array.to_list imps) c,pb)
        ::metasubst,
        evarsubst
    | Evar ev ->
      (* Currently unused: incompatible with eauto/eassumption backtracking *)
	metasubst,(ev,solve_pattern_eqn env (Array.to_list l) (Array.to_list imps) c)::evarsubst
    | _ -> assert false
...
let unify_0_with_initial_metas metas env sigma cv_pb mod_delta m n =
  ...
  let rec unirec_rec curenv pb ((metasubst,evarsubst) as substn) curm curn =
	...
	| _, Evar ev -> metasubst,((ev,cM)::evarsubst)
        (* Unify domain types even if they are implicit subterms! *)
	| Lambda (na,Expl,t1,c1), Lambda (_,Expl,t2,c2) ->
	    unirec_rec (push_rel_assum (na,Expl,t1) curenv) topconv
	      (unirec_rec curenv topconv substn t1 t2) c1 c2
        | Lambda (_,Impl,_,c1), _ ->
            unirec_rec curenv pb substn (lift(-1)c1) curn
        | _, Lambda (_,Impl,_,c2) ->
            unirec_rec curenv pb substn curm (lift(-1)c2)
	| Prod (na,imp1,t1,c1), Prod (_,imp2,t2,c2) when imp1=imp2 ->
	    unirec_rec (push_rel_assum (na,imp1,t1) curenv) (prod_pb pb) 
	      (unirec_rec curenv topconv substn t1 t2) c1 c2
	| LetIn (_,_,b,_,c), _ -> unirec_rec curenv  pb substn (subst1 b c) cN
	| _, LetIn (_,_,b,_,c) -> unirec_rec curenv  pb substn cM (subst1 b c)
	    
	| App (f1,l1,imps1), _ when
	    isMeta f1 & is_unification_pattern f1 l1 & not (dependent f1 cN) ->
	      solve_pattern_eqn_array curenv f1 l1 imps1 cN substn
	| _, App (f2,l2,imps2) when
	    isMeta f2 & is_unification_pattern f2 l2 & not (dependent f2 cM) ->
	      solve_pattern_eqn_array curenv f2 l2 imps2 cM substn

	| App (f1,l1,imps1), App (f2,l2,imps2) ->
		...
		if len1 = len2 then (f1,l1,f2,l2)
		  ...
		  let extras,restl2 = array_chop (len2-len1) l2 in 
                  let eimps,_ = array_chop (len2-len1) imps2 in
		    (f1, l1, appvect (f2,extras,eimps), restl2)
		else 
		  let extras,restl1 = array_chop (len1-len2) l1 in 
                  let eimps,_ = array_chop (len1-len2) imps1 in
		    (appvect (f1,extras,eimps), restl1, f2, l2) in
...
let rec unify_with_eta keptside mod_delta env sigma k1 k2 c1 c2 =
  ...
  | (Lambda (na,imp1,t1,c1'), Lambda (_,imp2,t2,c2')) when imp1=imp2 ->
      let env' = push_rel_assum (na,imp1,t1) env in
  ...
  let rec apprec n c cty evd =
    if n = 0 then 
      ...
      match kind_of_term (whd_betadeltaiota env (evars_of evd) cty) with
        | Prod (_,imp,c1,c2) ->
            let (evd',evar) = 
	      Evarutil.new_evar evd env ~src:(dummy_loc,GoalEvar) imp c1 in
	    apprec (n-1) (mkApp(c,[|evar|],[|imp|])) (subst1 evar c2) evd'
...
let w_merge env with_types mod_delta metas evars evd =
  let rec w_merge_rec evd metas evars eqns =
    ...
    | ((evn,_ as ev),rhs)::evars' ->
	  ...
	  | App (f,cl,imps) when is_mimick_head f ->
	      (try 
    ...
    (try 
     ...
     with ex when precatchable_exception ex ->
	  ...
	  | App (f,args,imps) ->
	      ...
	      assert (n>0);
	      let c1 =
                mkApp (f,Array.sub args 0 (n-1),Array.sub imps 0 (n-1)) in
	      let c2 = args.(n-1) in
	      (try 
		 matchrec c1
	       with ex when precatchable_exception ex -> 
...
let w_unify2 env mod_delta allow_K cv_pb ty1 ty2 evd =
  let c1, oplist1, imps1 = whd_stack ty1 in
  let c2, oplist2, imps2 = whd_stack ty2 in
  match kind_of_term c1, kind_of_term c2 with
    | Meta p1, _ ->
        (* Find the predicate *)
	let evd' =
...
let w_unify allow_K env cv_pb ?(mod_delta=true) ty1 ty2 evd =
  ...
  let hd1,l1, _ = whd_stack ty1 in
  let hd2,l2, _ = whd_stack ty2 in
