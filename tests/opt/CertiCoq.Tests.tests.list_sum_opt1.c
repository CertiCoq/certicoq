#ifndef CERTICOQ_TESTS_TESTS_LIST_SUM_OPT1_C
#define CERTICOQ_TESTS_TESTS_LIST_SUM_OPT1_C
#include <gc_stack.h>
#include <prim_string.h>
#include <prim_floats.h>
#include <prim_int63.h>
#include <coq_c_ffi.h>
#include "CertiCoq.Tests.tests.list_sum_opt1.h"
...
value body(struct thread_info *$tinfo)
{
  ...
  register value $CertiCoqdTestsdtestsdlist_sum_231;
  register value *$alloc;
  register value *$limit;
  register value *$args;
  $alloc = (*$tinfo).alloc;
  ...
  *((value *) $y_226 + 0LL) = $y_225;
  ...
  (*$tinfo).limit = $limit;
  /*skip*/;
  $CertiCoqdTestsdtestsdlist_sum_231 =
    ((value (*)(struct thread_info *, value, value)) fold_left_uncurried_known_102)
    ($tinfo, $y_230, $y_228);
  $alloc = (*$tinfo).alloc;
  $limit = (*$tinfo).limit;
  ...
  (*$tinfo).alloc = $alloc;
  (*$tinfo).limit = $limit;
  return $CertiCoqdTestsdtestsdlist_sum_231;
}


#endif /* CERTICOQ_TESTS_TESTS_LIST_SUM_OPT1_C */
