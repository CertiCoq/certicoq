...
(* $Id: ccalgo.ml 10042 2007-07-24 13:55:50Z corbinea $ *)
...
type term=
    Symb of constr
  | Eps
  | Appli of term*term*impl
  | Constructor of cinfo (* constructor arity + nhyps *)

type ccpattern =
    PApp of term * ccpattern list * impl list (* arguments are reversed *)
  | PVar of int 
...
let rec constr_of_term = function
  ...
  | Appli (s1,s2,imp)->
      make_app [(constr_of_term s2)] [imp] s1
and make_app l imps=function
    Appli (s1,s2,imp)->make_app ((constr_of_term s2)::l) (imp::imps) s1    
  | other -> applistc (constr_of_term other) l imps 
...
let rec inst_pattern subst = function
  ...
  | PApp (t, args,imps) -> 
      List.fold_right2
	(fun spat imp f -> Appli (f,inst_pattern subst spat,imp))
	   args imps t  
...
let add_inst state (inst,int_subst) = 
  ...
  let imps = Array.map (fun _ -> Expl) args in
  let _ = array_rev args in (* highest deBruijn index first *)
  let prf= mkApp(prfhead,args,imps) in
	...
	[] -> 
	  ...
	  let uf=state.uf in
		...
		PVar i -> 
	      ...
	      | PApp(f, (last_arg::rem_args as args), _::rimps) ->
		  (try 
