...
(* $Id: indtypes.ml 9831 2007-05-17 18:55:42Z herbelin $ *)
...
let rec infos_and_sort env t =
  match kind_of_term t with
	...
	let env1 = Environ.push_rel (name,imp,None,varj.utj_val) env in
...
let infer_constructor_packet env_ar params lc =
  ...
  let _ = Array.iter (fun j ->
    if not (noccur_implicit_env env_ar_par j.utj_val) then
      failwith "a constructor type uses an implicit parameter") jlc in
  let cparams = set_impl_rel_context Impl params in
  let lc'' =
    Array.map (fun j -> it_mkProd_or_LetIn j.utj_val cparams) jlc in
...
let typecheck_inductive env mie =
  ...
  let env_params, params, cst1 =
    infer_local_decls env mie.mind_entry_params in
  ...
  let cst, env_arities, rev_arity_list =
    List.fold_left
	 ...
	 let arity, cst2 = infer_type env_params ind.mind_entry_arity in
         if not (noccur_implicit_env env_params arity.utj_val) then
           failwith "arity uses an implicit parameter";
	 ...
	 let id = ind.mind_entry_typename in
	 let env_ar' = push_rel (Name id, Expl, None, full_arity) env_ar in
	 let lev =
  ...
  let param_ccls = List.fold_left (fun l (_,_,b,p) ->
    if b = None then 
...
let check_correct_par (env,n,ntypes,_) hyps l largs =
  ...
  let rec check k index = function
    | [] -> ()
    | (_,_,Some _,_)::hyps -> check k (index+1) hyps
    | _::hyps ->
...
(* start from 0, hyps will be in reverse order *)
  ...
  let rec find k index = 
	...
	| (lp,(_,_,Some _,_)::hyps) -> find k (index-1) (lp,hyps)
	| (p::lp,_::hyps) ->
 ...
 *) 
let ienv_push_var (env, n, ntypes, lra) (x,imp,a,ra) =
 (push_rel (x,imp,None,a) env, n+1, ntypes, (Norec,ra)::lra)

let ienv_push_inductive (env, n, ntypes, ra_env) (mi,lpar) =
  let auxntyp = 1 in
  let specif = lookup_mind_specif env mi in
  let env' =
    push_rel (Anonymous,Expl,None,
...
let check_positivity_one (env, _,ntypes,_ as ienv) hyps i indlc =
  ...
  let rec check_pos (env, n, ntypes, ra_env as ienv) nmr c = 
    let x,largs,aimps = decompose_app (whd_betadeltaiota env c) in
	...
	| Prod (na,imp,b,d) ->
		...
		None -> failwith_non_pos_list n ntypes [b]
              | Some b ->
	          check_pos (ienv_push_var ienv (na, imp, b, mk_norec)) nmr d)
	| Rel k ->
  ...
  and check_constructors ienv check_head nmr c = 
    let rec check_constr_rec (env,n,ntypes,ra_env as ienv) nmr lrec c = 
      let x,largs,_ = decompose_app (whd_betadeltaiota env c) in
	match kind_of_term x with
          | Prod (na,imp,b,d) -> 
	      assert (largs = []);
              let nmr',recarg = check_pos ienv nmr b in 
              let ienv' = ienv_push_var ienv (na,imp,b,mk_norec) in
...
let fold_inductive_blocks f =
  Array.fold_left (fun acc (_,_,lc,(arsign,_)) ->
    f (Array.fold_left f acc lc)
      (it_mkProd_or_LetIn (* dummy *) mkSet arsign))
...
let build_inductive env env_ar params isrecord isfinite inds nmr recargs cst =
  ...
  let build_one_packet (id,cnames,lc,(ar_sign,ar_kind)) recarg =
    ...
    let nf_lc =
      Array.map (fun (d,b) -> it_mkProd_or_LetIn b d) splayed_lc in
    let nf_lc = if nf_lc = lc then lc else nf_lc in
    let consnrealargs =
      Array.map
        (fun (d,_) -> rel_context_length d - rel_context_length params)
	splayed_lc in
      (* Elimination sorts *)
...
let check_inductive env mie =
  ...
  build_inductive env env_ar params
    mie.mind_entry_record mie.mind_entry_finite inds nmr recargs cst
