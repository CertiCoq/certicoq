...
Ltac gen_srewrite O I add mul sub opp eq Rsth Reqe ARth :=
  repeat first
     ...
     | match goal with
       | |- context [opp(add _ _)] => rewrite (ARopp_add ARth)
       | |- context [sub _ _] => rewrite (ARsub_def ARth)
       | |- context [mul (opp _) _] => rewrite <- (ARopp_mul_l ARth)
       | |- context [mul _ (opp _)] =>
            rewrite <- (ARopp_mul_r Rsth Reqe ARth)
       end].
