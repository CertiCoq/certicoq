...
let find_class_type t =
  let t', args, imps = decompose_app (Reductionops.whd_betaiotazeta t) in
  match kind_of_term t' with
    | Var id -> CL_SECVAR id, (args, imps)
    | Const sp -> CL_CONST sp, (args, imps)
    | Ind ind_sp -> CL_IND ind_sp, (args, imps)
    | Prod (_,_,_,_) -> CL_FUN, ([], [])
    | Sort _ -> CL_SORT, ([], [])
    |  _ -> raise Not_found
...
let class_of env sigma t = 
  let (t, n1, i, args) = 
    ...
    with Not_found ->
      ...
      (t, n1, i, args)
  in
  if List.length (fst args) = n1 then t, i else raise Not_found
...
let reference_arity_length ref =
  ...
  List.length
    (fst (Reductionops.splay_arity (Global.env()) Evd.empty t))
...
let discharge_coercion (_,(coe,stre,isid,cls,clt,ps)) =
  if stre = Local then None else 
    let n =
      try 
        let (inst,_) = Lib.section_instance coe in Array.length inst
      with Not_found -> 0 in
