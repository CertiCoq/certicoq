(** A big-step semantics for the Clight language.
    More or less the same as compcert's ClightBigstep.v, but with slight
    modification to put arguments into temporary variables rather than
    stack locals. *)
...
with eval_funcall: mem -> fundef -> list val -> trace -> mem -> val -> Prop :=
  | eval_funcall_internal: forall le f vargs t e m1 m2 m3 out vres m4,
      list_norepet (var_names f.(fn_vars)) ->
      list_norepet (var_names f.(fn_params)) ->
      list_disjoint (var_names f.(fn_params)) (var_names f.(fn_temps)) ->
      alloc_variables ge empty_env m1 f.(fn_vars) e m2 ->
      ...
      eval_funcall m1 (Internal f) vargs t m4 vres
...
with evalinf_funcall: mem -> fundef -> list val -> traceinf -> Prop :=
  | evalinf_funcall_internal: forall f vargs t e m1 m2 le,
      list_norepet (var_names f.(fn_vars)) ->
      list_norepet (var_names f.(fn_params)) ->
      list_disjoint (var_names f.(fn_params)) (var_names f.(fn_temps)) ->
      alloc_variables ge empty_env m1 f.(fn_vars) e m2 ->
      ...
      evalinf_funcall m1 (Internal f) vargs t.
