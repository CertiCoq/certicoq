...
(* $Id: detyping.ml 9919 2007-06-30 11:33:15Z herbelin $ *)
...
let rec decomp_branch n nal b (avoid,env as e) c =
  if n=0 then (List.rev nal,(e,c))
	...
	| Lambda (na,imp,_,c) -> na,imp,c,concrete_let_name
	| LetIn (na,imp,_,_,c) -> na,imp,c,concrete_name
	| _ -> 
	    Name (id_of_string "x"),Expl,
            (applist (lift 1 c, [mkRel 1],[Expl])), 
	    concrete_name in
    let na',avoid' = f (Some b) avoid env na c in
    decomp_branch (n-1) ((na',imp)::nal) b (avoid',add_name na' env) c
...
and align_tree nal isgoal (e,c as rhs) = match nal with
  ...
  | (na,imp)::nal ->
	...
	List.flatten
	    ...
	    clauses)
    | _ ->
	let pat = PatVar(dl,update_name na rhs,imp) in
...
let it_destRLambda_or_LetIn_names n c =
  let rec aux n nal c =
    if n=0 then (List.rev nal,c) else match c with
      | RLambda (_,na,imp,_,c) -> aux (n-1) ((na,imp)::nal) c
      | RLetIn (_,na,imp,_,c) -> aux (n-1) ((na,imp)::nal) c
      | _ ->
          (* eta-expansion *)
	  let rec next l =
...
(* 	    if occur_rawconstr x c then next (x::l) else x in *)
	  ...
	  let a = RVar (dl,x) in
	  aux (n-1) ((Name x,Expl) :: nal)
            (match c with
              | RApp (loc,p,l,imps) -> RApp (loc,c,l@[a],imps@[Expl])
              | _ -> (RApp (dl,c,[a],[Expl])))
  in aux n [] c
...
let detype_case computable detype detype_eqns testdep avoid data p c bl =
    ...
    then 
      ...
      match option_map detype p with
	    ...
	    let n,typ = match typ with 
	      ...
	      | _ -> Anonymous, typ in
	    let aliastyp =
	      if List.for_all (fun (na,_) -> na=Anonymous) nl then None
  ...
  | LetStyle when aliastyp = None -> 
      ...
      let (nal,d) =
        it_destRLambda_or_LetIn_names consnargsl.(0) bl'.(0) in
...
let rec detype (isgoal:bool) avoid env t =
  match kind_of_term (collapse_appl t) with
    ...
    | Prod (na,imp,ty,c) ->
        detype_binder isgoal BProd avoid env na imp ty c
    | Lambda (na,imp,ty,c) ->
        detype_binder isgoal BLambda avoid env na imp ty c
    | LetIn (na,imp,b,_,c) ->
        detype_binder isgoal BLetIn avoid env na imp b c
    | App (f,args,imps) ->
...
and share_names isgoal n l avoid env c t =
  match kind_of_term c, kind_of_term t with
    ...
    | Lambda (na,_,t,c), Prod (na',imp,t',c') ->
	...
	let id = next_name_away na avoid in 
        ...
        share_names isgoal (n-1) ((Name id,imp,None,t)::l) avoid env c c'
    (* May occur for fix built interactively *)
    | LetIn (na,imp,b,t',c), _ when n > 0 ->
        let t' = detype isgoal avoid env t' in
        let b = detype isgoal avoid env b in
	let id = next_name_away na avoid in 
        let avoid = id::avoid and env = add_name (Name id) env in
        share_names isgoal n ((Name id,imp,Some b,t')::l) avoid env c t
    (* Only if built with the f/n notation or w/o let-expansion in types *)
    | _, LetIn (_,_,b,_,t) when n > 0 ->
	share_names isgoal n l avoid env c (subst1 b t)
    (* If it is an open proof: we cheat and eta-expand *)
    | _, Prod (na',imp,t',c') when n > 0 ->
        let t' = detype isgoal avoid env t' in
	let id = next_name_away na' avoid in 
        let avoid = id::avoid and env = add_name (Name id) env in
        let appc = mkApp (lift 1 c,[|mkRel 1|],[|imp|]) in
        share_names isgoal (n-1) ((Name id,imp,None,t')::l) avoid env appc c'
...
and detype_eqn isgoal avoid env constr construct_nargs branch =
  let make_pat x imp avoid env b ids =
    if force_wildcard () & noccurn 1 b then
      PatVar (dl,Anonymous,imp),avoid,(add_name Anonymous env),ids
    else 
      let id = next_name_away_in_cases_pattern x avoid in
      PatVar (dl,Name id,imp),id::avoid,(add_name (Name id) env),id::ids
  in
  let rec buildrec ids patlist avoid env n b =
	...
	| Lambda (x,imp,_,b) -> 
	    let pat,new_avoid,new_env,new_ids =
              make_pat x imp avoid env b ids in
	...
	| _ -> (* eta-expansion : n'arrivera plus lorsque tous les
	    ...
	    let new_b = applist (lift 1 b, [mkRel 1],[Expl]) in
            let pat,new_avoid,new_env,new_ids =
	      make_pat Anonymous Expl avoid env new_b ids in
	    buildrec new_ids (pat::patlist) new_avoid new_env (n-1) new_b
	  
  in 
  buildrec [] [] avoid env construct_nargs branch

and detype_binder isgoal bk avoid env na imp ty c =
  let na',avoid' =
    if bk = BLetIn then
  ...
  match bk with
  | BProd -> RProd (dl, na',imp,detype isgoal avoid env ty, r)
  | BLambda -> RLambda (dl, na',imp,detype isgoal avoid env ty, r)
  | BLetIn -> RLetIn (dl, na',imp,detype isgoal avoid env ty, r)
...
let rec detype_rel_context where avoid env sign =
  ...
  | (na,imp,b,t)::rest ->
	...
	| Some c ->
      ...
      (na',imp,b,t) :: aux avoid' (add_name na' env) rest
...
let simple_cases_matrix_of_branches ind brns brs =
  list_map2_i (fun i n b ->
      ...
      let mkPatVar (na, imp) = PatVar (dummy_loc,na,imp) in
      let p =
        PatCstr (dummy_loc,(ind,i+1),List.map mkPatVar nal,Anonymous) in
      let ids = map_succeed (fun (na,_) ->Nameops.out_name na) nal in
      (dummy_loc,ids,[p],c))
    0 brns brs
...
let return_type_of_predicate ind nparams n pred =
  ...
  (fst(List.hd nal),
  Some (dummy_loc, ind, nparams, List.tl nal)), Some p
