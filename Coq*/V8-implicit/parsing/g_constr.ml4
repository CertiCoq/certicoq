...
(* $Id: g_constr.ml4 10007 2007-07-16 09:18:44Z corbinea $ *)
...
let rec index_and_rec_order_of_annot loc bl ann =
  match names_of_local_assums bl,ann with
    | [_], (None, r) -> Some 0, r
    | lids, (Some x, ro) ->
        let ids = List.map (fun ((_,na),_) -> na) lids in
...
let test_name_impl =
  Gram.Entry.of_parser "test_name_impl"
    (fun strm ->
      match Stream.npeek 1 strm with
        | [("IDENT","_")] -> Stream.junk strm; (Anonymous,Expl)
        | [("IDENT",s)] ->
            Stream.junk strm;
            (Name(id_of_string s),Expl)
        | [("","[")] ->
            (match Stream.npeek 2 strm with
	      | [_; ("IDENT",s)] ->
                  (match Stream.npeek 3 strm with
                    | [_; _; ("", "]")] ->
                        Stream.junk strm; Stream.junk strm; Stream.junk strm;
                        let na = if s="_" then Anonymous else
                          Name(id_of_string s) in
                        (na,Impl)
	            | _ -> raise Stream.Failure)
              | _ -> raise Stream.Failure)
        | _ -> raise Stream.Failure)
...
GEXTEND Gram
  ...
  operconstr:
    ...
    | "90" RIGHTA
      [ c1 = operconstr; "->"; c2 = binder_constr ->
        CArrow(loc,Expl,c1,c2)
      | c1 = operconstr; "->"; c2 = SELF -> CArrow(loc,Expl,c1,c2)
      | "["; c1 = operconstr; "]"; "->"; c2 = binder_constr ->
          CArrow(loc,Impl,c1,c2)
      | "["; c1 = operconstr; "]"; "->"; c2 = SELF ->
          CArrow(loc,Impl,c1,c2)]
    | "10" LEFTA
      [ f=operconstr; args=LIST1 appl_arg -> CApp(loc,(None,f),args)
      | "@"; f=global; args=LIST0 appl_earg ->
          CAppExpl(loc,(None,f),args)
      | "@"; "?"; (locid,id) = identref; args=LIST1 identref ->
          let args = List.map (fun x -> (CRef (Ident x),Expl), None) args in
          CApp(loc,(None,CPatVar(locid,(true,id))),args) ]
    | "9"
        [ ".."; c = operconstr LEVEL "0"; ".." ->
          CAppExpl (loc,(None,Ident (loc,Topconstr.ldots_var)),[c,Expl]) ]
    | "8" LEFTA [ ]
    | "1" LEFTA
      [ c=operconstr; ".("; f=global; args=LIST0 appl_arg; ")" ->
	CApp(loc,(Some (List.length args+1),CRef f),args@[(c,Expl),None])
      | c=operconstr; ".("; "@"; f=global;
        args=LIST0 appl_earg; ")" ->
        CAppExpl(loc,(Some (List.length args+1),f),args@[c,Expl]) 
  ...
  binder_constr:
    [ [ "forall"; bl = binder_list; ","; c = operconstr LEVEL "200" ->
      ...
      | "fun"; bl = binder_list; "=>"; c = operconstr LEVEL "200" ->
          mkCLambdaN loc bl c
      | "let"; (id,imp)=name_impl; bl = LIST0 binder_let; ty = type_cstr; ":=";
        c1 = operconstr LEVEL "200"; "in"; c2 = operconstr LEVEL "200" ->
          let loc1 = loc_of_binder_let bl in
          CLetIn(loc,id,imp,mkCLambdaN loc1 bl (mk_cast(c1,ty)),c2)
      | "let"; fx = single_fix; "in"; c = operconstr LEVEL "200" ->
          ...
          let (li,id) = match fixp with
            ...
            | CCoFix(_,id,_) -> id
            | _ -> assert false in
          CLetIn(loc,(li,Name id),Expl,fixp,c)
      | "let"; lb = ["("; l=LIST0 name_impl SEP ","; ")" -> l | "()" -> []];
	  po = return_type;
  ...
  appl_arg:
    [ [ id = lpar_id_coloneq; c=lconstr; ")" ->
	  ((c,Expl),Some (loc,ExplByName id))
      | id = lpar_id_coloneq; "["; c=lconstr; "]"; ")" ->
	  ((c,Impl),Some (loc,ExplByName id))
      | c=operconstr LEVEL "9" -> ((c,Expl),None)
      | "["; c=lconstr; "]" -> ((c,Impl),None) ] ]
  ;
  appl_earg:
    [ [ "["; c=lconstr; "]" -> (c,Impl)
      | c=operconstr LEVEL "9" -> (c,Expl) ] ]
  ;
  atomic_constr:
    [ [ g=global -> CRef g
      | s=sort -> CSort (loc,s)
  ...
  pattern:
    ...
    | "10" LEFTA
      [ p = pattern; lp = LIST1 NEXT ->
        (match p with
          | CPatAtom (_, Some r,Expl) -> CPatCstr (loc, r, lp)
          | _ -> Util.user_err_loc 
    ...
    | "0"
      [ r = Prim.reference -> CPatAtom (loc,Some r,Expl)
      | "["; r = Prim.reference; "]" -> CPatAtom (loc,Some r,Impl)
      | "_" -> CPatAtom (loc,None,Expl)
      | "["; "_"; "]" -> CPatAtom (loc,None,Impl)
      | "("; p = pattern LEVEL "200"; ")" ->
          (match p with
  ...
  ;
  name_impl0:[[(na,imp)=test_name_impl->((loc,na),imp)]];
  binder_list:
    [ [ "["; idl=LIST1 name; "]"; bl=binder_list_end ->
          bl loc (List.map (fun na -> (na,Impl)) idl)
      | id=name; bl=binder_list_end ->
          bl loc [(id,Expl)]
      | "["; idl=LIST1 name; ":"; c=lconstr; "]"; bl=LIST0 binder_let -> 
          LocalRawAssum (List.map(fun na->na,Impl)idl,c) :: bl
      | "("; idl=LIST1 name_impl; ":"; c=lconstr; ")"; bl=LIST0 binder_let ->
          LocalRawAssum (idl,c)::bl ] ]
  ;
  binder_list_end:
    [ [ idl=LIST0 name_impl0; ":"; c=lconstr ->
          fun _ idl0 -> [LocalRawAssum(idl0@idl,c)]
      | idl=LIST0 name_impl0; bl=LIST0 binder_let  ->
          fun loc idl0 -> LocalRawAssum(idl0@idl,CHole loc)::bl ] ]
  ;
  binder_let:
    [ [ na=name ->
          LocalRawAssum ([na,Expl],CHole loc)
      | "("; id=name_impl; idl=LIST1 name_impl; ":"; c=lconstr; ")" -> 
          LocalRawAssum (id::idl,c)
      | "("; id=name_impl; ":"; c=lconstr; ")" -> 
          LocalRawAssum ([id],c)
      | "["; na=name; "]" ->
          LocalRawAssum ([na,Impl],CHole loc)
      | "["; id=name; idl=LIST1 name; ":"; c=lconstr; "]" -> 
          LocalRawAssum (List.map(fun na->na,Impl)(id::idl),c)
      | "["; id=name; ":"; c=lconstr; "]" -> 
          LocalRawAssum ([id,Impl],c)
      | "("; (id,imp)=name_impl; ":="; c=lconstr; ")" ->
          LocalRawDef (id,imp,c)
      | "("; (id,imp)=name_impl; ":"; t=lconstr; ":="; c=lconstr; ")" -> 
          LocalRawDef
            (id,imp,CCast (join_loc (constr_loc t) loc,c,
             CastConv (DEFAULTcast,t)))
    ] ]
  ;
  binder:
    [ [ id=name_impl -> ([id],CHole loc)
      | "("; idl=LIST1 name_impl; ":"; c=lconstr; ")" -> (idl,c) ] ]
  ;
  type_cstr:
