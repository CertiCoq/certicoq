...
(* $Id: rawterm.ml 9718 2007-03-19 16:54:25Z msozeau $ *)
...
type rawconstr = 
  ...
  | RApp of loc * rawconstr * rawconstr list * impl list
  | RLambda of loc * name * impl * rawconstr * rawconstr
  | RProd of loc * name * impl * rawconstr * rawconstr
  | RLetIn of loc * name * impl * rawconstr * rawconstr
  | RCases of loc * rawconstr option * tomatch_tuple * cases_clauses
  | RLetTuple of loc * (name * impl) list * (name * rawconstr option) * 
      rawconstr * rawconstr
...
and rawdecl = name * impl * rawconstr option * rawconstr
...
and predicate_pattern =
    name * (loc * inductive * int * (name * impl) list) option
...
let cases_predicate_names tml =
  List.flatten (List.map (function
    | (tm,(na,None)) -> [na]
    | (tm,(na,Some (_,_,_,nal))) -> na::List.map fst nal) tml)
...
i*)
let map_rawdecl f (na,imp,obd,ty) = (na,imp,option_map f obd,f ty)

let map_rawconstr f = function
  | RVar (loc,id) -> RVar (loc,id)
  | RApp (loc,g,args,imps) -> RApp (loc,f g, List.map f args, imps)
  | RLambda (loc,na,imp,ty,c) -> RLambda (loc,na,imp,f ty,f c)
  | RProd (loc,na,imp,ty,c) -> RProd (loc,na,imp,f ty,f c)
  | RLetIn (loc,na,imp,b,c) -> RLetIn (loc,na,imp,f b,f c)
...
let map_rawconstr_with_binders_loc loc g f e = function
  ...
  | RLetIn (_,na,imp,b,c) ->
      let na,e = name_app g e na in RLetIn (loc,na,imp,f e b,f e c)
...
let occur_rawconstr id =
  let rec occur = function
    | RVar (loc,id') -> id = id'
    | RApp (loc,f,args,imps) -> (occur f) or (List.exists occur args)
    | RLambda (loc,na,_,ty,c) -> (occur ty) or ((na <> Name id) & (occur c))
    | RProd (loc,na,_,ty,c) -> (occur ty) or ((na <> Name id) & (occur c))
    | RLetIn (loc,na,imp,b,c) -> (occur b) or ((na <> Name id) & (occur c))
    | RCases (loc,rtntypopt,tml,pl) ->
	...
	occur_return_type rtntyp id
        or (occur b)
        or (not (List.mem (Name id) (List.map fst nal)) & (occur c))
...
let free_rawvars  =
  let rec vars bounded vs = function
    ...
    | RApp (loc,f,args,imps) -> List.fold_left (vars bounded) vs (f::args)
    | (RLambda (loc,na,imp,ty,c) | RProd (loc,na,imp,ty,c) |
       RLetIn (loc,na,imp,ty,c)) -> 
	...
	let bounded' =
          List.fold_left add_name_to_ids bounded (List.map fst nal) in
	vars bounded' vs2 c
    | RIf (loc,c,rtntyp,b1,b2) -> 
	let vs1 = vars_return_type bounded vs rtntyp in 
...
let rec cases_pattern_of_rawconstr na = function
  | RVar (loc,id) when na<>Anonymous ->
      ...
      raise Not_found
  | RVar (loc,id) -> PatVar (loc,Name id,Expl)
  | RHole (loc,_) -> PatVar (loc,na,Expl)
  | RRef (loc,ConstructRef cstr) ->
      PatCstr (loc,cstr,[],na)
  | RApp (loc,RRef (_,ConstructRef cstr),l,imps) ->
