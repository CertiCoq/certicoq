...
(*i $Id: extraction.ml 9455 2006-12-17 19:53:53Z letouzey $ i*)
...
let impl_aware = ref true

let rec flag_of_type env impt t = 
  let info() = if !impl_aware && impt = Impl then Logic else Info in
  let t = whd_betadeltaiota env none t in 
  match kind_of_term t with
    | Prod (x,imp,t,c) -> flag_of_type (push_rel (x,imp,None,t) env) impt c
    | Sort (Prop Null) -> (Logic,TypeScheme)
    | Sort _ -> (info(),TypeScheme)
    | _ ->
        if (sort_of env t) = InProp then (Logic,Default) else (info(),Default)

(*s Two particular cases of [flag_of_type]. *)

let is_default env imp t = (flag_of_type env imp t = (Info, Default))

exception NotDefault of kill_reason

let check_default env imp t = 
  match flag_of_type env imp t with 
    | _,TypeScheme -> raise (NotDefault Ktype)
    | Logic,_ -> raise (NotDefault Kother)
    | _ -> ()

let is_info_scheme env imp t = (flag_of_type env imp t = (Info, TypeScheme))
...
let rec type_sign env c = 
  match kind_of_term (whd_betadeltaiota env none c) with
    | Prod (n,imp,t,d) -> 
	(if is_info_scheme env imp t then Keep else Kill Kother)
	:: (type_sign (push_rel_assum (n,imp,t) env) d)
    | _ -> []

let rec type_scheme_nb_args env c = 
  match kind_of_term (whd_betadeltaiota env none c) with
    | Prod (n,imp,t,d) -> 
	let n = type_scheme_nb_args (push_rel_assum (n,imp,t) env) d in 
	if is_info_scheme env imp t then n+1 else n
    | _ -> 0
...
let rec type_sign_vl env c = 
  match kind_of_term (whd_betadeltaiota env none c) with
    | Prod (n,imp,t,d) -> 
	let s,vl = type_sign_vl (push_rel_assum (n,imp,t) env) d in 
	if not (is_info_scheme env imp t) then Kill Kother::s, vl
...
let rec nb_default_params env c = 
  match kind_of_term (whd_betadeltaiota env none c) with
    | Prod (n,imp,t,d) ->
	let n = nb_default_params (push_rel_assum (n,imp,t) env) d in 
	if is_default env imp t then n+1 else n
    | _ -> 0
...
let rec extract_type env db j c args imps = 
  match kind_of_term (whd_betaiotazeta c) with
    | App (d, args',imps') ->
	(* We just accumulate the arguments. *)
	extract_type env db j d (Array.to_list args' @ args)
          (Array.to_list imps' @ imps)
    | Lambda (_,_,_,d) -> 
	(match args,imps with 
	   | a :: args, _::imps ->
               extract_type env db j (subst1 a d) args imps
	   | _ -> assert false) (* otherwise the lambda would be reductible.*)
    | Prod (n,imp,t,d) ->
	assert (args = [] && imps=[]); 
	let env' = push_rel_assum (n,imp,t) env in 
	(match flag_of_type env imp t with 
	   | (Info, Default) -> 
	   ...
	   | (_, TypeScheme) when j > 0 -> 
	       (* A new type var. *)
               (* BB: logic type schemes treated here (case of implicit type)*)
    ...
    | _ when sort_of env (applist (c, args,imps)) = InProp -> Tdummy Kother
    | Rel n -> 
	(match lookup_rel n env with
           | (_,imp,Some t,_) ->
               assert (imp=Expl);
               extract_type env db j (lift n t) args imps
	   | _ -> 
	...
	(match flag_of_type env Expl typ with 
	   | (Info, TypeScheme) -> 
	       let mlt =
                 extract_type_app env db (r, type_sign env typ) args in 
	       (match cb.const_body with 
		  | None -> mlt
		  | Some _ when is_custom r -> mlt 
		  | Some lbody -> 
		      let newc = applist(Declarations.force lbody,args,imps) in 
	   ...
	   | _ -> (* only other case here: Info, Default, i.e. not an ML type *)
		  ...
		  | Some lbody -> 
		      (* We try to reduce. *)
		      let newc =
                        applist(Declarations.force lbody,args,imps) in 
...
and extract_type_scheme env db c p = 
  if p=0 then extract_type env db 0 c [] []
    ...
    match kind_of_term c with 
      | Lambda (n,imp,t,d) -> 
          extract_type_scheme (push_rel_assum (n,imp,t) env) db d (p-1)
      | _ ->  
          let rels = fst (splay_prod env none (type_of env c)) in
          let env = push_rels_assum rels env in  
          let eta_args = List.rev_map mkRel (interval 1 p) in 
          let imps = List.rev_map (fun (_,imp,_) -> imp) rels in 
          extract_type env db 0 (lift p c) eta_args imps
	...
	for j = 0 to Array.length types - 1 do 
	  ...
	  let nprods = List.length prods in 
          let args = match kind_of_term head with
            | App (f,args,imps) -> args (* [kind_of_term f = Ind ip] *)
            | _ -> [||]
          in
...
and extract_type_cons env db dbmap c i =
  match kind_of_term (whd_betadeltaiota env none c) with 
    | Prod (n,Impl,t,d) when !impl_aware -> 
	let env' = push_rel_assum (n,Impl,t) env in
...
let rec extract_term env mle mlt c args imps = 
  match kind_of_term c with
    | App (f,a,i) ->
	extract_term env mle mlt f (Array.to_list a @ args)
          (Array.to_list i @ imps)
    | Lambda (n, imp, t, d) ->
	...
	(match args,imps with 
	   | a :: l, _::il -> 
	       (* We make as many [LetIn] as possible. *)
 	       let d' =
                 mkLetIn (Name id,imp,a,t,
                  applistc d (List.map (lift 1) l) il)
	       in extract_term env mle mlt d' [] []
	   | [],[] -> 
	       let env' = push_rel_assum (Name id, imp,t) env in 
	       let id, a = try check_default env imp t; id, new_meta()
           ...
           | _ -> assert false)
    | LetIn (n, imp, c1, t1, c2) ->
	let id = id_of_name n in 
	let env' = push_rel (Name id, imp, Some c1, t1) env in 
	let args' = List.map (lift 1) args in 
	(try 
	  check_default env imp t1; 
	  let a = new_meta () in 
	  let c1' = extract_term env mle a c1 [] [] in
	  (* The type of [c1'] is generalized and stored in [mle]. *)
	  let mle' = Mlenv.push_gen mle a in 
	  MLletin (id, c1', extract_term env' mle' mlt c2 args' imps) 
	with NotDefault d -> 
	  let mle' = Mlenv.push_std_type mle (Tdummy d) in 
	  ast_pop (extract_term env' mle' mlt c2 args' imps))
    | Const kn ->
...
let rec decomp_lams_eta_n n env c t = 
  ...
  let rels = List.map (fun (id,imp,_,c) -> (id,imp,c)) rels in 
  let m = nb_lam c in 
  if m >= n then decompose_lam_n n c 
    ...
    let eta_rels = list_firstn d rels in
    let rels = eta_rels @ rels' in
    let eta_args = List.rev_map mkRel (interval 1 d) in 
    let imps = List.rev_map (fun (_,imp,_) -> imp) eta_rels in
    rels, applist (lift d c,eta_args,imps)
...
let extract_std_constant env kn body typ = 
  ...
  (* The lambdas names. *)
  let ids = List.map (fun (n,_,_) -> id_of_name n) rels in 
...
let constant_kind env cb = 
  match flag_of_type env Expl (Typeops.type_of_constant_type env cb.const_type) with 
    | (Logic,_) -> Logical
