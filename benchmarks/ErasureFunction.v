...
Section fix_sigma.
  ...
  Definition print_term Σ Γ t :=
    let ids := PCUICPretty.fresh_names Σ [] Γ in
    PCUICPretty.print_term Σ true ids true false t.

  Definition pr_term prefix (Σ : global_env_ext) Γ (t : PCUICAst.term) := 
    let s := (prefix ++ print_term Σ Γ t) in
    let _ := print_str s in
    let _ := print_newline tt in t.
  ...
  Equations(noeqns noind) is_arity Γ (HΓ : ∥wf_local Σ Γ∥) T (HT : welltyped Σ Γ T) :
    ...
    is_arity Γ HΓ T HT with inspect (reduce_to_sort Σ Γ T HT) => {
      | exist (Checked_comp H) rsort =>
         let _ := print_str "is a sort " in
         left _
      | exist (TypeError_comp _) rsort with inspect (reduce_to_prod Σ Γ T _) => {
        | exist (Checked_comp (na; A; B; H)) rprod with is_arity (Γ,, vass na A) _ B _ :=
          { | left isa => 
              let _ := print_str "is a product ending in a sort" in
              left _
            | right nisa =>
              let _ := print_str "is a product not ending in a sort" in
               right _ }
        | exist (TypeError_comp e) rprod => 
          let _ := print_str "is not a product" in
          right _ } }.
...
Program Definition is_erasable (Σ : wf_env_ext) (Γ : context) (t : PCUICAst.term) 
  ...
  (* let t := pr_term "infering type of " Σ Γ t in *)
  let T := @type_of_typing extraction_checker_flags _ Σ Γ t wt in
  let ty := T.π1 in
  (* let _ := pr_term "found type  " Σ Γ ty in *)
  let b := is_arity Σ Γ _ T.π1 _ in
  match b : {_} + {_} return _ with
  | left _ => 
    let _ := print_str"erasable arity" in
    let _ := print_newline tt in
    left _
  | right _ => 
      let K := @type_of_typing extraction_checker_flags _ Σ Γ T.π1 _ in
      (* let _ := pr_term "its sort is " Σ Γ K.π1 in *)
       match reduce_to_sort Σ Γ K.π1 _ with
       | Checked_comp (u; Hu) =>
          match is_propositional u with 
          | true =>
            let _ := print_str "erasable proof" in
            left _ 
          | false => 
            let _ := print_str "not erasable" in
            right _ 
          end
...
Next Obligation. destruct wt; sq; eauto. todo "foo". Qed.
Next Obligation. 
  sq. todo "foo".
Qed.
Next Obligation. 
...
(** We assume that there are no lets in contexts, so nothing has to be expanded.
  ...
  *)
  ...
  Proof.
    ...
    - cbn in he. todo "foo". discriminate. eapply inversion_Lambda in Ht as (? & ? & ? & ? & ?); auto.
      eexists; eauto.
...
Proof.
  apply (erase_elim Σ
    ...
    (fun Γ l awt e => True)
    (fun Γ p l awt e => True)
    (fun Γ l awt e => True)); intros => //. all:bang.
Qed.
(*
