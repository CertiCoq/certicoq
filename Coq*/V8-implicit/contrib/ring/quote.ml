...
let compute_lhs typ i nargsi =
  match kind_of_term typ with
    | Ind(sp,0) -> 
        ...
        mkApp (mkConstruct ((sp,0),i+1), argsi,Array.map(fun _->Expl)argsi)
    | _ -> i_can't_do_that ()
...
let compute_rhs bodyi index_of_f =
  let rec aux c = 
    match kind_of_term c with
      | App (j, args,imps) when j = mkRel (index_of_f) (* recursive call *) -> 
          ...
          | Case(_,p,c,lci) -> (* <p> Case c of c1 ... cn end *)
           ...
           (* First we test if the RHS is the RHS for constants *)
                   if bodyi = mkRel 1 then 
                     c_lhs := Some (compute_lhs (last_ty args3) i nargsi)
           (* Then we test if the RHS is the RHS for variables *)
			   ...
			   pf_conv_x gl vmf 
                           (Lazy.force coq_varmap_find)->
                             v_lhs := Some (compute_lhs (last_ty args3) 
                                              i nargsi)
           (* Third case: this is a normal LHS-RHS *)
                     | _ -> 
                         n_lhs_rhs :=
                         (compute_lhs (last_ty args3) i nargsi,
...
let rec closed_under cset t =
  ...
  (match (kind_of_term t) with
     ...
     | App(f,l,_) -> closed_under cset f && array_for_all (closed_under cset) l
     | _ -> false)
...
let btree_of_array a ty =
  ...
  and empty = mkApp (Lazy.force coq_Empty_vm, [| ty |],[|Impl|]) in
  let rec aux n =
    if n > size_of_a 
    then empty
    ...
    then mkApp (node, [| ty; a.(n-1); empty; empty |],
      ...
      (node, [| ty; a.(n-1); aux (2*n); aux (2*n+1) |],
       [|Impl;Expl;Expl;Expl|])
  in 
  aux 1
...
let rec subterm gl (t : constr) (t' : constr) = 
  (pf_conv_x gl t t') or
  (match (kind_of_term t) with 
     | App (f,args,_) -> array_exists (fun t -> subterm gl t t') args
...
let quote f lid gl =
  ...
  match ivs.variable_lhs with
    | None ->
        Tactics.convert_concl (mkApp (f, [| p |],[|Expl|])) DEFAULTcast gl
    | Some _ ->
        Tactics.convert_concl (mkApp (f, [| vm; p |],[|Expl;Expl|]))
          DEFAULTcast gl
