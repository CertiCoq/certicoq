...
let pos_of_bignat dloc x =
  ...
  let rec pos_of x =
    match div2_with_rest x with
      | (q,false) -> RApp (dloc, ref_xO,[pos_of q],[Term.Expl])
      | (q,true) when q <> zero -> RApp (dloc,ref_xI,[pos_of q],[Term.Expl])
      | (q,true) -> ref_xH
  in 
  pos_of x
...
let rec bignat_of_pos = function
  | RApp (_, RRef (_,b),[a],_) when b = glob_xO -> mult_2(bignat_of_pos a)
  | RApp (_, RRef (_,b),[a],_) when b = glob_xI ->
      add_1(mult_2(bignat_of_pos a))
...
let n_of_binnat dloc pos_or_neg n = 
  if n <> zero then
    RApp(dloc, RRef (dloc,glob_Npos), [pos_of_bignat dloc n],[Term.Expl])
  else 
    RRef (dloc, glob_N0)
...
let bignat_of_n = function
  | RApp (_, RRef (_,b),[a],_) when b = glob_Npos -> bignat_of_pos a
...
let z_of_int dloc n = 
  if n <> zero then
    ...
    RApp(dloc, RRef (dloc,sgn), [pos_of_bignat dloc n],[Term.Expl])
  else 
    RRef (dloc, glob_ZERO)
...
let bigint_of_z = function
  | RApp (_, RRef (_,b),[a],_) when b = glob_POS -> bignat_of_pos a
  | RApp (_, RRef (_,b),[a],_) when b = glob_NEG ->
      Bigint.neg (bignat_of_pos a)
