...
Require Import ImplicitAxioms.  
  ...
  Proof.
    ...
    simple induction 1.
    intro; apply (H0 x2 y0 H3).
    
    simple induction 1.
    intro; simple induction 1.
    clear H4 H2.
    intro; generalize y0; clear y0.
    rewrite H2.
    apply rev_ind with
      (A := A)
      (P := fun x3:List =>
    ...
    apply impl_PI; apply (H0 x4 y3); auto with sets.
    
    intros.
    generalize (dist_Desc_concat l (l0 ++ Cons x4 Nil) y1).
    destruct 1.
    generalize (desc_end x4 x1 l0 (conj H7 H5)); intro.
    generalize y1.
    rewrite <- (app_ass l l0 (Cons x4 Nil)); intro.
    generalize (HInd x4 H8 (l ++ l0)); intros HInd2.
    generalize (ltl_unit l0 x4 x1 H7 H5); intro.
    generalize (dist_Desc_concat (l ++ l0) (Cons x4 Nil) y0).
    simple induction 1; intros.
    generalize (H4 H11 H9); intro.
    generalize (Acc_inv H13).
    generalize (acc_app l l0 H11 H13).
    intros f g.
    generalize (HInd2 f); intro.
    apply Acc_intro.
    simple induction y2.
    unfold lex_exp at 1 in |- *; simpl in |- *; intros.
    apply impl_PI; apply H14; auto with sets.
  Qed.
