...
Proof.
  intros f g l a b pr1 pr2; unfold NewtonInt in |- *;
    case (NewtonInt_P5 f g l a b pr1 pr2); intros; case pr1; 
      intros; case pr2; intros; destruct (total_order_or a b) as [[a0|b0]|r].
  elim o; intro.
  elim o0; intro.
  elim o1; intro.
...
Proof.
  ...
  intros; elim H3; clear H3; intros; destruct (total_order_or x b) as [[a0|b0]|r].
  assert (H5 : a <= x <= b).
  ...
  exists (mkposreal _ H11); intros; case (Rle_dec x b); intro; apply impl_PI.
  case (Rle_dec (x + h) b); intro; apply impl_PI.
  apply H10.
  assumption.
  ...
  apply (cond_pos x2).
  apply (cond_pos x3).
  exists (mkposreal _ H16); intros; case (Rle_dec x b); intro; apply impl_PI.
  case (Rle_dec (x + h) b); intro; apply impl_PI.
  apply H15.
  assumption.
...
Proof.
  intros; unfold antiderivative in H, H0; elim H; clear H; elim H0; clear H0;
    intros; destruct (total_order_or a c) as [[a0|b0]|r].
...
Proof.
  intros; unfold antiderivative in H, H0; elim H; clear H; elim H0; clear H0;
    intros; destruct (total_order_or c b) as [[a0|b0]|r].
  right; unfold antiderivative in |- *; split.
  intros; apply H1; elim H3; intros; split;
...
Proof.
  ...
  destruct (total_order_T a b) as [[a0|b0]|r].
  destruct (total_order_T b c) as [[a1|b0]|r].
(* a<b & b<c *)
  ...
  elim (impl_PI (Rlt_irrefl _ (Rle_lt_trans _ _ _ H2 a1))).
  unfold antiderivative in H; elim H; clear H; intros _ H.
  elim (impl_PI (Rlt_irrefl _ (Rle_lt_trans _ _ _ H a0))).
(* a<b & b=c *)
  rewrite (impl_PI b0) in X; apply X.
(* a<b & b>c *)
  destruct (total_order_T a c) as [[a1|b0]|r0].
  ...
  elim (impl_PI (Rlt_irrefl _ (Rle_lt_trans _ _ _ H4 a1))).
  assumption.
  unfold antiderivative in H2; elim H2; clear H2; intros _ H2.
  elim (impl_PI (Rlt_irrefl _ (Rle_lt_trans _ _ _ H2 a0))).
  rewrite (impl_PI b0); apply NewtonInt_P1.
  ...
  assumption.
  unfold antiderivative in H4; elim H4; clear H4; intros _ H4.
  elim (impl_PI (Rlt_irrefl _ (Rle_lt_trans _ _ _ H4 r0))).
  unfold antiderivative in H2; elim H2; clear H2; intros _ H2.
  elim (impl_PI (Rlt_irrefl _ (Rle_lt_trans _ _ _ H2 a0))).
(* a=b *)
  rewrite (impl_PI b0); apply X0.
  destruct (total_order_T b c) as [[a0|b0]|r0].
(* a>b & b<c *)
  destruct (total_order_T a c) as [[a1|b0]|r0].
  unfold Newton_integrable in |- *; apply existT with F1.
  left.
...
(*****************)
  elim H0; intro.
  unfold antiderivative in H2; elim H2; clear H2; intros _ H2.
  elim (impl_PI (Rlt_irrefl _ (Rle_lt_trans _ _ _ H2 r))).
...
(* a>b & b>c *)
  ...
  apply NewtonInt_P3.
  apply NewtonInt_P7 with b; try apply impl_PI; assumption.
Defined.
...
Proof.
  ...
  case pr1; intros.
  case pr2; intros.
  destruct (total_order_or a b) as [[a0|b0]|r].
  destruct (total_order_or b c) as [[a1|b0]|r].
(* a<b & b<c *)
  elim o0; intro.
  elim o1; intro.
  elim o; intro.
  ...
  case (Rle_dec c b); intro; apply impl_PI.
...
(* a<b & b>c *)
  ...
  rewrite (H4 a).
  rewrite (H4 b).
  case (Rle_dec b c); intro; apply impl_PI.
  elim (Rlt_irrefl _ (Rle_lt_trans _ _ _ r0 r)).
  ...
  rewrite (H4 c).
  rewrite (H4 b).
  case (Rle_dec b a); intro; apply impl_PI.
  elim (Rlt_irrefl _ (Rle_lt_trans _ _ _ r0 a0)).
  case (Rle_dec c a); intro; apply impl_PI.
  ring.
  elim n0; unfold antiderivative in H1; elim H1; intros; assumption.
  split; [ left; assumption | right; reflexivity ].
  split; [ right; reflexivity | left; assumption ].
...
(* a=b *)
  ...
  split; [ right; reflexivity | assumption ].
(* a>b & b<c *)
  destruct (total_order_or b c) as [[a0|b0]|r0].
  elim o0; intro.
  unfold antiderivative in H; elim H; clear H; intros _ H.
  elim (Rlt_irrefl _ (Rle_lt_trans _ _ _ H r)).
  elim o1; intro.
