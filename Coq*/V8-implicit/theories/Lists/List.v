...
Require Import ImplicitAxioms.
  ...
  Section Remove.
    ...
    Proof.
      ...
      intro y; simpl; destruct (eq_dec y x) as [yeqx | yneqx]; apply impl_PI. 
      apply IHl.
...
(******************************)
  ...
  Proof. 
    ...
    exists (a::l'); exists a'; rewrite H; auto.
  Qed.
  ...
  Proof.
    ...
    simpl. intro x; destruct (eqA_dec y x) as [Heq|Hneq]; apply impl_PI.
  ...
  Proof.
    ...
    apply sym_eq.
    generalize (H x).
    simpl. destruct (eqA_dec x x) as [|HF]; apply impl_PI.
    trivial.
    elim HF; reflexivity.
    (* Case <- *)
  ...
  Proof.
    ...
    destruct (eqA_dec x y); [reflexivity | apply impl_PI; contradiction].
  Qed.
  
  Lemma count_occ_cons_neq : forall (l : list A) (x y : A), x <> y -> count_occ (x::l) y = count_occ l y.
  Proof.
    intros l x y H; simpl.
    destruct (eqA_dec x y); [apply impl_PI; contradiction | reflexivity]. 
  Qed.
...
(*********************************************)
  ...
  Proof.
    ...
    left; rewrite xeqy; rewrite leql'; trivial.
  Qed.
