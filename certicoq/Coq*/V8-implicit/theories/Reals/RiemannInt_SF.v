...
Proof.
  ...
  rewrite H10; rewrite (impl_PI p); rewrite <- INR_IZR_INZ; replace 1 with (INR 1);
  ...
  induction  x0 as [| x0 Hrecx0];
    [ rewrite (impl_PI p) in H7; rewrite <- INR_IZR_INZ in H7; simpl in H7;
  ...
  split with (pred x0); unfold E in H13; elim H13; intros; elim H12; intros;
    rewrite H10 in H15; rewrite (impl_PI p) in H15; rewrite <- INR_IZR_INZ in H15;
...
Proof. 
  ...
  rewrite H2; unfold Rmin in |- *; case (Rle_dec a b); intro;
    case (Rle_dec b a); intro; apply impl_PI; try reflexivity.
...
Proof. 
  ...
  simpl in |- *; unfold Rmin in |- *; case (Rle_dec a b); intro;
    apply impl_PI; [ reflexivity | elim n; assumption ].
...
Proof. 
  ...
  apply existT with (cons a (cons b nil)); unfold is_subdivision in |- *;
    apply existT with (cons c nil); apply impl_PI; apply (StepFun_P3 c r).
  apply existT with (cons b (cons a nil)); unfold is_subdivision in |- *;
    apply existT with (cons c nil); apply impl_PI; apply StepFun_P2; 
...
Proof. 
  ...
  rewrite H5 in H2; unfold Rmin in |- *; case (Rle_dec r2 b); intro;
    apply impl_PI; [ reflexivity | elim n; assumption ].
  unfold Rmax in |- *; case (Rle_dec r2 b); intro; apply impl_PI;
...
Proof. 
  ...
  simpl in |- *; unfold Rmin in |- *; case (Rle_dec a b); intro; apply impl_PI;
    [ reflexivity | elim n; assumption ].
  simpl in |- *; unfold Rmax in |- *; case (Rle_dec a b); intro; apply impl_PI;
    [ reflexivity | elim n; assumption ].
  ...
  unfold open_interval in |- *; simpl in |- *; rewrite H7; simpl in H13;
    rewrite H13; unfold Rmin in |- *; case (Rle_dec a b); 
      intro; apply impl_PI; [ assumption | elim n; assumption ].
  elim (le_Sn_O _ H10).
  intros; simpl in H8; elim (lt_n_O _ H8).
  intros; simpl in H8; inversion H8;
    [ simpl in |- *; assumption | elim (le_Sn_O _ H10) ].
  assert (Hyp_min : Rmin t2 b = t2).
  unfold Rmin in |- *; case (Rle_dec t2 b); intro; apply impl_PI;
    [ reflexivity | elim n; assumption ].
  ...
  simpl in |- *; simpl in H18; rewrite H18; unfold Rmax in |- *;
    case (Rle_dec a b); case (Rle_dec t2 b); intros; apply impl_PI; reflexivity || elim n;
      assumption.
  simpl in |- *; simpl in H20; apply H20.
  intros; simpl in H1; unfold constant_D_eq, open_interval in |- *; intros;
    induction  i as [| i Hreci].
  simpl in |- *; simpl in H6; destruct (total_order_or x t2) as [[a0|b0]|r3].
  apply (H17 0%nat);
    [ simpl in |- *; apply lt_O_Sn
  ...
  rewrite H9; simpl in |- *; simpl in H13; rewrite H13; unfold Rmax in |- *;
    case (Rle_dec t2 b); case (Rle_dec a b); intros; apply impl_PI; reflexivity || elim n;
      assumption.
...
Proof. 
  ...
  rewrite <- H12 in H1; case (Rle_dec r1 s2); intro; apply impl_PI; try assumption.
...
Lemma StepFun_P13 :
  forall (a b r r1 r3 s1 s2 r4:R) (r2 lf1 s3 lf2:Rlist) 
    ...
    adapted_couple_opt f a b (cons s1 (cons s2 s3)) (cons r4 lf2) -> r1 <= s2.
Proof. 
  intros; destruct (total_order_or a b) as [[a0|b0]|r0].
...
Proof. 
  intros; unfold RiemannInt_SF in |- *; case (Rle_dec a b); intro; apply impl_PI.
  replace
...
Proof. 
  ...
  elim (Rle_dec (pos_Rl lf (S x0)) (pos_Rl (cons_ORlist lf lg) i)); intro; apply impl_PI.
...
Proof. 
  intros; case (Rle_dec a b); intro;
    [ apply StepFun_P22 with g; try apply impl_PI; assumption
...
Proof. 
  ...
  symmetry  in |- *; apply S_pred with 0%nat; apply neq_O_lt; red in |- *;
    intro; rewrite <- H22 in H21; elim (lt_n_O _ H21).
  elim (Rle_dec (pos_Rl lg (S x0)) (pos_Rl (cons_ORlist lf lg) i)); intro; apply impl_PI.
  assert (H23 : (S x0 <= x0)%nat);
...
Proof. 
  intros a b f g lf lg H H0; case (Rle_dec a b); intro;
    [ apply StepFun_P24 with f; try apply impl_PI; assumption
...
Proof. 
  ...
  unfold Rmin in |- *; case (Rle_dec r1 b); intro; apply impl_PI;
    [ reflexivity | elim n; assumption ].
  ...
  simpl in H13; discriminate.
  reflexivity.
  unfold Rmax in |- *; case (Rle_dec a b); case (Rle_dec r1 b); intros; apply impl_PI;
    reflexivity || elim n; assumption.
  ...
  rewrite H16 in H12; rewrite H12 in H14; elim H14; clear H14; intros _ H14;
    unfold g' in |- *; case (Rle_dec r1 x); intro r3; apply impl_PI.
  ...
  assert (H18 := H16 H17); unfold constant_D_eq, open_interval in H18;
    unfold constant_D_eq, open_interval in |- *; intros;
      assert (H19 := H18 _ H14); rewrite <- H19; unfold g' in |- *;
        case (Rle_dec r1 x); intro; apply impl_PI.
  reflexivity.
...
Proof. 
  ...
  rewrite H10; rewrite H4; unfold Rmin, Rmax in |- *; case (Rle_dec a b);
    case (Rle_dec b c); intros; apply impl_PI;
  ...
  rewrite H15; rewrite <- minus_n_n; rewrite H10; unfold Rmin in |- *;
    case (Rle_dec b c); intro; apply impl_PI; [ reflexivity | elim n; left; assumption ].
  ...
  [ rewrite H4; unfold Rmax in |- *; case (Rle_dec a b); intro; apply impl_PI;
  ...
  unfold Rmin, Rmax in |- *; case (Rle_dec a b); intro; apply impl_PI;
...
Proof.
  ...
  exists (cons_Rlist l1 l2); exists (FF (cons_Rlist l1 l2) f);
    apply StepFun_P40 with b lf1 lf2; apply impl_PI; assumption.
  exists l1; exists lf1; rewrite Hbc in H1; apply impl_PI; assumption.
  elim (impl_PI (Rlt_irrefl _ (Rle_lt_trans _ _ _ H0 Hgtbc))).
  exists l2; exists lf2; rewrite <- Hab in H2; apply impl_PI; assumption.
  elim (impl_PI (Rlt_irrefl _ (Rle_lt_trans _ _ _ H Hgtab))).
Qed.
...
Proof. 
  ...
  case (Rle_dec a b); case (Rle_dec b c); case (Rle_dec a c); intros; apply impl_PI.
  elim r1; intro.
  elim r0; intro.
  ...
  unfold adapted_couple in H1, H2; decompose [and] H1; decompose [and] H2;
    clear H1 H2; rewrite H11; rewrite H5; unfold Rmax, Rmin in |- *;
      case (Rle_dec a b); case (Rle_dec b c); intros; apply impl_PI; reflexivity || elim n;
        assumption.
  eapply StepFun_P17;
  ...
  unfold adapted_couple in H2, H3; decompose [and] H2; decompose [and] H3;
    clear H3 H2; rewrite H10; rewrite H6; unfold Rmax, Rmin in |- *;
      case (Rle_dec a c); case (Rle_dec b c); intros; apply impl_PI;
        [ elim n; assumption
          | reflexivity
          | elim n0; assumption
          | elim n1; assumption ].
  ...
  unfold adapted_couple in H1, H3; decompose [and] H1; decompose [and] H3;
    clear H3 H1; rewrite H9; rewrite H5; unfold Rmax, Rmin in |- *;
      case (Rle_dec a c); case (Rle_dec a b); intros; apply impl_PI;
        [ elim n; assumption
          | elim n1; assumption
          | reflexivity
          | elim n1; assumption ].
  ...
  unfold adapted_couple in H2, H1; decompose [and] H2; decompose [and] H1;
    clear H1 H2; rewrite H11; rewrite H5; unfold Rmax, Rmin in |- *;
      case (Rle_dec a b); case (Rle_dec b c); intros; apply impl_PI;
        [ elim n1; assumption
          | elim n1; assumption
          | elim n0; assumption
          | reflexivity ].
...
Proof. 
  ...
  split with (cons r (cons c nil)); split with (cons r3 nil);
    unfold adapted_couple in H; decompose [and] H; clear H; 
      assert (H6 : r = a).
  simpl in H4; rewrite H4; unfold Rmin in |- *; case (Rle_dec a b); intro; apply impl_PI;
    [ reflexivity
  ...
  rewrite H6; unfold ordered_Rlist in |- *; intros; simpl in H8; inversion H8;
    [ simpl in |- *; assumption | elim (le_Sn_O _ H10) ].
  simpl in |- *; unfold Rmin in |- *; case (Rle_dec a c); intro; apply impl_PI;
    [ assumption | elim n; assumption ].
  simpl in |- *; unfold Rmax in |- *; case (Rle_dec a c); intro; apply impl_PI;
    [ reflexivity | elim n; assumption ].
  ...
  apply Rlt_le_trans with c; apply impl_PI; assumption.
  elim (le_Sn_O _ H11).
  ...
  elim H0; intros; apply Rle_trans with c; assumption.
  assert (H16 : r = a).
  simpl in H7; rewrite H7; unfold Rmin in |- *; case (Rle_dec a b); intro; apply impl_PI;
    [ reflexivity | elim n; assumption ].
  ...
  apply (H5 0%nat).
  simpl in |- *; apply lt_O_Sn.
  simpl in H12; rewrite H12; unfold Rmin in |- *; case (Rle_dec r1 c); intro; apply impl_PI;
    [ reflexivity | elim n; left; assumption ].
  ...
  unfold Rmax in |- *; case (Rle_dec r1 c); case (Rle_dec a c); intros; apply impl_PI;
    [ reflexivity
...
Proof. 
  ...
  split with (cons c (cons r1 r2)); split with (cons r3 lf1);
    unfold adapted_couple in H; decompose [and] H; clear H;
      unfold adapted_couple in |- *; repeat split.
  unfold ordered_Rlist in |- *; intros; simpl in H; induction  i as [| i Hreci].
  simpl in |- *; apply impl_PI; assumption.
  clear Hreci; apply (H2 (S i)); simpl in |- *; assumption.
  simpl in |- *; unfold Rmin in |- *; case (Rle_dec c b); intro; apply impl_PI;
    [ reflexivity | elim n; elim H0; intros; assumption ].
  replace (Rmax c b) with (Rmax a b).
  rewrite <- H3; reflexivity.
  unfold Rmax in |- *; case (Rle_dec a b); case (Rle_dec c b); intros; apply impl_PI;
    [ reflexivity
      | elim n; elim H0; intros; assumption
      | elim n; elim H0; intros; apply Rle_trans with c; assumption
  ...
  elim H0; intros; assumption.
  simpl in H4; rewrite H4; unfold Rmin in |- *; case (Rle_dec a b); intros; apply impl_PI;
    [ reflexivity
      | elim n; elim H0; intros; apply Rle_trans with c; assumption ].
  ...
  apply impl_PI; split; [ left; assumption | elim H0; intros; assumption ].
  eapply StepFun_P7;
    [ elim H0; intros; apply Rle_trans with c; [ apply H2 | apply H3 ]
      | apply H ].
Qed.
...
Proof. 
  ...
  apply StepFun_P41 with b; try apply impl_PI; assumption.
  ...
  apply StepFun_P6; apply StepFun_P41 with b;
    (apply impl_PI;auto with real) || apply StepFun_P6; assumption.
Qed.
