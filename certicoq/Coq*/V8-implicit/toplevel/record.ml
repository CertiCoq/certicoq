...
(* $Id: record.ml 9917 2007-06-30 09:55:33Z herbelin $ *)
...
let interp_decl sigma env = function
  | Vernacexpr.AssumExpr((_,id),imp,t) ->
      (id,imp,None,interp_type Evd.empty env t)
  | Vernacexpr.DefExpr((_,id),imp,c,t) ->
      let c = match t with
	| None -> c
	| Some t -> mkCastC (c, Rawterm.CastConv (DEFAULTcast,t))
      in
      let j = interp_constr_judgment Evd.empty env c in
      (id,imp,Some j.uj_val, refresh_universes j.uj_type)
...
let typecheck_params_and_fields id t ps fs =
  ...
  let env_ar =
    push_rel_context newps (push_rel (Name id,Expl,None,fullarity) env0) in
  let env2,newfs =
    List.fold_left
      (fun (env,newfs) d ->
...
let degenerate_decl (na,imp,b,t) =
  let id = match na with
    | Name id -> id
    | Anonymous -> anomaly "Unnamed record variable" in 
  match b with
    | None -> (id, Entries.LocalAssum (t,imp))
    | Some b -> (id, Entries.LocalDef (b,imp))
...
let declare_projections indsp coers fields =
  ...
  let cpdecls = Sign.set_impl_rel_context Impl paramdecls in
  let r = mkInd indsp in
  let inst,imps = extended_rel_list 0 paramdecls in
  let rp = applist (r,inst,imps) in
  let (paramargs,_) =
    extended_rel_list 1 paramdecls in (*def in [[params;x:rp]]*)
  ...
  let (_,kinds,sp_projs,_) =
    List.fold_left2
      (fun (nfi,kinds,sp_projs,subst) coe (fi,impci,optci,ti) ->
	let (sp_projs,subst) =
	  match fi, impci with
	  | Name fid, Expl ->
            (try
		...
		let p = mkLambda (x, Expl, lift 1 rp, ccl') in
		let branch =
                  it_mkLambda_or_LetIn (mkRel nfi) lifted_fields in
		let ci = Inductiveops.make_case_info env indsp LetStyle in
		mkCase (ci, p, mkRel 1, [|branch|]) in
	      let proj =
	        it_mkLambda_or_LetIn (mkLambda (x,Expl,rp,body)) cpdecls in
              let projtyp =
                it_mkProd_or_LetIn (mkProd (x,Expl,rp,ccl)) cpdecls in
	      let kn =
	        try
		  let cie = {
	      ...
	      let proj_args = (*Rel 1 refers to "x"*) paramargs@[mkRel 1] in
              let proj_imps = List.map(fun _->Impl)paramargs@[Expl] in
	      let constr_fip = applist (constr_fi,proj_args,proj_imps) in
...
let definition_structure ((is_coe,(_,idstruc)),ps,cfs,idbuild,s) =
  ...
  let extract_name acc = function
      Vernacexpr.AssumExpr((_,Name id),imp,_) -> id::acc
    | Vernacexpr.DefExpr ((_,Name id),imp,_,_) -> id::acc
    | _ -> acc in
  ...
  let (args,imps) = extended_rel_list nfields params in
  let ind = applist (mkRel (1+nparams+nfields), args, imps) in
