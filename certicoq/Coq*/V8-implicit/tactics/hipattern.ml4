...
let match_with_non_recursive_type t = 
  match kind_of_term t with 
    | App _ -> 
        let (hdapp,args,imps) = decompose_app t in
        (match kind_of_term hdapp with
           | Ind ind -> 
               if not (Global.lookup_mind (fst ind)).mind_finite then 
		 Some (hdapp,args,imps) 
	       else 
		 None 
           | _ -> None)
    | _ -> None
...
let match_with_forall_term c=
  match kind_of_term c with
    | Prod (nam,imp,a,b) -> Some (nam,imp,a,b)
    | _            -> None
...
let match_with_imp_term c=
  match kind_of_term c with
    | Prod (_,imp,a,b) when not (dependent (mkRel 1) b) ->Some (imp,a,b)
    | _              -> None
    ...
    match (kind_of_term hdapp) with
      | Ind ind  -> 
		...
		if array_for_all nodep_constr mip.mind_nf_lc then
		  ...
		  let imps=
		    if mip.mind_nrealargs=0 then imps else
		      fst (list_chop mib.mind_nparams imps) in
		    Some (hdapp,params,imps,mip.mind_nrealargs)
		else 
		  None
      | _ -> None
