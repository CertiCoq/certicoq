...
(* $Id: topconstr.ml 9868 2007-05-29 10:01:06Z herbelin $ *)
...
type aconstr =
  ...
  | ALambda of name * impl * aconstr * aconstr
  | AProd of name * impl * aconstr * aconstr
  | ALetIn of name * impl * aconstr * aconstr
  | ACases of aconstr option *
      (aconstr *
       (name * (inductive * int * (name * impl) list) option)) list *
      (cases_pattern list * aconstr) list
  | ALetTuple of
      (name * impl) list * (name * aconstr option) * aconstr * aconstr
...
let rec cases_pattern_fold_map loc g e = function
  | PatVar (_,na,imp) ->
      let e',na' = name_fold_map g e na in e', PatVar (loc,na',imp)
...
let rawconstr_of_aconstr_with_binders loc g f e = function
  ...
  | AApp (a,args) -> RApp (loc,f e a, List.map (f e) args,
                           List.map(fun _->Expl) args)
  | AList (x,y,iter,tail,swap) ->
      let t = f e tail in let it = f e iter in
      let innerl = (ldots_var,t)::(if swap then [] else [x,RVar(loc,y)]) in
      let inner = RApp (loc,RVar (loc,ldots_var),
                        [subst_rawvars innerl it],[Expl]) in
      let outerl = (ldots_var,inner)::(if swap then [x,RVar(loc,y)] else []) in
      subst_rawvars outerl it
  | ALambda (na,imp,ty,c) ->
      let e,na = name_fold_map g e na in RLambda (loc,na,imp,f e ty,f e c)
  | AProd (na,imp,ty,c) ->
      let e,na = name_fold_map g e na in RProd (loc,na,imp,f e ty,f e c)
  | ALetIn (na,imp,b,c) ->
      let e,na = name_fold_map g e na in RLetIn (loc,na,imp,f e b,f e c)
  | ACases (rtntypopt,tml,eqnl) ->
	...
	| Some (ind,npar,nal) -> 
	    let e',nal' =
              List.fold_right
                (fun (na,imp) (e',nal) -> 
	          let e',na' = name_fold_map g e' na in e',(na',imp)::nal)
                nal (e',[]) in
  ...
  | ALetTuple (nal,(na,po),b,c) ->
      let e,nal =
        list_fold_map
          (fun e (na,imp) ->
            let (e',na') = name_fold_map g e na in (e',(na',imp)))
          e nal in 
      let e,na = name_fold_map g e na in
...
let has_ldots =
  List.exists
    (function RApp (_,RVar(_,v),_,_) when v = ldots_var -> true | _ -> false)

let compare_rawconstr f t1 t2 = match t1,t2 with
  | RRef (_,r1), RRef (_,r2) -> r1 = r2
  | RVar (_,v1), RVar (_,v2) -> v1 = v2
  | RApp (_,f1,l1,imps1), RApp (_,f2,l2,imps2) when imps1=imps2 ->
      f f1 f2 & List.for_all2 f l1 l2
  | RLambda (_,na1,imp1,ty1,c1), RLambda (_,na2,imp2,ty2,c2)
      when na1 = na2 && imp1=imp2 ->
      f ty1 ty2 & f c1 c2
  | RProd (_,na1,imp1,ty1,c1), RProd (_,na2,imp2,ty2,c2)
      when na1 = na2 && imp1=imp2 ->
      f ty1 ty2 & f c1 c2
...
let aconstr_and_vars_of_rawconstr a =
  ...
  | RApp (_,f,args,imps) when has_ldots args ->
      make_aconstr_list f args imps 
  | RApp (_,RVar (_,f),[RApp (_,t,[c],[Expl]);d],[Expl]) when f = ldots_var ->
  ...
  | RApp (_,g,args,imps) -> AApp (aux g, List.map aux args)
  | RLambda (_,na,imp,ty,c) ->
      add_name found na; ALambda (na,imp,aux ty,aux c)
  | RProd (_,na,imp,ty,c) ->
      add_name found na; AProd (na,imp,aux ty,aux c)
  | RLetIn (_,na,imp,b,c) ->
      add_name found na; ALetIn (na,imp,aux b,aux c)
  | RCases (_,rtntypopt,tml,eqnl) ->
	  ...
	  option_iter
	    (fun (_,_,_,nl) ->
              List.iter (fun (na,_) -> add_name found na) nl) x;
          (aux tm,
          (na,
           option_map (fun (_,ind,n,nal) -> (ind,n,nal)) x))) tml,
        List.map f eqnl)
  | RLetTuple (loc,nal,(na,po),b,c) ->
      add_name found na;
      List.iter (fun (na,_) -> add_name found na) nal;
...
allowed in abbreviatable expressions"
  ...
  and terminator_of_pat f1 fimps ll1 lr1  = function
  | RApp (loc,f2,l2,imps) ->
	...
	error "Both ends of the recursive pattern have different lengths";
      ...
      let iter =
        if order then RApp (loc,f2,ll2@RVar (loc,ldots_var)::lr2,imps)
        else RApp (loc,f1,ll1@RVar (loc,ldots_var)::lr1,fimps) in
  ...
  and make_aconstr_list f args imps =
    let rec find_patterns acc = function
      | RApp(_,RVar (_,a),[c],_) :: l when a = ldots_var ->
          (* We've found the recursive part *)
	  let x,y,iter,term,lassoc =
            terminator_of_pat f imps (List.rev acc) l c in
...
let encode_list_value l =
  RApp (dummy_loc,RVar (dummy_loc,ldots_var),l, List.map(fun _->Expl) l)
...
let abstract_return_type_context pi mklam tml rtno =
  option_map (fun rtn ->
	...
	match t with Some x -> (pi x)@[na,Expl] | None -> [na,Expl]) tml) in
...
let abstract_return_type_context_rawconstr =
  abstract_return_type_context (fun (_,_,_,nal) -> nal)
    (fun (na,imp) c ->
      RLambda(dummy_loc,na,imp,RHole(dummy_loc,Evd.InternalHole),c))

let abstract_return_type_context_aconstr =
  abstract_return_type_context (fun (_,_,nal) -> nal)
    (fun (na,imp) c -> ALambda(na,imp,AHole Evd.InternalHole,c))
...
let rec match_cases_pattern metas acc pat1 pat2 =
  ...
  | PatVar (_,na1,_), PatVar (_,na2,_) -> match_names metas acc na1 na2
...
let rec match_ alp metas sigma a1 a2 = match (a1,a2) with
  ...
  | RApp (_,f1,l1,_), AApp (f2,l2) when List.length l1 = List.length l2 ->
      List.fold_left2 (match_ alp metas) (match_ alp metas sigma f1 f2) l1 l2
  | RApp (_,f1,l1,_), AList (x,_,(AApp (f2,l2) as iter),termin,lassoc) 
      when List.length l1 = List.length l2 ->
      match_alist alp metas sigma (f1::l1) (f2::l2) x iter termin lassoc
  | RLambda (_,na1,imp1,t1,b1), ALambda (na2,imp2,t2,b2) ->
     match_binders alp metas (na1,imp1) (na2,imp2)
       (match_ alp metas sigma t1 t2) b1 b2
  | RProd (_,na1,imp1,t1,b1), AProd (na2,imp2,t2,b2) ->
     match_binders alp metas (na1,imp1) (na2,imp2)
       (match_ alp metas sigma t1 t2) b1 b2
  | RLetIn (_,na1,imp1,t1,b1), ALetIn (na2,imp2,t2,b2) ->
     match_binders alp metas (na1,imp1) (na2,imp2)
       (match_ alp metas sigma t1 t2) b1 b2
  ...
  | RIf (_,a1,(na1,to1),b1,c1), AIf (a2,(na2,to2),b2,c2) ->
      let sigma =
        match_opt
          (match_binders alp metas (na1,Expl) (na2,Expl)) sigma to1 to2 in
      List.fold_left2 (match_ alp metas) sigma [a1;b1;c1] [a2;b2;c2]
  | RLetTuple (_,nal1,(na1,to1),b1,c1),
    ALetTuple (nal2,(na2,to2),b2,c2) 
      ...
      let (alp,sigma) =
	List.fold_left2
          (fun e (na1,imp1) (na2,imp2) ->
            if imp1<>imp2 then raise No_match;
            match_names metas e na1 na2) (alp,sigma) nal1 nal2 in
...
and match_binders alp metas (na1,imp1) (na2,imp2) sigma b1 b2 =
  if imp1<>imp2 then raise No_match;
...
type cases_pattern_expr =
  ...
  | CPatAtom of loc * reference option * impl
...
type constr_expr =
  ...
  | CArrow of loc * impl * constr_expr * constr_expr
  | CProdN of
      loc * ((name located * impl) list * constr_expr) list * constr_expr
  | CLambdaN of
      loc * ((name located * impl) list * constr_expr) list * constr_expr
  | CLetIn of loc * name located * impl * constr_expr * constr_expr
  | CAppExpl of loc * (proj_flag * reference) * (constr_expr * impl) list
  | CApp of loc * (proj_flag * constr_expr) * 
        ((constr_expr * impl) * explicitation located option) list
  | CCases of loc * constr_expr option *
      (constr_expr * (name option * constr_expr option)) list *
      (loc * cases_pattern_expr list list * constr_expr) list
  | CLetTuple of
      loc * (name located * impl) list * (name option * constr_expr option) *
...
and local_binder =
  | LocalRawDef of name located * impl * constr_expr
  | LocalRawAssum of (name located * impl) list * constr_expr
...
let names_of_local_binders bl =
  List.flatten
    (List.map
      (function LocalRawAssum(l,_)-> l
        |LocalRawDef(l,imp,_)->[l,imp]) bl)
...
let ids_of_cases_indtype =
  ...
  let rec vars_of = function
    ...
    | CApp (_,_,l) ->
        List.fold_left add_var [] (List.map (fun((a,_),_)->a) l)
    | CNotation (_,_,l) -> List.fold_left add_var [] l
    (* assume the ntn is applicative and does not instantiate the head !! *)
    | CAppExpl (_,_,l) -> List.fold_left add_var [] (List.map fst l)
...
let rec cases_pattern_fold_names f a = function
  ...
  | CPatAtom (_,Some (Ident (_,id)),_) when not (is_constructor id) -> f id a
...
let rec fold_constr_expr_binders g f n acc b = function
  | (nal,t)::l ->
      let nal = List.map (fun ((_,na),_) -> na) nal in
...
let fold_constr_expr_with_binders g f n acc = function
  | CArrow (loc,_,a,b) -> f n (f n acc a) b
  | CAppExpl (loc,(_,_),l) -> List.fold_left (f n) acc (List.map fst l)
  | CApp (loc,(_,t),l) ->
      List.fold_left (f n) (f n acc t) (List.map(fun((a,_),_)->a) l)
  | CProdN (_,l,b) | CLambdaN (_,l,b) -> fold_constr_expr_binders g f n acc b l
  | CLetIn (_,na,imp,a,b) ->
      fold_constr_expr_binders g f n acc b [[na,imp],a]
  ...
  | CLetTuple (loc,nal,(ona,po),b,c) ->
      let n' = List.fold_right (fun ((_,na),_) -> name_fold g na) nal n in
...
let mkLetInC (id,imp,a,b)   = CLetIn (dummy_loc,id,imp,a,b)
let mkProdC (idl,a,b)   = CProdN (dummy_loc,[idl,a],b)

let rec mkCProdN loc bll c =
  match bll with
  | LocalRawAssum (((loc1,_),_)::_ as idl,t) :: bll -> 
      CProdN (loc,[idl,t],mkCProdN (join_loc loc1 loc) bll c)
  | LocalRawDef ((loc1,_) as id,imp,b) :: bll -> 
      CLetIn (loc,id,imp,b,mkCProdN (join_loc loc1 loc) bll c)
  | [] -> c
...
let rec mkCLambdaN loc bll c =
  ...
  | LocalRawDef ((loc1,_) as id,imp,b) :: bll -> 
      CLetIn (loc,id,imp,b,mkCLambdaN (join_loc loc1 loc) bll c)
  | [] -> c
  | LocalRawAssum ([],_) :: bll -> mkCLambdaN loc bll c

let rec abstract_constr_expr c = function
  | [] -> c
  | LocalRawDef (x,imp,b)::bl -> mkLetInC(x,imp,b,abstract_constr_expr c bl)
...
let rec prod_constr_expr c = function
  | [] -> c
  | LocalRawDef (x,imp,b)::bl -> mkLetInC(x,imp,b,prod_constr_expr c bl)
  | LocalRawAssum (idl,t)::bl ->
...
let map_binder g e nal =
  List.fold_right (fun ((_,na),_) -> name_fold g na) nal e
...
let map_local_binders f g e bl =
  ...
  let h (e,bl) = function
    ...
    | LocalRawDef((loc,na),imp,ty) ->
        (name_fold g na e, LocalRawDef((loc,na),imp,f e ty)::bl) in
  let (e,rbl) = List.fold_left h (e,[]) bl in
  (e, List.rev rbl)

let map_constr_expr_with_binders g f e = function
  | CArrow (loc,imp,a,b) -> CArrow (loc,imp,f e a,f e b)
  | CAppExpl (loc,r,l) ->
      CAppExpl (loc,r,List.map (fun(a,imp) -> (f e a,imp)) l) 
  | CApp (loc,(p,a),l) -> 
      CApp (loc,(p,f e a),List.map (fun ((a,imp),i) -> ((f e a,imp),i)) l)
  | CProdN (loc,bl,b) ->
      let (e,bl) = map_binders f g e bl in CProdN (loc,bl,f e b)
  | CLambdaN (loc,bl,b) ->
      let (e,bl) = map_binders f g e bl in CLambdaN (loc,bl,f e b)
  | CLetIn (loc,na,imp,a,b) ->
      CLetIn (loc,na,imp,f e a,f (name_fold g (snd na) e) b)
  ...
  | CLetTuple (loc,nal,(ona,po),b,c) ->
      let e' = List.fold_right (fun ((_,na),_) -> name_fold g na) nal e in
