...
let raw_app (loc,hd,args,imps) =
  if args =[] then hd else RApp(loc,hd,args,imps) 
...
(*      Stdpp.raise_with_loc loc 
  ...
  | CPatAtom (_,Some (Libnames.Ident (_,id)),_) -> add_var id globs
  |  _  -> globs 
...
let type_tester_var body typ = 
  raw_app(dummy_loc,
       RLambda(dummy_loc,Anonymous,Expl,typ,
	       RSort (dummy_loc,RProp Null)),body,[Expl])

let abstract_one_hyp inject h raw = 
  match h with 
      Hvar (loc,(id,None)) ->  
	RProd
          (dummy_loc,Name id, Expl,RHole (loc,Evd.BinderType (Name id)), raw)
    | Hvar (loc,(id,Some typ)) ->  
	RProd (dummy_loc,Name id,Expl,fst typ, raw)
    | Hprop st -> 
	RProd (dummy_loc,st.st_label,Expl,inject st.st_it, raw)
...
let rec match_hyps blend names constr = function 
    [] -> [],substl names constr
  | hyp::q -> 
      let (name,imp,typ,body)=destProd constr in
...
let rec deanonymize ids = 
  function 
	...
	let _= ids:= (loc,new_id) :: found , new_id :: known in
	  PatVar (loc,Name new_id,imp)
    | PatVar (loc,Name id,_) as pat -> 
	let (found,known) = !ids in
...
let rec raw_of_pat =
  function 
      PatVar (loc,Anonymous,_) -> anomaly "Anonymous pattern variable" 
	...
	raw_app(loc,RRef(dummy_loc,Libnames.ConstructRef cstr),
	        add_params mind.Declarations.mind_nparams args,
                list_tabulate(fun _->Impl) mind.Declarations.mind_nparams
                @List.map(fun _ -> Impl) args) 
			 
let prod_one_hyp = function
    (loc,(id,None)) ->
      (fun raw ->  
	 RProd (dummy_loc,Name id, Expl,
...
let let_in_one_alias (id,pat) raw =
  RLetIn (dummy_loc,Name id,Expl,raw_of_pat pat, raw)
...
let rec match_args dest names constr = function 
  ...
  | _::q -> 
      let (name,imp,typ,body)=dest constr in
      let st={st_label=name;st_it=substl names typ} in
      let qnames=
	match name with
...
let rec match_aliases names constr = function 
    [] -> [],names,substl names constr
  | _::q -> 
      let (name,imp,c,typ,body)=destLetIn constr in
...
let interp_cases info sigma env params (pat:cases_pattern_expr) hyps =
  ...
  let app_ind =
    ...
    let args = rparams@rparams_rec@dum_args in
    raw_app(dummy_loc,rind,args,List.map(fun _->Expl)args) in
  ...
  let term2 =
    RLetIn(dummy_loc,Anonymous,Expl,
  ...
  let (_,_,pat_pat,pat_typ,rest1) = destLetIn rest2 in
  let blend st st' =
    match st'.st_it with 
  ...
  let push_one hyp env0 =
	...
	(Hprop st | Hvar st) ->
	  match st.st_label with
	      Name id -> Environ.push_named (id,Expl,None,st.st_it) env0
	    | _ -> env in
...
let abstract_one_arg = function
    (loc,(id,None)) ->
      (fun raw ->  
	 RLambda (dummy_loc,Name id,Expl, 
		RHole (loc,Evd.BinderType (Name id)), raw))
  | (loc,(id,Some typ)) -> 
      (fun raw ->  
	 RLambda (dummy_loc,Name id,Expl,fst typ, raw))
