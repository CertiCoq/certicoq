...
let l_true_equals_true = 
  lazy (mkApp(logic_constant "refl_equal", 
        [|data_constant "bool";data_constant "true"|],[|Impl;Impl|]))
  ...
  let cciterm=special_whd gls term  in
	...
	Prod(_,Expl,a,b) -> 
	  ...
	  if ind = Lazy.force li_False then
	    ...
	    make_atom atom_env (normalize term)
      | App(hd,argv,_) when Array.length argv = 2 ->
	  begin
	    try 
...
let rec make_hyps atom_env gls lenv = function
    [] -> []
  | (_,imp,Some body,typ)::rest -> 
      make_hyps atom_env gls (typ::body::lenv) rest  
  | (id,imp,None,typ)::rest ->
      let hrec=
	...
	then
	  ...
	  (id,imp,make_form atom_env gls typ)::hrec

let rec build_pos n =
  if n<=1 then force node_count l_xH 
  else if n land 1 = 0 then 
    mkApp (force node_count l_xO,[|build_pos (n asr 1)|],[|Expl|])
  else 
    mkApp (force node_count l_xI,[|build_pos (n asr 1)|],[|Expl|])

let rec build_form = function
    Atom n -> mkApp (force node_count l_Atom,[|build_pos n|],[|Expl|])
  | Arrow (f1,f2) -> 
	...
	mkApp (force step_count l_Ax,
	  ...
	  mkApp (force step_count l_I_And, 
		 [|build_proof pops size p1;
		   build_proof pops size p2|],[|Expl;Expl|])
      | E_And(i,p) -> 
	  mkApp (force step_count l_E_And,
		 [|build_pos (decal i pops);
		   build_proof pops (size + 2) p|],[|Expl;Expl|])
      | D_And(i,p) -> 
	  mkApp (force step_count l_D_And,
		 [|build_pos (decal i pops);
		   build_proof pops (size + 1) p|],[|Expl;Expl|])
      | I_Or_l(p) -> 
	  mkApp (force step_count l_I_Or_l,
		 [|build_proof pops size p|],[|Expl|])
      | I_Or_r(p) -> 
...
let build_env gamma=
  List.fold_right (fun (p,_) e -> 
		     mkApp(force node_count l_push,
                           [|mkProp;p;e|],[|Expl;Expl;Expl|])) 
    gamma.env (mkApp (force node_count l_empty,[|mkProp|],[|Expl|]))
...
let rtauto_tac gls=
  ...
  let formula=
    List.fold_left (fun gl (_,_,f)-> Arrow (f,gl)) glf hyps in  
  let search_fun = 
  ...
  let main = mkApp (force node_count l_Reflect,
		    [|build_env gamma;
		      build_form formula;
		      build_proof [] 0 prf|],[|Expl;Expl;Expl|]) in
  let term=
    Term.applist
      (main,List.rev_map (fun (id,_,_) -> mkVar id) hyps,
       List.rev_map(fun (_,imp,_)->imp)hyps) in
