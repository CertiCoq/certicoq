...
(* $Id: evd.ml 9878 2007-06-06 17:00:01Z herbelin $ *)
...
let define evd ev body = 
  ...
  if not (noccur_implicit oldinfo.evar_hyps body)
  then error "Evd.define: implicit variable in extraction of body";
  let newinfo =
    { oldinfo with
...
let instantiate_sign_including_let sign args =
  let rec instrec = function
    | ((id,_,b,_) :: sign, c::args) -> (id,c) :: (instrec (sign,args))
    | ([],[])                        -> []
    | ([],_) | (_,[]) ->
...
type clbinding =
  | Cltyp of name * impl * constr freelisted
  | Clval of
      name * impl * (constr freelisted * instance_status) * constr freelisted

let map_clb f = function
  | Cltyp (na,imp,cfl) -> Cltyp (na,imp,map_fl f cfl)
  | Clval (na,imp,(cfl1,pb),cfl2) ->
      Clval (na,imp,(map_fl f cfl1,pb),map_fl f cfl2)

(* name of defined is erased (but it is pretty-printed) *)
let clb_name = function
    Cltyp(na,_,_) -> (na,false)
  | Clval (na,_,_,_) -> (na,true)
...
let evar_declare hyps evn imp ty ?(src=(dummy_loc,InternalHole)) evd =
  let hyps = if imp=Impl then set_impl_named_val Expl hyps else hyps in
  { evd with
...
let metas_of evd = 
  List.map (function
    | (n,Clval(_,_,_,typ)) -> (n,typ.rebus)
    | (n,Cltyp (_,_,typ))  -> (n,typ.rebus))
    (meta_list evd)
...
let meta_impl evd mv =
  match Metamap.find mv evd.metas with
    | Cltyp (_,imp,_) -> imp
    | Clval(_,imp,_,_) -> imp
 
let meta_declare mv imp v ?(name=Anonymous) evd =
  { evd with
    metas = Metamap.add mv (Cltyp(name,imp,mk_freelisted v)) evd.metas }
...
let meta_assign mv (v,pb) evd =
  match Metamap.find mv evd.metas with
      Cltyp(na,imp,ty) ->
        { evd with
          metas =
          Metamap.add mv (Clval(na,imp,(mk_freelisted v,pb), ty)) evd.metas }
...
let retract_coercible_metas evd =
  let mc,ml = 
    Metamap.fold (fun n v (mc,ml) -> 
      match v with
	| Clval (na,imp,(b,(UserGiven,CoerceToType as s)),typ) ->
	    (n,b.rebus,s)::mc, Metamap.add n (Cltyp (na,imp,typ)) ml
	| v ->
...
let pr_meta_map mmap =
  ...
  let pr_meta_binding = function
    | (mv,Cltyp (na,imp,b)) ->
      	hov 0 
	  (if imp=Impl then (str"["++pr_meta mv++str"]") else pr_meta mv
            ++ pr_name na ++ str " : " ++ print_constr b.rebus ++ fnl ())
    | (mv,Clval(na,imp,(b,_),_)) ->
      	hov 0 
	  (if imp=Impl then (str"["++pr_meta mv++str"]") else pr_meta mv
            ++ pr_name na ++ str " := " ++ print_constr b.rebus ++ fnl ())
  in
...
let pr_idl idl =
  prlist_with_sep pr_spc
    (fun (id,imp) -> if imp=Impl then str"["++pr_id id++str"]"
    else pr_id id)
    idl

let pr_evar_info evi =
  let idl = List.map (fun (id,imp,_,_) -> (id,imp)) (evar_context evi) in
  let phyps = pr_idl (List.rev idl) in
