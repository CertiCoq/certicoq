...
(* $Id: declare.ml 9795 2007-04-25 15:13:45Z soubiran $ *)
...
let cache_variable ((sp,_),o) =
  ...
  let vd = match d with (* Fails if not well-typed *)
    | SectionLocalAssum ty ->
        let cst = Global.push_named_assum (id,Expl,ty) in
        let (_,_,bd,ty) = Global.lookup_named id in
        CheckedSectionLocalAssum (ty,cst)
    | SectionLocalDef (c,t,opaq) -> 
        let cst = Global.push_named_def (id,Expl,c,t) in
        let (_,_,bd,ty) = Global.lookup_named id in
...
let discharged_hyps kn sechyps =
  let (_,dir,_) = repr_kn kn in
  let args,_ = instance_from_named_context sechyps in
  let args = array_map_to_list destVar args in
...
let discharge_constant ((sp,kn),(cdt,dhyps,kind)) =
  ...
  let recipe =
    { d_from=cb; d_modlist=repl; d_abstract=sechyps } in
...
let get_variable id = 
  ...
  match x with
    | CheckedSectionLocalDef (c,ty,cst,opaq) -> (id,Expl,Some c,ty)
    | CheckedSectionLocalAssum (ty,cst) -> (id,Expl,None,ty)
...
let clear_proofs sign =
  List.fold_right
    (fun (id,imp,c,t as d) signv -> 
      let d = if variable_opacity id then (id,imp,None,t) else d in
