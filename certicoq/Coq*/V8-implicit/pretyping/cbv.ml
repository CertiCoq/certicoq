...
type cbv_value =
  | VAL of int * constr
  | LAM of int * (name * impl * constr) list * constr * cbv_value subs
  | FIXP of fixpoint * cbv_value subs * cbv_value array * impl array
  | COFIXP of cofixpoint * cbv_value subs * cbv_value array * impl array
  | CONSTR of constructor * cbv_value array * impl array
...
let rec shift_value n = function
  ...
  | FIXP (fix,s,args,imps) ->
      FIXP (fix,subs_shft (n,s), Array.map (shift_value n) args,imps)
  | COFIXP (cofix,s,args,imps) ->
      COFIXP (cofix,subs_shft (n,s), Array.map (shift_value n) args,imps)
  | CONSTR (c,args,imps) ->
      CONSTR (c, Array.map (shift_value n) args,imps)
let shift_value n v =
...
type cbv_stack =
  | TOP
  | APP of cbv_value array * impl array * cbv_stack
  | CASE of constr * constr array * case_info * cbv_value subs * cbv_stack

(* Adds an application list. Collapse APPs! *)
let stack_app appl imps stack =
  if Array.length appl = 0 then stack else
    match stack with
    | APP(args,i,stk) -> APP(Array.append appl args,Array.append imps i,stk)
    | _             -> APP(appl, imps, stack)
...
let strip_appl head stack =
  match head with
    | FIXP (fix,env,app,imps) ->
        (FIXP(fix,env,[||],[||]), stack_app app imps stack)
    | COFIXP (cofix,env,app,imps) ->
        (COFIXP(cofix,env,[||],[||]), stack_app app imps stack)
    | CONSTR (c,app,imps) -> (CONSTR(c,[||],[||]), stack_app app imps stack)
    | _ -> (head, stack)
...
let rec norm_head info env t stack =
  ...
  | App (head,args,imps) ->
      ...
      norm_head info env head (stack_app nargs imps stack)
  ...
  | LetIn (x, imp, b, t, c) ->
	...
	let normt =
	  mkLetIn (x, imp, cbv_norm_term info env b,
  ...
  | Prod (x,imp,t,c) -> 
      (VAL(0, mkProd (x, imp, cbv_norm_term info env t,
...
and cbv_stack_term info stack env t =
  match norm_head info env t stack with
    ...
    | (LAM (nlams,ctxt,b,env), APP (args, aimps, stk))
      when red_set (info_flags info) fBETA ->
        ...
        if nargs == nlams then
          ...
          let eimps = Array.sub aimps nlams (nargs-nlams) in
          cbv_stack_term info (APP(eargs,eimps,stk)) env' b
        else
          let ctxt' = list_skipn nargs ctxt in
          LAM(nlams-nargs,ctxt', b, subs_cons(args,env))

    (* a Fix applied enough -> IOTA *)
    | (FIXP(fix,env,_,_), stk)
        when fixp_reducible (info_flags info) fix stk ->
        let (envf,redfix) = contract_fixp env fix in
        cbv_stack_term info stk envf redfix

    (* constructor guard satisfied or Cofix in a Case -> IOTA *)
    | (COFIXP(cofix,env,_,_), stk)
        when cofixp_reducible (info_flags info) cofix stk->
        let (envf,redfix) = contract_cofixp env cofix in
        cbv_stack_term info stk envf redfix

    (* constructor in a Case -> IOTA *)
    | (CONSTR((sp,n),_,_), APP(args,imps,CASE(_,br,ci,env,stk)))
            when red_set (info_flags info) fIOTA ->
        let nargs = Array.length args - ci.ci_npar in
	let cargs = Array.sub args ci.ci_npar nargs in
        let cimps = Array.sub imps  ci.ci_npar nargs in
        cbv_stack_term info (stack_app cargs cimps stk) env br.(n-1)
         
    (* constructor of arity 0 in a Case -> IOTA *)
    | (CONSTR((_,n),_,_), CASE(_,br,_,env,stk))
            when red_set (info_flags info) fIOTA ->
                    cbv_stack_term info stk env br.(n-1)

    (* may be reduced later by application *)  
    | (head, TOP) -> head
    | (FIXP(fix,env,_,_), APP(appl,imps,TOP)) -> FIXP(fix,env,appl,imps) 
    | (COFIXP(cofix,env,_,_), APP(appl,imps,TOP)) ->
        COFIXP(cofix,env,appl,imps) 
    | (CONSTR(c,_,_), APP(appl,imps,TOP)) -> CONSTR(c,appl,imps)
...
and apply_stack info t = function
  | TOP -> t
  | APP (args,imps,st) ->
      apply_stack info (mkApp(t,Array.map (cbv_norm_value info) args, imps)) st
...
and cbv_norm_value info = function (* reduction under binders *)
  ...
  | LAM (n,ctxt,b,env) ->
      let nctxt =
        list_map_i (fun i (x,imp,ty) ->
          (x,imp,cbv_norm_term info (subs_liftn i env) ty)) 0 ctxt in
      compose_lam (List.rev nctxt)
        (cbv_norm_term info (subs_liftn n env) b)
  | FIXP ((lij,(names,lty,bds)),env,args,imps) ->
      mkApp
        (mkFix (lij,
		(names,
         ...
         imps)
  | COFIXP ((j,(names,lty,bds)),env,args,imps) ->
      mkApp
        (mkCoFix (j,
         ...
         imps)
  | CONSTR (c,args,imps) ->
      mkApp(mkConstruct c, Array.map (cbv_norm_value info) args, imps)
