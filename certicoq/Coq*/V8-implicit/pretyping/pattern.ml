...
(* $Id: pattern.ml 9718 2007-03-19 16:54:25Z msozeau $ *)
...
type constr_pattern =
  ...
  | PLambda of name * impl * constr_pattern * constr_pattern
  | PProd of name * impl * constr_pattern * constr_pattern
  | PLetIn of name * impl * constr_pattern * constr_pattern
  | PSort of rawsort
...
let rec occur_meta_pattern = function
  ...
  | PLambda (na,_,t,c)  -> (occur_meta_pattern t) or (occur_meta_pattern c)
  | PProd (na,_,t,c)  -> (occur_meta_pattern t) or (occur_meta_pattern c)
  | PLetIn (na,_,t,c)  -> (occur_meta_pattern t) or (occur_meta_pattern c)
  | PIf (c,c1,c2)  ->
...
let rec head_pattern_bound t =
  match t with
    | PProd (_,_,_,b)  -> head_pattern_bound b 
...
let rec pattern_of_constr t =
  match kind_of_term t with
    ...
    | LetIn (na,imp,c,_,b) ->
        PLetIn (na,imp,pattern_of_constr c,pattern_of_constr b)
    | Prod (na,imp,c,b)   ->
        PProd (na,imp,pattern_of_constr c,pattern_of_constr b)
    | Lambda (na,imp,c,b) -> 
        PLambda (na,imp,pattern_of_constr c,pattern_of_constr b)
    | App (f,a,_) ->
        PApp (pattern_of_constr f,Array.map pattern_of_constr a)
...
let map_pattern_with_binders g f l = function
  ...
  | PLambda (n,imp,a,b) -> PLambda (n,imp,f l a,f (g l) b)
  | PProd (n,imp,a,b) -> PProd (n,imp,f l a,f (g l) b)
  | PLetIn (n,imp,a,b) -> PLetIn (n,imp,f l a,f (g l) b)
...
let rec subst_pattern subst pat = match pat with
  ...
  | PSoApp (i,args) ->
	...
	if args' == args then pat else
	  PSoApp (i,args')
  | PLambda (name,imp,c1,c2) ->
      let c1' = subst_pattern subst c1 in
      let c2' = subst_pattern subst c2 in
	if c1' == c1 && c2' == c2 then pat else
	  PLambda (name,imp,c1',c2')
  | PProd (name,imp,c1,c2) ->
      let c1' = subst_pattern subst c1 in
      let c2' = subst_pattern subst c2 in
	if c1' == c1 && c2' == c2 then pat else
	  PProd (name,imp,c1',c2')
  | PLetIn (name,imp,c1,c2) ->
      let c1' = subst_pattern subst c1 in
      let c2' = subst_pattern subst c2 in
	if c1' == c1 && c2' == c2 then pat else
	  PLetIn (name,imp,c1',c2')
...
let mkPLambda (na, imp) b = PLambda(na,imp,PMeta None,b)
...
let rec pat_of_raw metas vars = function
  ...
  | RApp (_, RPatVar (_,(true,n)), cl, _) ->
      metas := n::!metas; PSoApp (n, List.map (pat_of_raw metas vars) cl)
  | RApp (_,c,cl,_) -> 
      PApp (pat_of_raw metas vars c,
	    Array.of_list (List.map (pat_of_raw metas vars) cl))
  | RLambda (_,na,imp,c1,c2) ->
      PLambda (na, imp,pat_of_raw metas vars c1,
	       pat_of_raw metas (na::vars) c2)
  | RProd (_,na,imp,c1,c2) ->
      PProd (na, imp,pat_of_raw metas vars c1,
	       pat_of_raw metas (na::vars) c2)
  | RLetIn (_,na,imp,c1,c2) ->
      PLetIn (na, imp, pat_of_raw metas vars c1,
	       pat_of_raw metas (na::vars) c2)
  | RSort (_,s) ->
      PSort s
  | RHole _ ->
  ...
  | RLetTuple (loc,nal,(_,None),b,c) ->
      let mkRLambda c (na, imp) =
        RLambda (loc,na,imp,RHole (loc,Evd.InternalHole),c) in
  ...
  | RCases (loc,p,[c,(na,indnames)],brs) ->
	...
	| Some p, Some (_,ind,n,nal) ->
	    rev_it_mkPLambda nal
              (mkPLambda (na, Expl) (pat_of_raw metas vars p)),
...
and pat_of_raw_branch loc metas vars ind brs i =
  ...
  match bri with
	...
	if ind <> None & ind <> Some indsp then
        ...
        let lna =
          (List.map
            (function PatVar(_,na,imp) -> na,imp
              | PatCstr(loc,_,_,_) ->
                  user_err_loc (loc,"pattern_of_rawconstr",
                                Pp.str "Not supported pattern")) lv) in
        let vars' = List.rev (List.map fst lna) @ vars in 
