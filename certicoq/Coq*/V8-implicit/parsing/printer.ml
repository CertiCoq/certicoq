...
(* $Id: printer.ml 10024 2007-07-18 15:20:49Z herbelin $ *)
...
let pr_var_decl env (id,imp,c,typ) =
  ...
  let ptyp = (str" : " ++ pt) in
  hv 2 (pr_impl imp (pr_id id) ++ cut() ++ pbody ++ ptyp)
...
let pr_rel_decl env (na,imp,c,typ) =
  ...
  let pvar =
    match na with
      | Anonymous -> pr_impl imp (str"<>")
      | Name id -> pr_impl imp (pr_id id) in
  hov 0 (pvar ++ pbody ++ str":" ++ spc () ++ ptyp)
...
let pr_impl_constr imp c =
  if imp=Impl then str"["++pr_lconstr c++str"]"
  else pr_constr c
...
let pr_prim_rule = function
  ...
  | Cut (b,id,imp,t) ->
      if b then
        (str"assert " ++ pr_impl_constr imp t)
      else
        (str"cut " ++ pr_impl_constr imp t ++
         str ";[intro " ++ pr_id id ++ str "|idtac]")
  ...
  | Convert_hyp (id,_,None,t) ->
      (str"change "  ++ pr_constr t  ++ spc ()  ++ str"in "  ++ pr_id id)

  | Convert_hyp (id,_,Some c,t) ->
