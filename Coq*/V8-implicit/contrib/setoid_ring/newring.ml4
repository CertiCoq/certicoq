...
(*i $Id: newring.ml4 10046 2007-07-24 17:28:08Z barras $ i*)
  ...
  match f_map t with
    ...
    | None ->
        (match kind_of_term t with
            App(f,args,imps) -> mk_clos_app_but f_map subs f args imps 0
          | Prod _ -> mk_clos_deep (mk_clos_but f_map) subs t
          | _ -> mk_atom t)

and mk_clos_app_but f_map subs f args imps n =
  if n >= Array.length args then mk_atom(mkApp(f, args, imps))
  else
    let fargs, args' = array_chop n args in
    let fimps, imps' = array_chop n imps in
    let f' = mkApp(f,fargs,fimps) in
    match f_map f' with
        Some map ->
          mk_clos_deep
            (fun s' -> unmark_arg (tag_arg (mk_clos_but f_map s') map s'))
            subs
            (mkApp (mark_arg (-1) f', Array.mapi mark_arg args',imps'))
      | None -> mk_clos_app_but f_map subs f args imps (n+1)
...
let lapp f imps args = mkApp(Lazy.force f,args,imps)
...
let option_reify ty f = function
    None -> lapp coq_None [|Impl|] [|ty|]
  | Some x -> lapp coq_Some [|Impl;Expl|] [|ty;f x|]

let dest_rel0 t =  
  match kind_of_term t with
  | App(f,args,imps) when Array.length args >= 2 ->
      let fargs = Array.sub args 0 (Array.length args - 2) in
      let fimps = Array.sub imps 0 (Array.length args - 2) in
      let rel = mkApp(f,fargs,fimps) in
      if closed0 rel then
...
let setoid_of_relation r =
  lapp coq_mk_Setoid [|Impl;Impl;Expl;Expl;Expl|]
    [|r.rel_a; r.rel_aeq;
      out_some r.rel_refl; out_some r.rel_sym; out_some r.rel_trans |]

let op_morph r add mul opp req m1 m2 m3 =
  lapp coq_mk_reqe [|Impl;Impl;Impl;Impl;Impl;Expl;Expl;Expl|]
    [| r; add; mul; opp; req; m1; m2; m3 |]

let op_smorph r add mul req m1 m2 =
  lapp coq_mk_seqe [|Impl;Impl;Impl;Impl;Expl;Expl|]
    [| r; add; mul; req; m1; m2 |]
...
let default_ring_equality (r,add,mul,opp,req) =
  ...
  match default_relation_for_carrier ~filter:is_setoid r with
      Leibniz _ ->
        let setoid = lapp coq_eq_setoid [|Impl|] [|r|] in
        let op_morph =
          match opp with
              Some opp ->
                lapp coq_eq_morph [|Impl;Impl;Impl;Impl|] [|r;add;mul;opp|]
            | None -> lapp coq_eq_smorph [|Impl;Impl;Impl|] [|r;add;mul|] in
        (setoid,op_morph)
    | Relation rel ->
...
let dest_morph env sigma m_spec =
  ...
  match kind_of_term m_typ with
      App(f,[|r;zero;one;add;mul;sub;opp;req;
              c;czero;cone;cadd;cmul;csub;copp;ceqb;phi|],_)
        when f = Lazy.force coq_ring_morph ->
          (c,czero,cone,cadd,cmul,Some csub,Some copp,ceqb,phi)
    | App(f,[|r;zero;one;add;mul;req;c;czero;cone;cadd;cmul;ceqb;phi|],_)
...
let reflect_coeff rkind =
  ...
  match rkind with
    ...
    | Computational c -> lapp coq_comp [|Expl|] [|c|]
    | Morphism m -> lapp coq_morph [|Expl|] [|m|]
...
let make_hyp env c =
  ...
  lapp coq_mkhypo [|Expl;Expl|] [|t;c|]

let make_hyp_list env lH =
  let carrier = Lazy.force coq_hypo in
  List.fold_right 
    (fun c l ->
      lapp coq_cons [|Impl;Expl;Expl|][|carrier; (make_hyp env c); l|]) lH
    (lapp coq_nil [|Impl|] [|carrier|])
...
let interp_power env pow =  
  ...
  | None -> 
      ...
      (TacArg(TacCall(dummy_loc,t,[])), lapp coq_None [|Impl|][|carrier|])
  | Some (tac, spec) -> 
      let tac = 
        match tac with
      ...
      (tac, lapp coq_Some [|Impl;Expl|][|carrier; spec|])

let interp_sign env sign =
  let carrier = Lazy.force coq_hypo in
  option_reify carrier (fun spec -> make_hyp env (ic spec)) sign
    (* Same remark on ill-typed terms ... *)

let interp_div env div =
  let carrier = Lazy.force coq_hypo in
  option_reify carrier (fun spec -> make_hyp env (ic spec)) div
    (* Same remark on ill-typed terms ... *)
...
let make_term_list carrier rl =
  List.fold_right
    (fun x l -> lapp coq_cons [|Impl;Expl;Expl|] [|carrier;x;l|]) rl
    (lapp coq_nil [|Impl|] [|carrier|])
...
let dest_field env sigma th_spec =
  let th_typ = Retyping.get_type_of env sigma th_spec in
  match kind_of_term th_typ with
    | App(f,[|r;zero;one;add;mul;sub;opp;div;inv;req|],_)
        when f = Lazy.force afield_theory ->
        let rth = lapp af_ar
          [|Impl;Impl;Impl;Impl;Impl;Impl;Impl;Impl;Impl;Impl;Expl|]
    ...
    | App(f,[|r;zero;one;add;mul;div;inv;req|],_)
        when f = Lazy.force sfield_theory ->
        let rth = lapp sf_sr
          [|Impl;Impl;Impl;Impl;Impl;Impl;Impl;Impl;Expl|]
...
let default_field_equality r inv req =
  ...
  match default_relation_for_carrier ~filter:is_setoid r with
      Leibniz _ ->
        mkLambda(Name(id_of_string"x"),Expl,r,
        mkLambda(Name(id_of_string"y"),Expl,r,
        mkApp
          ((Coqlib.build_coq_eq_data()).congr,[|r;r;inv;mkRel 2;mkRel 1|],
...
let add_field_theory name fth eqth morphth cst_tac inj (pre,post) power sign odiv =
  ...
  let cond_lemma =
    match inj with
      | Some thm -> mkApp(constr_of params.(8),[|thm|],[|Expl|])
