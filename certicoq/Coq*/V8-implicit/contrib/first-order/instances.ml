...
(*i $Id: instances.ml 9850 2007-05-22 21:37:55Z herbelin $ i*)
...
let mk_open_instance id gl m t=
  ...
  let var_id=
	...
	(* since we know we will get a product, 
      ...
      let (nam,_,_,_)=destProd (whd_betadeltaiota env evmap typ) in
	match nam with 
	    Name id -> id
  ...
  let rec aux n avoid=
	...
	(Name nid,Expl,None,dummy_constr)::(aux (n-1) (nid::avoid)) in
  let nt=it_mkLambda_or_LetIn revt (aux m []) in
  let rawt=Detyping.detype false [] [] nt in
  let rec raux n t=
    if n=0 then t else 
      match t with
	  RLambda(loc,name,imp,_,t0)->
	    let t1=raux (n-1) t0 in
	      RLambda(loc,name,imp,RHole (dummy_loc,Evd.BinderType name),t1)
...
let left_instance_tac (inst,id) continue seq=
  match inst with
	...
	if lookup (id,None) seq then 
	  ...
	  tclTHENS (cut Expl dom) 
	    [tclTHENLIST
	       [introf;
		(fun gls->generalize 
		    [mkApp(constr_of_global id,
			  [|mkVar (Tacmach.pf_nth_hyp_id gls 1)|],[|Expl|])]
                    [Expl]
                    gls);
		introf;
	  ...
	  let special_generalize=
		...
		let gt= 
		  it_mkLambda_or_LetIn 
		    (mkApp(constr_of_global id,[|ot|],[|Expl|])) rc in
		  generalize [gt] [Expl] gl
	    else
	      generalize [mkApp(constr_of_global id,[|t|],[|Expl|])][Expl]
	  in
	    tclTHENLIST 
	      [special_generalize;
	       introf; 
