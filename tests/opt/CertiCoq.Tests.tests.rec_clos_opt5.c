#ifndef CERTICOQ_TESTS_TESTS_REC_CLOS_OPT5_C
#define CERTICOQ_TESTS_TESTS_REC_CLOS_OPT5_C
#include <gc_stack.h>
#include <prim_string.h>
#include <prim_floats.h>
#include <prim_int63.h>
#include <coq_c_ffi.h>
#include "CertiCoq.Tests.tests.rec_clos_opt5.h"
extern struct thread_info *make_tinfo(void);
extern value aux_known_104(struct thread_info *, value, value, value, value);
...
value aux_known_104(struct thread_info *, value, value, value, value);
...
unsigned long long const aux_known_info_759[6] = { 0LL, 4LL, 0LL, 1LL, 2LL,
  3LL, };

value aux_known_104(struct thread_info *$tinfo, value $l_126, value $y_127, value $y_128, value $y_129)
{
  ...
  if (($l_126 & 1) == 0) {
    switch (*((value *) $l_126 + -1LL) & 255LL) {
      default:
        $z_131 = *((value *) $l_126 + 0LL);
        $zs_132 = *((value *) $l_126 + 1LL);
        $args = (*$tinfo).args;
        (*$tinfo).alloc = $alloc;
        (*$tinfo).limit = $limit;
        *(root + 3LL) = $zs_132;
        ...
        *(root + 0LL) = $y_138;
        ...
        $y_139 =
          ((value (*)(struct thread_info *, value, value, value, value)) 
            aux_known_104)
          ($tinfo, $zs_132, $y_127, $y_128, $y_129);
        $alloc = (*$tinfo).alloc;
        $limit = (*$tinfo).limit;
        if (!(3LL <= $limit - $alloc)) {
          *(root + 1LL) = $y_139;
...
value body(struct thread_info *$tinfo)
{
  ...
  *(root + 0LL) = $y_142;
  ...
  $CertiCoqdTestsdtestsdrec_clos_758 =
    ((value (*)(struct thread_info *, value, value, value, value)) aux_known_104)
    ($tinfo, $y_756, $y_142, $y_145, $y_149);
  $alloc = (*$tinfo).alloc;
  $limit = (*$tinfo).limit;
  /*skip*/;
  (*$tinfo).alloc = $alloc;
...
#endif /* CERTICOQ_TESTS_TESTS_REC_CLOS_OPT5_C */
