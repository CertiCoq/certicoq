...
Section quote_byte.
  Import Byte.
  Definition quote_byte (c : Byte.byte) : term :=
  match c with
  | x00 => <%x00%>
  | x01 => <%x01%>
  | x02 => <%x02%>
...
Fixpoint has_var (x : Kernames.ident) (e : term) {struct e} : bool. ltac1:(refine(
  match e with
  | tRel n => false
  | tVar x' => x ==? x'
...
Definition erase_ctx (C : Kernames.ident) (Cdecl : context_decl) (Γ : named_ctx) : named_ctx. ltac1:(refine(
  map (fun '(C', d) =>
   if C ==? C' then
