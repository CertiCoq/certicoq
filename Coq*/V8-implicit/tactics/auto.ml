...
(* $Id: auto.ml 9842 2007-05-20 17:44:23Z herbelin $ *)
...
let rec nb_hyp c = match kind_of_term c with
  | Prod(_,_,_,c2) -> if noccurn 1 c2 then 1+(nb_hyp c2) else nb_hyp c2
  | _ -> 0 
...
let make_exact_entry (c,cty) =
  ...
  match kind_of_term cty with
    ...
    | _ ->
        let hd,_,_ = head_constr cty in
	(head_of_constr_reference hd, { pri=0; pat=None; code=Give_exact c })
...
let make_resolve_hyp env sigma (hname,_,_,htyp) = 
  try
...
let make_trivial env sigma c =
  ...
  let hd,_,_ = head_constr t in
  let hd = head_of_constr_reference hd in
...
let subst_autohint (_,subst,(local,name,hintlist as obj)) = 
  ...
  let subst_hint (lab,data as hint) =
    ...
    let lab' =
     try
       let hd,_,_ = head_constr_bound elab' in
       head_of_constr_reference hd
...
let fmt_hint_term cl = 
  try 
    let (hdc,args,imps) = head_constr_bound cl in
	...
	map_succeed 
	  (fun (name, db) -> 
	     (name, db, Hint_db.map_auto (hd,applist(hdc,args,imps)) db)) 
	  dbs
    in 
    if valid_dbs = [] then 
...
and trivial_resolve db_list local_db cl = 
  try 
    let hdconstr,_,_ = head_constr_bound cl in
    priority 
...
let decomp_unary_term c gls = 
  ...
  let hd,_,_ = head_constr typc in 
...
let decomp_empty_term c gls = 
  let typc = pf_type_of gls c in 
  let (hd,_,_) = decompose_app typc in 
...
let rec search_gen decomp n db_list local_db extra_sign goal =
  ...
  let intro_tac = 
    tclTHEN intro 
      (fun g' -> 
	 let (hid,_,_,htyp as d) = pf_last_hyp g' in
	 let hintl = 
	   try 
...
let make_resolve_any_hyp env sigma (id,_,_,ty) =
  let ents = 
    map_succeed
      (fun f -> f (mkVar id,ty)) 
...
let search_superauto n to_add argl g = 
  let sigma =
    List.fold_right
      (fun (id,c) -> add_named_decl (id, Expl, None, pf_type_of g c))
