#ifndef GLUE_C
#define GLUE_C
#include <gc_stack.h>
#include <stdio.h>
#include "glue.h"
struct closure;
struct closure {
  value (*func)(struct thread_info *, value, value);
  value env;
};

extern int is_ptr(value);
unsigned long long get_unboxed_ordinal(value);
unsigned long long get_boxed_ordinal(value);
value *get_args(value);
value make_Coq_Init_Datatypes_nat_O(void);
value make_Coq_Init_Datatypes_nat_S(value, value *);
value alloc_make_Coq_Init_Datatypes_nat_S(struct thread_info *, value);
value make_Coq_Init_Datatypes_bool_true(void);
value make_Coq_Init_Datatypes_bool_false(void);
value make_Coq_Strings_Ascii_ascii_Ascii(value, value, value, value, value, value, value, value, value *);
value alloc_make_Coq_Strings_Ascii_ascii_Ascii(struct thread_info *, value, value, value, value, value, value, value, value);
value make_Coq_Strings_String_string_EmptyString(void);
value make_Coq_Strings_String_string_String(value, value, value *);
value alloc_make_Coq_Strings_String_string_String(struct thread_info *, value, value);
unsigned long long get_Coq_Init_Datatypes_nat_tag(value);
unsigned long long get_Coq_Init_Datatypes_bool_tag(value);
unsigned long long get_Coq_Strings_Ascii_ascii_tag(value);
unsigned long long get_Coq_Strings_String_string_tag(value);
void print_Coq_Init_Datatypes_nat(value);
void print_Coq_Init_Datatypes_bool(value);
void print_Coq_Strings_Ascii_ascii(value);
void print_Coq_Strings_String_string(value);
value call(struct thread_info *, value, value);
signed char const lparen_lit[2] = { 40, 0, };

signed char const rparen_lit[2] = { 41, 0, };

signed char const space_lit[2] = { 32, 0, };

signed char const fun_lit[6] = { 60, 102, 117, 110, 62, 0, };

signed char const type_lit[7] = { 60, 116, 121, 112, 101, 62, 0, };

signed char const unk_lit[6] = { 60, 117, 110, 107, 62, 0, };

signed char const prop_lit[7] = { 60, 112, 114, 111, 112, 62, 0, };

unsigned long long get_unboxed_ordinal(value $v)
{
  return (unsigned long long) $v >> 1LL;
}

unsigned long long get_boxed_ordinal(value $v)
{
  return (unsigned long long) *((unsigned long long *) $v + -1LL) & 255LL;
}

value *get_args(value $v)
{
  return (value *) $v;
}

signed char const names_of_Coq_Init_Datatypes_nat[2][2] = { 79, 0, 83, 0,
  /* skip 0 */ };

signed char const names_of_Coq_Init_Datatypes_bool[2][6] = { 116, 114, 117,
  101, 0, 0, 102, 97, 108, 115, 101, 0, /* skip 0 */ };

signed char const names_of_Coq_Strings_Ascii_ascii[1][6] = { 65, 115, 99,
  105, 105, 0, /* skip 0 */ };

signed char const names_of_Coq_Strings_String_string[2][12] = { 69, 109, 112,
  116, 121, 83, 116, 114, 105, 110, 103, 0, 83, 116, 114, 105, 110, 103, 0,
  0, 0, 0, 0, 0, /* skip 0 */ };

value make_Coq_Init_Datatypes_nat_O(void)
{
  return (value) 1;
}

value make_Coq_Init_Datatypes_nat_S(value $arg0, value *$argv)
{
  *($argv + 0LL) = (value) 1024LL;
  *($argv + 1LL) = $arg0;
  return $argv + 1LL;
}

value alloc_make_Coq_Init_Datatypes_nat_S(struct thread_info *$tinfo, value $arg0)
{
  register value *$argv;
  $argv = (*$tinfo).alloc;
  *($argv + 0LL) = 1024LL;
  *($argv + 1LL) = $arg0;
  (*$tinfo).alloc = (*$tinfo).alloc + 2LL;
  return $argv + 1LL;
}

value make_Coq_Init_Datatypes_bool_true(void)
{
  return (value) 1;
}

value make_Coq_Init_Datatypes_bool_false(void)
{
  return (value) 3;
}

value make_Coq_Strings_Ascii_ascii_Ascii(value $arg0, value $arg1, value $arg2, value $arg3, value $arg4, value $arg5, value $arg6, value $arg7, value *$argv)
{
  *($argv + 0LL) = (value) 8192LL;
  *($argv + 1LL) = $arg0;
  *($argv + 2LL) = $arg1;
  *($argv + 3LL) = $arg2;
  *($argv + 4LL) = $arg3;
  *($argv + 5LL) = $arg4;
  *($argv + 6LL) = $arg5;
  *($argv + 7LL) = $arg6;
  *($argv + 8LL) = $arg7;
  return $argv + 1LL;
}

value alloc_make_Coq_Strings_Ascii_ascii_Ascii(struct thread_info *$tinfo, value $arg0, value $arg1, value $arg2, value $arg3, value $arg4, value $arg5, value $arg6, value $arg7)
{
  register value *$argv;
  $argv = (*$tinfo).alloc;
  *($argv + 0LL) = 8192LL;
  *($argv + 1LL) = $arg0;
  *($argv + 2LL) = $arg1;
  *($argv + 3LL) = $arg2;
  *($argv + 4LL) = $arg3;
  *($argv + 5LL) = $arg4;
  *($argv + 6LL) = $arg5;
  *($argv + 7LL) = $arg6;
  *($argv + 8LL) = $arg7;
  (*$tinfo).alloc = (*$tinfo).alloc + 9LL;
  return $argv + 1LL;
}

value make_Coq_Strings_String_string_EmptyString(void)
{
  return (value) 1;
}

value make_Coq_Strings_String_string_String(value $arg0, value $arg1, value *$argv)
{
  *($argv + 0LL) = (value) 2048LL;
  *($argv + 1LL) = $arg0;
  *($argv + 2LL) = $arg1;
  return $argv + 1LL;
}

value alloc_make_Coq_Strings_String_string_String(struct thread_info *$tinfo, value $arg0, value $arg1)
{
  register value *$argv;
  $argv = (*$tinfo).alloc;
  *($argv + 0LL) = 2048LL;
  *($argv + 1LL) = $arg0;
  *($argv + 2LL) = $arg1;
  (*$tinfo).alloc = (*$tinfo).alloc + 3LL;
  return $argv + 1LL;
}

unsigned long long get_Coq_Init_Datatypes_nat_tag(value $v)
{
  register _Bool $b;
  register unsigned long long $t;
  $b = is_ptr($v);
  if ($b) {
    $t = get_boxed_ordinal($v);
    switch ($t) {
      case 0:
        return 1;
      
    }
  } else {
    $t = get_unboxed_ordinal($v);
    switch ($t) {
      case 0:
        return 0;
      
    }
  }
}

unsigned long long get_Coq_Init_Datatypes_bool_tag(value $v)
{
  register unsigned long long $t;
  $t = get_unboxed_ordinal($v);
  return $t;
}

unsigned long long get_Coq_Strings_Ascii_ascii_tag(value $v)
{
  register unsigned long long $t;
  $t = get_boxed_ordinal($v);
  return $t;
}

unsigned long long get_Coq_Strings_String_string_tag(value $v)
{
  register _Bool $b;
  register unsigned long long $t;
  $b = is_ptr($v);
  if ($b) {
    $t = get_boxed_ordinal($v);
    switch ($t) {
      case 0:
        return 1;
      
    }
  } else {
    $t = get_unboxed_ordinal($v);
    switch ($t) {
      case 0:
        return 0;
      
    }
  }
}

void print_Coq_Init_Datatypes_nat(value $v)
{
  register unsigned int $tag;
  register void *$args;
  $tag = get_Coq_Init_Datatypes_nat_tag($v);
  switch ($tag) {
    case 0:
      printf(*(names_of_Coq_Init_Datatypes_nat + $tag));
      break;
    case 1:
      $args = get_args($v);
      printf(lparen_lit);
      printf(*(names_of_Coq_Init_Datatypes_nat + $tag));
      printf(space_lit);
      print_Coq_Init_Datatypes_nat(*((value *) $args + 0));
      printf(rparen_lit);
      break;
    
  }
}

void print_Coq_Init_Datatypes_bool(value $v)
{
  register unsigned int $tag;
  $tag = get_Coq_Init_Datatypes_bool_tag($v);
  printf(*(names_of_Coq_Init_Datatypes_bool + $tag));
}

void print_Coq_Strings_Ascii_ascii(value $v)
{
  register unsigned int $tag;
  register void *$args;
  $tag = get_Coq_Strings_Ascii_ascii_tag($v);
  switch ($tag) {
    case 0:
      $args = get_args($v);
      printf(lparen_lit);
      printf(*(names_of_Coq_Strings_Ascii_ascii + $tag));
      printf(space_lit);
      print_Coq_Init_Datatypes_bool(*((value *) $args + 0));
      printf(space_lit);
      print_Coq_Init_Datatypes_bool(*((value *) $args + 1));
      printf(space_lit);
      print_Coq_Init_Datatypes_bool(*((value *) $args + 2));
      printf(space_lit);
      print_Coq_Init_Datatypes_bool(*((value *) $args + 3));
      printf(space_lit);
      print_Coq_Init_Datatypes_bool(*((value *) $args + 4));
      printf(space_lit);
      print_Coq_Init_Datatypes_bool(*((value *) $args + 5));
      printf(space_lit);
      print_Coq_Init_Datatypes_bool(*((value *) $args + 6));
      printf(space_lit);
      print_Coq_Init_Datatypes_bool(*((value *) $args + 7));
      printf(rparen_lit);
      break;
    
  }
}

void print_Coq_Strings_String_string(value $v)
{
  register unsigned int $tag;
  register void *$args;
  $tag = get_Coq_Strings_String_string_tag($v);
  switch ($tag) {
    case 0:
      printf(*(names_of_Coq_Strings_String_string + $tag));
      break;
    case 1:
      $args = get_args($v);
      printf(lparen_lit);
      printf(*(names_of_Coq_Strings_String_string + $tag));
      printf(space_lit);
      print_Coq_Strings_Ascii_ascii(*((value *) $args + 0));
      printf(space_lit);
      print_Coq_Strings_String_string(*((value *) $args + 1));
      printf(rparen_lit);
      break;
    
  }
}

value call(struct thread_info *$tinfo, value $clo, value $arg)
{
  register value $f;
  register value $envi;
  register value $tmp;
  $f = (*((struct closure *) $clo)).func;
  $envi = (*((struct closure *) $clo)).env;
  $tmp =
    ((value (*)(struct thread_info *, value, value)) $f)
    ($tinfo, $envi, $arg);
  return $tmp;
}


#endif /* GLUE_C */
