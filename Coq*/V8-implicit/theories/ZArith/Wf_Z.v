...
Proof.
  intro x; destruct x; intros;
	...
	intros; simpl in |- *; specialize (nat_of_P_o_P_of_succ_nat_eq_succ x0);
	  intro Hx0; rewrite <- H0 in Hx0; apply f_equal with (f := Zpos);
	    apply nat_of_P_inj; auto with arith
      | exists 0%nat(*dummy*); absurd (0 <= Zneg p);
	[ unfold Zle in |- *; simpl in |- *; do 2 unfold not in |- *;
	  auto with arith
	  | assumption ] ].
Qed.
...
Proof.
  intros P H x H0.
  destruct (Z_of_nat_complete_inf x H0).
   replace x with (Z_of_nat x0); trivial.
   apply impl_PI; auto.
Qed.
...
Proof.
  intros P H x H0.
  replace x with
   (Z_of_nat (proj1_sig (Z_of_nat_complete_inf x H0))).
   auto.

   apply (fun H => sig_rect'
     (fun n => Z_of_nat (proj1_sig n) = x) 
     H (Z_of_nat_complete_inf x H0)).
   simpl; intros.
   apply impl_PI; auto.
Qed.
...
Lemma natlike_rec :
 forall [P:Z -> Set],
   ...
   (forall x:Z, [0 <= x] -> P x -> P (Zsucc x)) -> forall x:Z, [0 <= x] -> P x.
Proof.
  intros P H H0 x H1. apply Z_of_nat_set0;
    [ simple induction n;
      [ simpl in |- *; assumption
	| intros; rewrite (inj_S n0); exact (H0 (Z_of_nat n0) (Zle_0_nat n0) H2) ]
      | assumption ].
