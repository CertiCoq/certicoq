...
let build_lambda toabstract stk (m : constr) = 
  let rec buildrec m p_0 p_1 = match p_0,p_1 with 
	...
	if List.mem n toabstract then
          buildrec (mkLambda (na,imp,t,m)) (n+1) tl
        else 
...
let matches_core convert allow_partial_app pat c = 
  let rec sorec stk sigma p t =
    ...
    match p,kind_of_term cT with
      ...
      | PApp (PMeta (Some n),args1), App (c2,args2,imps2)
          when allow_partial_app ->
          let p = Array.length args2 - Array.length args1 in
          if p>=0 then
            let args21, args22 = array_chop p args2 in
            let imps21, _ = array_chop p imps2 in
            let sigma =
	      let depth = List.length stk in
              let c = mkApp(c2,args21,imps21) in
      ...
      | PApp (c1,arg1), App (c2,arg2,imps2) ->
        (try array_fold_left2 (sorec stk) (sorec stk sigma c1 c2) arg1 arg2
         with Invalid_argument _ -> raise PatternMatchingFailure)

      | PProd (na1,imp1,c1,d1), Prod(na2,imp2,c2,d2) when imp1=imp2 ->
	  sorec ((na2,imp2,c2)::stk) (sorec stk sigma c1 c2) d1 d2

      | PLambda (na1,imp1,c1,d1), Lambda(na2,imp2,c2,d2) when imp1=imp2->
	  sorec ((na2,imp2,c2)::stk) (sorec stk sigma c1 c2) d1 d2

      | PLetIn (na1,imp1,c1,d1), LetIn(na2,imp2,c2,t2,d2) when imp1=imp2 ->
	  sorec ((na2,imp2,t2)::stk) (sorec stk sigma c1 c2) d1 d2
	  ...
	  let ctx,b2 =
            decompose_lam_n_assum ci.ci_cstr_nargs.(0) b2 in
	  let ctx',b2' =
            decompose_lam_n_assum ci.ci_cstr_nargs.(1) b2' in
	  let n = List.length ctx and n' = List.length ctx' in
	  if noccur_between 1 n b2 & noccur_between 1 n' b2' then
	    let s =
              List.fold_left (fun l (na,imp,_,t) -> (na,imp,t)::l)
                stk ctx in
	    let s' =
              List.fold_left (fun l (na,imp,_,t) -> (na,imp,t)::l)
                stk ctx' in
...
let rec sub_match nocc pat c =
 ...
 | Lambda (x,imp,c1,c2) ->
    ...
    | PatternMatchingFailure ->
      ...
      (lm,mkLambda (x,imp,List.hd lc,List.nth lc 1))
    | NextOccurrence nocc ->
  ...
  | Prod (x,imp,c1,c2) ->
    ...
    | PatternMatchingFailure ->
      let (lm,lc) = try_sub_match nocc pat [c1;c2] in
      (lm,mkProd (x,imp,List.hd lc,List.nth lc 1))
    | NextOccurrence nocc ->
      let (lm,lc) = try_sub_match (nocc - 1) pat [c1;c2] in
      (lm,mkProd (x,imp,List.hd lc,List.nth lc 1)))
  | LetIn (x,imp,c1,t2,c2) ->
    (try authorized_occ nocc ((matches pat c), mkMeta special_meta) with
    | PatternMatchingFailure ->
      let (lm,lc) = try_sub_match nocc pat [c1;t2;c2] in
      (lm,mkLetIn (x,imp,List.hd lc,List.nth lc 1,List.nth lc 2))
    | NextOccurrence nocc ->
  ...
  | App (c1,lc,imps) ->
    ...
    | PatternMatchingFailure ->
      ...
      (lm,mkApp (List.hd le, Array.of_list (List.tl le),imps))
    | NextOccurrence nocc ->
