...
Section CONTRACT.
  ...
  Proof.
    ...
    - simpl in H5.
      ...
      +  assert (H1v := H1 v).
         ...
         { replace 0 with (0 + 0) by omega. econstructor.
           constructor.
           simpl.
           rewrite rename_all_ns_fundefs_append.
           simpl.
         ...
         unfold ce'.
         split.
         replace 0 with (0 + 0) by omega. eapply gsr_clos_trans.
         * apply H6.
         * auto.
         * split; auto.
         * intro.
  ...
  Proof.
    ...
    - destruct a.
      ...
      2:{ unfold term_sub_inl_size. simpl. omega. }
    ...
    - simpl in Hc. 
      ...
      + destruct (get_c v count).
        ...
        * destruct (contract sig count e sub im) as [[[[? ?] ?] ?] ?]. 
          ...
          destruct s as [[[[? ?] ?] ?] ?]. simpl in *. inv Hc. symmetry in Heqs. eapply IHfds_c in Heqs. omega.
    - simpl in Hc. inv Hc. omega.
  Qed. 
  ...
  Proof.
    ...
    { (* Fcons *)
      ...
      - (* get_b v im = true *)
        ...
        etransitivity. apply H7. simpl. omega.        
    - (* get_b v im = false *)
      destruct (get_c v count) eqn:gvc.
      + (* get_c v count = 0 i.e. dead fun *)
        ...
        { (* by Fun_rem_s *)
          ...
          replace 0 with (0 + 0) by omega. eapply Trans_srw; [| now constructor ]. 
        ...
        * destructAll.
          ...
          replace (steps' - steps) with (0 + (steps' - steps)) by omega.
      ...
      + (* get_c v count = (S n) *)
        ...
        unfold ce'.
        split; auto.
        * replace (n0 + n1 - steps) with (n0 + (n1 - steps)) by omega. eapply gsr_clos_trans. eassumption.
...
(** dec_census_all_case removes all the occurences of variables in all branches from their respective tally in count *)
  ...
  Proof.
    intros. unfold apply_r.
    destruct (@M.get M.elt z sig) eqn:Heqz.
    + rewrite M.gso with (i := z). rewrite M.gempty. rewrite Heqz.
  ...
  Proof.
    ...
    assert (n > term_sub_inl_size (e, sub, inl)) by omega. clear Heqn. 
    ...
    - (* constr *)
      ...
      + (* dead constr *)
        ...
        { unfold ce. replace 1 with (1 + 0) by omega. eapply Trans_srw; [| now constructor ]. 
          constructor. constructor.
        ...
        * { destruct Heqs. destruct H10. destruct H11 as (H11, Hsig).
            ...
            - (* dead post *) inv H6. split; [| split; [| split ]].
              + eapply gsr_clos_trans. eassumption.
                rewrite <- H12. unfold ce'. simpl. replace 1 with (1 + 0) by omega. eapply Trans_srw; [| eapply Refl_srw ]. 
                constructor. constructor.
      ...
      + destruct (findtag l c0) eqn:ftlc0.
        * (* folded case *)
           ...
           { apply H3 in garvs. destructAll. unfold ce. repeat normalize_ctx. do 2 (rewrite <- app_ctx_f_fuse).
             replace 1 with (1 + 0) by omega. econstructor; [| eapply Refl_srw ]. constructor. simpl.
        ...
        * (* case not found, same as fun. contractcases_corresp with cl1 = nil and x = apply_r sig x*)
           ...
           2:{ intros. eapply IHn; eauto. unfold term_sub_inl_size in *. simpl in H7. simpl in *. omega. }
    ...
    - (* proj *)
      ...
      + (* dead v pre *)
        simpl in H6.
        assert (gsr_clos 1 ce (rename_all_ctx_ns sig (inlined_ctx_f c inl) |[ rename_all_ns sig e ]|)).
        { replace 1 with (1 + 0) by omega. eapply Trans_srw; [| now constructor ]. constructor.
      ...
      + { simpl in H6.
          ...
          { clear H6. intros H6.
            ...
            { (* dead v post *)
              ...
              - eapply gsr_clos_trans. eassumption. 
                ...
                replace 1 with (1 + 0) by omega. econstructor; [| now constructor ].
          ...
          - intro. specialize (H9 v1). apply num_occur_app_ctx in H9. destructAll.
                ...
                rewrite apply_r_list_empty. unfold get_c. rewrite M.gempty. simpl. omega.
              ...
              -  (* TODO: redo using rename_all_set_x *)
                ...
                { apply num_occur_app_ctx.
                  ...
                  - assert (num_occur_list [apply_r sig v0] (apply_r sig v1) = 0).
                    ...
                    unfold var in *. rewrite H6 in *. rewrite H11. omega. }
      ...
      + destruct (sub ! (apply_r sig v0)) as [[|] | ] eqn:Hsub. 
        ...
        * (* f in Vfun *)
          ...
          { (* function inlining *)
            ...
            assert (Hnin : ~ v \in FromList l0 :|: FromList l). { admit. }
            assert (Hdis : Disjoint positive (FromList l0) (FromList l)) by admit.  

            destruct (inline_letapp e0 v) as [[Cinl x'] | ] eqn:Hinl. 
            - assert (Hinl' := Hinl). eapply inline_letapp_rename with (sig := set_list (combine l0 l) (M.empty _)) in Hinl'. 
              2:{ rewrite apply_r_set_list2. rewrite apply_r_empty. reflexivity. intros Hc. eauto. }
              
              assert (Hdead1 : FromList l0 \subset (dead_var (rename_all_ns sig e))).
              { admit. (* params of function do not appear free in vars of e *) }
              ...
              remember (contract
                          (M.set v (apply_r (set_list (combine l0 (apply_r_list sig l)) sig) x')
                                 (set_list (combine l0 (apply_r_list sig l)) sig))
                          (update_count_inlined
                             (apply_r
                                (set_list
                                   (combine l0 (apply_r_list sig l)) sig)
                                x' :: apply_r_list sig l) 
                             (v :: l0) (M.set (apply_r sig v0) 0 count))
                          (Cinl |[ e ]|) sub (M.set (apply_r sig v0) true inl)) as s.
              destruct s as [[[[? ?] ?] ?] ?]. symmetry in Heqs.
              eapply IHn with (c := c) in Heqs.
              { destructAll. inv H6.
                assert (Heq : rename_all_ns
                                (M.set v (apply_r (set_list (combine l0 (apply_r_list sig l)) sig) x')
                                       (set_list (combine l0 (apply_r_list sig l)) sig))
                                (Cinl |[ e ]|) =
                ...
                { (* erewrite <- (proj1 (one_rename_all_ns_mut _ _ _ Hnin2 Hnin1)). *)
                  ...
                  - rewrite rename_all_ctx_ns_Disjoint. reflexivity.
                    ...
                    eapply Singleton_Included. admit. (* dead var rename? *) 
                  ...
                  - admit. (* from sig_inv *)
                  - admit. (* messy but ok *)
                  - rewrite Dom_map_set_list. eapply Union_Disjoint_r. repeat normalize_sets. eapply Disjoint_Singleton_l.
                    now eauto. admit. (* from sig_inv *) rewrite length_apply_r_list. eauto. } 
                    
                    , Dom_map_empty. 
                    eapply Included_Union_preserv_r. 
                    rewrite <- (proj1 (set_list_rename_all_ns _ _)).
                    rewrite rename_all_ns_join. eapply eq_P_rename_all_ctx_ns. eapply eq_env_P_set_not_in_P_l. eapply eq_env_P_refl.
                    reflexivity.
                     f_equal. erewrite (proj1 (rename_all_ns_app_ctx _ _)).                
                  rewrite rename_all_ctx_ns_Disjoint with (m := M.set _ _ _ ).
                  erewrite <- (proj1 (rename_all_ns_push _ _ _)).
                  rewrite rename_all_ns_Disjoint with (e := e). reflexivity. 
                  - rewrite Dom_map_set_list, Dom_map_empty. normalize_sets. sets.
                    eapply Disjoint_sym. eapply Complement_Disjoint. eassumption. eauto.
                  - intros Hc. eapply Range_map_set_list in Hc. eauto.
                  - rewrite Dom_map_set_list, Dom_map_empty. normalize_sets. eauto. eauto.
                  - rewrite Dom_map_set, Dom_map_empty. normalize_sets.
                    eapply Disjoint_sym. eapply Complement_Disjoint. eapply Singleton_Included. eassumption.
                  - eassumption. }
                rewrite Heq in H7.
                ...
                assert (Hctx_rw :
                          rename_all_ctx_ns
                            sig
                            (inlined_ctx_f (comp_ctx_f x (Efun1_c (fundefs_append x1 (Fcons (apply_r sig v0) f l0 e0 x2))  x0)) inl)
                       |[ Eletapp v (apply_r sig v0) f (apply_r_list sig l) (rename_all_ns sig e) ]| = 
                                                                                                     rename_all_ctx_ns sig (inlined_ctx_f x inl) |[
                         Efun (fundefs_append (rename_all_fun_ns sig (inlined_fundefs_f x1 inl)) (Fcons (apply_r sig v0) f l0 (rename_all_ns sig e0) (rename_all_fun_ns sig (inlined_fundefs_f x2 inl))))
                              (rename_all_ctx_ns sig (inlined_ctx_f x0 inl) |[ Eletapp v (apply_r sig v0) f (apply_r_list sig l) (rename_all_ns sig e) ]|) ]|).
                { ctx_inl_push. rewrite inlined_fundefs_append.
                  rewrite rename_all_ns_fundefs_append. simpl. rewrite Hel4. reflexivity. }
                assert (H_inl_ctx: (rename_all_ctx_ns (set_list (combine l0 (apply_r_list sig l)) sig)
                                    ...
                                    (rename_all_ctx_ns (set_list (combine l0 (apply_r_list sig l)) sig)
                                                       (inlined_ctx_f (comp_ctx_f x (Efun1_c (fundefs_append x1 (Fcons (apply_r sig v0) f l0 e0 x2)) x0))
                                                                      (M.set (apply_r sig v0) true inl))))).
                { repeat normalize_ctx. simpl. do 2 (rewrite inlined_fundefs_append). simpl. unfold get_b. rewrite M.gss. reflexivity. }
                ...
                assert (Heqp : eq_env_P
                                 ...
                                 (M.set v (apply_r (set_list (combine l0 (apply_r_list sig l)) sig) x') (set_list (combine l0 (apply_r_list sig l)) sig))
                                 sig).
                { eapply eq_env_P_set_not_in_P_l. eapply eq_env_P_antimon. eapply eq_P_set_list_not_in. 
                  eapply Complement_antimon.
                  repeat normalize_ctx. simpl. do 1 (rewrite inlined_fundefs_append). simpl.
                  assert (Hb : get_b (apply_r sig v0) im' = true). { eapply b_map_le_c in b0. eapply b0. unfold get_b. now rewrite M.gss. }
                  rewrite Hb. 
                    in Hel4.
                  runfold get_b. rewrite M.gss. reflexivity.
                erewrite !(proj1 eq_P_rename_all_ctx_ns) with (sub := M.set v (apply_r (set_list (combine l0 (apply_r_list sig l)) sig) x')
                                                                           (set_list (combine l0 (apply_r_list sig l)) sig)) (sub' := sig) in *.
                erewrite (proj1 eq_P_rename_all_ctx_ns) with (sub := M.set v (apply_r (set_list (combine l0 (apply_r_list sig l)) sig) x')
                                                                           (set_list (combine l0 (apply_r_list sig l)) sig)) (sub' := sig) in H7.
                erewrite (proj1 eq_P_rename_all_ctx_ns) with (sub' := sig) in H7.
                split; [| split; [| split ]].
                + rewrite plus_comm. eapply gsr_clos_trans. ; [| eassumption ].
                  ...
                  assert (Hclos := gs
                                     (* (* first IH *) eapply IHn in Heqs; eauto. (* second IH *) eapply IHn in Heqs'; eauto. *)
                                     (* + destructAll. split; [| split; [| split ]]. *)
                                     (*   (* Show goal *) *)
                                     (*   * unfold ce, ce'.  *)
            ...
            (*     inv H6. admit. *)
              admit. 
            - (* failed to inline. Shouldn't technically happpen but handled like other cases. Same. *) admit. }
          admit.
        * (* f is None *) admit. (* same *)
      + destruct (get_c (apply_r sig v0) count) as [| [|] ]; try congruence.        
        admit. admit. (* same *)
    - (* fun *)
      ...
      + assert (Hube0 := gsr_clos_preserves_clos _ _ _ H11 H12 H14). destructAll.
        ...
        2:{ intros. eapply IHn; eauto.
            ...
            { unfold term_sub_inl_size; simpl. assert (sub_inl_size sub b0 <= sub_inl_size sub inl).
              ...
              omega. auto. }
            assert ( S n >  funs_size f0 + sub_inl_size sub b0) by omega.
        ...
        * destruct Heqs as (Heqs1, (Heqs2, (Heqs3, (Heqs4, (Heqs5, Heqs6))))).
          ...
          destruct f1.
          -- inv H6. split; auto. unfold ce'. unfold ce. replace (n0 + n1) with (0 + (n0 + n1)) by omega.
             eapply gsr_clos_trans. eassumption. eapply gsr_clos_trans. eassumption.
             replace n1 with (n1 - 0) by omega. eassumption . 
             split. now auto. split. now auto.
             (* sig_inv_codom *) 
          ...
          -- (* additional RW with Fun_dead_s on Fnil *)
             ...
             ++ rewrite <- minus_n_O in Heqs1. replace (n0 + n1 + 1) with (0 + ((n0 + n1) + 1)) by omega. eapply gsr_clos_trans. eassumption.
                eapply gsr_clos_trans. eapply gsr_clos_trans. eassumption. eassumption.
                replace 1 with (1 + 0) by omega. eapply Trans_srw;[| now eapply Refl_srw  ]. constructor. constructor. 
      ...
      + assert ((forall im, sub_inl_size c0 im <= funs_size f + sub_inl_size sub im /\ funs_size f0 <= funs_size f))%nat.
        ...
        unfold term_sub_inl_size. simpl. unfold term_sub_inl_size in H; simpl in H. specialize (H14 inl). omega.
        ...
        * (* eligible to be inlined? *)
          ...
          { (* function inlining *)
            ...
            { assert (H9 := true).             
              ...
              clear H8.
              apply num_occur_ec_comp_ctx. exists 0, 0; auto. split; [| split; [| omega ]].              
              - (* x *)
              ...
              reflexivity. now apply H13. now apply H14. omega. }
                ...
                gsr_clos 1
                  ...
                  (rename_all_ctx_ns (set_list (combine l0 (apply_r_list sig l)) sig)
                                     (inlined_ctx_f (comp_ctx_f x (Efun1_c (fundefs_append x1 (Fcons (apply_r sig v) f l0 e x2)) x0))
                                                    (M.set (apply_r sig v) true inl))
                  |[ rename_all_ns (set_list (combine l0 (apply_r_list sig l)) sig) e ]|)).
            { replace 1 with (1 + 0) by omega. eapply gsr_clos_trans.
              replace 1 with (1 + 0) by omega. econstructor; [| now constructor ]. constructor. 
              apply Fun_inline_s.
              - rewrite length_apply_r_list. auto.
            ...
            - (* term size for IH *)
              unfold term_sub_inl_size in *; simpl in *.
              apply sub_inl_fun_size with (im := inl) in garvs'. omega.
                ...
                * apply H5co in H9.
                  ...
                  -- left.
                     ...
                     ++ apply num_occur_ec_comp_ctx. exists 0, 0; auto. split; auto. split; auto.
                        ...
                        repeat normalize_ctx.
                        eapply fundefs_append_num_occur; eauto. omega. 
                     ++ split; auto.
    ...
    - (* prim *)
      ...
      { (* dead prim post IH *)
        ...
        assert (gsr_clos
                  (rename_all_ctx_ns sig
                                     (inlined_ctx_f (comp_fctx_f c (Eprim_c v p l Hole_c)) im') |[ e'  ]|) ce').
        { unfold ce'.
          repeat normalize_ctx.
          rewrite <- app_ctx_f_fuse.
