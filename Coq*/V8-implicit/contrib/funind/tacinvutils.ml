...
let mkEq typ c1 c2 = 
  mkApp (build_coq_eq(),[| typ; c1; c2|],[|Expl;Expl;Expl|])

let mkRefl typ c1 = 
  mkApp ((build_coq_eq_data()).refl, [| typ; c1|],[|Impl;Impl|])
...
let constr_head_match u t=
  if isApp u 
  then 
	 let uhd,args,_= destApp u in
    uhd=t
  else false

(* My operations on constr *)
let lift1L l = (List.map (lift 1) l)
let mkArrow_lift imp t1 t2 = mkArrow imp t1 (lift 1 t2)
let mkProd_liftc nme imp c1 c2 = mkProd (nme,imp,c1,(lift 1 c2))
(* prod_it_lift x [a1 a2 ...] *)
let prod_it_lift ini lcpl =
  List.fold_right (function (a,imp,b) -> (fun c -> mkProd_liftc a imp b c)) ini lcpl;;

let prod_it_anonym_lift trm lst imps =
  List.fold_right2 mkArrow_lift imps lst trm

let lam_it_anonymous trm lst imps =
  List.fold_right2 
    (fun elt imp res -> mkLambda(Name(id_of_string "Hrec"),imp,elt,res))
    lst imps trm

let lambda_id id typeofid cstr =
  let cstr' = mkNamedLambda (id_of_string "FUNX") Expl typeofid cstr in
  substitterm 0 id (mkRel 0) cstr'

let prod_id id typeofid cstr =
  let cstr' = mkNamedProd (id_of_string "FUNX") Expl typeofid cstr in
  substitterm 0 id (mkRel 0) cstr'
...
let nth_dep_constructor indtype n =
  ...
  let indfam,_,_ = dest_ind_type indtypedef in
...
let rec hdMatchSub u t=
  let subres = 
    match kind_of_term u with
      | Lambda (nm,imp,tp,cstr) | Prod (nm,imp,tp,cstr) ->
                                    hdMatchSub (lift 1 cstr) t
      | Fix (_,(lna,tl,bl))  -> 
	  Array.fold_left 
...
let exchange_hd_prod subst_hd t =
  let hd,args,imps= destApplication t in mkApp (subst_hd,args,imps)
...
let exchange_reli_arrayi tarr (d,f) t =
  let hd,args,imps= destApp t in 
  let i = destRel hd in
  let res = whd_beta (mkApp (tarr.(f-i) ,args,imps)) in
  res
