...
(* $Id: recdef.ml4 9991 2007-07-13 10:36:24Z jforest $ *)
...
let rec (find_call_occs:
	   int -> constr -> constr ->
             (constr list ->constr)*(constr list list)*impl list list) =
 fun nb_lam f expr ->
  match (kind_of_term expr) with
    App (g, args,imps) when g = f -> 
      (fun l -> List.hd l), [Array.to_list args], [Array.to_list imps]
  | App (g, args, imps) ->
	 ...
	 []    -> (fun x -> []), [], []
       | a::upper_tl ->
         (match find_aux upper_tl with
          (cf, ((arg1::args) as args_for_upper_tl), impstl) -> 
           (match find_call_occs nb_lam f a with
             cf2, (_ :: _ as other_args), oimps ->
               let rec avoid_duplicates args imps =
                  match args,imps with
                  | [],[] -> (fun _ -> []), [],[]
                  | h::tl,imp::imps -> 
                      let recomb_tl, args_for_tl,impstl =
                          avoid_duplicates tl imps in
                    (match rank_for_arg_list h args_for_upper_tl with
                    | None -> 
                        (fun l -> List.hd l::recomb_tl(List.tl l)),
                        h::args_for_tl,imp::impstl
                    | Some i ->
                  ...
                  | _ -> assert false
               in
               let recombine, other_args', oimps' =
                   avoid_duplicates other_args oimps in
	       let len1 = List.length other_args' in                   
                   (fun l -> cf2 (recombine l)::cf(nthtl(l,len1))),
                     other_args'@args_for_upper_tl, oimps'@impstl

           | _, [], [] -> (fun x -> a::cf x), args_for_upper_tl, impstl
           | _ -> assert false)
	 | _, [], [] ->
	   (match find_call_occs nb_lam f a with
	     cf, (arg1::args),imps ->
               (fun l -> cf l::upper_tl), (arg1::args), imps
	   | _, [], [] -> (fun x -> a::upper_tl), [], []
           | _ -> assert false)
         | _ -> assert false) in
     begin
       match (find_aux largs) with
	   cf, [],[] -> (fun l -> mkApp(g, args,imps)), [], []
	 | cf, args, imps' ->
	     (fun l -> mkApp (g, Array.of_list (cf l),imps)), args, imps'
     end
  ...
  | Prod(_,_,_,_) -> error "find_call_occs : Prod"
  | Lambda(na,imp,t,b) -> 
      begin
  ...
  | LetIn(na,imp,v,t,b) -> 
	...
	match find_call_occs nb_lam f v, find_call_occs (succ nb_lam) f b with 
	  | (_,[],[]),(_,[],[]) -> 
	      ((fun l -> expr), [],[])
	  | (_,[],[]),(cf,(_::_ as l),limps) -> 
	      ((fun  l -> mkLetIn(na,imp,v,t,cf l)),l,limps)
	  | (cf,(_::_ as l),limps),(_,[],[]) -> 
	      ((fun l -> mkLetIn(na,imp,cf l,t,b)), l,limps)
  ...
  | Case(i,t,a,r) ->
      (match find_call_occs nb_lam f a with
	cf, (arg1::args),imps ->
          (fun l -> mkCase(i, t, (cf l), r)),(arg1::args), imps
      | _ -> (fun l -> expr),[],[])
...
let  mkCaseEq a  : tactic =
	...
	(* commentaire de Yves: on pourra avoir des problemes si
       ...
       (tclTHEN (h_generalize [Expl]
            [mkApp(delayed_force refl_equal,[|type_of_a; a|],[|Impl;Impl|])])
	  (tclTHEN 
	     (fun g2 ->
...
(* 	in *)
	let to_revert = 
	  Util.map_succeed 
	    ...
	    hyps
	in
	let to_revert_constr = List.rev_map (fun (id,_) -> mkVar id) to_revert in 
	let to_rev_imps = List.rev_map snd to_revert in
  
	(* commentaire de Yves: on pourra avoir des problemes si
	   a n'est pas bien type dans l'environnement du but *)
       let type_of_a = pf_type_of g expr in
       (tclTHEN (h_generalize (Expl::to_rev_imps)
             (mkApp(delayed_force refl_equal, [| type_of_a; expr|],[|Impl;Impl|])::to_revert_constr))
	  (tclTHEN 
	     (fun g2 ->
		change_in_concl None 
		  (pattern_occs [([-1], expr)] (pf_env g2) Evd.empty (pf_concl g2))
		  g2)
	     (simplest_case expr))), List.map fst to_revert
  ...
  let finalize () =  	if extra_eqn then
	  let teq = pf_get_new_id teq_id g in
		...
		(fun g1 -> 
		   ...
		   let teq_lhs,teq_rhs = 
		     let _,args,_ = try destApp ty_teq with _ -> Pp.msgnl (Printer.pr_goal (sig_it g1) ++ fnl () ++ pr_id teq ++ str ":" ++ Printer.pr_lconstr ty_teq); assert false in 
		     args.(1),args.(2) 
		   in
  ...
  | a::tl ->
	...
	(tclTHENS
	   (fun g -> 
	      ...
	      let m_id = 
		let _,_,_,t = destProd t_le_trans in 
		let na,_,_,_ = destProd t in 
		Nameops.out_name na
	      in
	      apply_with_bindings
		(le_trans,
...
let make_lt_proof pmax le_proof =
  tclTHENS
    (fun g -> 
       ...
       let m_id = 
	 let _,_,_,t = destProd t_le_lt_trans in 
	 let na,_,_,_ = destProd t in 
	 Nameops.out_name na
       in
       apply_with_bindings
...
let rec list_cond_rewrite k def pmax cond_eqs le_proofs =
  ...
  | eq::eqs ->
	 ...
	 let k_id,def_id = 
	   let k_na,_,_,t = destProd t_eq in 
	   let _,_,_,t  = destProd t in 
	   let def_na,_,_,_ = destProd t in 
...
let rec introduce_all_equalities func eqs values specs bound le_proofs 
  ...
  match specs with
    [] -> 
      fun g ->
	let ids = pf_ids_of_hyps g in
	let s_max = mkApp(delayed_force coq_S, [|bound|],[|Expl|]) in
	   ...
	   tclTHENS
	     ...
	     [
	       tclTHENLIST[h_intro h';
			   simplest_elim(mkApp(delayed_force lt_n_O,[|s_max|],[|Expl|]));
			   default_full_auto];
	       tclIDTAC
	     ];
  ...
  | spec1::specs ->
	...
	tclTHENLIST
	  [simplest_elim (mkVar spec1);
	   list_rewrite true eqs;
	   h_intros [p; heq];
	   simplest_elim (mkApp(delayed_force max_constr,
                                [| bound; mkVar p|],[|Expl;Expl|]));
...
let rec introduce_all_values is_mes acc_inv func context_fn
    eqs  hrec args imps values specs =
    (match args, imps with
    [],[] -> 
      tclTHENLIST
...
let rec_leaf_terminate is_mes acc_inv hrec (func:global_reference) eqs expr =
  ...
  | context_fn, args, imps ->
      observe_tac "introduce_all_values" 
	(introduce_all_values is_mes acc_inv func context_fn eqs  hrec
          args imps  [] [])
(*
	...
	match (kind_of_term expr) with
	    Case (ci, t, a, l) -> 
		 ...
		 | _, _::_,_ -> 
		     (
		       match find_call_occs  0 f_constr expr with
	     		   _,[],_ -> observe_tac "base_leaf" (base_leaf func eqs expr)
			 | _, _:: _,_ -> 
			     observe_tac "rec_leaf" 
			       (rec_leaf is_mes acc_inv hrec  func eqs expr)
		     )
	  ...
	  Array.of_list 
	    (lift 5 a_arrow_b:: mkRel 3::
	       ...
	       List.rev (list_map_i (fun i _ -> mkRel (6+i)) 0 rev_args)
            ...
            Array.of_list
              (Impl::Expl::Expl::
               List.rev(List.map(fun (_,imp,_)->imp) rev_args))
	 )
  in
  let right = mkRel 5 in 
  let equality = mkApp(delayed_force eq,
                       [|lift 5 b; left; right|],[|Expl;Expl;Expl|]) in
  let result = (mkProd ((Name def_id) , Expl, lift 4 a_arrow_b, equality)) in
  let cond = mkApp(delayed_force lt, [|(mkRel 2); (mkRel 1)|],[|Expl;Expl|]) in
  let nb_iter =
    mkApp(delayed_force ex,
	  [|delayed_force nat;
	    (mkLambda 
	       (Name p_id, Expl,
		  delayed_force nat, 
		  (mkProd (Name k_id, Expl, delayed_force nat, 
			   mkArrow Expl cond result))))|],
          [|Expl;Expl|])in
  let value = mkApp(delayed_force coq_sig, 
		    [|b; (mkLambda (Name v_id, Expl, b, nb_iter))|],
                    [|Expl;Expl|]) in
	...
	(wf_rec_arg::wf_thm::ids) in 
	  ...
	  lazy (
	    mkApp (
	      ...
	      [|input_type;relation;mkVar rec_arg_id|],[|Impl;Impl;Impl|]
	    )
	  )
      in
      tclTHEN
	   ...
	   [
	     ...
	     tclTHENS 
		  ...
		  (assert_tac 
		     true 
		     (Name wf_thm) Expl
		     (mkApp (delayed_force well_founded,
                     [|input_type;relation|],[|Expl;Expl|]))
		  )
	       )
	       [ 
		 ...
		 observe_tac 
		   "apply wf_thm" 
		   (h_simplest_apply 
                     (mkApp(mkVar wf_thm,[|mkVar rec_arg_id|],[|Expl|]))
		   )
	       ]
	     ;
	     (* rest of the proof *)
	     tclTHENSEQ 
		  ...
		  (onNLastHyps (nargs+1)
		     (fun (id,_,_,_) -> 
		       tclTHEN (h_generalize [Expl][mkVar id])
                         (h_clear false [id])
		     ))
	       ;
...
let rec instantiate_lambda t l = 
  ...
  | [] -> t
  | a::l -> 
      let (bound_name, _,_, body) = destLambda t in
...
let whole_start nb_args is_mes func input_type relation rec_arg_num  : tactic = 
  begin 
    fun g -> 
      ...
      let (f_name, _,_, body1) = destLambda func_body in
      let f_id =
	match f_name with
...
let build_and_l l = 
  ...
  let mk_and p1 p2 = 
    Term.mkApp(and_constr,[|p1;p2|],[|Expl;Expl|]) in 
  let rec f  = function 
...
let clear_goals = 
  let rec clear_goal t = 
    match kind_of_term t with 
      | Prod(Name id as na,imp,t,b) -> 
	  let b' = clear_goal b in 
	  if noccurn 1 b' && (is_rec_res id) 
	  then pop b' 
	  else if b' == b then t 
	  else mkProd(na,imp,t,b')
...
let (value_f:constr list -> global_reference -> constr) =
  fun al fterm ->
    ...
    let fun_body = 
      RCases
	(d0,None,
	 [RApp(d0, RRef(d0,fterm),
               List.rev_map (fun x_id -> RVar(d0, x_id)) rev_x_id_l,
               List.map(fun _ -> Expl) rev_x_id_l),
	  (Anonymous,None)],
	 [d0, [v_id], [PatCstr(d0,(ind_of_ref 
				     (delayed_force coq_sig_ref),1),
			       [PatVar(d0, Name v_id,Expl);
				PatVar(d0, Anonymous,Expl)],
			       Anonymous)],
	  RVar(d0,v_id)])
    in
    let value =
	...
	(fun acc x_id a -> 
	   RLambda
      	     (d0, Name x_id, Expl, RDynamic(d0, constr_in a),
	      acc
	     ) 
	)
	fun_body
...
let start_equation (f:global_reference) (term_f:global_reference) 
  ...
  let args,_ = decompose_prod (type_of_const terminate_constr) in 
  let imps = List.rev(List.map (fun (_,imp,_)->imp) args) in
  let x = 
    let rec f ids = function
        [] -> []
      | _::rargs ->
...
let base_leaf_eq func eqs f_id g =
  ...
  let hex = next_global_ident_away true hex_id (heq1::heq::v::p::k::ids) in
	...
	(rewriteRL 
	   (mkApp(mkVar heq1, 
		  [|mkApp (delayed_force coq_S, [|mkVar p|],[|Expl|]);
		    mkApp(delayed_force lt_n_Sn, [|mkVar p|],[|Expl|]);
                    f_id|],[|Expl;Expl;Expl|])));
      simpl_iter();
...
let f_S t = mkApp(delayed_force coq_S, [|t|],[|Expl|]);;


let rec introduce_all_values_eq  cont_tac functional termine 
    f p heq1 pmax bounds le_proofs eqs ids args imps =
  match args,imps with
      [],[] ->
	tclTHENLIST
	  [tclTHENS
	...
	let heq2 = next_global_ident_away true heq_id ids in
        let ids = heq2::ids in
	tclTHENLIST
	  [mkCaseEq(mkApp(termine, Array.of_list arg,Array.of_list imp));
	   h_intros [v'; hex'];
	   simplest_elim(mkVar hex');
	   h_intros [p'];
	   simplest_elim(mkApp(delayed_force max_constr,
                         [|mkVar pmax; mkVar p'|],[|Expl;Expl|]));
	   h_intros [new_pmax;hle1;hle2];
		...
		tclTHENLIST
	     ...
	     (p'::bounds)((mkVar pmax)::le_proofs) eqs
             (heq2::heq::hle2::hle1::new_pmax::p'::hex'::v'::ids) args imps]
    | _ -> assert false
  

let rec_leaf_eq termine f ids functional eqs expr fn args imps =
  let p = next_global_ident_away true p_id ids in
  let ids = p::ids in
  let v = next_global_ident_away true v_id ids in
  let ids = v::ids in
  ...
  let ids = hle1::ids in
    tclTHENLIST
      [h_intros [v;hex];
       simplest_elim (mkVar hex);
       h_intros [p;heq1];
       h_generalize [Expl] [mkApp(delayed_force le_n,[|mkVar p|],[|Impl|])];
       h_intros [hle1];
       observe_tac "introduce_all_values_eq" (introduce_all_values_eq
	 (fun _ _ -> tclIDTAC)
	 functional termine f p heq1 p [] [] eqs ids args imps);
  ...
  (match kind_of_term expr with
	...
	(match find_call_occs 0 f a with
	   ...
	   | _,_::_,_ ->
	     ...
	     _,[],_ -> base_leaf_eq functional eqs f
	   | fn,args,imps ->
	       fun g ->
		 let ids = ids_of_named_context (pf_hyps g) in
	       rec_leaf_eq termine f ids
		 (constr_of_reference functional)
		 eqs expr fn args imps g))
       | _ -> 
	   (match find_call_occs 0 f expr with
		_,[],_ -> base_leaf_eq functional eqs f
	      | fn,args,imps ->
...
let recursive_definition is_mes function_name rec_impls type_of_f r rec_arg_num eq 
  ...
  let env =
    push_named (function_name,Expl,None,function_type) (Global.env()) in
...
(*   Pp.msgnl (Printer.pr_lconstr equation_lemma_type); *)
  ...
  let env_eq' = Environ.push_rel_context (List.map (fun (x,imp,y) -> (x,imp,None,y)) res_vars) env in
...
(*     Pp.msgnl (str "eq' := " ++ str (string_of_int rec_arg_num)); *)
    match kind_of_term eq' with 
      | App(e,[|_;_;eq_fix|],_) -> 
	  mkLambda (Name function_name,Expl,function_type,subst_var function_name (compose_lam res_vars  eq_fix))
      | _ -> failwith "Recursive Definition (res not eq)"
  in
  let pre_rec_args,function_type_before_rec_arg =
    decompose_prod_n (rec_arg_num - 1) function_type in 
  let (_, _, rec_arg_type, _) = destProd function_type_before_rec_arg in
  let arg_types = List.rev_map (fun(_,_,ty)->ty)
    (fst (decompose_prod_n (List.length res_vars) function_type)) in
  ...
  let env_with_pre_rec_args =
    push_rel_context(List.map (function (x,imp,t) -> (x,imp,None,t)) pre_rec_args) env in 
  let relation = 
    interp_constr
      Evd.empty 
