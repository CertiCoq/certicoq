...
# coq_makefile -R ../common Common -R ../L1_MalechaQuoted L1 -R ../L2_typeStrippedL1 L2 -R . L3 L3.v unaryApplications.v wNorm.v wcbvEval.v wndEval.v program.v term.v -o Makefile.coq 
#
...
COQLIBS?=\
  -R "../common" Common\
  -R "../L1_MalechaQuoted" L1\
  -R "../L2_typeStrippedL1" L2\
  -R "." L3
COQDOCLIBS?=\
  -R "../common" Common\
  -R "../L1_MalechaQuoted" L1\
...
install:
	cd "." && for i in $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/L3/$$i`"; \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/L3/$$i; \
	done
...
uninstall_me.sh: Makefile.coq
	echo '#!/bin/sh' > $@ 
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/L3 && rm -f $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "L3" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
