...
let int31_of_pos_bigint dloc n = 
  ...
  RApp (dloc, ref_construct, List.rev (args 31 n),
        Util.list_tabulate (fun _ -> Term.Expl) 31)
...
let bigint_of_int31 = 
  ...
  in
  function 
  | RApp (_, RRef (_, c), args,_) when c=int31_construct ->
      args_parsing args zero
  | _ -> raise Non_closed
...
let word_of_pos_bigint dloc hght n =
  ...
  let rec decomp hgt n =
    if is_neg_or_zero hgt then
      ...
      RApp (dloc, ref_W0, [RHole (dloc, Evd.InternalHole)],[Term.Expl])
    else
      let (h,l) = split_at hgt n in
      RApp (dloc, ref_WW, [RHole (dloc, Evd.InternalHole);
			   decomp (sub_1 hgt) h;
			   decomp (sub_1 hgt) l],
            [Term.Expl;Term.Expl;Term.Expl])
  in
  decomp hght n
...
let bigint_of_word = 
  let rec get_height rc =
    match rc with
    | RApp (_,RRef(_,c), [_;lft;rght],_) when c = zn2z_WW -> 
  ...
  let rec transform hght rc =
    match rc with
    | RApp (_,RRef(_,c),_,_) when c = zn2z_W0-> zero
    | RApp (_,RRef(_,c), [_;lft;rght],_) when c=zn2z_WW-> let new_hght = sub_1 hght in
...
let bigint_of_bigN rc =
  match rc with
  | RApp (_,_,[one_arg],_) -> bigint_of_word one_arg
  | RApp (_,_,[_;second_arg],_) -> bigint_of_word second_arg
  | _ -> raise Non_closed
...
let interp_bigZ dloc n = 
  ...
  if is_pos_or_zero n then
    RApp (dloc, ref_pos, [bigN_of_pos_bigint dloc n],[Term.Expl])
  else
    RApp (dloc, ref_neg, [bigN_of_pos_bigint dloc (neg n)],[Term.Expl])

(* pretty printing functions for bigZ *)
let bigint_of_bigZ = function
  | RApp (_, RRef(_,c), [one_arg],_) when c = bigZ_pos -> bigint_of_bigN one_arg
  | RApp (_, RRef(_,c), [one_arg],_) when c = bigZ_neg -> 
