...
(*i $Id: reserve.ml 9718 2007-03-19 16:54:25Z msozeau $ i*)
...
let rec unloc = function
  ...
  | RApp (_,g,args,imps) ->
      RApp (dummy_loc,unloc g, List.map unloc args,imps)
  | RLambda (_,na,imp,ty,c) -> RLambda (dummy_loc,na,imp,unloc ty,unloc c)
  | RProd (_,na,imp,ty,c) -> RProd (dummy_loc,na,imp,unloc ty,unloc c)
  | RLetIn (_,na,imp,b,c) -> RLetIn (dummy_loc,na,imp,unloc b,unloc c)
  ...
  | RRec (_,fk,idl,bl,tyl,bv) ->
      RRec (dummy_loc,fk,idl,
            Array.map (List.map 
              (fun (na,imp,obd,ty) -> (na,imp,option_map unloc obd, unloc ty)))
              bl,
            Array.map unloc tyl,
            Array.map unloc bv)
