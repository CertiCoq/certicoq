...
(* $Id: reductionops.ml 9866 2007-05-28 22:54:04Z herbelin $ *)
...
let empty_stack = []
let append_stack_list l1 imps1 s =
  match (l1,s) with
  | ([],s) -> s
  | (l1, Zapp (l,i) :: s) -> Zapp (l1@l, imps1@i) :: s
  | (l1, s) -> Zapp (l1,imps1) :: s
let append_stack v imps s =
  append_stack_list (Array.to_list v) (Array.to_list imps) s
...
let rec stack_args_size = function
  | Zapp (l,_)::s -> List.length l + stack_args_size s
...
let rec decomp_stack = function
  | Zapp([v],[imp])::s -> Some (v, imp, s)
  | Zapp(v::l,imp::imps)::s -> Some (v, imp, (Zapp (l,imps) :: s))
  | Zapp ([],[]) :: s -> decomp_stack s
  | _ -> None
let rec decomp_stackn = function
  | Zapp ([],[]) :: s -> decomp_stackn s
  | Zapp (l,imps) :: s -> (Array.of_list l, Array.of_list imps, s)
  | _ -> assert false
let array_of_stack s =
  let rec stackrec rargs rimps = function
  | [] ->
      Array.of_list(List.concat(List.rev rargs)),
      Array.of_list(List.concat(List.rev rimps))
  | Zapp (args,imps) :: s -> stackrec (args::rargs) (imps::rimps) s
  | _ -> assert false
  in stackrec [] [] s
let rec list_of_stack = function
  | [] -> [],[]
  | Zapp (args,imps) :: s ->
      let (a,i) = list_of_stack s in
      (args @ a, imps @ i)
  | _ -> assert false
let rec app_stack = function
  ...
  | f, (Zapp (args,imps) :: s) -> 
      app_stack (applist (f, args,imps), s)
  | _ -> assert false
  ...
  | Zapp (args,imps) :: s ->
	...
	Zapp (args,imps) :: stack_assign s (p-q) c
        ...
        (match list_chop p args with
            (bef, _::aft) -> Zapp (bef@c::aft,imps) :: s
          | _ -> assert false)
  | _ -> s
let rec stack_tail p s =
  if p = 0 then s else
    match s with
	  ...
	  else Zapp (list_skipn p args, list_skipn p imps) :: s
      | _ -> failwith "stack_tail"
let rec stack_nth s p = match s with
  | Zapp (args,imps) :: s ->
      let q = List.length args in
      if p >= q then stack_nth s (p-q)
      else (List.nth args p, List.nth imps p)
  | _ -> raise Not_found
...
type  contextual_stack_reduction_function = 
    env ->  evar_map -> constr -> constr * constr list * impl list
type  stack_reduction_function =  contextual_stack_reduction_function
type local_stack_reduction_function =
    constr -> constr * constr list * impl list
...
let rec whd_state (x, stack as s) =
  match kind_of_term x with
    | App (f,cl,imps) -> whd_state (f, append_stack cl imps stack)
    | Cast (c,_,_)  -> whd_state (c, stack)
    | _             -> s

let appterm_of_stack (f,s) =
  let (args,imps) = list_of_stack s in (f,args,imps)
...
let rec strong_prodspine redfun c = 
  let x = redfun c in
  match kind_of_term x with
    | Prod (na,imp,a,b) -> mkProd (na,imp,a,strong_prodspine redfun b)
    | _ -> x
...
let rec stacklam recfun env t stack =
  match (decomp_stack stack,kind_of_term t) with
    | Some (h,imp,stacktl), Lambda (_,_,_,c) ->
        stacklam recfun (h::env) c stacktl
    | _ -> recfun (substl env t, stack)

let beta_applist (c,l,imps) =
  stacklam app_stack [] c (append_stack_list l imps empty_stack)
...
(*	let ncargs = (fst mia.mci).(i-1) in*)
	...
	let real_imps = list_skipn mia.mci.ci_npar mia.mimps in
        applist (mia.mlf.(i-1),real_cargs,real_imps)
    | CoFix cofix ->
	let cofix_def = contract_cofix cofix in
	mkCase (mia.mci, mia.mP, applist(cofix_def,mia.mcargs,mia.mimps), mia.mlf)
    | _ -> assert false
...
let fix_recarg ((recindices,bodynum),_) stack =
  ...
  try 
    Some (recargnum, fst (stack_nth stack recargnum))
  with Not_found ->
    None
  ...
  let rec whrec (x, stack as s) =
    match kind_of_term x with
	  ...
	  (match lookup_rel n env with
	     | (_,_,Some body,_) -> whrec (lift n body, stack)
	     | _ -> s)
      | Var id when red_delta flags ->
	  (match lookup_named id env with
	     | (_,_,Some body,_) -> whrec (body, stack)
	     | _ -> s)
	  ...
	  (match constant_opt_value env const with
	     | Some  body -> whrec (body, stack)
	     | None -> s)
      | LetIn (_,_,b,_,c) when red_zeta flags -> stacklam whrec [b] c stack
      | Cast (c,_,_) -> whrec (c, stack)
      | App (f,cl,imps)  -> whrec (f, append_stack cl imps stack)
      | Lambda (na,imp,t,c) ->
          (match decomp_stack stack with
             | Some (a,_,m) when red_beta flags -> stacklam whrec [a] c m
             | None when red_eta flags ->
		 let env' = push_rel (na,imp,None,t) env in
		 let whrec' = whd_state_gen flags env' sigma in
			...
			if napp > 0 then
			  let x', l' = whrec' (array_last cl, empty_stack) in
				...
				let li = Array.sub imps 0 (napp-1) in
				let u =
                                  if napp=1 then f else appvect (f,lc,li) in
      ...
      | Case (ci,p,d,lf) when red_iota flags ->
          ...
          if reducible_mind_case c then
            let (args, imps) = list_of_stack cargs in
            whrec (reduce_mind_case
                     {mP=p; mconstr=c; mcargs=args;
                      mimps=imps;  mci=ci; mlf=lf}, stack)
          else 
...
let rec whd_betaiotaevar_preserving_vm_cast env sigma t = 	 
   let rec stacklam_var subst t stack = 	 
     match (decomp_stack stack,kind_of_term t) with 	 
     | Some (h,_,stacktl), Lambda (_,_,_,c) -> 	 
    ...
    | App (f,l,imps) when isCast f ->
	    ...
	    match kind_of_term g with
            | App _ -> 
                ...
                mkLetIn
                  (Name h,Expl,g,t,mkApp(mkRel 1,Array.map (lift 1) l',imps))
            | _ -> mkApp (g,l',imps)
	    with Not_found -> mkApp (f,l',imps))
        | _ -> mkApp (irec f,l',imps))	
...
let splay_prod env sigma = 
  let rec decrec env m c =
    ...
    match kind_of_term t with
      | Prod (n,imp,a,c0) ->
	  decrec (push_rel (n,imp,None,a) env) ((n,imp,a)::m) c0
      | _ -> m,t
  in 
  decrec env []
...
let splay_prod_assum env sigma = 
  let rec prodec_rec env l c =
    ...
    match kind_of_term t with
    | Prod (x,imp,t,c)  ->
	prodec_rec (push_rel (x,imp,None,t) env)
	  (Sign.add_rel_decl (x, imp, None, t) l) c
    | LetIn (x,imp,b,t,c) ->
	prodec_rec (push_rel (x, imp, Some b, t) env)
	  (Sign.add_rel_decl (x, imp, Some b, t) l) c
...
let decomp_n_prod env sigma n = 
  let rec decrec env m ln c = if m = 0 then (ln,c) else 
    match kind_of_term (whd_betadeltaiota env sigma c) with
      | Prod (n,imp,a,c0) ->
	  decrec (push_rel (n,imp,None,a) env)
	    (m-1) (Sign.add_rel_decl (n,imp,None,a) ln) c0
...
let rec apprec env sigma s =
  ...
  match kind_of_term t with
    | Case (ci,p,d,lf) ->
        let (cr,crargs,imps) = whd_betadeltaiota_stack env sigma d in
        let rslt = mkCase (ci, p, applist (cr,crargs,imps), lf) in
  ...
  let rec whrec (x, stack as s) =
    match kind_of_term x with
	  ...
	  if occur_existential c then 
	    ...
	    whrec (mkApp (f, Array.sub cl 0 n, Array.sub imps 0 n),
                   append_stack [|c|] [|imps.(n)|] stack)
      | LetIn (_,_,b,_,c) ->
      ...
      | Lambda (_,_,_,c) ->
          (match decomp_stack stack with
             | None -> s
             | Some (a,_,m) -> stacklam whrec [a] c m)
      | Case (ci,p,d,lf) ->
...
let find_conclusion env sigma =
  let rec decrec env c =
    let t = whd_betadeltaiota env sigma c in
    match kind_of_term t with
      | Prod (x,imp,t,c0) -> decrec (push_rel (x,imp,None,t) env) c0
      | Lambda (x,imp,t,c0) -> decrec (push_rel (x,imp,None,t) env) c0
      | t -> t
  in 
  decrec env
...
let meta_reducible_instance evd b =
  ...
  let rec irec u =
    ...
    | App (f,l,imps) when isMeta f or isCast f & isMeta (pi1 (destCast f)) ->
	...
	(match
	  ...
	  with Not_found -> None
	 with
	   | Some g -> irec (mkApp (g,l,imps))
	   | None -> mkApp (f,Array.map irec l,imps))
    | Meta m ->
