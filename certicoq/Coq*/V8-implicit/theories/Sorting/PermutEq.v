...
Section Perm.
  ...
  Proof.
    ...
    inversion_clear H.
    subst a0.
    destruct (eq_dec a a) as [_|H]; apply impl_PI; auto with arith; destruct H; auto.
    destruct (eq_dec a a0) as [H1|H1]; apply impl_PI; auto with arith; simpl.
    rewrite <- IHl; auto.
    destruct (eq_dec a a0); apply impl_PI; auto.
    simpl in H.
  ...
  Proof.
    ...
    inversion_clear 1.
    rewrite IHl in H1.
    intros; destruct (eq_dec a a0) as [H2|H2]; apply impl_PI; simpl; auto.
    subst a0.
    rewrite multiplicity_In_O; auto.
    intros; constructor.
    rewrite multiplicity_In.
    generalize (H a).
    destruct (eq_dec a a) as [H0|H0]; apply impl_PI.
  ...
  Proof.
    ...
    destruct (In_dec eq_dec e l2) as [H|H]; apply impl_PI; auto.
  ...
  Proof.
    ...
    destruct (eq_dec b b) as [H|H]; apply impl_PI; [ | destruct H; auto].
    destruct (eq_dec a b); apply impl_PI; simpl; auto; intros; discriminate.
  Qed.
  ...
  Proof.
    ...
    destruct (eq_dec a1 a) as [H2|H2]; 
      destruct (eq_dec a2 a) as [H3|H3]; apply impl_PI; auto.
    ...
    generalize (P a); clear P; simpl.
    destruct (eq_dec a1 a) as [H2|H2]; 
      destruct (eq_dec b2 a) as [H3|H3]; apply impl_PI; auto.
