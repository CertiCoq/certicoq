...
let mkLApp(fc,v,i) = mkApp(Lazy.force fc, v,i)
...
let implement_theory env t th args imps =
...
let add_theory want_ring want_abstract want_setoid a aequiv asetth amorph aplus amult aone azero aopp aeq t cset = 
  ...
  let env = Global.env () in
	...
	not (implement_theory env t coq_Ring_Theory
	  [| a; aplus; amult; aone; azero; (unbox aopp); aeq |]
          (Array.create 7 Expl))) then
...
let build_spolynom gl th lc =
  ...
  let rec aux c = 
    match (kind_of_term (strip_outer_cast c)) with 
      | App (binop,[|c1; c2|],_) when safe_pf_conv_x gl binop th.th_plus ->
	  mkLApp(coq_SPplus, [|th.th_a; aux c1; aux c2 |],[|Impl;Expl;Expl|])
      | App (binop,[|c1; c2|],_) when safe_pf_conv_x gl binop th.th_mult ->
	  mkLApp(coq_SPmult, [|th.th_a; aux c1; aux c2 |],[|Impl;Expl;Expl|])
      | _ when closed_under th.th_closed c ->
	  mkLApp(coq_SPconst, [|th.th_a; c |],[|Impl;Expl|])
      | _ -> 
	  try Hashtbl.find varhash c 
	  with Not_found -> 
	    let newvar =
              mkLApp(coq_SPvar,
                    [|th.th_a; (path_of_int !counter) |],[|Impl;Expl|]) in
	    begin 
	      incr counter;
	      varlist := c :: !varlist;
...
let build_polynom gl th lc =
  ...
  let rec aux c = 
    match (kind_of_term (strip_outer_cast c)) with 
      | App (binop, [|c1; c2|],_) when safe_pf_conv_x gl binop th.th_plus ->
	  mkLApp(coq_Pplus,
                 [|th.th_a; aux c1; aux c2 |],[|Impl;Expl;Expl|])
      | App (binop, [|c1; c2|],_) when safe_pf_conv_x gl binop th.th_mult ->
	  mkLApp(coq_Pmult,
                 [|th.th_a; aux c1; aux c2 |],[|Impl;Expl;Expl|])
	  ...
	  when safe_pf_conv_x gl c
            (mkApp (th.th_plus,
                    [|c1; mkApp(unbox th.th_opp, [|c2|],[|Expl|])|],
                    [|Expl;Expl|])) ->
	    mkLApp(coq_Pplus,
                  [|th.th_a; aux c1;
	            mkLApp(coq_Popp, [|th.th_a; aux c2|],[|Impl;Expl|]) |],
                  [|Impl;Expl;Expl|])
      | App (unop, [|c1|],_) when safe_pf_conv_x gl unop (unbox th.th_opp) ->
	  mkLApp(coq_Popp, [|th.th_a; aux c1|],[|Impl;Expl|])
      | _ when closed_under th.th_closed c ->
	  mkLApp(coq_Pconst, [|th.th_a; c |],[|Impl;Expl|])
      | _ -> 
	  try Hashtbl.find varhash c 
	  with Not_found -> 
	    let newvar =
              mkLApp(coq_Pvar, [|th.th_a; (path_of_int !counter)|],[|Expl|]) in
	    begin 
	      incr counter;
	      varlist := c :: !varlist;
	      Hashtbl.add varhash c newvar;
    ...
    (fun p -> 
       (mkLApp(coq_interp_p,
	       [| th.th_a; th.th_plus; th.th_mult; th.th_zero;
                 (unbox th.th_opp); v; p |],
               [|Impl;Expl;Expl;Expl;Expl;Expl;Expl|])),
	mkLApp(coq_interp_cs,
	       [| th.th_a; th.th_plus; th.th_mult; th.th_one; th.th_zero; v;
		  pf_reduce cbv_betadeltaiota gl 
...
let build_aspolynom gl th lc =
  ...
  let rec aux c = 
    match (kind_of_term (strip_outer_cast c)) with 
      | App (binop, [|c1; c2|],_) when safe_pf_conv_x gl binop th.th_plus ->
	  mkLApp(coq_ASPplus, [| aux c1; aux c2 |],[|Expl;Expl|])
      | App (binop, [|c1; c2|],_) when safe_pf_conv_x gl binop th.th_mult ->
	  mkLApp(coq_ASPmult, [| aux c1; aux c2 |],[|Expl;Expl|])
	  ...
	  with Not_found -> 
	    let newvar =
              mkLApp(coq_ASPvar, [|(path_of_int !counter) |],[|Expl|]) in
	    begin 
	      incr counter;
	      varlist := c :: !varlist;
	      Hashtbl.add varhash c newvar;
  ...
  List.map 
	...
	mkLApp(coq_interp_acs,
		  ...
		  pf_reduce cbv_betadeltaiota gl 
		    (mkLApp(coq_aspolynomial_normalize,[|p|],[|Expl|])) |],
...
let build_apolynom gl th lc =
  ...
  let rec aux c = 
    match (kind_of_term (strip_outer_cast c)) with 
      | App (binop, [|c1; c2|],_) when safe_pf_conv_x gl binop th.th_plus ->
	  mkLApp(coq_APplus, [| aux c1; aux c2 |],[|Expl;Expl|])
      | App (binop, [|c1; c2|],_) when safe_pf_conv_x gl binop th.th_mult ->
	  mkLApp(coq_APmult, [| aux c1; aux c2 |],[|Expl;Expl|])
      (* The special case of Zminus *)
      | App (binop, [|c1; c2|],_) 
	  when safe_pf_conv_x gl c
            (mkApp(th.th_plus,
                   ...
                   [|Expl;Expl|])) ->
	    mkLApp(coq_APplus,
                   [|aux c1; mkLApp(coq_APopp,[|aux c2|],[|Expl|]) |],
                   [|Expl;Expl|])
      | App (unop, [|c1|],_) when safe_pf_conv_x gl unop (unbox th.th_opp) ->
	  mkLApp(coq_APopp, [| aux c1 |],[|Expl|])
	  ...
	  with Not_found -> 
	    let newvar =
              mkLApp(coq_APvar, [| path_of_int !counter |],[|Expl|]) in
	    begin 
	      incr counter;
	      varlist := c :: !varlist;
	      Hashtbl.add varhash c newvar;
    ...
    (fun p -> 
       (mkLApp(coq_interp_ap,
		  ...
		  th.th_zero; (unbox th.th_opp); v; p |],
               [|Impl;Expl;Expl;Expl;Expl;Expl;Expl;Expl|]),
	mkLApp(coq_interp_sacs,
	       [| th.th_a; th.th_plus; th.th_mult; 
		  th.th_one; th.th_zero; (unbox th.th_opp); v; 
		  pf_reduce cbv_betadeltaiota gl 
		    (mkLApp(coq_apolynomial_normalize, [|p|],[|Expl|])) |],
	  ...
	  when safe_pf_conv_x gl c
	    (mkApp(th.th_plus,
	      ...
	      mkLApp(coq_SetPplus,
                     [| th.th_a; aux c1;
			mkLApp(coq_SetPopp,
                               [|th.th_a; aux c2|],[|Impl;Expl|]) |],
                     [|Impl;Expl;Expl|])
      | App (unop, [|c1|],_) when safe_pf_conv_x gl unop (unbox th.th_opp) ->
	  mkLApp(coq_SetPopp, [| th.th_a; aux c1 |],[|Impl;Expl|])
      | _ when closed_under th.th_closed c ->
	  mkLApp(coq_SetPconst, [| th.th_a; c |],[|Impl;Expl|])
      | _ -> 
	  try Hashtbl.find varhash c 
	  with Not_found -> 
	    let newvar =
...
let build_setspolynom gl th lc =
  ...
  let rec aux c = 
    match (kind_of_term (strip_outer_cast c)) with 
      | App (binop, [|c1; c2|],_) when safe_pf_conv_x gl binop th.th_plus ->
	  mkLApp(coq_SetSPplus,
                 [|th.th_a; aux c1; aux c2 |],[|Impl;Expl;Expl|])
      | App (binop, [|c1; c2|],_) when safe_pf_conv_x gl binop th.th_mult ->
	  mkLApp(coq_SetSPmult,
                 [| th.th_a; aux c1; aux c2 |],[|Impl;Expl;Expl|])
      | _ when closed_under th.th_closed c ->
	  mkLApp(coq_SetSPconst, [| th.th_a; c |],[|Impl;Expl|])
      | _ -> 
	  try Hashtbl.find varhash c
	  with Not_found ->
	    let newvar =
              mkLApp(coq_SetSPvar,[|th.th_a; path_of_int !counter|],
                     [|Impl;Expl|]) in
	    begin 
	      incr counter;
	      varlist := c :: !varlist;
