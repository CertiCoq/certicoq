...
(* $Id: tactics.ml 9950 2007-07-06 14:00:59Z glondu $ *)
...
let rec nb_prod x =
  let rec count n c =
    match kind_of_term c with
      ...
      | LetIn(_,_,a,_,t) -> count n (subst1 a t)
...
let rec head_constr_bound t l impsl =
  let t = strip_outer_cast(collapse_appl t) in
  match kind_of_term t with
    | Prod (_,_,_,c2)  -> head_constr_bound c2 l impsl
    | LetIn (_,_,_,_,c2) -> head_constr_bound c2 l impsl
    | App (f,args,imps)  -> 
	head_constr_bound f
          (Array.to_list args@l) (Array.to_list imps@impsl)
    | (Const _|Ind _|Construct _|Var _) -> t,l,impsl
    | _                -> raise Bound

let head_constr_bound c = head_constr_bound c [] []
...
let reduct_in_hyp redfun ((_,id),where) gl =
  let (_,imp, c, ty) = pf_get_hyp gl id in
  let redfun' = (*under_casts*) (pf_reduce redfun gl) in
  match c with
  | None -> 
      if where = InHypValueOnly then
	errorlabstrm "" (pr_id id ++ str "has no value");
      convert_hyp_no_check (id,imp,None,redfun' ty) gl
  | Some b ->
      let b' = if where <> InHypTypeOnly then redfun' b else b in
      let ty' =	if where <> InHypValueOnly then redfun' ty else ty in
      convert_hyp_no_check (id,imp,Some b',ty') gl
...
let default_id env sigma = function
  ...
  | (name,_,Some b,_) -> id_of_name_using_hdchar env b name
...
let find_intro_names ctxt gl = 
  let _, res = List.fold_right 
    (fun decl acc -> 
      let wantedname,imp,x,typdecl = decl in
      let env,idl = acc in
      let name = fresh_id idl (default_id env gl.sigma decl) gl in
      let newenv = push_rel (wantedname,imp,x,typdecl) env in
      (newenv,(name::idl)))
    ctxt (pf_env gl , []) in
  List.rev res 
...
let rec intro_gen name_flag move_flag force_flag gl =
  match kind_of_term (pf_concl gl) with
    | Prod (name,imp,t,_) -> 
	build_intro_tac (find_name (name,imp,None,t) gl name_flag) move_flag gl
    | LetIn (name,imp,b,t,_) ->
	build_intro_tac
          (find_name (name,imp,Some b,t) gl name_flag) move_flag gl
    | _ -> 
...
let dependent_in_decl a (_,_,c,t) =
  match c with
    | None -> dependent a t
...
let move_to_rhyp rhyp gl =
  ...
  let (hyp,_,c,typ as decl) = List.hd sign in
...
let apply_type hdcty argl imps gl =
  refine (applist
    (mkCast (Evarutil.mk_new_meta(),DEFAULTcast, hdcty),argl,imps)) gl
    
let apply_term hdc argl imps gl =
  refine (applist (hdc,argl,imps)) gl
...
let bring_hyps hyps = 
  if hyps = [] then Refiner.tclIDTAC
    ...
    (fun gl ->
      let newcl =
        List.fold_right mkNamedProd_or_LetIn hyps (pf_concl gl) in
      let f = mkCast (Evarutil.mk_new_meta(),DEFAULTcast, newcl) in
      let (inst,imps) = instance_from_named_context hyps in
      refine_no_check (mkApp (f,inst,imps)) gl)
...
let cut imp c gl =
  match kind_of_term (hnf_type_of gl c) with
    | Sort _ ->
        ...
        let t = mkProd (Anonymous, imp, c, pf_concl gl) in
          tclTHENFIRST
            (internal_cut_rev id imp c)
            (tclTHEN (apply_type t [mkVar id] [imp]) (thin [id]))
            gl
    | _  -> error "Not a proposition or a type"

let cut_intro imp t = tclTHENFIRST (cut imp t) intro
...
let cut_replacing id imp t tac = 
  tclTHENS (cut imp t) [
      tclORELSE (intro_replacing id) (intro_erasing id); 
      tac (refine_no_check (mkVar id)) ]

let cut_in_parallel imps l = 
  let rec prec = function
    | [],[] -> tclIDTAC 
    | h::t,imp::imps -> tclTHENFIRST (cut imp h) (prec (t,imps))
    | _ -> error "not the right number of implicits"
  in 
    prec (List.rev l, List.rev imps)
...
let clenv_refine_in with_evars id clenv gl =
  ...
  let imp = clenv_imp clenv in
  tclTHEN
    (tclEVARS (evars_of clenv.evd))
    (cut_replacing id imp new_hyp_typ
...
let cut_and_apply impc c gl =
  let goal_constr = pf_concl gl in 
    match kind_of_term (pf_hnf_constr gl (pf_type_of gl c)) with
      | Prod (_,imp,c1,c2) when not (dependent (mkRel 1) c2) ->
	  let c2 = refresh_universes c2 in
	  tclTHENLAST
	    (apply_type (mkProd (Anonymous,impc,c2,goal_constr))
              [mkMeta(new_meta())] [impc])
	    (apply_term c [mkMeta (new_meta())] [imp]) gl
...
let new_hyp mopt (c,lbind) g =
  ...
  let (thd,tstack,timps) = whd_stack (clenv_value clause) in
  let nargs = List.length tstack in
  let args,imps =
    match mopt with
      | Some m ->
          if m < nargs then (list_firstn m tstack,list_firstn m timps)
          else (tstack,timps)
      | None   -> (tstack,timps) in
  let cut_pf = applist(thd, args,imps) in
...
let keep hyps gl =
  ...
  let cl,_ =
    fold_named_context_reverse (fun (clear,keep) (hyp,_,_,_ as decl) ->
...
let last_arg c = match kind_of_term c with
  | App (f,cl,imps) ->  
      if array_last imps = Impl then error "last_arg: implicit";
      array_last cl
  | _ -> anomaly "last_arg"
...
let assert_as first ipat imp c gl =
  match kind_of_term (hnf_type_of gl c) with
  | Sort s ->
      let id,tac = prepare_intros s ipat gl in
      tclTHENS
        ((if first then internal_cut else internal_cut_rev) id imp c)
	(if first then [tclIDTAC; tac] else [tac; tclIDTAC]) gl
  | _  -> error "Not a proposition or a type"

let assert_tac first na imp = assert_as first (ipat_of_name na) imp
...
let generalize_goal gl imp c cl =
  ...
  match kind_of_term c with
	...
	mkNamedProd id imp t cl
    | _        -> 
        let cl' = subst_term c cl in
        if noccurn 1 cl' then 
	  mkProd (Anonymous,imp,t,cl)
	  ...
	  prod_name (Global.env()) (Anonymous, imp, t, cl')

let generalize_dep imp c gl =
  let env = pf_env gl in
  let sign = pf_hyps gl in
  let init_ids = ids_of_named_context (Global.named_context()) in
  let rec seek d toquant =
    if List.exists (fun (id,_,_,_) -> occur_var_in_decl env id d) toquant
  ...
  let qhyps = List.map (fun (id,_,_,_) -> id) to_quantify_rev in
  ...
  let cl'' = generalize_goal gl imp c cl' in
  let (args,imps) = instance_from_named_context to_quantify_rev in
  tclTHEN
    (apply_type cl''
      (c::Array.to_list args) (Expl::List.rev(Array.to_list imps)))
    (thin (List.rev tothin'))
    gl
    
let generalize lconstr imps gl = 
  let newcl =
    List.fold_right2 (generalize_goal gl) imps lconstr (pf_concl gl) in
  apply_type newcl lconstr imps gl
...
let letin_tac with_eq name imp c occs gl =
  ...
  let newcl = mkNamedLetIn id imp c t tmpcl in
...
let letin_abstract id c occs gl =
  let env = pf_env gl in
  let compute_dependency _ (hyp,_,_,_ as d) depdecls =
    match occurrences_of_hyp hyp occs with
      | None -> depdecls
      | Some occ ->
          let newdecl = subst_term_occ_decl occ c d in
  ...
  let lastlhyp =
    if depdecls = [] then None else
      let(na,_,_,_) = list_last depdecls in Some na in
...
let letin_tac with_eq name imp c occs gl =
  ...
  let t = refresh_universes (pf_type_of gl c) in
  let newcl = mkNamedLetIn id imp c t ccl in
  tclTHENLIST
...
let forward usetac ipat c gl =
  ...
  | None -> 
      let t = refresh_universes (pf_type_of gl c) in
      tclTHENFIRST (assert_as true ipat Expl t) (exact_no_check c) gl
  | Some tac -> 
      tclTHENFIRST (assert_as true ipat Expl c) tac gl
...
let unfold_body x gl =
  ...
  let xval =
    match Sign.lookup_named x hyps with
        (_,_,Some xval,_) -> xval
      | _ -> errorlabstrm "unfold_body"
          (pr_id x ++ str" is not a defined hypothesis") in
  let aft = afterHyp x gl in
  let hl = List.fold_right (fun (y,_,yval,_) cl -> (([],y),InHyp) :: cl) aft [] in
  let xvar = mkVar x in
...
let unfold_all x gl =
  let (_,_,xval,_) = pf_get_hyp gl x in
...
let atomize_param_of_ind (indref,nparams) hyp0 gl =
  ...
  let _, indtyp = decompose_prod typ0 in
  let _,argl,aimps = decompose_app indtyp in
  ...
  let rec atomize_one i avoid gl =
	...
	| Var id ->
	    ...
	    tclTHEN
	      (letin_tac true (Name x) imp (mkVar id) allClauses)
	...
	| _ ->
	    ...
	    let x = fresh_id [] id gl in
	    tclTHEN
	      (letin_tac true (Name x) imp c allClauses)
	      (atomize_one (i-1) ((mkVar x)::avoid)) gl
    else 
      tclIDTAC gl
  in
...
let find_atomic_param_of_ind nparams indtyp =
  let _,argl,imps = decompose_app indtyp in
...
let cook_sign hyp0_opt indvars_init env =
  ...
  let seek_deps env (hyp,_,_,_ as decl) rhyp =
    if hyp = hyp0 then begin
	  ...
	  or List.exists (fun (id,_,_,_) -> occur_var_in_decl env id decl)
        !decldeps)
      then begin
  ...
  let compute_lstatus lhyp (hyp,_,_,_) =
...
(*     anomaly "hyp0 not found" *)
  ...
  with Shunt lhyp0 ->
    ...
    (statuslists, (if hyp0_opt=None then None else lhyp0) ,
     !indhyps, !decldeps)
...
let decompose_paramspred_branch_args elimt = 
  let rec cut_noccur elimt acc2 =
    match kind_of_term elimt with
      | Prod(nme,imp,tpe,elimt') -> 
	  let hd_tpe,_,_ = decompose_app (snd (decompose_prod_assum tpe)) in
	  if not (occur_rel 1 elimt') && isRel hd_tpe	    
	  then cut_noccur elimt' ((nme,imp,None,tpe)::acc2)
	  else
            let acc3,ccl = decompose_prod_assum elimt in
            acc2 , acc3 , ccl
      | App _ | Rel _ -> acc2 , [] , elimt
      | _ -> error "cannot recognise an induction schema" in
  let rec cut_occur elimt acc1 =
    match kind_of_term elimt with
      | Prod(nme,imp,tpe,c) when occur_rel 1 c ->
          cut_occur c ((nme,imp,None,tpe)::acc1)
      | Prod(nme,_,tpe,c) ->
          let acc2,acc3,ccl = cut_noccur elimt [] in acc1,acc2,acc3,ccl
      | App _ | Rel _ -> acc1,[],[],elimt
      | _ -> error "cannot recognise an induction schema" in
  ...
  if List.length acc2 <> 0 then acc1, acc2 , acc3, ccl
    ...
    let hd_ccl_pred,_,_ = decompose_app ccl in
    match kind_of_term hd_ccl_pred with
      | Rel i  ->
          let acc3,acc1 = list_chop (i-1) hyps in
          acc1 , [] , acc3 , ccl
...
let exchange_hd_app subst_hd t =
  let hd,args,imps= decompose_app t in
  mkApp (subst_hd,Array.of_list args,Array.of_list imps)
...
let compute_elim_sig ?elimc elimt =
  ...
  let preds,params =
    list_chop (List.length params_preds - nparams) params_preds in
  ...
  try 
	...
	| hiname,_,Some _,hi -> error "cannot recognize an induction schema"
	| hiname,_,None,hi -> 
	    let hi_ind, hi_args, hi_imps = decompose_app hi in
  ...
  with Exit -> (* Ending by computing indrev: *)
    match !res.indarg with
      | None -> !res (* No indref *)
      | Some ( _,_,Some _,_) -> error "Cannot recognise an induction scheme"
      | Some ( _,_,None,ind) -> 
	  let indhd,_,_ = decompose_app ind in
...
let compute_elim_signature elimc elimt names_info =
  ...
  let f,l,_ = decompose_app scheme.concl in
  (* Vérifier que les arguments de Qi sont bien les xi. *)
  match scheme.indarg with
    | Some (_,_,Some _,_) ->
        error "strange letin, cannot recognize an induction schema"
    | None -> (* Non standard scheme *)
	let npred = List.length scheme.predicates in 
	let is_pred n c = 
	  let hd,_,_ = decompose_app c in match kind_of_term hd with
	    | Rel q when n < q & q <= n+npred -> IndArg
	    | _ -> OtherArg in 
	let rec check_branch p c = 
	  match kind_of_term c with
	    | Prod (_,_,t,c) -> is_pred p t :: check_branch (p+1) c
	    | LetIn (_,_,_,_,c) -> OtherArg :: check_branch (p+1) c
	...
	let rec find_branches p lbrch = 
	  match lbrch with
	    ...
	    | (_,_,Some _,_)::_ -> error_ind_scheme "the branches of"
	    | [] -> [] in
	let indsign = Array.of_list (find_branches 0 (List.rev scheme.branches)) in
	indsign,scheme	
	
    | Some ( _,_,None,ind) -> (* Standard scheme from an inductive type *)
	let indhd,indargs,_ = decompose_app ind in
	let npred = List.length scheme.predicates in
	let is_pred n c = 
	  let hd,_,_ = decompose_app c in match kind_of_term hd with
	    | Rel q when n < q & q <= n+npred -> IndArg
	...
	let rec find_branches p lbrch =
	  match lbrch with
		...
		let ccl_arg_ok = is_pred (p + scheme.nargs + 1) f = IndArg in
                let (inst,_) = extended_rel_list 0 scheme.args in
		let ind_is_ok = list_lastn scheme.nargs indargs = inst in
		if not (ccl_arg_ok & ind_is_ok) then
		  error
                    "Cannot recognize the conclusion of an induction schema";
		[] 
	in
	let indsign =
          Array.of_list (find_branches 0 (List.rev scheme.branches)) in
	indsign,scheme
...
let recolle_clenv scheme lid elimclause gl = 
  let _,arr,imps = destApp elimclause.templval.rebus in
  let lindmv = 
    Array.map
      (fun x -> 
  ...
  let lidparams,lidargs = list_chop (scheme.nparams) lid in
...
let induction_from_context_l isrec elim_info lid names gl =
  ...
  let hyp0,indvars,lid_params = 
    match lid with
	  ...
	  let ivs,lp = list_chop nargs_without_first l in
	  e, ivs, lp in
  let statlists,lhyp0,indhyps,deps = cook_sign None (hyp0::indvars) env in
  let tmpcl = it_mkNamedProd_or_LetIn (pf_concl gl) deps in
  let names = compute_induction_names (Array.length indsign) names in
  let dephyps = List.map (fun (id,_,_,_) -> id) deps in
  let deps_cstr =
    List.fold_left
      (fun a (id,_,b,_) -> if b = None then (mkVar id)::a else a) [] deps in
  ...
  tclTHENLIST
    [ 
      ...
      if deps = [] then tclIDTAC else
        apply_type tmpcl deps_cstr (imps_from_named_context deps);
...
let new_induct_gen isrec elim names c gl =
  match kind_of_term c with
	...
	tclTHEN
	  (letin_tac true (Name id) Expl c allClauses)
	  (induction_with_atomization_of_ind_arg isrec elim names id) gl
...
let impE imp id gl =
  ...
  if is_imp_term (pf_hnf_constr gl t) then 
    let (dom, imp, _, rng) = destProd (pf_hnf_constr gl t) in 
    tclTHENLAST
      (cut_intro imp rng) 
      (apply_term (mkVar id) [mkMeta (new_meta())] [imp]) gl
  else 
    errorlabstrm "impE"
...
let build_sym hdcncl args imps =
  let imps = Array.of_list imps in
  match args with 
   | [t1; c1; t2; c2] -> mkApp (hdcncl, [| t2; c2; t1; c1 |],imps)
   | [typ;c1;c2] -> mkApp (hdcncl, [| typ; c2; c1 |],imps)
   | [c1;c2]     -> mkApp (hdcncl, [| c2; c1 |],imps)
   | _ -> assert false
...
let symmetry_red allowred gl =
  ...
  match match_with_equation concl with
    ...
    | Some (hdcncl,args,imps) ->
        let hdcncls = string_of_inductive hdcncl in
        begin 
	  try 
	    (apply (pf_parse_const gl ("sym_"^hdcncls)) gl)
          with  _ ->
            let symc = build_sym hdcncl args imps in
	    tclTHENFIRST (cut Expl symc)
              (tclTHENLIST 
		[ intro;
...
let symmetry_in id gl = 
  ...
  match match_with_equation t with
    ...
    | Some (hdcncl,args,aimps) -> 
        let symccl = build_sym hdcncl args aimps in
	tclTHENS (cut Expl (it_mkProd_or_LetIn symccl sign))
	  [ intro_replacing id;
...
let build_trans hd args imps t gl =
  let imps = Array.of_list imps in
  match args with 
    | [typ1;c1;typ2;c2] ->
        let typt = pf_type_of gl t in
        (mkApp(hd, [| typ1; c1; typt ;t |],imps),
 	 mkApp(hd, [| typt; t; typ2; c2 |], imps))
    | [typ;c1;c2] ->
	(mkApp (hd, [| typ; c1; t |],imps),
	 mkApp (hd, [| typ; t; c2 |],imps))
    | [c1;c2] ->
	(mkApp (hd, [| c1; t|],imps),
	 mkApp (hd, [| t; c2 |],imps))
    | _ -> assert false 
    ...
    | Some (hdcncl,args,imps) -> 
	  ...
	  try 
	    apply_list [(pf_parse_const gl ("trans_"^hdcncls));t] gl 
          with  _ -> 
            let eq1, eq2 = build_trans hdcncl args imps t gl in
            tclTHENFIRST (cut Expl eq2)
	      (tclTHENFIRST (cut Expl eq1)
                (tclTHENLIST
		  [ tclDO 2 intro;
		    tclLAST_HYP simplest_case;
		    assumption ])) gl
...
let interpretable_as_section_decl d1 d2 = match d1,d2 with
  | (_,_,Some _,_), (_,_,None,_) -> false
  | (_,imp1,Some b1,t1), (_,imp2,Some b2,t2) ->
      imp1 = imp2 & eq_constr b1 b2 & eq_constr t1 t2
  | (_,imp1,None,t1), (_,imp2,_,t2) -> imp1 = imp2 & eq_constr t1 t2
...
let abstract_subproof name tac gl = 
  ...
  let concl = it_mkNamedProd_or_LetIn (pf_concl gl) sign in
    ...
    in
    let (inst,imps) = instance_from_named_context sign in
      exact_no_check 
	(applist (lemme, 
		 List.rev (Array.to_list inst),
                 List.rev (Array.to_list imps)))
	gl
