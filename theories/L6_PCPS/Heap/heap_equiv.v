...
Module HeapEquiv (H : Heap).
    ...
    match v1, v2 with
      | Loc l1, Loc l2 =>
        ...
        match get l1 H1 with
          ...
          | Some (Clos (FunPtr B f) rho1) =>
            exists rho2,
              get l2 H2 = Some (Clos (FunPtr B f) rho2) /\
          ...
          | Some (Clos (Loc l1') rho1) =>
            exists l2' rho2, get l2 H2 = Some (Clos (Loc l2') rho2) /\
                        forall i, (i < n)%nat ->
                             match n with
                               | 0%nat => True
                               | S n' =>
                                 res_approx_fuel (n'-(n'-i)) (b1, (Loc l1', H1)) (b2, (Loc l2', H2))
                             end
          | None => True
        end
  ...
  Definition block_equiv (p1 : (loc -> loc) * heap block * block) (p2 : (loc -> loc) *  heap block * block)  :=
    ...
    match bl1, bl2 with
      ...
      | Clos (FunPtr B1 f1) rho1, Clos (FunPtr B2 f2) rho2 =>
        B1 = B2 /\ f1 = f2 /\ occurs_free_fundefs B1 |- (H1, rho1) ⩪_(b1, b2) (H2, rho2)
      | Clos (Loc l1 as l1')  _, Clos (Loc l2 as l2') _ => (l1', H1) ≈_(b1, b2) (l2', H2)
      | _, _ => False
    end.
  ...
  Definition heap_equiv (S : Ensemble loc) p1 p2 : Prop :=
    let '(b1, H1) := p1 in
    let '(b2, H2) := p2 in
    forall l, l \in S -> (Loc (b1 l), H1)  ≈_( b1 , b2 ) (Loc (b2 l), H2).
      ...
      | Loc l1, Loc l2 =>
        ...
        match get l1 H1 with
          ...
          | Some (Clos (Loc l1') rho1) =>
            exists l2' rho2, get l2 H2 = Some (Clos (Loc l2') rho2) /\
                        (forall i, (i < n)%nat ->
                              res_approx_fuel i (b1, (Loc l1', H1)) (b2, (Loc l2', H2)))
          | None => True
        end
      | FunPtr B1 f1, FunPtr B2 f2 => f1 = f2 /\ B1 = B2
      | _, _ => False
  ...
  Proof.
    ...
    - destruct (get l1 H1) as [[c1 vs1 | v1 rho1]|]; [| | now firstorder ].
      ...
      + split.
        * intros [Heq1 H]. split; eauto.
          destruct v1; eauto.
          destruct H as [rho2 [Hget [Hrho Hres]]].
          eexists. eexists. split; eauto.
          intros. omega.

          destruct H as [rho2 [Hget Hrho]].
          eexists; split; eauto.
          intros x Hin. destruct (Hrho x Hin) as [Hb1 | Hb1]; eauto. 
          destruct Hb1 as [v1'' [v2 [Hget1 [Hget2 Ht]]]].
          left. repeat eexists; eauto. intros; omega.
        * intros [Heq1 H]. split; eauto.
          destruct v1; eauto.
          destruct H as [rho2 [Hget [Hrho Hres]]].
          eexists. eexists. now split; eauto.
 ...
 (** Equivalent definition for [res_equiv] *)
  ...
  Proof.
    ...
    - intros H. assert (H0 := H 0%nat).
      ...
      + destruct v as [c1 vs1 |  [ lc | B f ]  rho1 ].
        ...
        * destruct Hal as [Heq1 Hal]. split; eauto.
          destruct Har as [Heq2 Har].
          destruct (get l2 H2) as [ [| [| ] ] | ] eqn:Hget2; eauto;
          try (destruct Har as [? [? _]]; congruence);
          try (destruct Hal as [? [? [? _]]]; congruence).

          intros n. specialize (H (S n)). rewrite !res_approx_fuel_eq in H. 
          edestruct H as [Hal' Har']. simpl in Hal', Har'.
          rewrite Hget1 in Hal'. rewrite Hget2 in Har'. 
          destruct Hal' as [Heq1' [l1' [rho1' [Hget2' Hi2]]]]. subst_exp.
          destruct Har' as [Heq2' [l2' [rho2' [Hget1' Hi1]]]]. subst_exp.
          split; eauto.

        * destruct Hal as [Heq1 [rho2 [Hget2 Hal]]]. rewrite Hget2 in *.
          split; eauto. split; eauto. split; eauto.
          destruct Har as [Heq2 [rho3 [Hget3 Har]]]. inv Hget3.
          split; intros x v Hin Hget.

          edestruct Hal as [[v1 [v2 [Hgetv1 [Hgetv2 Hb'']]]] | [Hn1 Hn2]]; eauto; try congruence.
          eexists; split; eauto.

          intros n. specialize (H (S n)). rewrite !res_approx_fuel_eq in H.
          edestruct H as [Hal' Har']. simpl in Hal', Har'.
          rewrite Hget1 in Hal'. rewrite Hget2 in Har'. 
          destruct Hal' as [_ [rho2' [Hget2' Hal']]].
          destruct Har' as [_ [rho3' [Hget3' Har']]].
          repeat subst_exp.

          edestruct Hal' as [[v1' [v2' [Hgetv1' [Hgetv2' Hb']]]] | [Hn1 Hn2]]; eauto; try congruence.
          edestruct Har' as [[v3' [v4' [Hgetv3' [Hgetv4' Hb]]]] | [Hn1 Hn2]]; eauto; try congruence.
          repeat subst_exp. now split; eauto.
          ...
          edestruct Har' as [[v3' [v4' [Hgetv3' [Hgetv4' Hb]]]] | [Hn1 Hn2]]; eauto; try congruence.
          repeat subst_exp. now split; eauto.
      + destruct Har as [Heq Har].
        destruct (get l2 H2); eauto.
        destruct b as [ | [| ] ].
        destruct Har as [vs2 [Hget _]]; discriminate.
        destruct Har as [? [? [Hget' _]]]; discriminate.
        destruct Har as [? [Hget _]]; discriminate.
    - destruct (get l1 H1) eqn:Hget1.
      + destruct b as [ c vs1 | v1 rho1 ].
        ...
        * destruct (get l2 H2) eqn:Hget2; try now firstorder.
          ...
          { destruct b as [ | [l2' |] rho2]; try now firstorder.
            intros [Heq1 Hres] n. rewrite !res_approx_fuel_eq.
            simpl. rewrite Hget1, Hget2. split.
            
            split; eauto.
            do 2 eexists; split; eauto. now intros; eapply Hres. 
            split; eauto.
            do 2 eexists; split; eauto. now intros; eapply Hres. 
          }
          
          { destruct b as [ | [l2' |] rho2]; try now firstorder.
            intros [Heq1 [Heq2 [Heq3 Heq]]] n; subst.
            ...
            - split; eauto. eexists. split. reflexivity.
              ...
              eapply Heq in Hgetv1; eauto.
              edestruct Hgetv1 as [v3' [Hgetv2 Hi]]; try eassumption.
              left; repeat eexists; eauto. intros. now eapply Hi.

              right. split; eauto. 
              destruct (M.get x rho2) as [v2'|] eqn:Hgetv2; eauto.
              eapply Heq in Hgetv2. edestruct Hgetv2 as [v2'' [Hgetv1' Hi']]; try eassumption.
  ...
  Definition collect (S : Ensemble loc) (H1 H2 : heap block) β : Prop :=
    size_heap H2 <= size_heap H1 /\
    S |- H1 ≃_(β, id) H2 /\
    injective_subdomain (reach' H1 S) β.
  
  (** [live S H1 H2] iff H2 is the live portion of H1, using S as roots *)
  Definition live (S : Ensemble loc) {_ : ToMSet S} (H1 H2 : heap block) β : Prop :=
 ...
 *)
  ...
  Proof.
    ...
    - destruct v; destruct v0; try contradiction.
      ...
      split; eauto. eexists. eexists.
      split; eauto. intros; eauto. now eapply Heq. 

      destruct Heq as [Heq3 [Heq4 Henv]]; subst.
      split; eauto. eexists. split; eauto.
      intros x Hinx. destruct Henv as [Hal Har].
      destruct (M.get x e) eqn:Hgetx1; destruct (M.get x e0) eqn:Hgetx2;
      simpl in *; eauto.
      + left. repeat eexists; eauto.
        edestruct Hal as [l' [Hgetx2' Ha]]; eauto.
        subst_exp. intros i Hleq; destruct (Ha i); eassumption.
      + edestruct Hal as [l' [Hgetx2' Ha]]; eauto.
        congruence.
  ...
  Proof.
    ...
    - induction i as [i IHi] using lt_wf_rec.
      ...
      + split; eauto. eexists; eexists; split; eauto.
        ...
        intros i' Hlt.
        specialize (IHi i' Hlt (β, (Loc l0, H1))). now eapply IHi; eauto.
      + split; eauto.
        eexists; split; eauto.
        ...
        intros.
        specialize (IHi i0 H (β, (v0, H1))). now eapply IHi; eauto.
    ...
    - induction i as [i IHi]  using lt_wf_rec1.
      ...
      { destruct (get l1 H1) eqn:Hget1; eauto.
        ...
        + destruct vq.
          ...
          edestruct Hap1 as [Heq1 [l1' [rho1' [Hget1' Hall1']]]].
          rewrite Hget1' in Hap2.
          edestruct Hap2 as [Heq2 [l2' [rho2' [Hget2' Hall2']]]].
          split. congruence. eexists; eexists; split; eauto.
          
          intros i' Hlt.
          now eapply IHi; eauto.

          edestruct Hap1 as [Heq1 [rho1' [Hget1' Hall1']]].
          rewrite Hget1' in Hap2.
          edestruct Hap2 as [Heq2 [rho2' [Hget2' Hall2']]].
          split. congruence. eexists; split; eauto.
          
          intros x Hin.
          edestruct Hall1' with (x := x) as [[l1' [l2' [Hgetl1 [Hgetl2 Hi]]]] | [Hn1 Hn2]]; eauto.
          * edestruct Hall2' with (x := x) as [[l2'' [l3' [Hgetl2' [Hgetl3 Hi']]]] | [Hn1 Hn2]]; eauto.
            subst_exp. subst_exp.
            ...
            eapply IHi; eauto.
            congruence.
          * edestruct Hall2' as [[l2'' [l3' [Hgetl2' [Hgetl3 Hi']]]] | [Hn2' Hn3]]; eauto.
            congruence.
  ...
  Proof.
    ...
    - destruct v; destruct v0; destruct v1; try contradiction.
      simpl. eapply Equivalence_Transitive; eassumption.

      destruct Hb1 as [Heq1 [Heq1' Hb1]].
      destruct Hb2 as [Heq2 [Heq2' Hb2]]. subst.
      split. reflexivity. split. reflexivity.
  ...
  Proof.
    constructor.
    - intros [b H]. intros x Hin; reflexivity.
    - intros [b H1] [b' H2] Hsym x Hin. symmetry.
      eauto.
    - intros [b1 H1] [b2 H2] [b3 H3] Ht1 Ht2 x Hin.
      eapply Equivalence_Transitive. eapply Ht1; eauto.
      eapply Ht2; eauto.
  Qed.
  ...
  Proof.
    ...
    - edestruct Heq1 as [Hb2 Heq1'].
      ...
      destruct v; destruct v0; try contradiction.
      now symmetry.
      destruct Hbl2 as [Hb'1 [Hb'2 Hres]].
      split; eauto. split; eauto. subst. symmetry; eauto.
  ...
  Proof.
    ...
    - revert b1 b1' r1' H1' b2' r2' H2' Heq2.
      ...
      + simpl. destruct (get l1 H1) as [b | ] eqn:Hgetl1; eauto. 
        ...
        * destruct vq.
          intros [Heqb [l2' [rho2' [Hget2' Hall]]]].
          split. rewrite <- Heq2. eassumption. 
          now repeat eexists; eauto.

          intros [Heqb [rho2' [Hget2' Hall]]].
          split. rewrite <- Heq2. eassumption. 
          repeat eexists; eauto.
          
          intros x Hin. destruct (Hall x Hin) as [[v1' [v2' [Hget1 [Hget2 Hs]]]] | Hn]; eauto.
          left. repeat eexists; eauto.
    ...
    - revert b1 b1' r1' H1' b2' r2' H2' Heq2.
      ...
      + simpl. destruct (get l1 H1) as [b | ] eqn:Hgetl1; eauto. 
        ...
        * destruct vq.
          ...
          intros [Heqb [l2' [rho2' [Hgetl2 Hall]]]]. split.
          rewrite Heq2. eassumption.
          repeat eexists; eauto.
          ...
          intros x Hin. destruct (Hall x) as [[v1' [v2' [Hget1 [Hget2 Hs]]]] | Hn]; eauto.
          left. repeat eexists; eauto.
        * intros [Heq _]; split; eauto. rewrite Heq2. eauto.
      + simpl; eauto.
  Qed.
  ...
  Proof. 
    ...
    destruct bl2 as [c vs | vq rho ]; destruct bl2' as [c' vs' | vq' rho' ];
    [| now firstorder | now firstorder | ].
    ...
    - assert (Heq : (f_eq * eq)%signature (b1, (vq, H2)) (b2, (vq, H2))) by (split; eauto).
      ...
      destruct vq; destruct vq'; [ | now firstorder | now firstorder | ].
      rewrite Heq. now firstorder.
  ...
  Proof. 
    ...
    assert (Heqp : (f_eq * eq)%signature (b1', (Loc (b2' x), H2')) (b2', (Loc (b2' x), H2'))) by (split; eauto).
    rewrite H. simpl. rewrite <- Heqp, <- H. eapply Heqh. eassumption.

    assert (Heqp : (f_eq * eq)%signature (b1', (Loc (b2' x), H2')) (b2', (Loc (b2' x), H2'))) by (split; eauto).
    rewrite H. simpl. rewrite Heqp. eapply Heqh. eassumption.
  Qed. 
  ...
  Proof.
    ...
    simpl. rewrite <- Heqp. simpl. eapply Heqh. eassumption.
  ...
  Proof.
    ...
    - simpl in Hres1, Hres2. simpl. destruct (get l1 H1) as [b1 |] eqn:Hget1; eauto.
      ...
      + destruct v1.
        ...
        destruct Hres1 as [Hbeq [l1' [rho1' [Hget1' Hall1]]]].
        rewrite Hget1' in Hres2. 
        destruct Hres2 as  [Hbeq' [l2' [rho2' [Hget2 Hall2]]]].
        split; eauto.
        unfold compose, id in *. congruence.
        eexists; eexists; split; eauto.
        ...
        intros i' Hlt.
        rewrite !res_approx_fuel_eq in *.
        eapply IHn; try eassumption; rewrite <- !res_approx_fuel_eq.
        now eapply Hall1; eauto.
        now eapply Hall2; eauto.


        destruct Hres1 as [Hbeq [rho1' [Hget1' Hall1]]].
        rewrite Hget1' in Hres2. 
        destruct Hres2 as  [Hbeq' [rho2' [Hget2 Hall2]]].
        split; eauto.
        unfold compose, id in *. congruence.
        eexists; split; eauto.
        
        intros x Hinx.
        destruct (Hall1 x Hinx) as [[v3' [v4' [Hgx1 [Hgx2 Hi']]]] | [Hn1 Hn2]];
          destruct (Hall2 x Hinx) as [[v3'' [v4'' [Hgx1' [Hgx2' Hj']]]] | [Hn1' Hn2']];
  ...
  Proof.
    ...
    - simpl in Hres1, Hres2. simpl. destruct (get l1 H1) as [b1 |] eqn:Hget1; eauto.
      ...
      + destruct v1.
        ...
        intros j Hlt.
        rewrite !res_approx_fuel_eq in *.
        now eapply IHn; eauto; rewrite <- !res_approx_fuel_eq; eauto.
        ...
        intros x Hinx.
        destruct (Hall1 x Hinx) as [[v3' [v4' [Hgx1 [Hgx2 Hi']]]] | [Hn1 Hn2]];
          destruct (Hall2 x Hinx) as [[v3'' [v4'' [Hgx1' [Hgx2' H'j]]]] | [Hn1' Hn2']];
          try congruence; eauto.
        left. do 2 eexists. split. eassumption. split. eassumption.
        intros j Hlt.
        rewrite !res_approx_fuel_eq in *.
        eapply IHn; eauto; rewrite <- !res_approx_fuel_eq; eauto.
        subst_exp. eauto.
  ...
  Lemma heap_env_approx_f_compose_l S (β1 β2 β3 β4 : loc -> loc) (H1 H2 H3 : heap block)
    ...
    Proof.
      ...
      - destruct v; destruct v0; destruct v1; try contradiction.
        now eapply res_equiv_f_compose; eauto.

        destruct Hb1 as [Heq1 [Heq2 Henv]].
        destruct Hb2 as [Heq1' [Heq2' Henv']]. subst.
        split. reflexivity. split. reflexivity.
        now eapply heap_env_equiv_f_compose; eauto.
    Qed.
    
    (* Lemma heap_equiv_compose S (β1 β3 β4 : loc -> loc) (H1 H2 H3 : heap block) : *)
    (*   S |- H1 ≃_(β1, β4) H2 -> *)
    (*   S |- H2 ≃_(β4, β3) H3 -> *)
    (*   S |- H1 ≃_(β1, β3) H3. *)
    (* Proof. *)
    (*   intros Hyp1 Hyp2 x Hin.  *)
    (*   eapply res_equiv_f_compose  with ( β4 := β4). *)
    (*   eapply Hyp1 . eassumption. *)
    (*   eapply res_equiv_f_compose; eauto. *)
  ...
  Proof with (now eauto with Ensembles_DB).
    ...
    { edestruct Hwf1 as [b1' [Hget1 Hsub1']].
      ...
      - destruct v.
        ...
        destruct Heq as [Hbeq [l1' [rho1' [Hget1' Hall1]]]].
        split; eauto. subst_exp.
        eexists; eexists; split; eauto.
        
        intros j Hlt.
        rewrite !res_approx_fuel_eq in *.
        eapply H; eauto. now rewrite <- !res_approx_fuel_eq; eauto.

        destruct Heq as [Hbeq [rho1' [Hget1' Hall1]]].
        split; eauto. subst_exp.
        eexists; eexists. eauto.
        
        intros x Hin.
        edestruct Hall1 as [[v1 [v2 [Hs1 [Hs2 Hi']]]] | Hnone]; eauto.
  ...
  Proof with (now eauto with Ensembles_DB).
    ...
    { edestruct Hdom1 as [b1 Hget1]; eauto. reflexivity.
      ...
      - destruct v.
        ...
        { destruct Heq as [Hbeq [l1' [Hget1' Hall1]]].
          split; eauto. subst_exp.
          eexists; split; eauto.
  ...
  Proof with (now eauto with Ensembles_DB).
    ...
    - destruct v.
      ...
      + destruct Hres as [l1' [rho1' [Hget1' Hall1]]].
        split; eauto.
        * rewrite <- Hfeq, <- Hfeq'. eassumption.
          now eapply reach'_extensive.
          now eapply reach'_extensive.
        * do 2 eexists. split. eassumption.
          setoid_rewrite res_approx_fuel_eq in Hall1.

          intros i' Hi'.
          rewrite res_approx_fuel_eq in *. eapply IHk; try eassumption.
          eapply Hall1. eassumption.
          ...
          eapply Included_Union_preserv_r. eapply reach'_set_monotonic.
          simpl; rewrite post_Singleton; [| eassumption ]...
      + destruct Hres as [rho1' [Hget1' Hall1]].
        split; eauto.
        * rewrite <- Hfeq, <- Hfeq'. eassumption.
          now eapply reach'_extensive.
          now eapply reach'_extensive.
        *  eexists. split. eassumption.
          ...
          edestruct Hall1 as [[v1 [v2 [Hs1 [Hs2 Hi']]]] | Hnone]; eauto.
          left. repeat eexists; eauto. intros i' Hlt.
          rewrite res_approx_fuel_eq. eapply IHk; eauto.
  ...
  Proof with (now eauto with Ensembles_DB) .
    ...
    - destruct v; destruct v0; try contradiction.
      ...
      now eapply res_equiv_rename_ext; eauto.

      destruct Hb1 as [Heq1 [Heq2 Hres]].
      split; eauto. 
      split; eauto. subst.
  ...
  Proof with (now eauto with Ensembles_DB).
    ...
    - rewrite M.gss in Hget. inv Hget.
      ...
      + split; eauto.
        ...
        * eapply res_equiv_weakening. exact Hc1. exact Hc2.
          eassumption.
          eapply alloc_subheap; eauto.
          now eapply alloc_subheap; eauto.
          eapply Included_trans; [| eassumption ]...
          eapply Included_trans; [| eassumption ]...
        * destruct Hb as [Heq1 [Heq2 Henv]]; subst.
          split; eauto. split. eauto.
  ...
  Proof with (now eauto with Ensembles_DB).
    ...
    - rewrite M.gss in Hget. inv Hget.
      ...
      + split; eauto.
        ...
        { destruct Hb as [Hb1 [Hb2 Heq']]; subst.
          split; eauto. split; eauto.
          eapply heap_env_equiv_weaking_alt;
            [ exact Hc1 | exact Hc2 | | | | | | |]; try eassumption.
          * now eapply alloc_subheap; eauto.
          * now eapply alloc_subheap; eauto. }
    - rewrite M.gso in Hget; eauto.
      destruct Heq as [Heq1 Heq2].
  ...
  Proof.
    ...
    - simpl in Hdef. destruct (def_closures B B0 rho H l) as [H'' rho''] eqn:Heqd.
      destruct (alloc (Clos (FunPtr B0 v) _) H'') as [l' H'''] eqn:Heqa.
      inv Hdef. 
    ...
    - simpl in Hres.
      ...
      + destruct Hres as [Hbs1 [vs2 [Hget2 Hi]]].
        ...
        * replace (S n) with (n + 1) by omega.
          ...
          eapply IHHi; eauto. 
      + destruct Hres as [Hbs1 Hres].
        destruct v1.
        
        { destruct Hres as [l1' [rho2' [Hget2 Hi]]].
          destruct n. simpl.
          * rewrite !image_Singleton. rewrite Hbs1. reflexivity.
          * replace (S n) with (n + 1) by omega.
            rewrite !app_plus. unfold compose. simpl.
            rewrite (proper_post_n H1);
              [| rewrite !post_Singleton; try eassumption; reflexivity ].
            rewrite (proper_post_n H2);
              [| rewrite !post_Singleton; try eassumption; reflexivity ].
            simpl.
            eapply IHn with (v1 := Loc l) (v2 := Loc l1'). omega.
            rewrite <- res_approx_fuel_eq. eapply Hi. omega. }
        { destruct Hres as [rho2' [Hget2 Hi]].
          destruct n. simpl.
          * simpl. rewrite !image_Singleton.
            rewrite Hbs1. reflexivity.
          * replace (S n) with (n + 1) by omega.
            ...
            intros l3 [z [HHin Hget]]. 
            destruct (M.get z rho1) as [v3 | ] eqn:Hgetz; [| now inv Hget ].
            destruct v3 as [l1' |]; try now inv Hget. inv Hget.
            destruct (Hi z) as [ [v3 [v4 [Hget1 [Hget2 Hi' ]]]] | [Hn1 Hn2] ]; [ eassumption | | congruence ].
            subst_exp. specialize (Hi' n (NPeano.Nat.lt_succ_diag_r n)).
            rewrite res_approx_fuel_eq in Hi'. 
            destruct v4 as [ l4 |]; try contradiction. eexists. split.
            eexists. split. eassumption. rewrite Hget2. reflexivity.
            rewrite res_approx_fuel_eq. eassumption.
            
            intros l3 [z [Hin Hget]].
            destruct (M.get z rho2') as [v2 | ] eqn:Hgetz; [| now inv Hin ].
            destruct v2 as [l1' |]; try now inv Hin. inv Hget.
            destruct (Hi z Hin) as [ [v3 [v4 [Hget1 [Hget2 Hi' ]]]] | [Hn1 Hn2] ]; [| congruence ].
            subst_exp. specialize (Hi' n (NPeano.Nat.lt_succ_diag_r n)).
            rewrite res_approx_fuel_eq in Hi'.
            destruct v3 as [l4 |]; try contradiction. eexists. split.
            eexists. split. eassumption. rewrite Hget1. reflexivity.
            rewrite res_approx_fuel_eq. eassumption. }
      + destruct Hres as [Heqb _]. destruct n.
        * simpl. rewrite !image_Singleton, Heqb. reflexivity.
        * simpl val_loc. rewrite post_n_Singleton_None; eauto.
  ...
  Lemma live_exists S H (_ : ToMSet S) b :
    exists H', live S H H' b.
  Proof.
  Admitted.
  ...
  Lemma heap_equiv_image_eq S H1 H2 b1 b2 :
    S |- H1 ≃_(b1, b2) H2 ->
    image b1 S <--> image b2 S.
  Proof.
    intros Heqh. split.
    - intros x [y [Hin Heq]]. subst.
      edestruct Heqh as [Heq _]; eauto. simpl in Heq. 
      rewrite Heq. eexists; split; eauto.
    - intros x [y [Hin Heq]]. subst.
      edestruct Heq2 as [Heq _]; eauto.
      rewrite Heq. eexists; split; eauto.
  Qed.
  
  Lemma live_respects_heap_equiv S (HS : ToMSet S)
        β1 β2 (H1 H2 H3 : heap block) (rho1 rho2 : env) : 
    S |- (H1, rho1) ⩪_(β1, β2) (H2, rho2) ->
    live (env_locs rho1 S) H1 H3 ->
    live (env_locs rho2 S) H2 H3.
  Proof.
    intros Heq [Hl1 [b [Hl2 Hl3]]].
    split. 
    - eapply Included_trans. eassumption.
    - eexists. 
    
    
  Lemma live'_respects_heap_equiv S1 S2 (HS1 : ToMSet S1) (HS2 : ToMSet S2) H1 H2 H3 : 
    S |- (H1, rho1) ⩪_(β1, β2) (H2, rho2) ->
    live S1 H1 H2 ->
    live S1 H1 H3.
  Proof.
  ...
  Proof.
    ...
    - destruct v; contradiction.
    - destruct v; destruct v0; try contradiction.
      + simpl in Hb. simpl. eapply val_loc_in_dom in Hb.
        eassumption. eassumption.
      + destruct Hb as [Hb1 [Hb2 Hres]]. simpl. subst.
        eapply env_locs_in_dom; try eassumption.
  Qed.
  ...
  Proof.
    ...
    - destruct v; destruct v0; try contradiction.
      + inv Hin.
        edestruct res_equiv_exists_loc with (v1 := (Loc l1)) as [l2 [Hl2in Heq]]; eauto.
        reflexivity.
      + destruct Hb as [Heq1 [Heq2 Henv]]. subst.
        edestruct heap_env_equiv_exists_loc as [l2 [Hin' Hres]]; eauto.
  Qed.
  ...
  Proof.
    ...
    + split; eauto.
      ...
      do 2 eexists. split.
      eapply Hsub. eassumption.
      intros. now eapply IHk; eauto. 

      eexists; split.
      eapply Hsub. eassumption.

      intros x. destruct (M.get x rho1); eauto.
  ...
  Proof with now eauto with Ensembles_DB.
    ...
    + split; eauto.
      ...
      eexists; split. eassumption.
      intros x. destruct (M.get x rho1) eqn:Hgetx; eauto.
      left. do 2 eexists. split. reflexivity.
      split. reflexivity.
      intros i Hlt. eapply IHk; eauto.
      eapply Included_trans; [| eapply reachable_closed; try eassumption ].
  ...
  Proof with now eauto with Ensembles_DB.
    ...
    + simpl.
      destruct v1.
      eapply subheap_res_equiv; eassumption.
  ...
  Lemma heap_env_equiv_def_funs_strong S β1 H1 H2 rho1 rho2 clo_rho1 clo_rho2 B B0 :
    ...
    env_locs clo_rho1 (occurs_free_fundefs B0) \subset (reach' H1 (env_locs rho1 (Setminus _ S (name_in_fundefs B)))) ->
    env_locs clo_rho2 (occurs_free_fundefs B0) \subset (reach' H2 (env_locs rho2 (Setminus _ S (name_in_fundefs B)))) ->

    (occurs_free_fundefs B0) \subset (Setminus _ S (name_in_fundefs B)) ->
    
    (occurs_free_fundefs B0) |- (H1, clo_rho1) ⩪_(id, β1) (H2, clo_rho2) ->

    injective_subdomain (reach' H2 (env_locs rho2 (Setminus _ S (name_in_fundefs B)))) β1 ->

    S \\ (name_in_fundefs B) |- (H1, rho1) ⩪_(id, β1) (H2, rho2) ->

    (exists β1', S |- (def_closures B B0 rho1 H1 clo_rho1) ⩪_(id, β1') (def_closures B B0 rho2 H2 clo_rho2) /\
            f_eq_subdomain (dom H2) β1 β1' /\
            let '(H1', rho1') := (def_closures B B0 rho1 H1 clo_rho1) in
            let '(H2', rho2') := (def_closures B B0 rho2 H2 clo_rho2) in
            well_formed (reach' H1' (env_locs rho1' S)) H1' /\
            well_formed (reach' H2' (env_locs rho2' S)) H2' /\
            env_locs rho1' S \subset dom H1' /\
            env_locs rho2' S \subset dom H2' /\
            injective_subdomain (reach' H2' (env_locs rho2' S)) β1').
  Proof with (now eauto with Ensembles_DB).
    revert S rho1 rho2; induction B;
    intros S rho1 rho2 Hc1 Hc2 Hdom1 Hdom2 He1 He2 Hsub Hloc Hinj Heq; simpl; eauto.
    - destruct (def_closures B B0 rho1 H1 _) as [H1' rho1'] eqn:Hd1.
      destruct (def_closures B B0 rho2 H2 _) as [H2' rho2'] eqn:Hd2.
      destruct (alloc (Clos (FunPtr B0 v) clo_rho1) _) as [l1' H1''] eqn:Ha1.
      destruct (alloc (Clos (FunPtr B0 v) clo_rho2) _) as [l2' H2''] eqn:Ha2.
      ...
      + rewrite Hd1, Hd2 in Hinj'. destruct Hinj' as [Hwf1' [Hwf2' [Henv1 [Henv2 Hinj']]]].
        ...
        { eapply heap_env_equiv_alloc_alt; (try now apply Ha1); (try now apply Ha2).
          ...
          + simpl. 
            ...
            eapply Included_trans. now apply He1.
            eapply reachable_in_dom; try eassumption.
          + simpl.
            eapply Included_trans; [| eapply dom_subheap; eapply def_funs_subheap; now eauto ].
            eapply Included_trans. now apply He2.
            eapply reachable_in_dom; try eassumption.
          + simpl.
            eapply Included_trans. eapply Included_post_reach'.
            rewrite (reach'_idempotent H1' (env_locs rho1' (S \\ [set v]))).
          ...
          + simpl.
            eapply Included_trans. eapply Included_post_reach'.
            rewrite (reach'_idempotent  H2' (env_locs rho2' (S \\ [set v]))).
            eapply reach'_set_monotonic. eapply Included_trans. eassumption.
            simpl. rewrite <- Setminus_Union. eapply Included_trans.
            eapply reach'_heap_monotonic. eapply def_funs_subheap; eauto.
            eapply reach'_set_monotonic. eapply def_closures_env_locs_subset.  eassumption.
          ...
          + simpl. split. reflexivity. split. reflexivity.
            ...
            eapply Included_trans. eapply Included_post_reach'.
            rewrite (reach'_idempotent H2 (env_locs rho2 _)).
            eapply reach'_set_monotonic. eassumption.
        ...
        { eapply well_formed_antimon with
          ...
          now eauto with Ensembles_DB typeclass_instances.
          eapply well_formed_reach_alloc_in_dom; try eassumption.
          simpl. eapply Included_trans. simpl.  
          eapply post_set_monotonic. eapply Included_trans. eassumption.
          eapply Included_trans. eapply reach'_heap_monotonic.
          now eapply def_funs_subheap; eauto.
          reflexivity. 
          eapply Included_trans. eapply Included_post_reach'.
          rewrite <- reach'_idempotent. eapply reach'_set_monotonic.
          eapply Included_trans. Focus 2.
          eapply def_closures_env_locs_subset. eassumption. simpl.
          eapply env_locs_monotonic...

          eapply Included_trans; [| now eapply HL.dom_subheap; eapply def_funs_subheap; eauto ].
        ...
        { eapply injective_subdomain_antimon;
          [| now eapply reach'_set_monotonic; eapply env_locs_set_Inlcuded' ]. 
          simpl.
          rewrite reach'_Union. rewrite reach_unfold. simpl.
          rewrite post_Singleton; [| erewrite gas; eauto ]. simpl.

          rewrite <- !Union_assoc.
          rewrite (Union_Same_set (reach' H2'' (env_locs clo_rho2 (occurs_free_fundefs B0)))).
          (* rewrite (Union_Same_set (reach' H2'' (post H2'' (env_locs clo_rho2 (Full_set var))))). *)
          rewrite <- well_formed_reach_alloc_same; try eassumption.
          
          eapply injective_subdomain_extend. eassumption.
          rewrite Hd1, Hd2 in HBs. 
          intros Hc.
          apply image_monotonic with (S' :=  reach' H2' (env_locs rho2' (S \\ [set v]))) in Hc;
            try now eauto with Ensembles_DB.
          erewrite <- heap_env_equiv_image_reach with (β1 := id) in Hc; try eassumption.
          rewrite image_id in Hc.
          eapply reachable_in_dom in Hc; try eassumption.
          destruct Hc as [v1' Hget1']. erewrite alloc_fresh in Hget1'; eauto.
          discriminate.
          ...
          eapply reach'_heap_monotonic. eapply def_funs_subheap; eauto.
          eapply Included_trans. 
          eapply reach'_heap_monotonic. eapply HL.alloc_subheap. eassumption.
          eapply reach'_set_monotonic. eapply def_closures_env_locs_subset.
          eassumption. }
    - eexists. simpl in *.
