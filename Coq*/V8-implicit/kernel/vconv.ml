...
let rec conv_eq pb t1 t2 cu =
  if t1 == t2 then cu 
    ...
    | Prod (_,imp1,t1,c1), Prod (_,imp2,t2,c2) when imp1=imp2 -> 
	conv_eq pb c1 c2 (conv_eq CONV t1 t2 cu)
    | Lambda (_,Expl,t1,c1), Lambda (_,Expl,t2,c2) -> conv_eq CONV c1 c2 cu
    | Lambda (_,Impl,_,c1), _ -> conv_eq CONV (lift(-1)c1) t2 cu
    | _, Lambda (_,Impl,_,c2) -> conv_eq CONV t1 (lift(-1)c2) cu
    | LetIn (_,Expl,b1,t1,c1), LetIn (_,Expl,b2,t2,c2) -> 
	conv_eq pb c1 c2 (conv_eq CONV b1 b2 cu)
    | LetIn (_,Impl,_,_,c1), _ -> conv_eq CONV (lift(-1)c1) t2 cu
    | _, LetIn (_,Impl,_,_,c2) -> conv_eq CONV t1 (lift(-1)c2) cu
    | App (c1,l1,imps1), App (c2,l2,imps2) ->
        let l1 = expl_filter l1 imps1 in
        let l2 = expl_filter l2 imps2 in
	...
	let ccu = conv_eq CONV c1 c2 cu in
	conv_eq_vect bl1 bl2 ccu
    | Fix (ln1,(_,tl1,bl1)), Fix (ln2,(_,tl2,bl2)) ->
	if ln1 = ln2 then conv_eq_vect bl1 bl2 cu
	else raise NotConvertible
