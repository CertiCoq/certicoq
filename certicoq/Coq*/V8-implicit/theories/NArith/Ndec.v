...
(*i $Id: Ndec.v 9883 2007-06-07 18:44:59Z letouzey $ i*)
...
Proof.
  intros. elim (sumbool_of_bool (Neqb a a'));
    intro H0; apply impl_PI.
...
Proof.
  intros. elim (sumbool_of_bool (Neqb (Ndouble a0) a));
    intro H0; apply impl_PI.
...
Proof.
  intros. elim (sumbool_of_bool (Neqb (Ndouble_plus_one a0) a));
    intro H0; apply impl_PI.
  rewrite <- (Neqb_complete _ _ H0) in H. 
  unfold Neven in H.
...
Proof.
  intros. elim (sumbool_of_bool (Neqb a (Ndouble_plus_one a0)));
    intro H0; apply impl_PI.
...
Proof.
  intros. elim (sumbool_of_bool (Neqb (Ndiv2 a) (Ndiv2 a')));
    intro H1; apply impl_PI.
...
Proof.
  intros. elim (sumbool_of_bool (Nleb (Ndouble a) (Ndouble b)));
     intro H0; apply impl_PI.
...
Proof.
  intros. elim (sumbool_of_bool (Nleb (Ndouble_plus_one a) (Ndouble_plus_one b)));
    intro H0; apply impl_PI.
...
Proof.
  intros. elim (sumbool_of_bool (Nleb a b));
    intro H0; apply impl_PI.
  rewrite (Nleb_double_mono _ _ H0) in H.
  discriminate H.
  trivial.
Qed.
...
Proof.
  ...
  unfold Nmin'; elim (sumbool_of_bool (Nleb a b)).
   intro H. rewrite (impl_PI H).
   apply Nleb_refl.

   intro H. apply impl_PI. rewrite H. apply Nltb_leb_weak. assumption.
Qed.

Lemma Nmin_le_2 : forall a b, Nleb (Nmin a b) b = true.
Proof.
  intros; rewrite Nmin_Nmin'.
  unfold Nmin'; elim (sumbool_of_bool (Nleb a b)); intro H; apply impl_PI.
  rewrite H. assumption.
  rewrite H. apply Nleb_refl.
Qed.
...
Proof.
  ...
  unfold Nmin' in *; elim (sumbool_of_bool (Nleb b c)); intro H0; apply impl_PI.
  rewrite H0 in H.
  assumption.
  rewrite H0 in H. apply Nltb_leb_weak. apply Nleb_ltb_trans with (b := c); assumption.
Qed.
...
Proof.
  intros. elim (Nmin_choice b c); intro H1;apply impl_PI.
...
Proof.
  intros; rewrite Nmin_Nmin' in *. 
  unfold Nmin' in *. intros.
  elim (sumbool_of_bool (Nleb b c)); intro H0; apply impl_PI.
  rewrite H0 in H.
  assumption.
  rewrite H0 in H. apply Nltb_trans with (b := c); assumption.
Qed.
