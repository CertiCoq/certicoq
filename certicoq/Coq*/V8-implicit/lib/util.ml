...
(* $Id: util.ml 9969 2007-07-11 21:48:04Z letouzey $ *)
...
let array_fold_right4 f v1 v2 v3 v4 a =
  let lv1 = Array.length v1 in
  let rec fold a n =
    if n=0 then a
    else
      let k = n-1 in
      fold (f v1.(k) v2.(k) v3.(k) v4.(k) a) k in
  if Array.length v2 <> lv1 ||
     Array.length v3 <> lv1 ||
     Array.length v4 <> lv1 then invalid_arg "array_fold_right2";
  fold a lv1
