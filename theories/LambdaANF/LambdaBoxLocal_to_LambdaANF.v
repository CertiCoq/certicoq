...
Section Translate.
  ...
  Section COMMON.
    ...
    - an L6 inductive environment (ind_env) mapping tags (ind_tag) to constructors
      ...
      - a map (conId_map) from LambdaBoxLocal tags (conId) to L6 constructor tags (ctor_tag)
  ...
  (** The actual map grows from 1 onward. We keep the pointer p to the last entry of the map.
    ...
    Section Convert.
      ...
      Fixpoint convert_anf (e : expression.exp) (vm : var_map) : anfM anf_term :=
        ...
        | App_e e1 e2 =>
          ...
          '(x1, C1) <- anf_term_to_ctx a1 def_name ;;
          '(x2, C2) <- anf_term_to_ctx a2 def_name ;;
    ...
    Inductive anf_term_to_exp_rel : Ensemble var -> anf_term -> Ensemble var -> cps.exp -> Prop :=
    | Anf_var_to_exp:
      forall S x C, anf_term_to_exp_rel S (Anf_Var x, C) S (C |[ Ehalt x ]|)
    | Anf_app_to_exp:
      forall S f x C, anf_term_to_exp_rel S (Anf_App f x, C) S (C |[ Eapp f func_tag [x] ]|)
    | Anf_constr_to_exp:
        forall S c xs C x,
          x \in S ->
          anf_term_to_exp_rel S (Constr c xs, C) (S \\ [set x]) (C |[ Econstr x c xs (Ehalt x) ]|)
    | Anf_proj_to_exp:
        forall S c n y C x,
          x \in S ->
          anf_term_to_exp_rel S (Proj c n y, C) (S \\ [set x]) (C |[ Eproj x c n y (Ehalt x) ]|)
    | Anf_fun_to_exp:
        forall S ft y e C x,
          x \in S ->
          anf_term_to_exp_rel S (Fun ft y e, C) (S \\ [set x]) (C |[ Efun (Fcons x ft [y] e Fnil) (Ehalt x) ]|)
    | Anf_prim_val_to_exp:
        forall S p C x,
          x \in S ->
          anf_term_to_exp_rel S (Prim_val p , C) (S \\ [set x]) (C |[ Eprim_val x p (Ehalt x) ]|)
    | Anf_prim__to_exp:
        forall S pr xs C x,
          x \in S ->
          anf_term_to_exp_rel S (Prim pr xs, C) (S \\ [set x]) (C |[ Eprim x pr xs (Ehalt x) ]|).

    Inductive anf_term_to_ctx_rel : Ensemble var -> anf_term -> Ensemble var -> var -> exp_ctx -> Prop :=
    | Anf_var_to_ctx:
      forall S x C, anf_term_to_ctx_rel S (Anf_Var x, C) S x C
    | Anf_app_to_ctx:
      forall S f x C, anf_term_to_ctx_rel S (Anf_App f x, C) S x (comp_ctx_f C (Eletapp_c x f func_tag [x] Hole_c))
    | Anf_constr_to_ctx:
        forall S c xs C x,
          x \in S ->
          anf_term_to_ctx_rel S (Constr c xs, C) (S \\ [set x]) x (comp_ctx_f C (Econstr_c x c xs Hole_c))
    | Anf_proj_to_ctx:
        forall S c n y C x,
          x \in S ->
          anf_term_to_ctx_rel S (Proj c n y, C) (S \\ [set x]) x (comp_ctx_f C (Eproj_c x c n y Hole_c))
    | Anf_fun_to_ctx:
        forall S ft y e C x,
          x \in S ->
          anf_term_to_ctx_rel S (Fun ft y e, C) (S \\ [set x]) x (comp_ctx_f C (Efun1_c (Fcons x ft [y] e Fnil) Hole_c))
    | Anf_prim_val_to_ctx:
        forall S p C x,
          x \in S ->
          anf_term_to_ctx_rel S (Prim_val p , C) (S \\ [set x]) x (comp_ctx_f C (Eprim_val_c x p Hole_c))
    | Anf_prim__to_ctx:
        forall S pr xs C x,
          x \in S ->
          anf_term_to_ctx_rel S (Prim pr xs, C) (S \\ [set x]) x (comp_ctx_f C (Eprim_c x pr xs Hole_c)).
  ...
  | e_Match :
      forall S1 S2 S3 e1 e1' bs bs' vn k x1 k1 n tgm,
        x1 \in S1 ->
        k1 \in (S1 \\ [set x1]) ->
        convert_anf_rel (S1 \\ [set x1] \\ [set k1]) e1 vn k1 tgm S2 e1' ->
        convert_anf_rel_branches S2 bs vn k x1 tgm S3 bs' ->
        convert_anf_rel S1
                    (Match_e e1 n bs)
                    vn
                    k
  ...
  | e_Fix :
      forall S1 S2 i f fnlst fnames vn k fdefs tgm,
        ...
        NoDup fnames ->
        List.length fnames = efnlength fnlst ->
        convert_anf_rel_efnlst (S1 \\ (FromList fnames)) fnlst (List.rev fnames ++ vn) fnames tgm S2 fdefs ->
        nth_error fnames (N.to_nat i) = Some f ->
        convert_anf_rel S1
                    (Fix_e fnlst i)
                    vn
  ...
  | e_Prf :
      forall S vn k tgm x,
        x \in S ->
        convert_anf_rel S Prf_e vn k tgm (S \\ [set x]) (Econstr x default_tag nil (Eapp k kon_tag (x::nil)))

  | e_Prim_val :
    forall S vn k p tgm x,
          x \in S ->
          convert_anf_rel S (Prim_val_e p) vn k tgm (S \\ [set x]) (Eprim_val x p (Eapp k kon_tag (x::nil)))


  with convert_anf_rel_efnlst :
         Ensemble var ->
         expression.efnlst ->
         list var ->
         list var ->
  ...
  | e_Eflnil :
      forall S vn tgm,
        convert_anf_rel_efnlst S eflnil vn [] tgm S Fnil
  | e_Eflcons :
      forall S1 S2 S3 vn fnames e1 e1' fnlst fdefs n n1 na x1 k1 tgm,
        x1 \in S1 ->
        k1 \in (S1 \\ [set x1]) ->
        convert_anf_rel (S1 \\ [set x1] \\ [set k1]) e1 (x1::vn) k1 tgm S2 e1' ->
        convert_anf_rel_efnlst S2 fnlst vn fnames tgm S3 fdefs ->
        convert_anf_rel_efnlst
          S1 (eflcons n1 (Lam_e na e1) fnlst)
          vn (n :: fnames) tgm S3
          (Fcons n func_tag (k1::x1::nil) e1' fdefs)
  with convert_anf_rel_branches :
         Ensemble var ->
         expression.branches_e ->
         list var ->
         var (* continuation variable *) ->
  ...
  | e_Brnil :
      forall S vn k r tgm,
        convert_anf_rel_branches S brnil_e vn k r tgm S []
  | e_Brcons:
      forall S1 S2 S3 vn k r e ce bs' cbs' vars lnames n ctx_p tg dc tgm,
        tg = dcon_to_tag dc tgm ->
        FromList vars \subset S2 ->
        ...
        convert_anf_rel_branches S1 bs' vn k r tgm S2 cbs' ->

        convert_anf_rel (S2 \\ (FromList vars)) e (vars ++ vn) k tgm S3 ce ->

        convert_anf_rel_branches
          S1 (brcons_e dc (n, lnames) e bs') vn k r tgm S3 ((tg, app_ctx_f ctx_p ce)::cbs').
