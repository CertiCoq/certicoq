...
(*i $Id: Znat.v 10028 2007-07-18 22:38:06Z letouzey $ i*)
...
Proof.
  intros x y H.
  destruct (le_lt_dec x y) as [H0|H0].
   apply impl_PI; auto.

   elimtype False.
   assert (H1:=inj_lt _ _ (impl_PI H0)).
   red in H; red in H1.
...
Proof.
  intros x y H.
  destruct (le_lt_dec y x) as [H0|H0].
   elimtype False.
   assert (H1:=inj_le _ _ (impl_PI H0)).
   red in H; red in H1.
...
Proof.
  ...
  destruct (le_lt_dec y x) as [H0|H0].
   ...
   elimtype False.
   assert (H1:=inj_gt _ _ (impl_PI H0)).
   red in H; red in H1.
   rewrite <- Zcompare_antisym in H; rewrite H1 in H; auto.
Qed.
...
Proof.
  ...
  destruct (le_lt_dec x y) as [H0|H0].
   elimtype False.
   assert (H1:=inj_ge _ _ (impl_PI H0)).
   red in H; red in H1.
   rewrite <- Zcompare_antisym in H1; rewrite H in H1; auto.
...
Proof.
 ...
 rewrite (inj_minus1 _ _ (impl_PI H)).
 assert (H':=Zle_minus_le_0 _ _ (inj_le _ _ (impl_PI H))).
 unfold Zle in H'.
 rewrite <- Zcompare_antisym in H'.
 destruct Zcompare; simpl in *; intuition.

 rewrite (inj_minus2 _ _ (impl_PI H)).
 assert (H':=Zplus_lt_compat_r _ _ (- Z_of_nat m) (inj_lt _ _ (impl_PI H))).
