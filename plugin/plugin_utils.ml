...
let extract_constant (g : Names.GlobRef.t) (s : string) : (BasicAst.kername * char list) * Datatypes.nat =
  match g with
...
let rec debug_mappings (ms : (BasicAst.kername * string) list) : unit =
  match ms with
  | [] -> ()
  | (k, s) :: ms ->     
     Feedback.msg_debug (str ("Kername: " ^ (string_of_chars (BasicAst.string_of_kername k)) ^ " C: "  ^ s));
     debug_mappings ms
