...
let detype_param v =
  match v with
  | (Name id,imp,None,p)   -> id, Entries.LocalAssum(p,imp)
  | (Name id,imp,Some p,_) -> id, Entries.LocalDef(p,imp)
  | (Anonymous,_,_,_)    -> anomaly"Unnamed inductive local variable"
...
let abstract_inductive hyps nparams inds =
  let chyps = set_impl_named_context Impl hyps in
  let ntyp = List.length inds in 
  let nhyp = named_context_length hyps in
  let (args,imps) = instance_from_named_context (List.rev hyps) in
  let subs =
    list_tabulate (fun k -> lift nhyp (mkApp(mkRel (k+1),args,imps))) ntyp in
  let inds' =
    List.map
	...
	let lc'' =
          List.map
            (fun b -> Termops.it_mkNamedProd_wo_LetIn b chyps) lc' in
...
(* To be sure to be the same as before, should probably be moved to process_inductive *) 
  let params' = let (_,arity,_,_) = List.hd inds' in 
		let (params,_) =
                  decompose_prod_n_assum nparams' arity in
