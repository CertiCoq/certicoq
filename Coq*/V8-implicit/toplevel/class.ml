...
let prods_of t = 
  let rec aux acc imps d = match kind_of_term d with
    | Prod (_,imp,c1,c2) -> aux (c1::acc) (imp::imps) c2
    | Cast (c,_,_) -> aux acc imps c
    | _ -> (d,acc,imps)
  in 
  aux [] [] t
...
let build_id_coercion idf_opt source =
  ...
  let (inst,imps) = extended_rel_list 0 lams in
  let val_f =
    it_mkLambda_or_LetIn
      (mkLambda (Name (id_of_string "x"),Expl,
		 applistc vs inst imps,
		 mkRel 1))
       lams
  in
  let typ_f =
    it_mkProd_wo_LetIn
      (mkProd (Anonymous, Expl,
               applistc vs inst imps,
               lift 1 t))
      lams
  in
