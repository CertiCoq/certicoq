...
(* $Id: himsg.ml 9868 2007-05-29 10:01:06Z herbelin $ *)
...
let explain_generalization env (name,var) j =
  ...
  let (pc,pt) = pr_ljudge_env (push_rel_assum (name,Expl,var) env) j in
...
let explain_implicit_lambda env t =
  let pt = pr_lconstr_env env t in 
  str "Term " ++ pt ++ str" is an implicit abstraction," ++ spc() ++
  str "but the bound variable is used in an explicit position"
...
let explain_type_error env err =
  ...
  | ImplicitLambdaError t ->
      explain_implicit_lambda env t
...
let explain_cannot_infer_predicate env typs =
  ...
  let pr_branch (cstr,typ) =
    let cstr,_,_ = decompose_app cstr in
