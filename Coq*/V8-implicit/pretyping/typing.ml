...
(* $Id: typing.ml 9879 2007-06-07 07:45:49Z herbelin $ *)
  ...
  match kind_of_term cstr with
	...
	let j =
	  match kind_of_term f with
		...
		execute env evd f
	in
	fst (judge_of_apply env j jl imps)
	    
    | Lambda (name,imp,c1,c2) -> 
        let j = execute env evd c1 in
	let var = type_judgment env j in
	let env1 = push_rel (name,imp,None,var.utj_val) env in
        let j' = execute env1 evd c2 in 
        judge_of_abstraction env1 name imp var j'
	...
	judge_of_product env name imp varj varj'
     ...
     | LetIn (name,imp,c1,c2,c3) ->
        ...
        judge_of_letin env name imp j1 j2 j3
