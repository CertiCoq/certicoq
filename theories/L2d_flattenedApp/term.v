...
Ltac not_is5 :=
  ...
  with aa := fresh "a"
  with cc := fresh "c" in
  intros hh; destruct hh as [xx [yy [zz [aa [cc jj]]]]]; discriminate.
Ltac not_is7 :=
  let hh := fresh "h"
  with xx := fresh "x"
  with jj := fresh "j"
...
Ltac is1_inv h := 
  let xx := fresh "x"
  with jj := fresh "j"
  in destruct h as [xx jj]; discriminate.
Ltac is2_inv h :=
  let xx := fresh "x"
  with yy := fresh "y"
  with jj := fresh "j"
  in destruct h as [xx [yy jj]]; discriminate.
Ltac is3_inv h :=
  let xx := fresh "x"
  with yy := fresh "y"
  with zz := fresh "z"
...
Ltac is4_inv h :=
  ...
  with zz := fresh "z"
  with jj := fresh "j"
  in destruct h as [xx [yy [ww [zz jj]]]]; discriminate.
Ltac is_inv h := first [is1_inv h | is2_inv h | is3_inv h | is4_inv h].
****************)             
...
Proof.
  ...
  - change (WFapp (TLambda nm0 (instantiate (S n) bod))).
    constructor.
    + apply H0. assumption.
