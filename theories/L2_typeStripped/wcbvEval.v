...
| wLam: forall nm bod, (* Crct p 1 bod -> *)
          WcbvEval p (TLambda nm bod) (TLambda nm bod)
| wProd: forall nm bod, (* Crct p 1 bod -> *)
...
| wInd: forall i, (* Crct p 0 prop -> *) WcbvEval p (TInd i) (TInd i) 
| wSort: forall srt, (* Crct p 0 prop -> *) WcbvEval p (TSort srt) (TSort srt)
| wFix: forall dts m, (* CrctDs p (dlength dts) dts -> *)
          WcbvEval p (TFix dts m) (TFix dts m)
| wAx: (* Crct p 0 prop -> *) WcbvEval p TAx TAx
...
| wAppFixCong: forall dts m (fn arg arg' x t t':Term)
...
Proof.
  ...
  - specialize (H _ e1). specialize (H0 _ e4). specialize (H1 _ e6).
    ...
    refine (wAppFixCong H e2 _ H0 e5 H1).
...
Lemma pre_WcbvEval_wcbvEval:
  ...
  - destruct H, H0. exists (S (max x x0)). intros mx h.
    ...
    destruct fn'; try reflexivity; repeat (try (apply f_equal; assumption)).
    + elim n; auto.
    + elim n0; auto.
  - destruct H, H0, H1. exists (S (max x0 (max x1 x2))). intros mx h.
    assert (j1:= max_fst x0 (max x1 x2)). 
