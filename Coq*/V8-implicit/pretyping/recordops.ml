...
type obj_typ = {
  ...
  o_IMPS : impl list; (* implicit args of o_TABS *)
...
let compute_canonical_projections (con,ind) =
  ...
  let lty = List.rev (List.map(fun(_,_,ty)->ty)lt) in
  let imps = List.rev (List.map (fun(_,imp,_)->imp) lt) in
  let (_,args,_) = decompose_app t in
  ...
  List.map (fun (refi,c,argj) ->
    (refi,c),
    {o_DEF=v; o_TABS=lty; o_IMPS=imps; o_TPARAMS=params; o_TCOMPS=argj})
    comp
