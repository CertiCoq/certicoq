...
let app f v imps = mkApp(Lazy.force f,v,imps)
let appex f v = app f v (Array.map (fun _ -> Term.Expl) v)

(* \subsection{Oformula vers COQ reel} *)

let rec coq_of_formula env t =
  let rec loop = function
  | Oplus (t1,t2) -> appex Z.plus [| loop t1; loop t2 |]
  | Oopp t -> appex Z.opp [| loop t |]
  | Omult(t1,t2) -> appex Z.mult [| loop t1; loop t2 |]
  | Oint v ->   Z.mk v
  | Oufo t -> loop t
  | Oatom var ->
  ...
  | Ominus(t1,t2) -> appex Z.minus [| loop t1; loop t2 |] in
  loop t
...
let rec reified_of_formula env = function
  | Oplus (t1,t2) ->
      appex coq_t_plus
        [| reified_of_formula env t1; reified_of_formula env t2 |]
  | Oopp t ->
      appex coq_t_opp [| reified_of_formula env t |]
  | Omult(t1,t2) ->
      appex coq_t_mult
        [| reified_of_formula env t1; reified_of_formula env t2 |]
  | Oint v ->  appex coq_t_int [| Z.mk v |]
  | Oufo t -> reified_of_formula env t
  | Oatom i -> appex coq_t_var [| mk_nat (reified_of_atom env i) |]
  | Ominus(t1,t2) ->
      appex coq_t_minus
        [| reified_of_formula env t1; reified_of_formula env t2 |]
...
let rec reified_of_proposition env = function
    Pequa (_,{ e_comp=Eq; e_left=t1; e_right=t2 }) -> 
      appex coq_p_eq [| reified_of_formula env t1; reified_of_formula env t2 |]
  ...
  | Pequa(_,{ e_comp=Gt; e_left=t1; e_right=t2 }) -> 
      appex coq_p_gt [| reified_of_formula env t1; reified_of_formula env t2 |]
  | Pequa(_,{ e_comp=Lt; e_left=t1; e_right=t2 }) ->  
      appex coq_p_lt [| reified_of_formula env t1; reified_of_formula env t2 |]
  ...
  | Pnot t -> 
      appex coq_p_not [| reified_of_proposition env t |]
  | Por (_,t1,t2) -> 
      appex coq_p_or
  ...
  | Pimp(_,t1,t2) -> 
      appex coq_p_imp
	[| reified_of_proposition env t1; reified_of_proposition env t2 |]
  | Pprop t -> appex coq_p_prop [| mk_nat (add_prop env t) |]
...
let reified_of_omega env body constant = 
  let coeff_constant = 
    appex coq_t_int [| Z.mk constant |] in
  let mk_coeff {c=c; v=v} t =
    let coef = 
      appex coq_t_mult 
	[| reified_of_formula env (unintern_omega env v); 
	   appex coq_t_int [| Z.mk c |] |] in
    appex coq_t_plus [|coef; t |] in
...
and oproposition_of_constr env ((negated,depends,origin,path) as ctxt) gl c = 
  match Z.parse_rel gl c with 
    ...
    | Riff (t1,t2) ->
	binprop env ctxt negated negated gl 
	  (fun i x y -> Pand(i,x,y))
          (Term.mkArrow Term.Expl t1 t2) (Term.mkArrow Term.Expl t2 t1)
    | _ -> Pprop c
...
let add_stated_equations env tree = 
  ...
  let add_env st = 
    ...
    let term_to_generalize =
      app coq_refl_equal [|Lazy.force Z.typ; coq_v|][|Term.Impl;Term.Impl|] in
...
let really_useful_prop l_equa c =
  let rec real_of = function
      Pequa(t,_) -> t
    | Ptrue ->  appex  coq_True [||]
    | Pfalse -> appex  coq_False [||]
    | Pnot t1 -> appex coq_not [|real_of t1|]
    | Por(_,t1,t2) -> appex coq_or [|real_of t1; real_of t2|]
    | Pand(_,t1,t2) -> appex coq_and [|real_of t1; real_of t2|]
    (* Attention : implications sur le lifting des variables à comprendre ! *)
    | Pimp(_,t1,t2) -> Term.mkArrow Term.Expl (real_of t1) (real_of t2)
    | Pprop t -> t in
  let rec loop c = 
    match c with 
      Pequa(_,e) ->
...
let mkAppEx(f,args) = mkApp(f,args,Array.map(fun _->Term.Expl)args)
...
let replay_history env env_hyp =
  let rec loop env_hyp t =
    match t with
	  ...
	  mkAppEx (Lazy.force coq_s_contradiction,
		 [| trace ; mk_nat (get_hyp env_hyp e1.id); 
		    mk_nat (get_hyp env_hyp e2.id) |])
      | DIVIDE_AND_APPROX (e1,e2,k,d) :: l ->
	  mkAppEx (Lazy.force coq_s_div_approx,
		 [| Z.mk k; Z.mk d; 
	  ...
	  let e2_body = map_eq_linear (fun c -> c / k) e1.body in
          mkAppEx (Lazy.force coq_s_not_exact_divide,
		 [|Z.mk k; Z.mk d; 
	  ...
	  let e2_constant = floor_div e1.constant k in
          mkAppEx (Lazy.force coq_s_exact_divide,
		 [|Z.mk k; 
		   reified_of_omega env e2_body e2_constant; 
		   mk_nat (List.length e2_body);
		   loop env_hyp l; mk_nat (get_hyp env_hyp e1.id)|])
	  ...
	  let n1 = get_hyp env_hyp e1.id and n2 = get_hyp env_hyp e2 in
          mkAppEx (Lazy.force coq_s_merge_eq,
	  ...
	  let trace = shuffle_path k1 e1.body k2 e2.body in
          mkAppEx (Lazy.force coq_s_sum,
		 [| Z.mk k1; mk_nat n1; Z.mk k2;
		    mk_nat n2; trace; (loop (CCEqua e3 :: env_hyp) l) |])
      | CONSTANT_NOT_NUL(e,k) :: l -> 
          mkAppEx (Lazy.force coq_s_constant_not_nul,
		 [|  mk_nat (get_hyp env_hyp e) |])
      | CONSTANT_NEG(e,k) :: l ->
          mkAppEx (Lazy.force coq_s_constant_neg,
		 [|  mk_nat (get_hyp env_hyp e) |])
	  ...
	  mkAppEx (Lazy.force coq_s_state,
		 [| Z.mk m; trace; mk_nat n1; mk_nat n2; 
		    loop (CCEqua new_eq.id :: env_hyp) l |])
      |	HYP _ :: l -> loop env_hyp l
      |	CONSTANT_NUL e :: l ->
	  mkAppEx (Lazy.force coq_s_constant_nul, 
		 [|  mk_nat (get_hyp env_hyp e) |])
      |	NEGATE_CONTRADICT(e1,e2,true) :: l ->
	  mkAppEx (Lazy.force coq_s_negate_contradict, 
		 [|  mk_nat (get_hyp env_hyp e1.id);
		     mk_nat (get_hyp env_hyp e2.id) |])
      |	NEGATE_CONTRADICT(e1,e2,false) :: l ->
	  mkAppEx (Lazy.force coq_s_negate_contradict_inv, 
		 [|  mk_nat (List.length e2.body); 
		     mk_nat (get_hyp env_hyp e1.id);
		     mk_nat (get_hyp env_hyp e2.id) |])
	  ...
	  mkAppEx (Lazy.force coq_s_split_ineq, 
...
and decompose_tree_hyps trace env ctxt = function
    [] -> appex coq_e_solve [| replay_history env ctxt trace |]
  | (i::l) -> 
      let equation =
	...
	decompose_tree_hyps trace env 
      ...
      appex coq_e_extract [|mk_nat index;
...
let resolution env full_reified_goal systems_list =
  ...
  let l_reified_stated =
     List.map (fun (_,_,(l,r),_) -> 
                 appex coq_p_eq [| reified_of_formula env l; 
  ...
  let reified = 
    appex coq_interp_sequent 
       [| reified_concl;env_props_reified;env_terms_reified;reified_goal|] in
  let normalize_equation e = 
    let rec loop = function
	[] -> appex (if e.e_negated then coq_p_invert else coq_p_step)
	          [| e.e_trace |]
      |	((O_left | O_mono) :: l) -> appex coq_p_left [| loop l |]
      |	(O_right :: l) -> appex coq_p_right [| loop l |] in
    let correct_index = 
    ...
    in 	
    appex coq_pair_step [| mk_nat correct_index; loop e.e_origin.o_path |] in
  ...
  let gen_list = l_generalize_arg @ List.map Term.mkVar (List.tl l_hyps) in
  Tactics.generalize gen_list (List.map (fun _->Term.Expl)gen_list) >>
  Tactics.change_in_concl None reified >> 
  Tactics.apply
    (appex coq_do_omega [|decompose_tactic; normalization_trace|]) >>
  show_goal >>
