...
(* $Id: logic.ml 9868 2007-05-29 10:01:06Z herbelin $ *)
...
let remove_hyp_body env sigma id =
  let sign =
	...
	| Some c ->(id,imp,None,t))
      (fun (id',_,c,t as d) sign ->
	(if !check then
	  begin 
...
let move_after with_dep toleft (left,(idfrom,_,_,_ as declfrom),right) hto =
  let env = Global.env() in
  let test_dep (hyp,_,c,typ as d) (hyp2,_,c,typ2 as d2) =
    if toleft
...
let impl_hyps imp hyps =
  if imp=Expl then hyps else set_impl_named_val Expl hyps

let rec mk_refgoals sigma goal goalacc conclty trm imp =
  let env = evar_env goal in
  let hyps = goal.evar_hyps in
  let mk_goal imp hyps concl =
    let hyps = impl_hyps imp hyps in
    mk_goal hyps concl goal.evar_extra in
(*
...
*)
  match kind_of_term trm with
	...
	(mk_goal imp hyps (nf_betaiota conclty))::goalacc, conclty

    | Cast (t,_, ty) ->
	check_typability env sigma ty;
	check_conv_leq_goal env sigma trm ty conclty;
	mk_refgoals sigma goal goalacc ty t imp

    | App (f,l,imps) ->
        let imps = Array.map (fun i -> impl_merge [imp;i]) imps in
	let (acc',hdty) =
	  match kind_of_term f with
	    ...
	    | _ -> 
		mk_hdgoals sigma goal goalacc f imp
	in
	let (acc'',conclty') =
	  mk_arggoals sigma goal acc' hdty
            (Array.to_list l) (Array.to_list imps) in
	check_conv_leq_goal env sigma trm conclty' conclty;
        (acc'',conclty')

    | Case (_,p,c,lf) ->
	let (acc',lbrty,conclty') =
          mk_casegoals sigma goal goalacc p c imp in
	check_conv_leq_goal env sigma trm conclty' conclty;
	let acc'' = 
	  array_fold_left2
            (fun lacc ty fi -> fst (mk_refgoals sigma goal lacc ty fi imp))
            acc' lbrty lf 
	in
	(acc'',conclty')
...
and mk_hdgoals sigma goal goalacc trm imp =
  let env = evar_env goal in
  let hyps = goal.evar_hyps in
  let mk_goal imp hyps concl =
    let hyps = impl_hyps imp hyps in
  ...
  match kind_of_term trm with
	...
	check_typability env sigma ty;
	(mk_goal imp hyps (nf_betaiota ty))::goalacc,ty
	...
	let (acc',hdty) = 
	  ...
	  else mk_hdgoals sigma goal goalacc f imp
	in
	mk_arggoals sigma goal acc' hdty (Array.to_list l) (Array.to_list imps)

    | Case (_,p,c,lf) ->
	let (acc',lbrty,conclty') = mk_casegoals sigma goal goalacc p c imp in
	let acc'' = 
	  array_fold_left2
            (fun lacc ty fi -> fst (mk_refgoals sigma goal lacc ty fi imp))
            acc' lbrty lf 
...
and mk_arggoals sigma goal goalacc funty allargs imps =
  match allargs,imps with
  | [],[] -> goalacc,funty
  | harg::tlargs, imp::tlimps ->
      let t = whd_betadeltaiota (evar_env goal) sigma funty in
      (match kind_of_term t with
	| Prod (_,_,c1,b) ->
	    let (acc',hargty) =
              mk_refgoals sigma goal goalacc c1 harg imp in
	    mk_arggoals sigma goal acc' (subst1 harg b) tlargs tlimps
	| LetIn (_,_,c1,_,b) ->
	    mk_arggoals sigma goal goalacc (subst1 c1 b) allargs imps
	| _ -> raise (RefinerError (CannotApply (t,harg))))
  | _ -> assert false

and mk_casegoals sigma goal goalacc p c imp =
  let env = evar_env goal in
  let (acc',ct) = mk_hdgoals sigma goal goalacc c imp in 
  let (acc'',pt) = mk_hdgoals sigma goal acc' p Impl in
...
let convert_hyp sign sigma (id,b,bt) =
  apply_to_hyp sign id
    (fun _ (_,imp,c,ct) _ ->
...
let illegal_implicit_vars sign c =
  let hyps = named_context_of_val sign in
  let vars = ref Idset.empty in
  let check id =
    if not (is_expl_named hyps id) then
      vars := Idset.add id !vars in
  let _ =
    extract_closed_gen (fun _ -> true) (fun id -> check id; true) c in
  Idset.elements !vars
...
let prim_refiner r sigma goal =
  ...
  match r with
    ...
    | Intro id ->
	  ...
	  error "New variable is already declared";
	   ...
	   | Prod (_,imp,c1,b) ->
	       ...
	       let sg =
                 mk_goal 
                   (push_named_context_val (id,imp,None,c1) sign)
		   (subst1 (mkVar id) b) in
	       ([sg], sigma)
	   | LetIn (_,imp,c1,t1,b) ->
	       if occur_meta c1 or occur_meta t1 then error_use_instantiate();
               if imp=Expl && not(noccur_implicit sign c1) then
                 error"Intro-let contains implicit variable";
	       let sg =
		 mk_goal (push_named_context_val (id,imp,Some c1,t1) sign)
		   (subst1 (mkVar id) b) in
	       ([sg], sigma)
	   | _ ->
	...
	(match kind_of_term (strip_outer_cast cl) with
           | Prod (_,imp,c1,b) ->
	       if occur_meta c1 then error_use_instantiate();
	       let sign' = replace_hyp sign id (id,imp,None,c1) cl in
	       ...
	       if occur_meta c1 then error_use_instantiate();
               if imp=Expl && not(noccur_implicit sign c1) then
                 error"Intro-let contains implicit variable";
	       let sign' = replace_hyp sign id (id,imp,Some c1,t1) cl in
	       let sg = mk_goal sign' (subst1 (mkVar id) b) in
	       ([sg], sigma)
	   | _ ->
	       raise (RefinerError IntroNeedsProduct))
    ...
    | Cut (b,id,imp,t) ->
	  ...
	  error "New variable is already declared";
        ...
        let sign1 = impl_hyps imp sign in
        let sg1 = mk_goal sign1 (nf_betaiota t) in
        let sg2 = mk_goal (push_named_context_val (id,imp,None,t) sign) cl in
     	...
     	let rec check_ind env k cl = 
          match kind_of_term (strip_outer_cast cl) with 
            | Prod (na,imp,c1,b) -> 
            	if k = 1 then 
                  (if imp=Impl then
                    error "cannot do a fixpoint on an implicit argument";
		  try 
		    let(ind,_,_) = find_inductive env sigma c1 in ind
		  with Not_found -> 
		    error "cannot do a fixpoint on a non inductive type")
            	else 
		  check_ind (push_rel (na,imp,None,c1) env) (k-1) b
	...
	let all = (f,n,cl)::rest in
	  ...
	  | (f,n,ar)::oth ->
		...
		error "name already used in the environment";
	      mk_sign (push_named_context_val (f,Expl,None,ar) sign) oth
	  | [] -> 
    ...
    | Cofix (f,others) ->
	  ...
	  let b = whd_betadeltaiota env sigma cl in
          match kind_of_term b with
            | Prod (na,imp,c1,b) ->
                check_is_coind (push_rel (na,imp,None,c1) env) b
            | _ -> 
		try 
    ...
    | Refine c ->
        ...
        if not (noccur_implicit sign c) then
          (let ids = illegal_implicit_vars sign c in
           errorlabstrm "Logic.prim_refiner"
             (str"implicit variable(s) " ++
              prlist_with_sep spc pr_id ids ++ spc() ++
              str "in extraction of body"));
	let (sgl,cl') = mk_refgoals sigma goal [] cl c Expl in
    ...
    match pft.ref with
	  ...
	  (match kind_of_term (strip_outer_cast cl) with
		...
		let cty = subst_proof_vars vl ty in
		  mkLambda (Name id, imp,cty, subfun (add_proof_var id vl) spf)
	    | LetIn (_,imp,b,ty,_) ->
		let cb = subst_proof_vars vl b in
		let cty = subst_proof_vars vl ty in
		mkLetIn
                  (Name id, imp,cb, cty, subfun (add_proof_var id vl) spf)
      ...
      | Some (Prim (Cut (b,id,imp,t)),[spf1;spf2]) ->
          let spf1, spf2 = if b then spf1, spf2 else spf2, spf1 in
	    mkLetIn (Name id,imp,subfun vl spf1,subst_proof_vars vl t,
