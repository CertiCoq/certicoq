...
| aConst: forall (s:string) (t:Term),
            LookupDfn s p t -> awndEval p (TConst s) t
| aBeta: forall (nm:name) (ty bod arg:Term) (args:Terms),
           awndEval p (TApp (TLambda nm ty bod) arg args)
...
| aCase0: forall (n:nat) (ty s:Term) (i:inductive) (brs:Terms),
            ...
            awndEval p (TCase 0 ty (TConstruct i n) brs) s
...
Definition no_awnds_step (p:environ) (ts:Terms) : Prop :=
  no_step (awndEvals p) ts.

Lemma awndEval_tappendl:
  forall p bs cs, awndEvals p bs cs ->
  forall ds, awndEvals p (tappend bs ds) (tappend cs ds).
Proof.
  induction 1; intros.
...
Lemma wndEval_tappendr:
  forall p bs cs, awndEvals p bs cs ->
  forall ds, awndEvals p (tappend ds bs) (tappend ds cs).
Proof.
...
Lemma awndEval_Lam_inv:
  forall p nm tp bod s,
    awndEval p (TLambda nm tp bod) s ->
    exists tp', awndEval p tp tp' /\ s = (TLambda nm tp' bod).
...
Lemma awndEval_Prod_inv:
  forall p nm tp bod s,
    awndEval p (TProd nm tp bod) s ->
    exists tp', awndEval p tp tp' /\ s = (TProd nm tp' bod).
intros p nm tp bod s h. inversion_Clear h.
- exists t2. split; [assumption | reflexivity].
Qed.
...
Lemma awndEval_Cast_inv:
  forall p tm ck ty s,
    awndEval p (TCast tm ck ty) s -> tm = s.
...
Proof. 
  ...
  - inversion H. subst. eapply aCasen; eassumption.
  - destruct (WFapp_mkApp_WFapp H0 _ _ eq_refl).
    apply awndEval_mkApp_mkApp; try assumption.
    + apply H; assumption.
Qed.
...
Proof. 
  intros p hp. apply awndEvalEvals_ind; intros; try (constructor; assumption);
              try (inversion H0; subst; constructor; apply H; assumption).
  - inversion H. subst. eapply sCasen; eassumption.
  - inversion H0. subst. rewrite <- (mkApp_goodFn _ _ H4). apply sAppFn.
    apply H. assumption.
Qed.
