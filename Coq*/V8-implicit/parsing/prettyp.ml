...
(* $Id: prettyp.ml 9864 2007-05-28 20:14:48Z herbelin $ *)
...
type object_pr = {
  ...
  print_named_decl          : named_declaration -> std_ppcmds;
...
let pr_implicit imp = pr_id (fst(name_of_implicit imp))
...
let print_ref reduce ref =
  ...
  let typ = 
    if reduce then
      let ctx,ccl =
        Reductionops.splay_prod_assum (Global.env()) Evd.empty typ
...
let need_expansion impl ref = 
  ...
  let nprods = List.length (List.filter (fun (_,_,b,_) -> b=None) ctx) in
...
let opacity env = function
  | VarRef v when let(_,_,b,_)=Environ.lookup_named v env in b <> None -> 
...
let print_named_def name imp body typ =
  let pbody = pr_lconstr body in
  let ptyp = pr_ltype typ in
  let pbody = if isCast body then surround pbody else pbody in
  (str "*** [" ++ Ppconstr.pr_impl imp (str name) ++ str " " ++
     hov 0 (str ":=" ++ brk (1,2) ++ pbody ++ spc () ++
	      str ":" ++ brk (1,2) ++ ptyp) ++
	   str "]" ++ fnl ())

let print_named_assum name imp typ =
  (str "*** [" ++ Ppconstr.pr_impl imp (str name) ++
   str " : " ++ pr_ltype typ ++ str "]" ++ fnl ())

let gallina_print_named_decl (id,imp,c,typ) =
  let s = string_of_id id in
  match c with
    | Some body -> print_named_def s imp body typ
    | None -> print_named_assum s imp typ
...
let build_inductive sp tyi =
  ...
  let args,_ = extended_rel_list 0 params in
  let env = Global.env() in
  let fullarity = match mip.mind_arity with
    | Monomorphic ar -> ar.mind_user_arity 
    | Polymorphic ar ->
	it_mkProd_or_LetIn (mkSort (Type ar.poly_level))
...
let get_fields =
  let rec prodec_rec l subst c =
	...
	prodec_rec ((id,imp,true,substl subst t)::l) (mkVar id::subst) c
    | LetIn (na,imp,b,_,c) ->
	let id = match na with Name id -> id | Anonymous -> id_of_string "_" in
	prodec_rec ((id,imp,false,substl subst b)::l) (mkVar id::subst) c
    | _               -> l
  in 
  prodec_rec [] []
...
let print_name ref = 
  ...
  try  (* Var locale de but, pas var de section... donc pas d'implicits *)
    ...
    let (_,imp,c,typ) = Global.lookup_named str in 
    (print_named_decl (str,imp,c,typ))
  with Not_found ->
  try 
...
let rec dest_lams acc c =
  match kind_of_econstr c with
      ELambda(x,b) -> dest_lams (x::acc) b
    | _ -> List.rev acc, c
let rec dest_prods acc c =
  match kind_of_econstr c with
      EProd(x,imp,a,b) -> dest_prods ((x,imp,a)::acc) b
    | _ -> List.rev acc, c
...
let push_name x names = x::names

let push_names l names =
  List.fold_left (fun names x -> push_name x names) names l
...
let new_name names x =
  match x with
      Anonymous -> x
    | Name id -> Name(next_ident_away id (ids names))

let new_names names nal =
  let (names',nalrev) =
    Array.fold_left(fun (names,nal) x -> 
      let x' = new_name names x in
      (x'::names, x'::nal)) names nal in
  (names',Array.of_list(List.rev nalrev))

let rec rename names c = fold_econstr (
  match kind_of_econstr c with
  | EProd(x,imp,a,b) ->
      let x' = new_name names x in
      EProd(x',imp,rename names a, rename (x'::names) b)
  | ELambda(x,b) ->
      let x' = new_name names x in
      ELambda(x', rename (x'::names) b)
  | ELetIn(x,a,b) ->
      let x' = new_name names x in
      ELetIn(x',rename names a, rename (x'::names) b)
  | EApp(f,v) -> EApp(rename names f, Array.map (rename names) v)
  | ECase(i,b,br) ->
      let rename_branch (args,c) =
        let (names',nal) = new_names (names,[]) args in
        (nal, rename names' c) in
      ECase(i,rename names b, Array.map rename_branch br)
  | EFix(f,(na,bd)) ->
      let (names',nal) = new_names (names,[]) na in
      EFix(f,(nal,Array.map(rename names') bd))
  | ECoFix(f,(na,bd)) ->
      let (names',nal) = new_names (names,[]) na in
      ECoFix(f,(nal,Array.map(rename names') bd))
  | c -> c)


let constr_num = ref false

let pr_construct (_,i as c) =
  if !constr_num then
    (str "#" ++ int (i-1) ++
     str"(*" ++ pr_constructor (Global.env()) c ++ str "*)")
  else
     pr_constructor (Global.env()) c

let rec print_extr ilev names c =
  let (lev,strm) =
  match kind_of_econstr c with 
      ERel n -> latom, pr_name (List.nth names (n-1))
    | EVar id -> latom, pr_id id
    | EMeta mv -> latom, str "?" ++ int mv
    | EEvar (k,_) -> latom, str "?" ++ int k
    | ESort s -> latom, pr_sort s
    | EProd _ ->
        let (bnds,b) = dest_prods [] c in
        let (rbnds,names') = List.fold_left
          (fun (rb,ns) (na,_,_ as b) -> ((ns,b)::rb,na::ns))
          ([],names) bnds in
        let bnds = List.rev rbnds in
        let pr_bnd (names,(x,imp,a)) =
          let bnd = pr_name x ++ str":" ++ print_extr ltop names a in
          if imp=Impl then str"["++bnd++str"]" else surround bnd in
        let bnds = hov 0 (prlist_with_sep spc pr_bnd bnds) in
        lprod, hov 2
          (str"forall" ++ spc() ++ bnds ++ str"," ++ spc() ++
          print_extr lprod names' b)
    | ELambda _ ->
        let (nal,b) = dest_lams [] c in
        let names' = push_names nal names in
        labs, hov 2
          (str "fun " ++ prlist_with_sep(fun _ -> str" ") pr_name nal ++
          str " =>" ++ spc() ++ print_extr labs names' b)
    | ELetIn (x,a,b) ->
        let names' = push_name x names in
        llet,hov 2
          (str "let " ++ pr_name x ++ str " :=" ++ spc() ++
          print_extr ltop names a ++ str " in" ++ spc() ++
          print_extr llet names' b)
    | EApp(f,args) ->
        lapp, hov 2
          (print_extr lapp names f ++ spc() ++
          prvect_with_sep spc
            (print_extr (lapp-1) names) args)
(*        lapp, hov 2
          (print_extr lapp names f ++ str"(" ++
          prvect_with_sep (fun _ -> str","++cut())
            (print_extr ltop names) args) ++ str")"*)
    | EConst c -> latom, pr_constant (Global.env()) c
    | EInd i -> latom, pr_inductive (Global.env()) i
    | EConstruct c ->
        latom, hov 2 (pr_construct c)
    | ECase(_,b,[|(args,c)|]) ->
        let names' = push_names (Array.to_list args) names in
        llet, hov 0
          (str "let (" ++
          prvect_with_sep (fun _ -> str",") pr_name args ++
          str ") :=" ++ brk(1,2) ++
          print_extr ltop names b ++ str " in" ++ spc() ++
          print_extr llet names' c)
    | ECase(ind,b,br) ->
        latom, hov 0 (
          hov 0 (str "match " ++ print_extr ltop names b ++ str" with") ++
          fnl() ++
          prvecti (fun i (args,c) ->
            let names' = push_names (Array.to_list args) names in
            str"| " ++ pr_construct (ind,i+1) ++
            prvect (fun na -> str" "++pr_name na) args ++
            str" => " ++ print_extr ltop names' c ++ fnl()) br ++ str "end")
    | EFix((rv,k),(na,bd)) ->
        let names' = push_names(Array.to_list na) names in
        labs, hov 0 (prvecti (fun i b ->
          let (nal,b) = dest_lams [] b in
          let names'' = push_names nal names' in
          (if i=0 then str"fix " else (fnl()++str"with ")) ++
          pr_name na.(i) ++ str "/" ++ int (rv.(i)+1) ++
          prlist (fun na -> str" "++pr_name na) nal ++ str " :=" ++
          brk(1,2) ++ print_extr labs names'' b) bd ++
        (if Array.length na = 1 then mt()
        else (fnl() ++ str "for " ++ pr_name na.(k))))
    | ECoFix (k,(na,bd)) ->
        let names' = push_names(Array.to_list na) names in
        labs, hov 0 (prvecti (fun i b ->
          (if i=0 then str"cofix " else (fnl()++str"with ")) ++
          pr_name na.(i) ++ str " :=" ++
          brk(1,2) ++ print_extr labs names' b) bd ++
        (if Array.length na = 1 then mt()
        else (fnl() ++ str "for " ++ pr_name na.(k)))) in
  if lev>ilev then surround strm else strm

let pr_extr c = print_extr ltop [] (rename [] c)
...
let print_opaque_name qid = 
  ...
  match global qid with
    ...
    | VarRef id ->
        let (_,imp,c,ty) = lookup_named id env in 
	print_named_decl (id,imp,c,ty)
...
let unfold_head_fconst = 
  let rec unfrec k = match kind_of_term k with
    ...
    | Lambda (na,imp,t,b) -> mkLambda (na,imp,t,unfrec b)
    | App (f,v,imps) -> appvect (unfrec f,v,imps)
    | _ -> k
  in 
  unfrec
