...
Module ClosureConversionCorrect (H : Heap).
  ...
  Proof.
    intros (Hwf & vs & l' & Hget1 & Hget2 & Hall) Hnd Hget1'. subst_exp.
  ...
  Proof.
    ...
    edestruct Hfun as (l1 & lenv & B1 & g1 & rhoc & B2 & g2 & Hget1 & Hsub & Hdis & Hget2 & Hget3 & Hget4 & Henv & Heq).
    eassumption. eassumption.  
  ...
  Proof.
    intros Hres l [x1 [Hgetx1 Heq1]].
    destruct (M.get x1 rho1) as [ [|] | ] eqn:Hgety; try inv Heq1.
    edestruct (Hres 0) as (l1 & lenv & B1 & g1 & rhoc & B2 & g2 & Hget1 & Hsub & Hdis & Hget2 & Hget3 & Hget4 & Henv & Heq).
  ...
  Proof.
    ...
    destruct (M.get x1 rho1) as [ [|] | ] eqn:Hgety; try inv Heq1.
    edestruct (Hres (1 + n))
      as (l1 & lenv & B1 & g1 & rhoc & B2 & g2 & Hget1 & Hsub & Hdis & Hget2 & Hget3 & Hget4 & Henv & Heq).
  ...
  Proof.
    ...
    edestruct (Hres 1) as (l1 & lenv & B1 & g1 & rhoc & B2 & g2 & Hget1 & Hsub & Hdis & Hget2 & Hget3 & Hget4 & Henv & Heq).
  ...
  Proof with (now eauto with Ensembles_DB).
    ...
    edestruct (Hfun j) as (l1 & lenv & B1 & g1 & rhoc & B2 & g2 & Hget1 & Hdis & Hsub & Hget2 & Hget3 & Hget4 & Henv & Heq);
      try eassumption.
    do 7 eexists; repeat split; try eassumption.
  ...
  Proof. 
    ...
    destruct (M.get f rho1) as [[l1|] |] eqn:Hgetf; try contradiction.
    inv Hget. inv Hnin.
    edestruct Hfun as
        (l1 & lenv & B1 & g1 & rhoc & B2 & g2 & Hget1 & Hdis & Hsub &
            Hget2 & Hget3 & Hget4 & Henv & Heq); try eassumption.
    eapply Hsub. repeat subst_exp. eassumption. 
  Qed.
  ...
  Lemma project_var_correct GII GI k  H1 rho1 H2 rho2 H2' rho2' b d
    ...
    Disjoint _ (image b (reach' H1 (env_locs rho1 (Scope :|: (FromList FVs \\ (Scope :|: Funs))))))
             (env_locs rho2 (image fenv (Funs \\ Scope)) :|: image' d (reach' H1 (env_locs rho1 (Scope :|: (FromList FVs \\ (Scope :|: Funs)))))) ->
  ...
  Proof with (now eauto with Ensembles_DB).
    ...
    - inv Hctx. simpl in H13. 
      ...
      edestruct (Hfun 0)
        ...
        try eassumption.
      repeat subst_exp.
      edestruct (alloc (Constr Size.Util.clo_tag [FunPtr B2 g2; Loc lenv]) H2) as [l' H2''] eqn:Ha. inv H4. 
      ...
      + intros j.
        edestruct (Hfun j)
         ...
         try eassumption.
        do 4 subst_exp.
        rewrite Ha in Heq'. eapply cc_approx_env_P_union.
      ...
      + assert (Hsub' : reach' H1
                              (env_locs rho1
                                        (x |: Scope :|: (FromList FVs \\ ((x |: Scope) :|: (Funs \\ [set x]))))) \\
                              [set l1] \subset
                       reach' H1
                              (env_locs rho1
        ...
        { rewrite <- Union_assoc, env_locs_Union, env_locs_Singleton; eauto.
          ...
          rewrite (Union_Setminus_Included (x |: Scope) Funs [set x]); tci; [| now eauto with Ensembles_DB ].
          rewrite Setminus_Same_set_Empty_set, Union_Empty_set_neut_l.
      ...
      + assert (Hsub' : reach' H1
        ...
        *  eapply Union_Disjoint_r.
          ...
          left. eapply get_In_env_locs. eexists; split; eauto. constructor; eassumption.
    ...
    - inv Hctx. inv H18.
      ...
      + assert (Hseq' : x |: Scope :|: (FromList FVs \\ (x |: Scope :|: Funs')) <-->
        ...
        rewrite !Hseq' at 1. split; eassumption.
  Qed.
  ...
  Lemma project_var_env_locs_dis (Scope Scope' Funs Funs' : Ensemble var) 
    ...
    project_var Size.Util.clo_tag Scope Funs fenv c Γ FVs x C1 Scope' Funs' ->
  ...
  Lemma project_vars_env_locs_dis (Scope Scope' Funs Funs' : Ensemble var) 
        ...
        (rho2 : env) (H2 : heap block) (m : nat) S :
    project_vars Size.Util.clo_tag Scope Funs fenv c Γ FVs x C1 Scope' Funs' ->
    ctx_to_heap_env_CC C1 H1 rho1 H2 rho2 m ->
    Disjoint _ S (Scope' \\ Scope) ->
    env_locs rho1 S <--> env_locs rho2 S.
  Admitted.  
  ...
  Proof with (now eauto with Ensembles_DB).
    ...
    - edestruct ctx_to_heap_env_CC_comp_ctx_f_l as [rho3 [H3 [m1 [m2 [Hctx2 [Hctx3 Heq]]]]]].
      ...
      + eapply Disjoint_Included_r; [| eassumption ].
        ...
        eapply project_var_Funs_l. eassumption.
        eapply project_var_Scope_l. eassumption.
        eapply project_var_env_locs_dis. eassumption.
        eassumption. eapply Disjoint_Included; [| | eapply Hdis ].
        erewrite project_var_FV_eq; try eassumption.
        unfold FV...
        now eauto with Ensembles_DB. 
      + erewrite <- project_var_FV_eq; try eassumption.
