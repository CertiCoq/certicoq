...
(* $Id: g_tactic.ml4 10007 2007-07-16 09:18:44Z corbinea $ *)
...
let mk_fix_tac (loc,id,bl,ann,ty) =
  let n =
    match bl,ann with
        [([_],_)], None -> 1
      | _, Some x ->
          let ids = List.map (fun ((_,na),_) -> na)
            (List.flatten (List.map fst bl)) in
...
GEXTEND Gram
  ...
  ;
  constr_impl:
    [ [ c = constr -> (Term.Expl,c)
      | "["; c = lconstr; "]" -> (Term.Impl,c) ] ]
  ;
  red_flag:
    [ [ IDENT "beta" -> FBeta
  ...
  simple_tactic:
    [ [ 
	  ...
	  TacLetTac (Names.Name id,Term.Expl,b,nowhere)
      | IDENT "pose"; b = constr ->
	  TacLetTac (Names.Anonymous,Term.Expl,b,nowhere)
      | IDENT "set"; id = lpar_id_coloneq; c = lconstr; ")"; p = clause ->
	  TacLetTac (Names.Name id,Term.Expl,c,p)
      | IDENT "set"; c = constr; p = clause ->
          TacLetTac (Names.Anonymous,Term.Expl,c,p)
      ...
      | IDENT "cut"; (imp,c) = constr_impl -> TacCut (imp,c)
      | IDENT "generalize"; lc = LIST1 constr_impl ->
          let (imps,lc) = List.split lc in
          TacGeneralize(imps,lc)
      | IDENT "generalize"; IDENT "dependent"; (imp,c) = constr_impl ->
          TacGeneralizeDep (imp,c)

      | IDENT "specialize"; n = OPT natural; lcb = constr_with_bindings ->
	  TacSpecialize (n,lcb)
      | IDENT "lapply"; (imp,c) = constr_impl -> TacLApply (imp,c)
