...
let make_options (l : command_args list) (pr : (Kernames.kername * Kernames.ident) list) (fname : string) : options =
...
let quote opts gr =
  ...
  let _ = Feedback.msg_info (Pp.str "Calling quoting in certicoqc") in
  let term = Metacoq_template_plugin.Ast_quoter.quote_term_rec ~bypass:false ~with_universes:false env (EConstr.to_constr sigma c) in
  let time = (Unix.gettimeofday() -. time) in
  debug_msg debug (Printf.sprintf "Finished quoting in %f s.. compiling to L7." time);
  term

(* Compile Quoted term with the bootstrapped CertiCoq *)
let compile opts term imports =
  let debug = opts.debug in
  let options = make_pipeline_options opts in
  let _ = Feedback.msg_info (Pp.str"Calling certicoq pipeline") in
  let p = Certicoq_pipeline.compile options term in
  let _ = Feedback.msg_info (Pp.str"Certicoq pipeline returned") in
  match p with
