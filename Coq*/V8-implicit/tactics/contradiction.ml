...
(* $Id: contradiction.ml 9868 2007-05-29 10:01:06Z herbelin $ *)
...
let absurd c gls =
  ...
  (tclTHENS
     (tclTHEN (elim_type (build_coq_False ())) (cut Expl c)) 
     ([(tclTHENS
          (cut Expl (applist(build_coq_not (),[c],[Expl]))) 
	  ([(tclTHEN intros
	       ((fun gl ->
		   let ida = pf_nth_hyp_id gl 1
                   and idna = pf_nth_hyp_id gl 2 in
                   exact_no_check (applist(mkVar idna,[mkVar ida],[Expl])) gl)));
            tclIDTAC]));
       tclIDTAC])) gls
...
let filter_hyp f tac gl =
  let rec seek = function
    | [] -> raise Not_found
    | (id,_,_,t)::rest when f t -> tac id gl
    | _::rest -> seek rest in
  seek (pf_hyps gl)
...
let contradiction_context gl =
  ...
  let rec seek_neg l gl = match l with
	...
	if is_empty_type typ then
	  ...
	  | Prod (na,imp,t,u) when is_empty_type u ->
	      (try
		filter_hyp (fun typ -> pf_conv_x_leq gl typ t) 
		  (fun id' -> simplest_elim
                      (mkApp (mkVar id,[|mkVar id'|],[|imp|])))
		  gl
...
let is_negation_of env sigma typ t =
  match kind_of_term (whd_betadeltaiota env sigma t) with
    | Prod (na,Expl,t,u) -> is_empty_type u & is_conv_leq env sigma typ t
    | _ -> false
...
let contradiction_term (c,lbind as cl) gl =
  ...
  if is_empty_type ccl then
	...
	filter_hyp (is_negation_of env sigma typ)
	  (fun id -> simplest_elim (mkApp (mkVar id,[|c|],[|Expl|]))) gl
      else
	raise Not_found
