...
(* $Id: equality.ml 9954 2007-07-06 16:35:07Z letouzey $ *)
...
let multi_replace clause c2 c1 unsafe try_prove_eq_opt gl = 
  ...
  if unsafe or (pf_conv_x gl t1 t2) then
    ...
    let eq = applist (e, [t1;c1;c2],[Expl;Expl;Expl]) in
...
let find_positions env sigma t1 t2 =
  let rec findrec sorts posn t1 t2 =
    let hd1,args1,imps1 = whd_betadeltaiota_stack env sigma t1 in
    let hd2,args2,imps2 = whd_betadeltaiota_stack env sigma t2 in
    match (kind_of_term hd1, kind_of_term hd2) with
      ...
      | _ ->
	  let t1_0 = applist (hd1,args1,imps1) 
          and t2_0 = applist (hd2,args2,imps2) in
...
let descend_then sigma env head dirn =
  ...
  (dirn_nlams,
   ...
   (fun dirnval (dfltval,resty) ->
      ...
      let p =
	it_mkLambda_or_LetIn (lift (mip.mind_nrealargs+1) resty)
          deparsign in
      let build_branch i =
...
let discrimination_pf e (t,t1,t2) discriminator lbeq =
  ...
  (applist (eq_elim, [t;t1;mkNamedLambda e Expl t discriminator;i;t2],
            [Impl;Impl;Impl;Expl;Impl]), absurd_term)
...
let discr_positions env sigma (lbeq,(t,t1,t2)) id cpath dirn sort =
  ...
  let e_env = push_named (e,Expl,None,t) env in
  let discriminator =
    build_discriminator sigma e_env dirn (mkVar e) sort cpath in
  let (pf, absurd_term) = discrimination_pf e (t,t1,t2) discriminator lbeq in
  tclCOMPLETE 
    ((tclTHENS (cut_intro Expl absurd_term)
      [onLastHyp gen_absurdity; 
       refine (mkApp (pf,[|mkVar id|],[|Expl|]))]))
...
let onNegatedEquality tac gls =
  ...
  let eq =
    try match kind_of_term (hnf_constr (pf_env gls) (project gls) ccl) with
      | Prod (_,Expl,t,u) when is_empty_type u ->
...
let make_tuple env sigma (rterm,rty) lind =
  ...
  let (na,_,_,_) = lookup_rel lind env in
  (* We move [lind] to [1] and lift other rels > [lind] by 1 *)
  let rty = lift (1-lind) (liftn lind (lind+1) rty) in
  (* Now [lind] is [mkRel 1] and we abstract on (na:a) *)
  let p = mkLambda (na, Expl, a, rty) in
  (applist(exist_term,[a;p;(mkRel lind);rterm],[Impl;Impl;Expl;Expl]),
   applist(sig_term,[a;p],[Expl;Expl]))
...
let sig_clausal_form env sigma sort_of_ty siglen ty dflt =
  ...
  let rec sigrec_clausal_form siglen p_i =
	...
	error "Cannot solve an unification problem"
      ...
      let (a,p_i_minus_1,imps) = match whd_beta_stack p_i with
	| (_sigS,[a;p],[imp1;imp2]) -> (a,p,imp2)
 	| _ -> anomaly "sig_clausal_form: should be a sigma type" in
      let ev = Evarutil.e_new_evar isevars env Expl a in
      let rty = beta_applist(p_i_minus_1,[ev],[Expl]) in
...
let simplify_args env sigma t = 
  ...
  match decompose_app t with
    | eq, [t;c1;c2], imps ->
        applist (eq,[t;nf env sigma c1;nf env sigma c2],imps)
    | eq, [t1;c1;t2;c2], imps ->
        applist (eq,[t1;nf env sigma c1;t2;nf env sigma c2],imps)
    | _ -> t
...
let inject_at_positions env sigma (eq,(t,t1,t2)) id posns =
  ...
  let injectors =
	...
	let injfun = mkNamedLambda e Expl t injbody in
	let pf = applist(eq.congr,[t;resty;injfun;t1;t2;mkVar id],
                         [Impl;Impl;Impl;Impl;Impl;Expl]) in
  ...
  tclMAP
    (fun (pf,ty) -> tclTHENS (cut Expl ty) [tclIDTAC; refine pf])
    injectors
...
let swap_equands gls eqn =
  ...
  applist(lbeq.eq,[t;e2;e1],[Expl;Expl;Expl])

let swapEquandsInConcl gls =
  let (lbeq,(t,e1,e2)) = find_eq_data_decompose (pf_concl gls) in
  let sym_equal = lbeq.sym in
  refine
    (applist(sym_equal,[t;e2;e1;Evarutil.mk_new_meta()],[Impl;Impl;Impl;Expl]))
    gls

let swapEquandsInHyp id gls =
  cut_replacing id Expl (swap_equands gls (pf_get_hyp_typ gls id))
...
let bareRevSubstInConcl lbeq body (t,e1,e2) gls =
  ...
  let p = lambda_create (pf_env gls) (Expl,t,body) in
  (* apply substitution scheme *)
  refine (applist(eq_elim,[t;e1;p;Evarutil.mk_new_meta();
                           e2;Evarutil.mk_new_meta()],
                  [Impl;Impl;Impl;Expl;Impl;Expl])) gls
...
let decomp_tuple_term env c t = 
  let rec decomprec inner_code ex exty =
    try
      ...
      let car_code = applist (p1,[a;p;inner_code],[Impl;Impl;Expl])
      and cdr_code = applist (p2,[a;p;inner_code],[Impl;Impl;Expl]) in
      let cdrtyp = beta_applist (p,[car],[Expl]) in
...
let subst_tuple_term env sigma dep_pair b =
  ...
  let abst_B =
    List.fold_right
      (fun (e,t) body -> lambda_create env (Expl,t,subst_term e body))
      e_list b in
  applist(abst_B,proj_list,List.map(fun _ ->Expl) proj_list)
...
let cutSubstInHyp_LR eqn id gls =
  ...
  assert (dependent (mkRel 1) body);
  cut_replacing id Expl (subst1 e2 body)
...
let unfold_body x gl =
  ...
  let hl =
    List.fold_right (fun (y,_,yval,_) cl -> (([],y),InHyp) :: cl) aft [] in
  let xvar = mkVar x in
...
let subst_one x gl = 
  ...
  let (abshyps,absimps) =
    List.split
      (map_succeed
        (fun (id,imp,v,_) ->
          if v=None then (mkVar id,imp) else failwith "caught")
        depdecls) in
  ...
  let introtac = function
      (id,_,None,_) -> intro_using id
    | (id,imp,Some hval,htyp) ->
        letin_tac true (Name id) imp
