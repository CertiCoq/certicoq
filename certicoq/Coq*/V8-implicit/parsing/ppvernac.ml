...
(* $Id: ppvernac.ml 9873 2007-05-30 17:31:50Z aspiwack $ *)  
...
let rec factorize = function
  ...
  | (c,((idl,t),imp))::l ->
	...
	| ((xl,t'),imp')::l' when t' = (c,t) && imp=imp' ->
            ((idl@xl,t'),imp')::l'
	| l' -> ((idl,(c,t)),imp)::l'

let pr_ne_params_list pr_c l =
  match factorize l with
    | [p,Term.Expl] -> pr_params pr_c p
    | l ->
	prlist_with_sep spc
          (fun (p,imp) ->
            let lp,ep = if imp=Term.Impl then ("[","]") else ("(",")") in
            hov 1 (str lp ++ pr_params pr_c p ++ str ep)) l
(*
...
let rec pr_vernac = function
  ...
  | VernacAssumption (stre,_,l) ->
      let n =
        List.length (List.flatten (List.map(fun(_,((idl,_),_))->idl) l)) in
      hov 2
  ...
  | VernacFixpoint (recs,b) ->
      let name_of_binder = function
        | LocalRawAssum (nal,_) -> List.map fst nal
  ...
  | VernacRecord (b,(oc,name),ps,s,c,fs) ->
      let pr_record_field = function
        | (oc,AssumExpr (id,imp,t)) ->
            let pf =
              pr_lname id ++
              (if oc then str" :>" else str" :") ++ spc() ++
              pr_lconstr_expr t in
            if imp=Term.Impl then hov 1 (str"["++pf++str"]")
            else hov 1 pf
        | (oc,DefExpr(id,imp,b,opt)) -> (match opt with
	    | Some t ->
                let pf =
                  pr_lname id ++
                  (if oc then str" :>" else str" :") ++ spc() ++
                  pr_lconstr_expr t ++ str" :=" ++ pr_lconstr b in
                if imp=Term.Impl then hov 1 (str"["++pf++str"]")
                else hov 1 pf
	    | None ->
                let pf = pr_lname id ++ str" :=" ++ spc() ++ pr_lconstr b in
                if imp=Term.Impl then hov 1 (str"["++pf++str"]")
                else hov 1 pf) in
      hov 2
...
(* spiwack: command printing all the axioms and section variables used in a 
	...
	| PrintNeededAssumptions qid -> str"Print Assumptions"++spc()++pr_reference qid
        | PrintExtr c -> str"Print Extraction"++spc()++pr_constr c
      in pr_printable p
