...
Module IntProperties (I:Int). 
 ...
 Proof.
  intros.
  rewrite le_lt_iff.
  destruct (lt_dec n m); apply impl_PI; intuition.
 Qed.
 ...
 Proof.
  intros.
  destruct (le_is_lt_or_eq _ _ H); apply impl_PI; intuition.
 Qed.

 Lemma le_trans : forall n m p, n<=m -> m<=p -> n<=p.
 Proof.
  intros n m p; do 3 rewrite le_lt_iff; intros A B C.
  destruct (lt_eq_lt_dec p m) as [[H|H]|H]; apply impl_PI; subst; auto.
 ...
 Proof.
 intros.
 destruct (lt_eq_lt_dec n 0) as [[Hn|Hn]|Hn]; apply impl_PI; auto; 
  destruct (lt_eq_lt_dec m 0) as [[Hm|Hm]|Hm]; apply impl_PI; auto; elimtype False.
 ...
 Proof.
 intros.
 destruct (le_is_lt_or_eq _ _ H1); apply impl_PI.

 apply le_trans with (i*l).
 destruct (le_is_lt_or_eq _ _ H0); apply impl_PI; [ | subst; apply le_refl].
 apply lt_le_weak.
 apply mult_lt_compat_l; auto.

 generalize (le_trans _ _ _ H2 H0); clear H0 H1 H2; intros.
 rewrite (mult_comm i), (mult_comm j).
 destruct (le_is_lt_or_eq _ _ H0); apply impl_PI;
  [ | subst; do 2 rewrite mult_0_l; apply le_refl].
 destruct (le_is_lt_or_eq _ _ H); apply impl_PI;
  [ | subst; apply le_refl].
 apply lt_le_weak.
 apply mult_lt_compat_l; auto.
 ...
 clear H1; subst y.
 destruct (eq_dec 0 x) as [H'|H']; apply impl_PI; auto.
 ...
 destruct (lt_eq_lt_dec z 0) as [[G|G]|G]; apply impl_PI.
 ...
 rewrite <- opp_eq_mult_neg_1.
 destruct (lt_eq_lt_dec 0 x) as [[H'|H']|H']; apply impl_PI.
 auto.
 congruence.
 right.
 ...
 Proof.
  red; intros; destruct (eq_dec i j); apply impl_PI; auto.
 Qed.
 ...
 Proof.
  red; intros; destruct (le_dec i j); apply impl_PI; auto.
 Qed.

 Lemma dec_lt : forall i j:int, decidable (i<j).
 Proof.
  red; intros; destruct (lt_dec i j); apply impl_PI; auto.
 Qed.

 Lemma dec_ge : forall i j:int, decidable (i>=j).
 Proof.
  red; intros; rewrite ge_le_iff; destruct (le_dec j i); apply impl_PI; auto.
 Qed.

 Lemma dec_gt : forall i j:int, decidable (i>j).
 Proof.
  red; intros; rewrite gt_lt_iff; destruct (lt_dec j i); apply impl_PI; auto.
 Qed.
...
Proof.
 ...
 | intros t11 H1 t12 H2 t2; case t2; try trivial_case; simpl in |- *;
    ...
    elim andb_false_elim with (1 := H3); intros H5; apply impl_PI;
