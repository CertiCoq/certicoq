...
(* $Id: tacticals.ml 9850 2007-05-22 21:37:55Z herbelin $ *)
...
let tclNTH_HYP m (tac : constr->tactic) gl =
  tac (try mkVar(let (id,_,_,_) = List.nth (pf_hyps gl) (m-1) in id) 
...
let afterHyp id gl =
  fst (list_splitby (fun (hyp,_,_,_) -> hyp = id) (pf_hyps gl))
...
let general_elim_then_using 
  ...
  in
  let pmv =
    let p, _, _ = decompose_app elimclause.templtyp.Evd.rebus in
  ...
  let after_tac ce i gl =
    let (hd,largs,imps) = decompose_app ce.templtyp.Evd.rebus in
  ...
  let rec makerec (assums,cargs,constargs,recargs,indargs) lb lc =
    match lb,lc with 
      | ([], _) -> 
          { ba = ba;
            assums    = assums}
      | ((true::tl), ((idrec,_,_,_ as recarg)::(idind,_,_,_ as indarg)::idtl)) ->
	  makerec (recarg::indarg::assums,
		   idrec::cargs,
		   idrec::recargs,
		   constargs,
		   idind::indargs) tl idtl
      | ((false::tl), ((id,_,_,_ as constarg)::idtl))      ->
  ...
  let rec makerec (assums,cargs,constargs,recargs) p_0 p_1 =
    match p_0,p_1 with 
      | ([], _) -> 
          { ba = ba;
            assums    = assums}
      | ((true::tl), ((idrec,_,_,_ as recarg)::idtl)) ->
