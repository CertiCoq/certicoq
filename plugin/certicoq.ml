...
let certicoq_run_functions = 
  Summary.ref ~name:"CertiCoq Run Functions Table" ~local:true
...
module CompileFunctor (CI : CompilerInterface) = struct
  ...
  let certicoq_eval_named opts env sigma c global_id imports =
    ...
    let packages = ["coq-core"; "coq-core.plugins.ltac"; "coq-metacoq-template-ocaml"; 
      "coq-core.interp"; "coq-core.kernel"; "coq-core.library"; "coq-certicoq.plugin"] in
    let pkgs = String.concat "," packages in
    let dontlink = "str,unix,dynlink,threads,zarith,coq-core,coq-core.plugins.ltac,coq-core.interp,coq-certicoq.plugin" in
