                ...
                (fun fO fS n -> if n=0 then fO () else fS (n-1))
                  ...
                  (fun fO fS n -> if n=0 then fO () else fS (n-1))
                    ...
                    (fun fO fS n -> if n=0 then fO () else fS (n-1))
                      ...
                      ascii_of_nat
                        (add
                          (sub n (Pervasives.succ (Pervasives.succ
                            (Pervasives.succ (Pervasives.succ
                            (Pervasives.succ (Pervasives.succ
                            (Pervasives.succ (Pervasives.succ
                            (Pervasives.succ (Pervasives.succ 0)))))))))))
...
let rec nat2string modulus x =
  ...
  then (digit2ascii x)::[]
       ...
       append (nat2string modulus m0)
         ((digit2ascii (sub x (mul modulus m0)))::[])

(** val nat2string10 : int -> char list **)

let nat2string10 =
  nat2string (Pervasives.succ (Pervasives.succ (Pervasives.succ
    (Pervasives.succ (Pervasives.succ (Pervasives.succ (Pervasives.succ
...
module Make =
 ...
 struct
  ...
  if p<=1 then f1 () else if p mod 2 = 0 then f2p (p/2) else f2p1 (p/2))
        (fun n0 ->
        treeify_cont (treeify_aux false n0) (treeify_aux pred0 n0))
        (fun n0 ->
        treeify_cont (treeify_aux pred0 n0) (treeify_aux true n0))
...
let rec app_ctx_f c e =
  ...
  | Ecase_c (x, te, t0, c0, te') ->
    Ecase (x, (app te ((t0, (app_ctx_f c0 e)) :: te')))
...
and app_f_ctx_f c e =
  ...
  | Fcons2_c (f, t0, ys, e', cfds) ->
    Fcons (f, t0, ys, e', (app_f_ctx_f cfds e))
...
and comp_f_ctx_f c c2 =
  match c with
  | Fcons1_c (f, t0, ys, c0, fds) ->
    Fcons1_c (f, t0, ys, (comp_ctx_f c0 c2), fds)
  | Fcons2_c (f, t0, ys, e', cfds) ->
    Fcons2_c (f, t0, ys, e', (comp_f_ctx_f cfds c2))
...
| Efun (fds, e0) ->
  add (add (Pervasives.succ 0) (funs_size fds)) (term_size e0)
...
(** val update_census :
    r_map -> exp -> (var -> c_map -> int) -> c_map -> c_map **)
...
let init_census e =
  update_census M.empty e (fun v c -> add (getd 0 v c) (Pervasives.succ 0))
    M.empty
...
let dec_census_list sig0 ys count =
  update_census_list sig0 ys (fun v c ->
    sub (getd 0 v c) (Pervasives.succ 0)) count
...
let rec contract_func x =
  ...
  let contract0 = fun sig1 count0 e0 sub1 im0 ->
    contract_func (ExistT (sig1, (ExistT (count0, (ExistT (e0, (ExistT (sub1,
      im0))))))))
  in
  (match e with
   | Econstr (x0, t0, ys, e') ->
            ...
            | SVconstr (_, ys) ->
              (match nthN ys n with
               | Some yn ->
                 ...
                 let count' =
                   M.set y' (sub (getd 0 y' count) (Pervasives.succ 0)) count
                 in
                 let count'' =
                   M.set v 0
                     (M.set yn' (add (getd 0 v count) (getd 0 yn' count))
                       count')
                 in
               ...
               | None ->
                 ...
                 ((fun fO fS n -> if n=0 then fO () else fS (n-1))
                    ...
                    let count'' = dec_census_list sig0 (y :: []) count' in
                    ExistT (((e', count''), im'), __))
                    (fun _ -> ExistT ((((Eproj (v, t0, n, y', e')), count'),
                    im'), __))
                    (getd 0 v count')))
            | SVfun (_, _, _) ->
              let ExistT (x0, _) = contract0 sig0 count e0 sub0 im in
        ...
        (fun fO fS n -> if n=0 then fO () else fS (n-1))
          ...
          (match filtered_var0 with
           | Some s ->
             (match s with
              | SVconstr (_, _) ->
                ExistT ((((Eapp (f', t0, ys')), count), im), __)
              | SVfun (t', xs, m0) ->
                let filtered_var1 =
                  (&&) (Coq_Pos.eqb t' t0)
                ...
                then let im' = M.set f' true im in
                     let count' =
                       update_count_inlined ys' xs (M.set f' 0 count)
                     in
                     let ExistT (x0, _) =
                       contract0 (set_list (combine xs ys') sig0) count' m0
                         sub0 im'
                     in
                     ExistT (x0, __)
...
let contract sig0 count e sub0 im =
  contract_func (ExistT (sig0, (ExistT (count, (ExistT (e, (ExistT (sub0,
    im))))))))
...
let l5_conId_dec x y =
  let (i, n) = x in
  let (i0, n0) = y in
  let h = inductive_dec i i0 in if h then eq_dec0 nEq n n0 else false
...
(** val set_t_f_list :
    fTag -> var list -> nVar list -> conv_env -> conv_env **)
...
(** val convert :
    cTag -> fTag -> fTag -> (nVar, l5Opid) nTerm -> s_map -> s_map ->
                ...
                | [] ->
                  pbind (pMonad_Monad (Obj.magic monad_option))
                    ...
                    ret (Obj.magic monad_option)
                      (((app_ctx_f ctx_v (Ehalt vn)), (Coq_Pos.succ n')),
                      tgm0))
                | _ :: _ -> None)
             | _ :: _ -> None))
       | CRet ->
                            ...
                            | [] ->
                              pbind (pMonad_Monad (Obj.magic monad_option))
                                ...
                                pbind (pMonad_Monad (Obj.magic monad_option))
                                  ...
                                  (fun k0 ->
                                  pbind
                                    (pMonad_Monad (Obj.magic monad_option))
                                    (Obj.magic __) (getVar (Obj.magic v))
                                    (fun v0 ->
                                    let fv =
                                      if varClass0 varClassNVar0 f0
                                      then sv
                                      else sk
                                    in
                                    let kv =
                                      if varClass0 varClassNVar0 k0
                                      then sv
                                      else sk
                                    in
                                    let vv =
                        ...
                        | y6 :: ls' ->
                          ...
                          pbind (pMonad_Monad (Obj.magic monad_option))
                            ...
                            ret (Obj.magic monad_option) ((((((((tg,
                              ...
                              sk0), tgm2), r), n'''))))) lbt
                     (ret (Obj.magic monad_option) (((((([], ls), sv), sk),
                       tgm0), vn), n'))) (fun r2 ->
                   let (y1, n'') = r2 in
                   let (y2, vn0) = y1 in
                   let (y3, tgm1) = y2 in
                  ...
                  (match l3 with
                   | [] ->
                     (match l0 with
                      | [] ->
                        let tgm0 =
                          set_nt (Coq_Pos.succ n) ((snd k), kon_tag) tgm
                        in
                        ...
                        pbind (pMonad_Monad (Obj.magic monad_option))
                          ...
                          let fds = Fcons (n', fun_tag,
                            ((Coq_Pos.succ n) :: (n :: [])), e', Fnil)
                          in
                          ret (Obj.magic monad_option) ((((Efun1_c (fds,
                            Hole_c)), n'), (Coq_Pos.succ n')), tgm3))
                      | _ :: _ -> None)
                   | _ :: _ -> None))))
       | CKLambda ->
         (match lbt with
                ...
                | [] ->
                  (match l0 with
                   | [] ->
                     ...
                     pbind (pMonad_Monad (Obj.magic monad_option))
                       (Obj.magic __)
                       (convert0 e sv (M.set (fst k) n sk) tgm0
                         (Coq_Pos.succ n)) (fun r ->
                       let (y, tgm1) = r in
                       let (e', n') = y in
                       let fds = Fcons (n', kon_tag, (n :: []), e', Fnil) in
                       ret (Obj.magic monad_option) ((((Efun1_c (fds,
                         Hole_c)), n'), (Coq_Pos.succ n')),
         ...
         (match lbt with
          ...
          | b :: _ ->
                                        ...
                                        | [] ->
                                          pbind (pMonad_Monad monad_option)
                                            ...
                                            (Obj.magic convert0 e
                                              ...
                                              (M.set (fst k)
                                                (Coq_Pos.succ n0) sk0)
                                              (set_nt (Coq_Pos.succ n0)
                                                ((snd k), kon_tag)
                                                (set_n n0 (snd x) tgm0))
                                              (Coq_Pos.add n0 ((fun p->2*p)
                                                1))) (fun r1 ->
                                            let (y, tgm1) = r1 in
                                            let (ce, n') = y in
                                            pbind (pMonad_Monad monad_option)
            ...
            pbind (pMonad_Monad (Obj.magic monad_option)) (Obj.magic __)
              ...
              let (y, tgm1) = r in
              let (fds, n') = y in
              ret (Obj.magic monad_option) ((((Efun1_c (fds, Hole_c)),
                (nth i nfds 1)), n'), tgm1)))
       | CDCon (dcn, _) ->
         let convert_v_list =
             ...
             | y :: lv' ->
               ...
               pbind (pMonad_Monad monad_option) (Obj.magic __)
                 ...
                 pbind (pMonad_Monad monad_option) (Obj.magic __)
                   ...
                   let (y3, n'') = y2 in
                   let (ctx_lv', ln') = y3 in
                   ret monad_option ((((comp_ctx_f ctx_v ctx_lv'),
                     (vn :: ln')), n''), tgm2)))
           in convert_v_list
         in
    ...
    (match itC with
     ...
     | cst :: icT' ->
       ...
       convert_cnstrs tyname cct' icT' ind (N.add nCon 1) niT
         (M.set cn (((((NNamed cname), (NNamed tyname)), niT),
  ...
  (match typ with
   ...
   | y :: typ' ->
     ...
     let ityi =
       combine cct
         (map (fun c ->
           let { cnstrNm = _; cnstrArity = n0 } = c in N.of_nat n0) itC)
     in
     convert_typack typ' idBundle (add n (Pervasives.succ 0))
       (((((M.set niT ityi ie), ce'), ncT'), (Coq_Pos.succ niT)), dcm'))
...
let rec convert_env' g ice =
  ...
  | [] -> ice
  | p :: g' ->
    let (id0, ty) = p in convert_env' g' (convert_typack ty id0 0 ice)
...
let convert_env default_tag default_itag g =
  ...
  let default_cEnv =
    M.set default_tag ((((NAnon, NAnon), default_itag), 0), 0) M.empty
  in
  convert_env' g ((((default_iEnv, default_cEnv),
    (Coq_Pos.succ default_tag)), (Coq_Pos.succ default_itag)), [])
...
(** val freshen_term :
    exp -> int M.t -> int -> int list -> (exp * int) * int list **)
...
(** val update_inFun :
    'a1 inlineHeuristic -> var -> tag -> var list -> exp -> r_map -> 'a1 ->
    'a1 **)
...
let freshen_exp e =
  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
    ...
    let (e', n) = p in
    pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
      ((Obj.magic monadState_state).put n) (fun _ ->
      ret (Obj.magic monad_state) e'))
...
let rec add_fundefs fds del0 =
  match fds with
  | Fcons (f, t0, xs, e, fds0) ->
    M.set f ((t0, xs), e) (add_fundefs fds0 del0)
  | Fnil -> del0
...
(** val beta_contract_func :
    ...
    M.t, 'a1) sigT) sigT) sigT -> exp freshM **)
  ...
  (match e0 with
       ...
       (let rec beta_list br s0 =
          ...
          | h :: br' ->
            ...
            pbind (pMonad_Monad monad_state) (Obj.magic __)
              ...
              pbind (pMonad_Monad monad_state) (Obj.magic __)
                (beta_list br' s0) (fun br'' ->
                ret monad_state ((t0, e') :: br'')))
   ...
   | Eapp (f, t0, ys) ->
     ...
     then (match o with
           | Some p0 ->
             ...
             ((fun fO fS n -> if n=0 then fO () else fS (n-1))
                (fun _ ->
                Obj.magic ret monad_state (Eapp (f', t0, ys')))
                (fun d' ->
                ...
                Obj.magic pbind (pMonad_Monad (Obj.magic monad_state)) __
                  (freshen_exp e2) (fun e' ->
                  beta_contract0 (d'0, e') sig' del0 s0))
                n)
   ...
   | Eprim (x0, t0, ys, e1) ->
     Obj.magic pbind (pMonad_Monad monad_state) __
       (Obj.magic beta_contract0 (d0, e1) sig0 del0 s) (fun e' ->
       let ys' = apply_r_list sig0 ys in
       ret monad_state (Eprim (x0, t0, ys', e')))
   | Ehalt x0 ->
     let x' = apply_r sig0 x0 in Obj.magic ret monad_state (Ehalt x'))
...
(** val beta_contract_top :
    'a1 inlineHeuristic -> exp -> int -> 'a1 -> exp **)
     ...
     | Some y ->
       ((fun fO fS n -> if n=0 then fO () else fS (n-1))
          ...
          (fun fO fS n -> if n=0 then fO () else fS (n-1))
            ...
            | [] -> (s, false)
            | k :: _ ->
              ((M.set f 0 (M.set k (Pervasives.succ (Pervasives.succ 0)) s)),
                true))
            (fun n0 ->
            (fun fO fS n -> if n=0 then fO () else fS (n-1))
              (fun _ -> (s, true))
...
(** val inlineSmallOrUncurried :
    int -> (bool M.t * int M.t) inlineHeuristic **)
...
let copyVar _ =
  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
    ...
    let (y, b) = p3 in
    pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
      ((Obj.magic monadState_state).put (((((((Coq_Pos.add y 1), b), aenv),
        fenv), ft), lm), s0)) (fun _ -> ret (Obj.magic monad_state) y))
...
let mark_as_uncurried x =
  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
    ...
    let (p3, aenv) = p2 in
    let (y, b) = p3 in
    (Obj.magic monadState_state).put ((((((y, b), aenv), fenv), ft),
      (M.set x true lm)), s0))
...
| x :: xs' ->
  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
    (Obj.magic copyVar x) (fun y ->
    pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
      (copyVars xs') (fun ys -> ret (Obj.magic monad_state) (y :: ys)))
...
let click =
  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
    ...
    let (p3, aenv) = p2 in
    let (y, _) = p3 in
    (Obj.magic monadState_state).put ((((((y, true), aenv), fenv), ft), lm),
      s0))
     ...
     | None ->
       pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
         ((Obj.magic monadState_state).put ((((((y, b), (M.set p4 ft aenv)),
           (M.set ft (n, (fromN 0 (N.to_nat n))) fenv)), (Coq_Pos.succ ft)),
           lm), s0)) (fun _ -> ret (Obj.magic monad_state) ft)))
...
| Econstr (x, ct, vs, e1) ->
  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
    ...
    ret (Obj.magic monad_state) (Econstr (x, ct, vs, e1')))
| Ecase (x, arms) ->
  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
     ...
     | h :: t0 ->
       ...
       pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
         ...
         pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
           ...
           ret (Obj.magic monad_state) ((s, e') :: t')))
     in uncurry_list arms) (fun arms' ->
    ret (Obj.magic monad_state) (Ecase (x, arms')))
| Eproj (x, ct, n, y, e1) ->
  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
    (uncurry_exp e1) (fun e1' ->
    ret (Obj.magic monad_state) (Eproj (x, ct, n, y, e1')))
| Efun (fds, e1) ->
  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
    (Obj.magic uncurry_fundefs fds) (fun fds' ->
    pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
      (uncurry_exp e1) (fun e1' ->
      ret (Obj.magic monad_state) (Efun (fds', e1'))))
| Eprim (x, p, xs, e1) ->
  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
    (uncurry_exp e1) (fun e1' ->
    ret (Obj.magic monad_state) (Eprim (x, p, xs, e1')))
          ...
          | Fcons (g, gt, gvs, ge, f1) ->
            (match f1 with
             | Fcons (_, _, _, _, _) ->
               pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
                 (Obj.magic uncurry_exp fe) (fun fe' ->
                 ret (Obj.magic monad_state) (Fcons (f, f_ft, fvs, fe',
                   fds1')))
             | Fnil ->
               (match e with
                | Eapp (fk', fk_ft, l) ->
                        ...
                        pbind (pMonad_Monad (Obj.magic monad_state))
                          ...
                          pbind (pMonad_Monad (Obj.magic monad_state))
                            ...
                            then pbind (pMonad_Monad (Obj.magic monad_state))
                                   (Obj.magic __) (Obj.magic copyVars gvs)
                                   (fun gvs' ->
                                   pbind
                                     ...
                                     pbind
                                       (pMonad_Monad (Obj.magic monad_state))
                                       (Obj.magic __) (Obj.magic copyVar f)
                                       (fun f' ->
                                       pbind
                                         (pMonad_Monad
                                           (Obj.magic monad_state))
                                         (Obj.magic __)
                                         ...
                                         pbind
                                           ...
                                           pbind
                                             (pMonad_Monad
                                               (Obj.magic monad_state))
                                             (Obj.magic __)
                                             (Obj.magic markToInline
                                               fp_numargs f g) (fun _ ->
                                             pbind
                                               (pMonad_Monad
                                                 (Obj.magic monad_state))
                                               (Obj.magic __)
                                   ...
                                   (fk :: fvs0), (Efun ((Fcons (g, gt, gvs,
                                   ge', Fnil)), (Eapp (fk', fk_ft,
                                   (g' :: []))))), fds1'))))
                      | _ :: _ ->
                        pbind (pMonad_Monad (Obj.magic monad_state))
                          (Obj.magic __) (Obj.magic uncurry_exp fe)
                          (fun fe' ->
                          ret (Obj.magic monad_state) (Fcons (f, f_ft, fvs,
                            fe', fds1')))))
                | _ ->
                  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
                    (Obj.magic uncurry_exp fe) (fun fe' ->
...
let uncurry e aenv fenv ft lm freshvar s =
  let (e0, s0) =
    runState (uncurry_exp e) ((((((freshvar, false), aenv), fenv), ft), lm),
      s)
  in
  let (p, s1) = s0 in
  let (p0, lm0) = p in
...
let rec uncurry_fuel' n e aenv fenv ft lm freshvar s =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    ...
    | Some p ->
      ...
      let (p4, fenv') = p3 in
      let (e', aenv') = p4 in
      uncurry_fuel' m0 e' aenv' fenv' ft' lm' freshvar0 s'
    | None -> ((e, s), fenv))
    n
...
let uncurry_fuel n e fenv =
  ...
  uncurry_fuel' n e M.empty fenv (Coq_Pos.succ max_ft) M.empty freshvar (0,
    M.empty)
...
let get_name_entry x =
  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
    ...
    let { next_var = _; nect_cTag = _; next_iTag = _; cenv = _; name_env =
      names } = p
    in
...
let set_name_entry x name0 =
  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
    (Obj.magic monadState_state).get (fun p ->
    let { next_var = n; nect_cTag = c; next_iTag = i; cenv = e; name_env =
      names } = p
    in
    pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
      ((Obj.magic monadState_state).put { next_var = n; nect_cTag = c;
        next_iTag = i; cenv = e; name_env = (M.set x name0 names) })
...
let get_name old_var suff =
  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
    ...
    in
    pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
      ((Obj.magic monadState_state).put { next_var = (Coq_Pos.add n 1);
        nect_cTag = c; next_iTag = i; cenv = e; name_env = names }) (fun _ ->
      pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
        (Obj.magic add_name_suff n old_var suff) (fun _ ->
        ret (Obj.magic monad_state) n)))
...
let make_record_cTag n =
  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
    (Obj.magic monadState_state).get (fun p ->
    let { next_var = x; nect_cTag = c; next_iTag = i; cenv = e; name_env =
      names } = p
    in
    let inf = ((((NAnon, NAnon), i), n), 0) in
    let e' = M.set c inf e in
    pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
      ((Obj.magic monadState_state).put { next_var = x; nect_cTag =
        (Coq_Pos.add c 1); next_iTag = (Coq_Pos.add i 1); cenv = e';
        name_env = names }) (fun _ -> ret (Obj.magic monad_state) c))
    ...
    (match entry with
     | FVar pos ->
       pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
         ...
         ret (Obj.magic monad_state) (y, (fun e -> Eproj (y, c, pos,
           _UU0393_, e))))
     | MRFun code_ptr ->
       pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
...
(** val get_vars :
    cTag -> var list -> varInfoMap -> cTag -> var -> (var list * (exp ->
    exp)) ccstate **)
  ...
  | x :: xs0 ->
    pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
      ...
      pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
        ...
        let (ys, f') = t2 in
        ret (Obj.magic monad_state) ((y :: ys), (fun e -> f (f' e)))))
...
let rec make_full_closure clo_tag defs mapfv_new mapfv_old _UU0393_ =
  ...
  | Fcons (f, _, _, _, defs') ->
    pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
      ...
      pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
        (make_full_closure clo_tag defs' mapfv_new mapfv_old _UU0393_)
        (fun t0 ->
        let (p, g') = t0 in
        ...
        ret (Obj.magic monad_state) ((mapfv_new'', mapfv_old''), (fun e ->
          Econstr (f, clo_tag, (code_ptr :: (_UU0393_ :: [])), (g' e))))))
  ...
  let rec exp_closure_conv0 e mapfv c _UU0393_ =
    ...
    | Econstr (x, tag0, ys, e') ->
      pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
        ...
        let (ys', f) = t1 in
        pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
          (exp_closure_conv0 e' (PTree.set x BoundVar mapfv) c _UU0393_)
          (fun ef ->
          ret (Obj.magic monad_state) ((Econstr (x, tag0, ys',
            (snd ef (fst ef)))), f)))
    | Ecase (x, pats) ->
      pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
        (let rec mapM_cc = function
         ...
         | y0 :: xs ->
           ...
           pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
             ...
             pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
               (mapM_cc xs) (fun xs' ->
               ret (Obj.magic monad_state) ((y, (snd ef (fst ef))) :: xs')))
         in mapM_cc pats) (fun pats' ->
        pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
          (Obj.magic get_var clo_tag x mapfv c _UU0393_) (fun t1 ->
          let (x', f1) = t1 in
          ret (Obj.magic monad_state) ((Ecase (x', pats')), f1)))
    | Eproj (x, tag0, n, y, e') ->
      pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
        ...
        pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
          (exp_closure_conv0 e' (PTree.set x BoundVar mapfv) c _UU0393_)
          (fun ef ->
          ret (Obj.magic monad_state) ((Eproj (x, tag0, n, y',
            (snd ef (fst ef)))), f)))
    | Efun (defs, e0) ->
      ...
      pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
        ...
        (fun _UU0393_' ->
        pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
          (Obj.magic make_env clo_tag fv PTree.empty mapfv c _UU0393_'
            _UU0393_) (fun t1 ->
          let (p, g1) = t1 in
          ...
          pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
            ...
            pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
              ...
              pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
                ...
                ret (Obj.magic monad_state) ((Efun (defs',
                  (g2 (snd ef (fst ef))))), g1))))))
    | Eapp (f, ft, xs) ->
      pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
        ...
        pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
          ...
          pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
            ...
            pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
              ...
              ret (Obj.magic monad_state) ((Eproj (ptr, clo_tag, 0, f',
    ...
    | Eprim (x, prim0, ys, e') ->
      pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
        ...
        pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
          (exp_closure_conv0 e' (PTree.set x BoundVar mapfv) c _UU0393_)
          (fun ef ->
          ret (Obj.magic monad_state) ((Eprim (x, prim0, ys',
            (snd ef (fst ef)))), f)))
    | Ehalt x ->
      pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
        (Obj.magic get_var clo_tag x mapfv c _UU0393_) (fun t1 ->
        ...
        pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
          ...
          pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
            ...
            let code_ptr =
              ...
              | None -> f
            in
            ret (Obj.magic monad_state) (Fcons (code_ptr, tag0,
...
let closure_conversion_hoist clo_tag e ctag itag cenv' nmap =
  ...
  let state1 = { next_var = next; nect_cTag = ctag; next_iTag = itag; cenv =
    cenv'; name_env = nmap }
  in
  let (p, s) =
    runState (exp_closure_conv clo_tag e PTree.empty 1 _UU0393_) state1
  in
...
let get_name0 =
  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
    ...
    pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
      ((Obj.magic monadState_state).put { next_var0 = (Coq_Pos.add n 1);
        next_fTag = f }) (fun _ -> ret (Obj.magic monad_state) n))
...
let rec get_names n =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    ...
    (fun n0 ->
    pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
      (Obj.magic get_name0) (fun x ->
      pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
        (get_names n0) (fun xs -> ret (Obj.magic monad_state) (x :: xs))))
    n
...
let get_tag =
  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
    ...
    let { next_var0 = n; next_fTag = f } = p in
    pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
      ((Obj.magic monadState_state).put { next_var0 = n; next_fTag =
        (Coq_Pos.add f 1) }) (fun _ -> ret (Obj.magic monad_state) f))
...
(** val add_functions :
    fundefs -> var list -> funInfoMap -> funInfoMap state0 **)
...
let rec add_functions b fvs m0 =
  ...
  | Fcons (f, _, _, _, b0) ->
    pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
      ...
      pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
        (Obj.magic get_name0) (fun f' ->
        pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
          (Obj.magic get_tag) (fun ft' ->
          ret (Obj.magic monad_state) (M.set f (Fun (f', ft', fvs)) m'))))
  | Fnil -> ret (Obj.magic monad_state) m0

(** val add_free_vars :
    var list -> varInfoMap0 -> (var list * varInfoMap0) state0 **)
...
let rec add_free_vars fvs m0 =
  ...
  | y :: ys ->
    pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
      (add_free_vars ys m0) (fun p ->
      pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
        (Obj.magic get_name0) (fun y' ->
        let (ys', m') = p in
        ret (Obj.magic monad_state) ((y' :: ys'), (M.set y y' m'))))
  ...
  let rec exp_true_fv_aux e scope fvset =
    ...
    | Eapp (x, _, xs) ->
      let fvset' =
        ...
        | Some inf ->
          let Fun (f', _, fvs) = inf in union_list0 fvset (f' :: fvs)
      ...
      (let rec mapM_ll = function
       ...
       | y :: p0 ->
         ...
         pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
           ...
           pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
             (mapM_ll p0) (fun p' ->
             ret (Obj.magic monad_state) ((c, e') :: p')))
       in mapM_ll p) (fun p' ->
      ret (Obj.magic monad_state) (Ecase ((rename fvm x), p')))
  | Eproj (x, t0, n, y, e0) ->
    pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
      (exp_lambda_lift e0 fvm fm) (fun e' ->
         ...
         pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
           ...
           pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
             ...
             pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
               ...
               ret (Obj.magic monad_state) (Fcons (f', ft', (app xs ys), e',
                 (Fcons (f, ft, xs', (Eapp (f', ft',
                 (app xs' (rename_lst fvm fvs)))), b'))))))))
...
let lambda_lift e ftag =
  ...
  let (e0, _) = runState (exp_lambda_lift e PTree.empty PTree.empty) state1 in
  e0
...
let rec get_live_initial b =
  let yss = get_vars0 b in let bss = get_bools b in Live (yss, bss)
...
let rec find_fun x funs n =
  ...
  | f :: funs' ->
    if peq f x
    then (true, n)
    else find_fun x funs' (add n (Pervasives.succ 0))
...
(** val add_fun_args :
    int -> var list -> bool list -> live_fun -> live_fun **)
...
(** val add_fun_vars :
    var -> var list -> var list -> live_fun -> int -> live_fun **)

let rec add_fun_vars f funs xs l m0 =
  let (b, n) = find_fun f funs 0 in
  if b
  then let bss = bool_live l in
       let bs = nth n bss [] in add_fun_args m0 xs bs l
  else add_vars xs l m0
...
(** val live_expr :
    fundefs -> live_fun -> var list -> int -> exp -> live_fun **)
...
let rec live b l funs n =
  ...
  | Fcons (_, _, _, e, b') ->
    ...
    live b' l' funs (add n (Pervasives.succ 0))
  | Fnil -> l
...
let rec check_list_equality l1 l2 =
  ...
  | hd1 :: tl1 ->
    (match l2 with
     | [] -> false
     | hd2 :: tl2 ->
       if eqb0 hd1 hd2 then check_list_equality tl1 tl2 else false)
...
(** val find_live_helper :
    fundefs -> live_fun -> var list -> int -> live_fun **)

let rec find_live_helper b prev_L funs n =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    (fun _ -> prev_L)
    (fun n' ->
...
| Efun (b, e') ->
  ...
  let n = num_vars b 0 in find_live_helper b l' funs n
| _ -> Live (([] :: []), ([] :: []))
...
let rec live_args ys bs =
  ...
  | y :: ys' ->
    (match bs with
     | [] -> ys
     | b :: bs' ->
       if eqb0 b true then y :: (live_args ys' bs') else live_args ys' bs')

(** val eliminate_expr : live_fun -> var list -> exp -> exp **)

let rec eliminate_expr l funs e = match e with
| Econstr (x, t0, ys, e') -> Econstr (x, t0, ys, (eliminate_expr l funs e'))
| Ecase (x, p) ->
  let p' =
  ...
  in
  Ecase (x, p')
| Eproj (x, t0, m0, y, e') -> Eproj (x, t0, m0, y, (eliminate_expr l funs e'))
| Efun (_, _) -> e
| Eapp (f, t0, ys) ->
  let (b, n) = find_fun f funs 0 in
  if b
  then let bss = bool_live l in
       let ys' = live_args ys (nth n bss []) in Eapp (f, t0, ys')
  else Eapp (f, t0, ys)
| Eprim (x, f, ys, e') -> Eprim (x, f, ys, (eliminate_expr l funs e'))
| Ehalt x -> Ehalt x

(** val eliminate_fundefs :
    fundefs -> live_fun -> var list -> int -> fundefs **)
...
let rec eliminate_fundefs b l funs n =
  ...
  (match b with
   | Fcons (f, ft, ys, e, b') ->
     ...
     Fcons (f, ft, ys', e',
     (eliminate_fundefs b' l funs (add n (Pervasives.succ 0))))
   | Fnil -> b)
...
let certiL5_t0_L6 v =
  ...
  | Some r ->
    ...
    let (p3, _) =
      uncurry_fuel (Pervasives.succ (Pervasives.succ (Pervasives.succ
        (Pervasives.succ (Pervasives.succ (Pervasives.succ (Pervasives.succ
        (Pervasives.succ (Pervasives.succ (Pervasives.succ (Pervasives.succ
        (Pervasives.succ (Pervasives.succ (Pervasives.succ (Pervasives.succ
        (Pervasives.succ (Pervasives.succ (Pervasives.succ (Pervasives.succ
    ...
    let (_, s) = s0 in
    let e1 =
      inline_uncurry_contract e0 s (Pervasives.succ (Pervasives.succ
        (Pervasives.succ (Pervasives.succ (Pervasives.succ (Pervasives.succ
        (Pervasives.succ (Pervasives.succ (Pervasives.succ (Pervasives.succ
    ...
    in
    let (cenv', nenv') = p4 in
    Ret ((((M.empty, (add_cloTag bogus_cloTag bogus_cloiTag cenv')), nenv'),
    M.empty), (M.empty, (shrink_top t')))
  | None ->
    Exc
...
let show_con cenv0 tg =
  ...
  | Some c ->
    ...
    (match n0 with
     | NAnon ->
       App ((Str ('c'::('o'::('n'::('_'::[]))))), (Str (show_pos tg)))
     | NNamed s -> Str s)
              ...
              pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
                (let rec iter0 = function
                 ...
                 | p :: tail ->
                   let (tg, e0) = p in
                   pbind (pMonad_Monad (Obj.magic monad_state))
                     (Obj.magic __) (tab indent) (fun _ ->
                     pbind (pMonad_Monad (Obj.magic monad_state))
                       (Obj.magic __) (emit (Str ('|'::(' '::[])))) (fun _ ->
                       pbind (pMonad_Monad (Obj.magic monad_state))
                         (Obj.magic __) (emit (show_con cenv0 tg)) (fun _ ->
                         pbind (pMonad_Monad (Obj.magic monad_state))
                           (Obj.magic __)
                           (emit (Str (' '::('='::('>'::(' '::[]))))))
                           (fun _ ->
                           pbind (pMonad_Monad (Obj.magic monad_state))
                             (Obj.magic __) newline (fun _ ->
                             pbind (pMonad_Monad (Obj.magic monad_state))
                               (Obj.magic __)
                               (emit_exp nenv cenv0 ftag_flag
                                 (add (Pervasives.succ (Pervasives.succ 0))
                                   indent) e0) (fun _ -> iter0 tail))))))
                 in iter0 arms) (fun _ ->
                pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
                  (tab indent) (fun _ ->
                  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
              ...
              pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
                ...
                pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
                  ...
                  pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
                    (emit (Str (' '::[]))) (fun _ ->
                    pbind (pMonad_Monad (Obj.magic monad_state))
                      (Obj.magic __) (emit (show_var nenv y)) (fun _ ->
                      pbind (pMonad_Monad (Obj.magic monad_state))
                        (Obj.magic __)
                        (emit (Str (' '::('i'::('n'::(' '::[])))))) (fun _ ->
                        pbind (pMonad_Monad (Obj.magic monad_state))
                          (Obj.magic __) newline (fun _ ->
                          emit_exp nenv cenv0 ftag_flag indent e0))))))))))
    | Efun (fds, e0) ->
      pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
        (emit (Str
          ('l'::('e'::('t'::('r'::('e'::('c'::(' '::('['::[]))))))))))
          ...
          pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
            (let rec iter0 = function
             | Fcons (x, tg, xs, e1, fds') ->
               pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
                 (tab (add (Pervasives.succ (Pervasives.succ 0)) indent))
                 (fun _ ->
                 pbind (pMonad_Monad (Obj.magic monad_state)) (Obj.magic __)
                   (emit (Str ('f'::('u'::('n'::(' '::[])))))) (fun _ ->
                   pbind (pMonad_Monad (Obj.magic monad_state))
                     (Obj.magic __) (emit (show_var nenv x)) (fun _ ->
                     pbind (pMonad_Monad (Obj.magic monad_state))
                       (Obj.magic __) (emit (show_ftag ftag_flag tg))
                       (fun _ ->
                       pbind (pMonad_Monad (Obj.magic monad_state))
                         (Obj.magic __) (emit (show_vars nenv xs)) (fun _ ->
                         pbind (pMonad_Monad (Obj.magic monad_state))
                           (Obj.magic __)
                           (emit (Str (' '::(':'::('='::(' '::[]))))))
                           (fun _ ->
...
let show_exp nenv cenv0 ftag_flag x =
  chr_newline::(show_tree
                 (snd (runState (emit_exp nenv cenv0 ftag_flag 0 x) Emp)))
...
(** val comp_L6 :
    (ienv * l5Term, ienv * l5Term) cTerm exception0 -> (l6env * l6term,
    l6val) cTerm exception0 **)
...
let test1_L5 =
  Ret
    ...
    0) } :: [])) } :: [])) :: []), (Oterm (CRet, ((Bterm ([], (Oterm
    (CKLambda, ((Bterm (((((fun p->1+2*p) 1), (NNamed ('k'::[]))) :: []),
    (Oterm (CRet, ((Bterm ([], (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p)
    1), (NNamed ('k'::[]))) :: []), (Oterm (CRet, ((Bterm ([], (Vterm
    (((fun p->1+2*p) 1), (NNamed ('k'::[])))))) :: ((Bterm ([], (Oterm
    (CLambda, ((Bterm (((((fun p->2*p) 1), (NNamed
    ('C'::('o'::('q'::('.'::('I'::('n'::('i'::('t'::('.'::('N'::('a'::('t'::('.'::('a'::('d'::('d'::[])))))))))))))))))) :: ((((fun p->1+2*p)
    1), (NNamed ('k'::[]))) :: [])), (Oterm (CRet, ((Bterm ([], (Oterm
    (CKLambda, ((Bterm (((((fun p->1+2*p) 1), (NNamed ('k'::[]))) :: []),
    (Oterm (CRet, ((Bterm ([], (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p)
    1), (NNamed ('k'::[]))) :: []), (Oterm (CRet, ((Bterm ([], (Vterm
    (((fun p->1+2*p) 1), (NNamed ('k'::[])))))) :: ((Bterm ([], (Oterm
    (CLambda, ((Bterm (((((fun p->2*p) ((fun p->2*p) 1)), (NNamed
    ('C'::('e'::('r'::('t'::('i'::('C'::('o'::('q'::('.'::('C'::('o'::('m'::('p'::('i'::('l'::('e'::('r'::('.'::('a'::('l'::('l'::('I'::('n'::('s'::('t'::('a'::('n'::('c'::('e'::('s'::('#'::('T'::('E'::('S'::('T'::('_'::('L'::('6'::('#'::('s'::('i'::('m'::('p'::('l'::('e'::('_'::('t'::('e'::('s'::('t'::[])))))))))))))))))))))))))))))))))))))))))))))))))))) :: ((((fun p->1+2*p)
    1), (NNamed ('k'::[]))) :: [])), (Oterm (CRet, ((Bterm ([], (Oterm
    (CKLambda, ((Bterm (((((fun p->1+2*p) 1), (NNamed ('k'::[]))) :: []),
    (Oterm (CRet, ((Bterm ([], (Vterm (((fun p->1+2*p) 1), (NNamed
    ('k'::[])))))) :: ((Bterm ([], (Vterm (((fun p->2*p) ((fun p->2*p) 1)),
    (NNamed
    ('C'::('e'::('r'::('t'::('i'::('C'::('o'::('q'::('.'::('C'::('o'::('m'::('p'::('i'::('l'::('e'::('r'::('.'::('a'::('l'::('l'::('I'::('n'::('s'::('t'::('a'::('n'::('c'::('e'::('s'::('#'::('T'::('E'::('S'::('T'::('_'::('L'::('6'::('#'::('s'::('i'::('m'::('p'::('l'::('e'::('_'::('t'::('e'::('s'::('t'::[]))))))))))))))))))))))))))))))))))))))))))))))))))))))) :: [])))))) :: []))))) :: ((Bterm
    ([], (Vterm (((fun p->1+2*p) 1), (NNamed
    ('k'::[])))))) :: [])))))) :: []))))) :: [])))))) :: []))))) :: ((Bterm
    ([], (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) ((fun p->2*p) 1)),
    (NNamed ('k'::('a'::('p'::('f'::[])))))) :: []), (Oterm (CRet, ((Bterm
    ([], (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) 1), (NNamed
    ('k'::[]))) :: []), (Oterm (CRet, ((Bterm ([], (Vterm (((fun p->1+2*p)
    1), (NNamed ('k'::[])))))) :: ((Bterm ([], (Oterm (CLambda, ((Bterm
    (((((fun p->2*p) ((fun p->2*p) 1)), (NNamed
    ('x'::[]))) :: ((((fun p->1+2*p) 1), (NNamed ('k'::[]))) :: [])), (Oterm
    (CRet, ((Bterm ([], (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) 1),
    (NNamed ('k'::[]))) :: []), (Oterm (CRet, ((Bterm ([], (Vterm
    (((fun p->1+2*p) 1), (NNamed ('k'::[])))))) :: ((Bterm ([], (Oterm
    (CLambda, ((Bterm (((((fun p->2*p) ((fun p->1+2*p) 1)), (NNamed
    ('y'::[]))) :: ((((fun p->1+2*p) 1), (NNamed ('k'::[]))) :: [])), (Oterm
    (CRet, ((Bterm ([], (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) 1),
    (NNamed ('k'::[]))) :: []), (Oterm (CRet, ((Bterm ([], (Oterm (CKLambda,
    ((Bterm (((((fun p->1+2*p) 1), (NNamed ('k'::[]))) :: []), (Oterm (CRet,
    ((Bterm ([], (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) 1), (NNamed
    ('k'::[]))) :: []), (Oterm (CRet, ((Bterm ([], (Vterm (((fun p->1+2*p)
    1), (NNamed ('k'::[])))))) :: ((Bterm ([], (Vterm (((fun p->2*p) 1),
    (NNamed
    ...
    ([], (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) 1), (NNamed
    ('k'::[]))) :: []), (Oterm (CRet, ((Bterm ([], (Vterm (((fun p->1+2*p)
    1), (NNamed ('k'::[])))))) :: ((Bterm ([], (Vterm (((fun p->2*p)
    ((fun p->2*p) 1)), (NNamed
    ('x'::[])))))) :: [])))))) :: []))))) :: ((Bterm ([], (Oterm (CKLambda,
    ((Bterm (((((fun p->1+2*p) ((fun p->1+2*p) 1)), (NNamed
    ('k'::('a'::('p'::('A'::('r'::('g'::[])))))))) :: []), (Oterm (CCall,
    ((Bterm ([], (Vterm (((fun p->1+2*p) ((fun p->2*p) 1)), (NNamed
    ('k'::('a'::('p'::('f'::[]))))))))) :: ((Bterm ([], (Vterm
    (((fun p->1+2*p) 1), (NNamed ('k'::[])))))) :: ((Bterm ([], (Vterm
    ...
    ([], (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) 1), (NNamed
    ('k'::[]))) :: []), (Oterm (CRet, ((Bterm ([], (Vterm (((fun p->1+2*p)
    1), (NNamed ('k'::[])))))) :: ((Bterm ([], (Oterm ((CDCon
    (({ inductive_mind =
    ('C'::('o'::('q'::('.'::('I'::('n'::('i'::('t'::('.'::('D'::('a'::('t'::('a'::('t'::('y'::('p'::('e'::('s'::('.'::('n'::('a'::('t'::[]))))))))))))))))))))));
    inductive_ind = 0 }, 1), (Pervasives.succ 0))), ((Bterm ([], (Oterm
    ...
    ([], (Vterm (((fun p->1+2*p) 1), (NNamed
    ('k'::[])))))) :: [])))))) :: []))))) :: [])))))) :: []))))) :: ((Bterm
    ([], (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) ((fun p->1+2*p) 1)),
    (NNamed ('k'::('a'::('p'::('A'::('r'::('g'::[])))))))) :: []), (Oterm
    (CCall, ((Bterm ([], (Vterm (((fun p->1+2*p) ((fun p->2*p) 1)), (NNamed
    ('k'::('a'::('p'::('f'::[]))))))))) :: ((Bterm ([], (Vterm
    (((fun p->1+2*p) 1), (NNamed ('k'::[])))))) :: ((Bterm ([], (Vterm
    (((fun p->1+2*p) ((fun p->1+2*p) 1)), (NNamed
    ('k'::('a'::('p'::('A'::('r'::('g'::[]))))))))))) :: []))))))) :: []))))) :: [])))))) :: []))))) :: [])))))) :: []))))) :: ((Bterm
    ...
    ([], (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) 1), (NNamed
    ('k'::[]))) :: []), (Oterm (CRet, ((Bterm ([], (Vterm (((fun p->1+2*p)
    1), (NNamed ('k'::[])))))) :: ((Bterm ([], (Oterm ((CFix
    ((Pervasives.succ 0), 0)), ((Bterm (((((fun p->2*p) 1), (NNamed
    ('a'::('d'::('d'::[]))))) :: []), (Oterm (CLambda, ((Bterm
    (((((fun p->2*p) ((fun p->2*p) 1)), (NNamed
    ('n'::[]))) :: ((((fun p->1+2*p) 1), (NNamed ('k'::[]))) :: [])), (Oterm
    (CRet, ((Bterm ([], (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) 1),
    (NNamed ('k'::[]))) :: []), (Oterm (CRet, ((Bterm ([], (Vterm
    (((fun p->1+2*p) 1), (NNamed ('k'::[])))))) :: ((Bterm ([], (Oterm
    (CLambda, ((Bterm (((((fun p->2*p) ((fun p->1+2*p) 1)), (NNamed
    ('m'::[]))) :: ((((fun p->1+2*p) 1), (NNamed ('k'::[]))) :: [])), (Oterm
    (CRet, ((Bterm ([], (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) 1),
    (NNamed ('k'::[]))) :: []), (Oterm (CRet, ((Bterm ([], (Oterm (CKLambda,
    ((Bterm (((((fun p->1+2*p) 1), (NNamed ('k'::[]))) :: []), (Oterm (CRet,
    ((Bterm ([], (Vterm (((fun p->1+2*p) 1), (NNamed
    ('k'::[])))))) :: ((Bterm ([], (Vterm (((fun p->2*p) ((fun p->2*p) 1)),
    (NNamed ('n'::[])))))) :: [])))))) :: []))))) :: ((Bterm ([], (Oterm
    (CKLambda, ((Bterm (((((fun p->1+2*p) ((fun p->2*p) 1)), (NNamed
    ('k'::('m'::('d'::[]))))) :: []), (Oterm ((CMatch ((({ inductive_mind =
    ('C'::('o'::('q'::('.'::('I'::('n'::('i'::('t'::('.'::('D'::('a'::('t'::('a'::('t'::('y'::('p'::('e'::('s'::('.'::('n'::('a'::('t'::[]))))))))))))))))))))));
    inductive_ind = 0 }, 0), 0) :: ((({ inductive_mind =
    ('C'::('o'::('q'::('.'::('I'::('n'::('i'::('t'::('.'::('D'::('a'::('t'::('a'::('t'::('y'::('p'::('e'::('s'::('.'::('n'::('a'::('t'::[]))))))))))))))))))))));
    inductive_ind = 0 }, 1), (Pervasives.succ 0)) :: []))), ((Bterm ([],
    (Vterm (((fun p->1+2*p) ((fun p->2*p) 1)), (NNamed
    ('k'::('m'::('d'::[])))))))) :: ((Bterm ([], (Oterm (CRet, ((Bterm ([],
    (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) 1), (NNamed
    ('k'::[]))) :: []), (Oterm (CRet, ((Bterm ([], (Vterm (((fun p->1+2*p)
    1), (NNamed ('k'::[])))))) :: ((Bterm ([], (Vterm (((fun p->2*p)
    ((fun p->1+2*p) 1)), (NNamed
    ('m'::[])))))) :: [])))))) :: []))))) :: ((Bterm ([], (Vterm
    (((fun p->1+2*p) 1), (NNamed ('k'::[])))))) :: [])))))) :: ((Bterm
    (((((fun p->2*p) ((fun p->2*p) ((fun p->2*p) 1))), NAnon) :: []), (Oterm
    (CRet, ((Bterm ([], (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) 1),
    (NNamed ('k'::[]))) :: []), (Oterm (CRet, ((Bterm ([], (Oterm (CKLambda,
    ((Bterm (((((fun p->1+2*p) 1), (NNamed ('k'::[]))) :: []), (Oterm (CRet,
    ((Bterm ([], (Vterm (((fun p->1+2*p) 1), (NNamed
    ('k'::[])))))) :: ((Bterm ([], (Oterm (CLambda, ((Bterm (((((fun p->2*p)
    ((fun p->1+2*p) ((fun p->2*p) 1))), (NNamed
    ('p'::[]))) :: ((((fun p->1+2*p) 1), (NNamed ('k'::[]))) :: [])), (Oterm
    (CRet, ((Bterm ([], (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) 1),
    (NNamed ('k'::[]))) :: []), (Oterm (CRet, ((Bterm ([], (Oterm (CKLambda,
    ((Bterm (((((fun p->1+2*p) 1), (NNamed ('k'::[]))) :: []), (Oterm (CRet,
    ((Bterm ([], (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) 1), (NNamed
    ('k'::[]))) :: []), (Oterm (CRet, ((Bterm ([], (Oterm (CKLambda, ((Bterm
    (((((fun p->1+2*p) 1), (NNamed ('k'::[]))) :: []), (Oterm (CRet, ((Bterm
    ([], (Vterm (((fun p->1+2*p) 1), (NNamed ('k'::[])))))) :: ((Bterm ([],
    (Vterm (((fun p->2*p) 1), (NNamed
    ('a'::('d'::('d'::[])))))))) :: [])))))) :: []))))) :: ((Bterm ([],
    (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) ((fun p->2*p) 1)), (NNamed
    ('k'::('a'::('p'::('f'::[])))))) :: []), (Oterm (CRet, ((Bterm ([],
    (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) 1), (NNamed
    ('k'::[]))) :: []), (Oterm (CRet, ((Bterm ([], (Vterm (((fun p->1+2*p)
    1), (NNamed ('k'::[])))))) :: ((Bterm ([], (Vterm (((fun p->2*p)
    ((fun p->1+2*p) ((fun p->2*p) 1))), (NNamed
    ('p'::[])))))) :: [])))))) :: []))))) :: ((Bterm ([], (Oterm (CKLambda,
    ((Bterm (((((fun p->1+2*p) ((fun p->1+2*p) 1)), (NNamed
    ('k'::('a'::('p'::('A'::('r'::('g'::[])))))))) :: []), (Oterm (CCall,
    ((Bterm ([], (Vterm (((fun p->1+2*p) ((fun p->2*p) 1)), (NNamed
    ('k'::('a'::('p'::('f'::[]))))))))) :: ((Bterm ([], (Vterm
    ...
    ('k'::('a'::('p'::('A'::('r'::('g'::[]))))))))))) :: []))))))) :: []))))) :: [])))))) :: []))))) :: [])))))) :: []))))) :: ((Bterm
    ([], (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) ((fun p->2*p) 1)),
    (NNamed ('x'::('0'::('k'::('d'::('c'::('o'::('n'::[]))))))))) :: []),
    (Oterm (CRet, ((Bterm ([], (Vterm (((fun p->1+2*p) 1), (NNamed
    ('k'::[])))))) :: ((Bterm ([], (Oterm ((CDCon (({ inductive_mind =
    ('C'::('o'::('q'::('.'::('I'::('n'::('i'::('t'::('.'::('D'::('a'::('t'::('a'::('t'::('y'::('p'::('e'::('s'::('.'::('n'::('a'::('t'::[]))))))))))))))))))))));
    ...
    ('k'::('a'::('p'::('A'::('r'::('g'::[]))))))))))) :: []))))))) :: []))))) :: [])))))) :: []))))) :: [])))))) :: []))))) :: ((Bterm
    ([], (Oterm (CKLambda, ((Bterm (((((fun p->1+2*p) 1), (NNamed
    ('k'::[]))) :: []), (Oterm (CHalt, ((Bterm ([], (Vterm (((fun p->1+2*p)
    1), (NNamed ('k'::[])))))) :: []))))) :: []))))) :: [])))))
...
let compL6_and_print_twice testL5 =
  ...
  | Ret c ->
    ...
    in
    let w = concat z (show_exp cenv0 cenv' true (eliminate t0)) in print w
