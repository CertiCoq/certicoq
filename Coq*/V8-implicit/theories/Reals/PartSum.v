...
(*i $Id: PartSum.v 9454 2006-12-15 15:30:59Z bgregoir $ i*)
...
Proof.
  ...
  assert (H2 := O_or_S N).
  elim H2; intros.
  elim a; intros; apply impl_PI.
  rewrite <- p.
  simpl in |- *; reflexivity.
  apply impl_PI; rewrite <- b in H1; elim (lt_irrefl _ H1).
...
Proof.
  ...
  elim H4; intro; apply impl_PI.
  elim a; intro; apply impl_PI.
...
Proof.
  intros; destruct (total_order_or (sum_f_R0 An N) l) as [[a|b]|r].
  left; apply a.
  right; apply b.
...
Proof.
  intros; case (total_order_T (Rabs l1) l2); intro; apply impl_PI.
  elim s; intro; apply impl_PI.
  left; apply a.
  right; apply b.
  ...
  case (Rcase_abs (Rabs l1 - Rabs (SP fn N x))); intro; apply impl_PI.
  ...
  case (Rcase_abs (sum_f_R0 An N - l2)); intro; apply impl_PI.
