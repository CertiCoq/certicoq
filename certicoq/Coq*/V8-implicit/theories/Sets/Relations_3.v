...
Section Relations_3.
   ...
   Definition noetherian_ind' [P : U -> Prop]
     (f : forall x : U,
          (forall y : U, R x y -> noetherian y) ->
          (forall y : U, R x y -> P y) -> P x):=
   fix F (x : U) (a : noetherian x) {struct a} : P x :=
   match a with
   | definition_of_noetherian a0 =>
       f x a0 (fun (y : U) (r : R x y) => F y (a0 y r))
   end.
