...
let rec compile_constr reloc c sz cont =
  ...
  | Prod(id,imp,dom,codom) ->
      let cont1 = 
        Kpush ::
        compile_constr reloc dom (sz+1) (Kmakeblock(3,accu_tag) ::
        cont) in
      let cont2 =
        Kpush :: 
        compile_constr reloc (mkLambda(id,Expl,dom,codom)) sz cont1 in
      Kconst(Const_b0 (Obj.magic imp)) :: cont2
  | Lambda _ ->
