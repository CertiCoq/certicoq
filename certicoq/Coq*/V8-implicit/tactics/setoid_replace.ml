...
(* $Id: setoid_replace.ml 9852 2007-05-23 14:00:25Z letouzey $ *)
...
let gen_app c imps v =
(*  assert (Array.length v = Array.length imps);*)
  mkApp(Lazy.force c,v,imps)
...
let coq_Build_Morphism_Theory =
  lazy(constant ["Setoid"] "Build_Morphism_Theory")
let app_Build_Morphism_Theory i o p1 p2 =
  gen_app coq_Build_Morphism_Theory [|Impl;Impl;Expl;Expl|] [|i;o;p1;p2|]
let coq_Compat = lazy(constant ["Setoid"] "Compat")
let app_Compat i o mt =
  gen_app coq_Compat [|Impl;Impl;Expl|] [|i;o;mt|]
...
let app_seq_refl a r sth =
  gen_app coq_seq_refl [|Impl;Impl;Expl|] [|a;r;sth|]
let coq_seq_sym = lazy(constant ["Setoid"] "Seq_sym")
let app_seq_sym a r sth =
  gen_app coq_seq_sym [|Impl;Impl;Expl|] [|a;r;sth|]
let coq_seq_trans = lazy(constant ["Setoid"] "Seq_trans")
let app_seq_trans a r sth =
  gen_app coq_seq_trans [|Impl;Impl;Expl|] [|a;r;sth|]
...
let app_singl t x = gen_app coq_singl [|Impl;Expl|] [|t;x|]
let app_cons t x l = gen_app coq_cons [|Impl;Expl;Expl|] [|t;x;l|]
...
let app_App = gen_app coq_App [|Impl;Impl;Expl;Expl;Expl;Expl;Expl|]
let app_ToReplace = gen_app coq_ToReplace [|Impl;Impl|]
let app_ToKeep = gen_app coq_ToKeep [|Impl;Impl;Expl;Expl;Expl|]
let app_ProperElementToKeep =
  gen_app coq_ProperElementToKeep [|Impl;Impl;Expl;Expl;Expl;Expl|]
let app_fcl_singl =
  gen_app coq_fcl_singl [|Impl;Impl;Expl;Expl;Expl;Expl;Expl|]
let app_fcl_cons =
  gen_app coq_fcl_cons
    [|Impl;Impl;Expl;Expl;Expl;Expl;Expl;Expl;Expl|]
 ...
 with
  Not_found ->
   ...
   match kind_of_term rel with
    | App (eq,[|ty|],_) when eq_constr eq (Lazy.force coq_eq) ->
        Leibniz (Some ty)
...
let cic_relation_class_of_X_relation_class typ value =
 function
    Relation {rel_X_relation_class=x_relation_class} ->
     mkApp (x_relation_class, [| typ ; value |],[|Expl;Expl|])
  | Leibniz (Some t) ->
     mkApp ((Lazy.force coq_Leibniz), [| typ ; t |],[|Impl;Expl|])
...
let cic_precise_relation_class_of_relation =
 function
    {rel_a=rel_a; rel_aeq=rel_aeq; rel_refl=Some refl; rel_sym=None} ->
      ...
      [|Expl;Expl;Expl|])
  | {rel_a=rel_a; rel_aeq=rel_aeq; rel_refl=Some refl; rel_sym=Some sym} ->
      mkApp ((Lazy.force coq_RSymmetric),
      [| rel_a ; rel_aeq; sym ; refl |],
      [|Expl;Expl;Expl;Expl|])
  | {rel_a=rel_a; rel_aeq=rel_aeq; rel_refl=None; rel_sym=None} ->
      mkApp ((Lazy.force coq_AAsymmetric),
      [| rel_a ; rel_aeq |], [|Expl;Expl|])
  | {rel_a=rel_a; rel_aeq=rel_aeq; rel_refl=None; rel_sym=Some sym} ->
      mkApp ((Lazy.force coq_ASymmetric),
      [| rel_a ; rel_aeq; sym |],[|Expl;Expl;Expl|])
...
let cic_precise_relation_class_of_relation_class =
 function
  ...
  | Leibniz (Some t) ->
     mkApp ((Lazy.force coq_eq), [| t |],[|Expl|]),
      mkApp ((Lazy.force coq_RLeibniz), [| t |],[|Expl|]), true
  | Leibniz None -> assert false
...
let cic_arguments_of_argument_class_list args =
 let rec aux =
  function
     [] -> assert false
   | [last] -> app_singl (Lazy.force coq_Argument_Class) last
   | he::tl -> app_cons (Lazy.force coq_Argument_Class) he (aux tl)
 in
...
let gen_compat_lemma_statement quantifiers_rev output args m =
 ...
 args, output,
   compose_prod quantifiers_rev
    (mkApp ((Lazy.force coq_make_compatibility_goal),
    [| args ; output ; m |],[|Expl;Expl;Expl|]))
...
let apply_to_rels c l =
 if l = [] then c
  ...
  let args = Util.list_map_i (fun i _ -> mkRel (len - i)) 0 l in
   applistc c args (List.map (fun _ -> Expl) args)

let apply_to_relation subst imps rel =
 if Array.length subst = 0 then rel
   ...
   { rel_a = mkApp (rel.rel_a, subst,imps) ;
     rel_aeq = mkApp (rel.rel_aeq, subst,imps) ;
     rel_refl = option_map (fun c -> mkApp (c,subst,imps)) rel.rel_refl ; 
     rel_sym = option_map (fun c -> mkApp (c,subst,imps)) rel.rel_sym;
     rel_trans = option_map (fun c -> mkApp (c,subst,imps)) rel.rel_trans;
     rel_quantifiers_no = new_quantifiers_no;
     rel_X_relation_class = mkApp (rel.rel_X_relation_class, subst,imps);
     rel_Xreflexive_relation_class =
      mkApp (rel.rel_Xreflexive_relation_class, subst,imps) }

let add_morphism lemma_infos mor_name
    (m,quantifiers_rev,args,aimps,output) =
 let lem =
  match lemma_infos with
     None ->
      ...
      let applied_args =
       ...
       in
       let imps = Array.of_list(List.map pi2 quantifiers_rev) in
       List.map
         (fun (v,rel) ->
           match rel with
              Leibniz (Some t) ->
            ...
            | Relation rel ->
                v, Relation (apply_to_relation subst imps rel)) args
      in
       compose_lam quantifiers_rev 
        (app_Compat(cic_arguments_of_argument_class_list applied_args)
 ...
 let args,imps =
  match kind_of_term t with
     App (he',args',imps') ->
      ...
      let imps1 = Array.sub imps' 0 argsno in
      let imps2 = Array.sub imps' argsno rel.rel_quantifiers_no in
       if is_conv env Evd.empty rel.rel_a (mkApp (he',args1,imps1)) then
        (args2,imps2)
       else
   ...
   | _  ->
       ...
       in
       Array.of_list args, Array.of_list imps
       with Reduction.NotConvertible -> 
	error_cannot_unify_signature env rel.rel_quantifiers_no t rel.rel_a
 in
  apply_to_relation args imps rel
...
let unify_morphism_with_arguments gl (c,av,avimps)
     {args=args; imps=imps; output=output; lem=lem;
      morphism_theory=morphism_theory} t
=
 ...
 let qimps,eimps = array_chop (avlen-argsno) avimps in
 let c' = mkApp (c,quantifiersv,qimps) in
 ...
 let ty = Tacmach.pf_type_of gl (mkApp (c,av,avimps)) in
 let output' = unify_relation_class_carrier_with_type (pf_env gl) output ty in
 let lem' = mkApp (lem,quantifiersv,qimps) in
 let morphism_theory' = mkApp (morphism_theory,quantifiersv,qimps) in
 ({args=args'; imps=imps; output=output'; lem=lem';
   morphism_theory=morphism_theory'},
  c',(Array.of_list al',eimps))
...
let new_morphism m signature id hook =
 if Nametab.exists_cci (Lib.make_path id) or is_section_variable id then
  ...
  let args_ty_quantifiers_rev,args,aimps,args_instance,output,output_instance =
    match signature with
	None ->
	  if args_ty = [] then
            errorlabstrm "New Morphism"
	  ...
	  let args =
            List.map
              (fun (_,_,ty) -> None,default_relation_for_carrier ty) args_ty in
          let imps = List.map(fun(_,imp,_)->imp) args_ty in
	  let output = default_relation_for_carrier output in
            [],args,imps,args,output,output
      | Some (args,output') ->
	  assert (args <> []);
	  ...
	  let number_of_quantifiers = args_ty_len - number_of_arguments in
		...
		let args_ty_quantifiers_rev, real_args_ty, real_output =
		  check_is_dependent number_of_quantifiers
                    args_ty output in
		let quantifiers_rel_context =
		  List.map (fun (n,imp,t) -> n,imp,None,t)
                    args_ty_quantifiers_rev in
		...
		let args, args_instance =
		  List.split
		    (List.map2 find_relation_class_v args real_args_ty) in
                let imps = List.map(fun _->Expl) args in
		let output,output_instance= find_relation_class output' real_output in
		  args_ty_quantifiers_rev,
                  args, imps, args_instance, output, output_instance
              end
  in
  ...
  let lem = Tacred.simpl env Evd.empty lem in
	...
	let lemma_infos = Some (id,argsconstr,outputconstr) in
	  add_morphism lemma_infos mor_name
            (m,args_ty_quantifiers_rev,args,aimps,output)
      end
    else
      begin
	new_edited id
	  (m,args_ty_quantifiers_rev,
           args,aimps,argsconstr,output,outputconstr);
...
let morphism_hook _ ref =
  ...
  let (m,quantifiers_rev,args,imps,argsconstr,output,outputconstr) =
   what_edited pf_id in
  if (is_edited pf_id)
  then 
   begin
    add_morphism (Some (pf_id,argsconstr,outputconstr)) mor_id
     (m,quantifiers_rev,args,imps,output) ;
...
let check_eq env a_quantifiers_rev a aeq =
 let typ =
  Sign.it_mkProd_or_LetIn
   (mkApp ((Lazy.force coq_relation),
   [| apply_to_rels a a_quantifiers_rev |],[|Expl|]))
    ...
    (Sign.it_mkProd_or_LetIn
     (mkApp ((Lazy.force coq_prop),
       [| apply_to_rels a   a_quantifiers_rev ;
          apply_to_rels aeq a_quantifiers_rev |],[|Expl;Expl|]))
      a_quantifiers_rev))
 then
  errorlabstrm "Add Relation Class"
...
let check_refl env a_quantifiers_rev a aeq refl =
  check_property env a_quantifiers_rev
    a aeq "reflexivity" coq_reflexive refl

let check_sym env a_quantifiers_rev a aeq sym =
  check_property env a_quantifiers_rev 
    a aeq "symmetry" coq_symmetric sym

let check_trans env a_quantifiers_rev a aeq trans =
  check_property env a_quantifiers_rev
    a aeq "transitivity" coq_transitive trans
 ...
 let a_quantifiers_rev = check_a env a in
  ...
  let x_relation_class =
   let subst,imps =
    ...
    Array.of_list
      (Util.list_map_i (fun i _ -> mkRel (len - i + 2)) 0 a_quantifiers_rev),
    Array.of_list (List.map(fun(_,imp,_,_)->imp) a_quantifiers_rev) in
   cic_relation_class_of_X_relation
    (mkRel 2) (mkRel 1) (apply_to_relation subst imps aeq_rel) in
  let _ =
   Declare.declare_internal_constant id
    (DefinitionEntry
      {const_entry_body =
        Sign.it_mkLambda_or_LetIn x_relation_class
         ([ Name (id_of_string "v"),Expl,None,mkRel 1;
            Name (id_of_string "X"),Expl,None,mkType (Termops.new_univ ())] @
            a_quantifiers_rev);
       const_entry_type = None;
       const_entry_opaque = false;
       const_entry_boxed = Options.boxed_definitions()},
  ...
  let xreflexive_relation_class =
   let subst,imps =
    ...
    Array.of_list (List.map(fun(_,imp,_,_)->imp) a_quantifiers_rev) in
   cic_precise_relation_class_of_relation
     (apply_to_relation subst imps aeq_rel) in
  let _ =
   Declare.declare_internal_constant id_precise
    (DefinitionEntry
      {const_entry_body =
        Sign.it_mkLambda_or_LetIn
          xreflexive_relation_class a_quantifiers_rev;
       const_entry_type = None;
       const_entry_opaque = false;
       const_entry_boxed = Options.boxed_definitions() },
      IsDefinition Definition) in
   ...
   | Some trans ->
      ...
      let _ =
       Declare.declare_internal_constant mor_name
        (DefinitionEntry
          {const_entry_body =
            Sign.it_mkLambda_or_LetIn lemma a_quantifiers_rev;
           const_entry_type = None;
           const_entry_opaque = false;
          const_entry_boxed = Options.boxed_definitions()},
          IsDefinition Definition)
      in
       let a_quantifiers_rev =
        List.map (fun (n,imp,b,t) -> assert (b = None); n,imp,t)
          a_quantifiers_rev in
       add_morphism None mor_name
        (aeq,a_quantifiers_rev,
         [aeq_rel_class_and_var1; aeq_rel_class_and_var2], [Expl;Expl],
         output)
...
let add_setoid id a aeq th =
 ...
 let a_quantifiers_rev = check_a env a in
  ...
  check_setoid_theory env a_quantifiers_rev a aeq  th ;
   ...
   let refl =
    Sign.it_mkLambda_or_LetIn
      (app_seq_refl a_instance aeq_instance th_instance) a_quantifiers_rev in
   let sym =
    Sign.it_mkLambda_or_LetIn
     (app_seq_sym a_instance aeq_instance th_instance) a_quantifiers_rev in
   let trans =
    Sign.it_mkLambda_or_LetIn
     (app_seq_trans a_instance aeq_instance th_instance) a_quantifiers_rev in
...
type constr_with_marks =
  | MApp of
      constr * morphism_class * constr_with_marks array * impl array *
      direction
  | ToReplace
...
let direction_of_constr_with_marks hole_direction =
 function
    MApp (_,_,_,_,dir) -> cic_direction_of_direction dir
  | ToReplace -> hole_direction
  | ToKeep (_,_,dir) -> cic_direction_of_direction dir

type argument =
   Toapply of impl * constr         (* apply the function to the argument *)
 | Toexpand of name * impl * types
   (* beta-expand the function w.r.t. an argument of this type *)
let beta_expand c args_rev =
 let rec to_expand =
  function
     [] -> []
   | (Toapply _)::tl -> to_expand tl
   | (Toexpand (name,imp,s))::tl -> (name,imp,s)::(to_expand tl) in
 let rec aux n =
  function
     [] -> []
   | (Toapply (imp,arg))::tl -> (imp,arg)::(aux n tl)
   | (Toexpand (_,imp,_))::tl -> (imp,mkRel n)::(aux (n + 1) tl)
 in
 let exp = to_expand args_rev in
 let (aimps,args) = List.split(List.rev(aux 1 args_rev)) in
  compose_lam exp
   (mkApp (c, Array.of_list args, Array.of_list aimps))
...
let relation_class_that_matches_a_constr caller_name new_goals hypt =
 let (heq, hargs,himps) = decompose_app hypt in
 ...
 let all_aeq_imps = get_all_but_last_two himps in
 let rec find_relation l limps subst imps =
  let aeq = mkApp (heq,Array.of_list l,Array.of_list limps) in
  try
   ...
   match rel,new_goals with
    ...
    | Relation rel,_ ->
        Relation
          (apply_to_relation (Array.of_list subst) (Array.of_list imps) rel)
  with Not_found ->
   if l = [] then
    errorlabstrm caller_name
     (pr_lconstr
       (mkApp (aeq, Array.of_list all_aeq_args, Array.of_list all_aeq_imps)) ++
      str " is not a registered relation.")
   else
    let last,others = Util.list_sep_last l in
    let imp,oimps = Util.list_sep_last limps in
    find_relation others oimps (last::subst) (imp::imps)
 in
  find_relation all_aeq_args all_aeq_imps [] []
      ...
      | App (c,al,aimps) -> 
         let mors_and_cs_and_als =
          let mors_and_cs_and_als =
           ...
           let rec aux acc accimps =
            function
               [],[] ->
                let c' = mkApp (c, Array.of_list acc, Array.of_list accimps) in
                let al' = [||] in
                let imps' = [||] in
                List.map (fun m -> m,c',(al',imps')) (morphism_table_find c')
             | (he::tl) as l, (himp::tlimps as imps) ->
                let c' = mkApp (c, Array.of_list acc, Array.of_list accimps) in
                let al' = Array.of_list l in
                let imps' = Array.of_list imps in
                let acc' = acc @ [he] in
                let accimps' = accimps @ [himp] in
                (List.map (fun m -> m,c',(al',imps'))
                  (morphism_table_find c')) @
                (aux acc' accimps' (tl,tlimps))
             | _ -> assert false
           in
            aux [] [] (Array.to_list al, Array.to_list aimps) in
          let mors_and_cs_and_als =
           List.map
            (function (m,c,al) ->
          ...
          let mors_and_cs_and_als =
           List.fold_left
            (fun l (m,c,(al,imps)) ->
	       try (unify_morphism_with_arguments gl (c,al,imps) m t) :: l
	       with Impossible -> l
	    ) [] mors_and_cs_and_als
          in
           List.filter
            (fun (mor,_,(_,_)) -> subrelation gl mor.output output_relation)
            mors_and_cs_and_als
         in
          (* First we look for well typed morphisms *)
          let res_mors =
           List.fold_left
            (fun res (mor,c,(al,aimps)) ->
              let a =
                ...
                (List.map
                  (fun a ->
                    if not (get_mark a) then
                     ...
                     MApp (c,ACMorphism mor,a,aimps,output_direction))
                  a'))
		 output_directions) @ res
          ...
          let res_functions =
           (* the tactic works only if the function type is
            ...
            let rec find_non_dependent_function
              env c c_args_rev typ f_args_rev f_imps_rev
              a_rev aimps_rev =
             function
                [],[] ->
                 if a_rev = [] then
		  List.map (fun output_direction ->
		   ...
		   output_directions
                  ...
                  in
                  let aimps = Array.of_list (List.rev aimps_rev) in
                   List.fold_left
                    (fun res a ->
                      if not (get_mark a) then
			List.map (fun output_direction ->
			 ...
			 output_directions @ res
                       ...
                       in
                        if err then res
                         ...
                         let mor =
                          ACFunction
                            {f_args=List.rev f_args_rev;
                             f_imps=List.rev f_imps_rev;
                             f_output=typ} in
                         let func = beta_expand c c_args_rev in
			 List.map (fun output_direction ->
                          (MApp (func,mor,a,aimps,output_direction)))
			  output_directions @ res
                    ) [] a'
              | (he::tl) as a, (imp::tlimps as imps) ->
                  ...
                  (match kind_of_term typnf with
                    Cast (typ,_,_) ->
                     find_non_dependent_function env c c_args_rev typ
                      f_args_rev f_imps_rev a_rev aimps_rev (a,imps)
                  | Prod (name,imp,s,t) ->
                     let env' = push_rel (name,imp,None,s) env in
                     let he =
                       ...
                       match noccurn 1 t, he0 with
                          _, ToKeep (arg,_,_) ->
                           ...
                           find_non_dependent_function
                             env' c ((Toapply(imp,arg))::c_args_rev)
                             (subst1 arg t) f_args_rev f_imps_rev
                             a_rev aimps_rev (tl,tlimps)
                        | true, _ ->
                           (* non-dependent product, to replace *)
                           find_non_dependent_function
                            env' c ((Toexpand (name,imp,s))::c_args_rev)
                             (lift 1 t) (s::f_args_rev) (imp::f_imps_rev)
                             (he::a_rev) (imp::aimps_rev) (tl,tlimps)
                        | false, _ ->
            ...
            in
             find_non_dependent_function (Tacmach.pf_env gl) c [] typ
               [] [] [] []
               (Array.to_list al, Array.to_list aimps)
          in
...
=
 let check =
  function
     (None,dir,dir') -> 
      mkApp ((Lazy.force coq_MSNone), [| dir ; dir' |],[|Expl;Expl|])
   | (Some true,dir,dir') ->
      assert (dir = dir');
      mkApp ((Lazy.force coq_MSCovariant), [| dir |],[|Expl|])
   | (Some false,dir,dir') ->
      assert (dir <> dir');
      mkApp ((Lazy.force coq_MSContravariant), [| dir |],[|Expl|]) in
 let rec aux =
  function
     [] -> assert false
   | [(variance,out),(value,direction)] ->
      app_singl (Lazy.force coq_Argument_Class) out,
      app_fcl_singl
   ...
   | ((variance,out),(value,direction))::tl ->
      ...
      app_cons (Lazy.force coq_Argument_Class) out outtl,
      app_fcl_cons
...
let rec cic_type_nelist_of_list =
 function
    [] -> assert false
  | [value] -> app_singl (mkType (Termops.new_univ ())) value
  | value::tl ->
      app_cons (mkType (Termops.new_univ ()))
        value (cic_type_nelist_of_list tl)
...
let syntactic_but_representation_of_marked_but hole_relation hole_direction =
 let rec aux out (rel_out,precise_out,is_reflexive) =
  function
           ...
           let mt =
            ...
            then
              mkApp ((Lazy.force coq_morphism_theory_of_predicate),
               [| cic_type_nelist_of_list f_args; f|],[|Expl;Expl|])
            else
   ...
   | ToReplace -> app_ToReplace [| hole_relation ; hole_direction |]
   | ToKeep (c,_,direction) ->
      let direction = cic_direction_of_direction direction in
       if is_reflexive then
        ...
        let c_is_proper =
         let typ = mkApp (rel_out, [| c ; c |],[|Expl;Expl|]) in
          mkCast (Evarutil.mk_new_meta (),DEFAULTcast, typ)
        in
         app_ProperElementToKeep
...
let check_evar_map_of_evars_defs evd =
 ...
 in
  List.iter
   (fun (_,binding) ->
     match binding with
        Evd.Cltyp (_,_,{Evd.rebus=rebus; Evd.freemetas=freemetas}) ->
         check_freemetas_is_empty rebus freemetas
      | Evd.Clval (_,_,({Evd.rebus=rebus1; Evd.freemetas=freemetas1},_),
  ...
  try
   ...
   let if_output_relation_is_iff gl =
    ...
    in
     ...
     in
     let impl1 = mkProd (Anonymous, Expl, hyp2, lift 1 hyp1) in
     let impl2 = mkProd (Anonymous, Expl, hyp1, lift 1 hyp2) in
      let th' = mkApp (proj, [|impl2; impl1; th|],[|Impl;Impl;Expl|]) in
       Tactics.refine
...
let analyse_hypothesis gl c =
 ...
 let (equiv, args,imps) = decompose_app (Clenv.clenv_type eqclause) in
 ...
 let oimps,_ = split_last_two imps in
  eqclause,mkApp (equiv, Array.of_list others, Array.of_list oimps),c1,c2
...
let general_setoid_replace rewrite_tac try_prove_eq_tac_opt relation c1 c2 ~new_goals gl =
   ...
   match relation with
    ...
    | None ->
       match default_relation_for_carrier (pf_type_of gl c1) with
        ...
        | Leibniz _ -> raise Optimize
  in
   let eq_left_to_right =
     mkApp (relation.rel_aeq, [| c1 ; c2 |],[|Expl;Expl|]) in
   let eq_right_to_left =
     mkApp (relation.rel_aeq, [| c2 ; c1 |],[|Expl;Expl|]) in
   let replace dir eq =
    tclTHENS (assert_tac false Anonymous Expl eq)
      [onLastHyp (fun id ->
        tclTHEN
 ...
 with
  Optimize -> (* (!replace tac_opt c1 c2) gl *)
    let eq =
      mkApp (Lazy.force  coq_eq, [| pf_type_of gl c1;c2 ; c1 |],
      [|Expl;Expl;Expl|]) in 
    tclTHENS (assert_tac false Anonymous Expl eq)
      [onLastHyp (fun id ->
		    tclTHEN
...
let setoid_symmetry_in id gl =
 let new_hyp =
  let _,he,c1,c2 = analyse_hypothesis gl (mkVar id) in
   mkApp (he, [| c2 ; c1 |],[|Expl;Expl|])
 in
 cut_replacing id Expl new_hyp (tclTHEN setoid_symmetry) gl
        ...
        | Some trans ->
           ...
           let binder =
            match List.rev argsrev with
               _::(Name n2,_,None,_)::_ -> Rawterm.NamedHyp n2
             | _ -> assert false
           in
            apply_with_bindings
