...
Definition slowFibx := (slowFib 3).
Quote Recursively Definition cbv_slowFibx :=
  ltac:(let t:=(eval cbv in slowFibx) in exact t).
Definition ans_slowFibx :=
  Eval cbv in (main (program_Program cbv_slowFibx)).
(* [program] of the program *)
Quote Recursively Definition p_slowFibx := slowFibx.
Definition P_slowFibx := Eval cbv in (program_Program p_slowFibx).
Goal
  let env := (env P_slowFibx) in
  let main := (main P_slowFibx) in
  wcbvEval (env) 90 (main) = Ret ans_slowFibx.
  vm_compute. reflexivity.
Qed.
...
Definition fibx := fib 0.
Quote Recursively Definition cbv_fibx :=
  ltac:(let t:=(eval cbv in fibx) in exact t).
Definition ans_fibx :=
  Eval cbv in (main (program_Program cbv_fibx)).
(* [program] of the program *)
Quote Recursively Definition p_fibx := fibx.
Definition P_fibx := Eval cbv in (program_Program p_fibx).

(************
Goal
  let env := (env P_fibx) in
  let main := (main P_fibx) in
  WcbvEval env main ans_fibx.
Proof.
  intros. unfold main, AstCommon.main, P_fibx, ans_fibx.
  eapply wConst.
  - reflexivity.
  - eapply wAppLam.
    + eapply wConst.
...
Goal
  let env := (env P_fibx) in
  let main := (main P_fibx) in
  wcbvEval (env) 100 (main) = Ret ans_fibx.
  vm_compute. reflexivity.
Qed.
...
Function deProof t := 
  match t with
    ...
    | TApp fn arg => TApp (deProof fn) (deProof arg)
    | TConst nm => TConst nm
...
Definition ans_and_rect_x :=
  Eval cbv in (wcbvEval (env P_and_rect_x) 1000 (main (program_Program cbv_and_rect_x))).

Goal
  let envx := env P_and_rect_x in
  let mainx := main P_and_rect_x in
  wcbvEval envx 1000 mainx = ans_and_rect_x.
  vm_compute. reflexivity.
Qed.
...
Print P_and_rectx.
Goal
  let P_envx := env P_and_rectx in
  let P_mainx := main P_and_rectx in
  wcbvEval P_envx 1000 P_mainx = Ans_and_rectx.
  vm_compute. reflexivity.
Qed.
...
Goal
    ...
    wcbvEval env 10 main = Ret (deProof ans_yyyyX).
  vm_compute. reflexivity.
Qed.
...
(****** something wrong?? "Const inductive or axiom ********
Time Quote Recursively Definition p_myMain := vs.myMain.
Time Definition P_myMain :=
  Eval vm_compute in (L2d.compile.program_Program p_myMain).
