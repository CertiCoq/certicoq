...
From L6.Heap Require Import heap heap_defs heap_equiv space_sem
     cc_log_rel size_cps closure_conversion
...
>>>>>>> 2a16e45e44aeb3acad389f282112e7c4acae95bf
...
Module ClosureConversionCorrect (H : Heap).
  ...
  Definition Fun_inv k j GI GP b d rho1 H1 rho2 H2 Scope Funs Γ FVs :=
    forall f, ~ f \in Scope -> f \in Funs ->
         exists l1 lenv B g,
           M.get f rho1 = Some (Loc l1) /\
           (* Funs locations are fresh *)
           ~ l1 \in (reach' H1 (env_locs rho1 (FV Scope Funs FVs \\ [set f]))) /\
           (* post is already reachable *)
           post H1 [set l1] \subset (reach' H1 (env_locs rho1 (Scope :|: (FromList FVs \\ (Scope :|: Funs))))) /\
  ...
  Lemma FV_inv_cc_approx_clos  (k j : nat) (IP : GIInv) (P : GInv) (b : Inj) (d : EInj)
    ...
    FV_inv k j IP P b d rho1 H1 rho2 H2 c (Empty_set _) (Empty_set _) Γ FVs ->
    NoDup FVs ->
    M.get Γ rho2 = Some (Loc l) ->
  ...
  Proof.
    ...
    - inv Hnd.
      ...
      edestruct IHHall as (FLs & Heq & Hnd' & Hget' & Hal'). eassumption. 
  ...
  Proof with (now eauto with Ensembles_DB).
    ...
    - simpl in Hdef.
      ...
      inv Hdef.
      assert (Hdef := Hdef').
      eapply FV_inv_set_not_in_FVs_l. eapply IHB1 in Hdef'.
      + eapply FV_inv_Scope_mon.
        eapply FV_inv_heap_mon; [ | | | | | | eassumption ].
        * eapply well_formed_antimon.
          eapply reach'_set_monotonic.
          rewrite <- Union_assoc, Union_commut, <- Setminus_Union.
  ...
  Lemma Fun_inv_set_r k j GI GP b d rho1 H1 rho2 H2 Scope Funs Γ FVs f v :
    Fun_inv k j GI GP b d rho1 H1 rho2 H2 Scope Funs Γ FVs ->
    ~ f \in (Funs \\ Scope) -> f <> Γ ->
    Fun_inv k j GI GP b d rho1 H1 (M.set f v rho2) H2 Scope Funs Γ FVs.
  Proof.
    intros Hfun Hnin1 Hnin2 x Hnin Hin.
    edestruct Hfun as (l1 & lenv & B1' & g & Hget1 & Hsub & Hdis & Hget2 & Hget3 & Heq).
    eassumption. eassumption.  
    do 4 eexists. repeat split; try eassumption. rewrite M.gso. eassumption.
    intros Hc. subst. eapply Hnin1; constructor; eauto.
    rewrite M.gso. eassumption. intros Hc. now subst.
  Qed.

  (* Lemma Setminus_Subset_Inclduded {A} (S1 S2 S3 : Ensemble A) : *)
  (*   S1 \subset S3 -> *)
  (*   S1 :|: (S2 \\ S1) \subset S3 :|: (S2 \\ S3). *)
  (* Proof. *)
  (*   intros Hsub l1 Hin. inv Hin; eauto. *)
  (*   inv H.  *)
  ...
  Lemma Fun_inv_subheap k j GI GP b d rho1 H1 H1' rho2 H2 H2' Scope Funs Γ FVs :
    ...
    Fun_inv k j GI GP b d rho1 H1' rho2 H2' Scope Funs Γ FVs.
  Proof with (now eauto with Ensembles_DB).
    intros Hwf1 Henv1 hwf2 Henv2 Him Hfun Hsub1 Hsub2 x Hin Hnin.
    edestruct Hfun as (l1 & lenv & B1' & g & Hget1 & Hdis & Hsub & Hget2 & Hget3 & Heq); try eassumption.
    do 4 eexists; repeat split; try eassumption.
    
    intros Hc. eapply Hdis. rewrite reach'_subheap; [ eassumption | | | eassumption ].
    ...
    left. right. now constructor. eapply Included_trans; [| eassumption ].
    ...
    - rewrite Combinators.compose_id_right. 
      ...
      { eapply Him. left. 
        ...
        now left; right; constructor; eauto.
  ...
  Proof.
    ...
    edestruct (Hfv f) as (l1 & len & B & g & Hgfet & Hdis & Hsub & Hgetf2 & Hgete & Hres).
    eassumption. rewrite Hseq. eassumption.
    do 4 eexists; repeat split; try eassumption.
    rewrite <- Hseq. eassumption.
    rewrite <- Hseq. eassumption.
    edestruct (Hfv f) as (l1 & len & B & g & Hgfet & Hdis & Hsub & Hgetf2 & Hgete & Hres).
    eassumption. now eapply Hseq. do 4 eexists; repeat split; try eassumption.
    rewrite Hseq. eassumption.
    rewrite Hseq. eassumption.
  Qed.
  ...
  Proof.
    ...
    split; intros Hfv f Hin Hnin.
    edestruct (Hfv f) as (l1 & len & B & g & Hgfet & Hdis & Hsub & Hgetf2 & Hgete & Hres).
    rewrite Hseq. eassumption. eassumption.
    do 4 eexists; repeat split; try eassumption.
    rewrite <- Hseq. eassumption.
  ...
  Lemma Fun_inv_locs_Disjoint k j GI GP b d rho1 H1 rho2 H2 Funs Scope Γ FVs f :
    Fun_inv k j GI GP b d rho1 H1 rho2 H2 Scope Funs Γ FVs ->
    f \in (Funs \\ Scope) ->
  ...
  Proof. 
    ...
    destruct Hin as [f [Hnin Hget]].
    destruct (M.get f rho1) as [[l1|] |] eqn:Hgetf; try contradiction. inv Hget. inv Hnin. 
    edestruct Hfun as (l1 & lenv & B1' & g & Hget1 & Hsub & Hdis & Hget2 & Hget3 & Heq).
    eassumption. eassumption.  
    eapply Hdis. repeat subst_exp. eassumption. 
  Qed.
  ...
  Lemma project_vars_ctx_to_heap_env Scope Scope' Funs c Γ FVs xs C vs1 rho1 rho2 H2:
    ...
    exists H2' rho2' s, ctx_to_heap_env_CC C H2 rho2 H2' rho2' s.
  Proof.
    revert Scope Scope' Funs Γ FVs C vs1
           rho1 rho2 H2.
    induction xs;
      intros Scope Scope' Funs Γ FVs C vs1
             rho1 rho2 H2 Hnin Hvars Hfun HFV Hget.
    ...
    - inv Hvars. simpl in Hget.
      ...
      + intros f Hnin' Hin.
        ...
        intros Hc; inv Hc; eauto. eapply Hnin.
      ...
      + edestruct HFV as [v' [vs [Hget [Hget1 Hall]]]]; eauto.
        ...
        * erewrite <- project_var_get; try eassumption.
          ...
          erewrite project_var_FV_eq; try eassumption.
          left. now left.
        * erewrite <- (project_var_subheap _ _ _ _ _ _ _ _ _ _ _ _ _ H7). reflexivity.
          eassumption. eassumption.
 ...
 *)
  ...
  Lemma project_var_correct GII GI k  H1 rho1 H2 rho2 H2' rho2' b d
        Scope Scope' Funs c Γ FVs x C m :
    (* well-formedness *)
    well_formed (reach' H1 (env_locs rho1 (FV Scope Funs FVs))) H1 ->
    env_locs rho1 (FV Scope Funs FVs)  \subset dom H1  ->
    ...
    (forall j, Fun_inv k j GII GI b d rho1 H1 rho2 H2 Scope Funs Γ FVs) ->
    (forall j, FV_inv k j GII GI b d rho1 H1 rho2 H2 c Scope Funs Γ FVs) ->

    injective_subdomain (reach' H1 (env_locs rho1 (Scope :|: (FromList FVs \\ (Scope :|: Funs))))) b ->
    Disjoint _ (image b (reach' H1 (env_locs rho1 (Scope :|: (FromList FVs \\ (Scope :|: Funs))))))
             (env_locs rho2 [set Γ] :|: image' d (reach' H1 (env_locs rho1 (Scope :|: (FromList FVs \\ (Scope :|: Funs)))))) ->
    ...
    exists b' d', 
      ...
      Disjoint _ (image b' (reach' H1 (env_locs rho1 (Scope' :|: (FromList FVs \\ (Scope' :|: Funs))))))
               (env_locs rho2' [set Γ] :|: image' d' (reach' H1 (env_locs rho1 (Scope' :|: (FromList FVs \\ (Scope' :|: Funs)))))).
  Proof with (now eauto with Ensembles_DB).
    intros Hwf1 Henv1 Hwf2 Henv2 Hproj Hcc Hfun Hfv Hinj Hd Hctx Hneq Hbind.
    inv Hproj.
    ...
    - inv Hctx. simpl in H13.
      ...
      destruct (M.get Γ rho2) as [v2|] eqn:Hgetg; try congruence. inv H13.
      edestruct (alloc (Constr Size.Util.clo_tag [v1; v2]) H2) as [l' H2''] eqn:Ha.
      inv H14. inv H15.
      edestruct (Hfun 0)
          as (l1 & lenv & B & f' & Hget1 & _ & _ & Hget2 & Hget3 & _); try eassumption.
      repeat subst_exp.
      ...
      + intros j.
        edestruct (Hfun j)
          as (l1' & lenv' & B' & f'' & Hget1' & _ &  _ & Hget2' & Hget3' & Hrel); try eassumption.
        repeat subst_exp.
        rewrite Ha in Hrel. eapply cc_approx_env_P_union.
        * intros y Hin. inv Hin. intros v1 Hget. repeat subst_exp. 
          eexists. rewrite M.gss. split. reflexivity. eassumption.
        * eapply cc_approx_env_P_set_not_in_P_r; [| eassumption ].
          ...
          intros Hc. eapply Fun_inv_locs_Disjoint. eapply (Hfun 0). now constructor; eauto.
      ...
      + intros j.
        ...
        * 
          ...
          eapply reach'_set_monotonic.
          eapply Included_trans; [| eapply Fun_inv_post_Included; try now apply (Hfun 0) ].
          ...
          eapply Included_trans; [| eapply Fun_inv_post_Included; try now apply (Hfun 0) ].
          eapply post_set_monotonic. eapply env_locs_monotonic...
          rewrite <- !reach'_idempotent at 1.
        ...
        * eapply f_eq_subdomain_extend_not_In_S_r. 
          intros Hc. eapply Fun_inv_locs_Disjoint. now eapply (Hfun 0). now constructor; eauto.
          constructor; eauto.
      ...
      + intros j.
        ...
        * eapply HL.subheap_refl.
        * eapply HL.alloc_subheap. eassumption.
        * eapply FV_inv_rename_ext. now eapply Hfv.
          ...
          reflexivity.
        * eauto.
      + assert (Hsub : reach' H1
                              (env_locs rho1
                                        (x |: Scope :|: (FromList FVs \\ ((x |: Scope) :|: Funs)))) \\
                              [set l1] \subset
                       reach' H1
                              (env_locs rho1
                                        (Scope :|: (FromList FVs \\ (Scope :|: Funs))))).
        { rewrite <- Union_assoc, env_locs_Union, env_locs_Singleton; eauto.
          rewrite reach'_Union, reach_unfold, <- Union_assoc. rewrite Setminus_Union_distr.
          rewrite Setminus_Same_set_Empty_set, Union_Empty_set_neut_l.
          rewrite Union_Same_set. 
          eapply Included_trans. eapply Setminus_Included.
          eapply reach'_set_monotonic; eapply env_locs_monotonic...
          rewrite <- (Union_assoc [set x] Scope), (Union_Same_set [set x]).
          rewrite (reach'_idempotent  H1 (env_locs rho1 (Scope :|: (FromList FVs \\ _)))).
          eapply reach'_set_monotonic.
          eapply Included_trans; [| eapply Fun_inv_post_Included ].
          eapply post_set_monotonic. eapply get_In_env_locs. constructor; eauto.
          eassumption. now eapply (Hfun 0).  eapply Singleton_Included... }
      ...
      + assert (Hsub : reach' H1
        ...
        eapply Disjoint_Included; 
           [ eapply Included_Union_compat; [ reflexivity |  eapply image'_extend_Included_Some ]
           | eapply image_extend_Included' | ].
        eapply Union_Disjoint_l; eapply Union_Disjoint_r.
        * eapply Disjoint_Included; [| eapply image_monotonic; eassumption | now apply Hd ].
          rewrite env_locs_set_not_In; [| intros Hc; inv Hc; eauto ]...
        * eapply Union_Disjoint_r.

          eapply Disjoint_Included; [| | now apply Hd ]...
          
          eapply Disjoint_Singleton_r. intros Hc.
          eapply Hd. constructor. eapply image_monotonic; eassumption.
          left. eapply get_In_env_locs. reflexivity. eassumption. reflexivity. 
        * constructor. intros l2 Hc. inv Hc. inv H3. eapply Hcn.
          eapply reachable_in_dom. eassumption. eassumption.
          eapply reach'_extensive. rewrite env_locs_Singleton in H4; [| rewrite M.gso; eauto ].
          inv H4.
          eapply get_In_env_locs; eauto.
          unfold FV_cc... reflexivity. 
        * eapply Union_Disjoint_r.
          ...
          eapply reachable_in_dom. eassumption. eassumption.
          eapply reach'_extensive. eapply get_In_env_locs; unfold FV_cc; try eassumption...
    - inv Hctx. inv H18.
      assert (Hseq : FV (x |: Scope) Funs FVs <--> FV Scope Funs FVs).
      { unfold FV. rewrite <- (Union_assoc [set x] Scope Funs) at 1.
        rewrite !(Union_commut [set x]) at 2.
      ...
      + intros j.
        edestruct (Hfv j) as (v1 & vs1 & Hget1 & Hget2 & Hall).
      ...
      + assert (Hseq' : x |: Scope :|: (FromList FVs \\ (x |: Scope :|: Funs)) <-->
        ...
        rewrite !Hseq' at 1.
        rewrite env_locs_set_not_In. split; eassumption.
        intros Hc; inv Hc; eauto.
  Qed.
  ...
  Lemma project_var_binding_in_map Scope Scope' Funs c Γ FVs x C1 rho1 H1 rho2 H2 m :
    project_var Size.Util.clo_tag Scope Funs c Γ FVs x C1 Scope' ->
  ...
  Lemma project_vars_binding_in_map Scope Scope' Funs c Γ FVs xs C1 rho1 H1 rho2 H2 m :
    project_vars Size.Util.clo_tag Scope Funs c Γ FVs xs C1 Scope' ->
    ctx_to_heap_env_CC C1 H1 rho1 H2 rho2 m ->
    binding_in_map Scope rho1 ->
    binding_in_map Scope' rho2. 
  Proof.
  ...
  Lemma project_vars_correct GII GI k  H1 rho1 H2 rho2 H2' rho2' b d
        Scope Scope' Funs c Γ FVs xs C m :
    (* well-formedness *)
    well_formed (reach' H1 (env_locs rho1 (FV Scope Funs FVs))) H1 ->
    env_locs rho1 (FV Scope Funs FVs)  \subset dom H1  ->
    well_formed (reach' H2 (env_locs rho2 (FV_cc Scope Funs Γ))) H2 ->
    ...
    exists b' d', 
      ...
      Disjoint _ (image b' (reach' H1 (env_locs rho1 (Scope' :|: (FromList FVs \\ (Scope' :|: Funs))))))
               (env_locs rho2' [set Γ] :|: image' d' (reach' H1 (env_locs rho1 (Scope' :|: (FromList FVs \\ (Scope' :|: Funs)))))).
  Proof with (now eauto with Ensembles_DB).
    intros Hwf1 Hlocs1 Hwf2 Hlocs2 Hvars.
    revert b d m H1 rho1 H2 rho2 H2' rho2' Hwf1 Hlocs1 Hwf2 Hlocs2.
    induction Hvars;
      intros b d m H1 rho1 H2 rho2 H2' rho2' Hwf1 Hlocs1 Hwf2 Hlocs2 Hcc Hfun Hfv
             Hinj Hd Hctx Hnin Hbin.
    - inv Hctx. do 2 eexists. split; [| split; [| split ]]; eauto.
    - edestruct ctx_to_heap_env_CC_comp_ctx_f_l as [rho3 [H3 [m1 [m2 [Hctx2 [Hctx3 Heq]]]]]].
      ...
      edestruct project_var_correct as (b' & d' & Hcc' & Hfun' & Hfv' & Hinj' & Hd');
        try eassumption.
      intros Hc; eapply Hnin. subst. rewrite FromList_cons...
  ...
  Lemma Closure_conversion_fundefs_correct
    ...
    forall  B1 B1' B2 B2'
      ...
      well_formed (reach' H1 (env_locs rho1 (FV Scope Funs FVs))) H1 ->
      env_locs rho1 (FV Scope Funs FVs) \subset dom H1  ->
      well_formed (reach' H2 (env_locs rho2 (Γ' |: FV_cc Scope Funs Γ))) H2 ->
      env_locs rho2 (Γ' |: FV_cc Scope Funs Γ) \subset dom H2  ->
            
      (forall j, (H1, rho1) ⋞ ^ (Scope; k; j; Pre; fun _ : nat => Post 0; b; d) (H2, rho2)) ->
      (forall j, Fun_inv k j Pre (fun _ : nat => Post 0) b d rho1 H1 rho2 H2 Scope Funs Γ FVs) ->
      (forall j, FV_inv k j Pre (fun _ : nat => Post 0) b d rho1 H1 rho2 H2 c Scope Funs Γ FVs) ->

      injective_subdomain (reach' H1 (env_locs rho1 (Scope :|: (FromList FVs \\ (Scope :|: Funs))))) b ->
      Disjoint _ (image b (reach' H1 (env_locs rho1 (Scope :|: (FromList FVs \\ (Scope :|: Funs))))))
               (env_locs rho2 [set Γ] :|:
                         image' d (reach' H1 (env_locs rho1 (Scope :|: (FromList FVs \\ (Scope :|: Funs)))))) ->
      ...
      (forall S, env_locs rho1c S \subset reach' H1 (env_locs rho1 (FromList FVs'))) ->
      ...
      (forall j, Fun_inv k j Pre (fun _ : nat => Post 0) b d
                    rho1' H1' (def_funs B2 B2' rho2) H2
                    (Scope \\ name_in_fundefs B1) (name_in_fundefs B1 :|: Funs) Γ' FVs).
  Proof with (now eauto with Ensembles_DB).
    Opaque cc_approx_exp.
    induction B1;
    intros B1' B2 B2' H1 H1' H2 rho1 rho1c rho1' rho2 b d
           Scope Funs H FVs FVs' c Γ Γ' 
           Hwf1 Hlocs1 Hwf2 Hlocs2 Hcc Hfun Hfv Hinj Hd Hfvs' Hfveq
           Hun1 Hun2 Hincl Hincfv Hccf1 Hccf2 Hclo j.
    destruct (Hfvs' j) as (vs & lenv & Hgetlenv & Hget' & HallP).
    simpl in Hclo.
    - inv Hccf1. simpl def_funs; simpl name_in_fundefs. 
      destruct (def_closures B1 B1' rho1 H1 rho1c) as [Hc rhoc] eqn:Hdef'.
      destruct (alloc (Clos (FunPtr B1' v) rho1c) Hc) as [l1 H1''] eqn:Ha. 
      inv Hclo. intros f1 Hin Hnin.
      assert (Hwfc :  well_formed
                        (reach' Hc
                                (env_locs rhoc
                                          (FV (Scope \\ (name_in_fundefs (Fcons f1 f l e B1)))
                                              (name_in_fundefs (Fcons f1 f l e B1) :|: Funs) FVs \\ [set f1])))
                        Hc).
      { eapply well_formed_antimon;
        [| eapply well_formed_reach_alloc_def_funs;
           [ now apply Hwf1 | eassumption | | eassumption ]]. 
        eapply reach'_set_monotonic. eapply env_locs_monotonic. unfold FV. 
        rewrite !Setminus_Union_distr.
        eapply Union_Included.
        eapply Union_Included. now eauto with Ensembles_DB. 
        simpl. rewrite !Setminus_Union_distr. 
        rewrite Setminus_Included_Empty_set; [| now eauto with Ensembles_DB ].
        rewrite Union_Empty_set_neut_l.
        eapply Union_Included.
        now eauto with Ensembles_DB.   
        eapply Included_Union_preserv_l.
        rewrite (Union_Setminus_Included Scope Funs Scope).
        now eauto with Ensembles_DB.   
        admit. reflexivity.
        rewrite (Union_Setminus_Included _ (FromList FVs) (Scope :|: Funs)).
        now eauto with Ensembles_DB. admit.
        rewrite (Union_Setminus_Included Scope Funs Scope).
        now eauto with Ensembles_DB.   
        admit. reflexivity.
        eapply Included_trans. eapply Hincl. 
        eapply reach'_set_monotonic. eapply env_locs_monotonic.
        eapply Included_trans... }
      
      destruct (var_dec v f1); subst.
      + setoid_rewrite M.gss.
        exists l1, lenv, B2', f1. split; [ | split; [ | split; [ | split; [ | split ] ] ]].
        * (* M.get f rho1 = Some (Loc l1) *)
          reflexivity.
        * (* ~ l1 \in reach H1' *)
          intros Hin'. rewrite env_locs_set_not_In in Hin'.
          rewrite <- well_formed_reach_alloc_same in Hin'; try eassumption.
          
          eapply reachable_in_dom in Hin'. destruct Hin' as [v1 Hgetl1].
          erewrite alloc_fresh in Hgetl1; eauto. congruence.
          eassumption.
          admit. admit.

          intros Hc'; inv Hc'; eauto.
        * rewrite post_Singleton; [| now erewrite gas; eauto ]. simpl.
          rewrite env_locs_set_not_In.  eapply Included_trans.
          now eapply Hincl.
          eapply Included_trans; [| eapply reach'_heap_monotonic ].

          eapply reach'_set_monotonic. 
          eapply Included_trans; [| eapply def_closures_env_locs_subset; try eassumption ].
          eapply env_locs_monotonic. rewrite Setminus_Union_distr. 
          eapply Included_Union_preserv_l. rewrite Setminus_Union.
          eapply Included_trans; [ eassumption |]...

          eapply HL.subheap_trans. now eapply def_funs_subheap; eauto.
          eapply HL.alloc_subheap. eassumption.

          intros Hc'. now inv Hc'; inv H0; eauto.

        * reflexivity.
        * admit. (* M.get def_funs *)
        * destruct (alloc (Constr Size.Util.clo_tag [FunPtr B2' f1; Loc lenv]) H2)
            as [lenv2 H2'] eqn:Ha'.
          simpl cc_approx_val'. erewrite !gas; eauto.
          ...
          { split.
            ...
            - intros i Hlt. setoid_rewrite <- Hfveq.
              eapply FV_inv_cc_approx_clos; [| admit | eassumption ]. 
              (* Empty because ~ names B1 \subset FromSet FVs' *)
              admit.
      ...
      + setoid_rewrite (@M.gso _ f1 v); eauto.
        repeat setoid_rewrite (@M.gso _ Γ' v); eauto. 
        edestruct IHB1 with (j := j)
          as (l1' & lenv' & B1'' & g & Hget1 & Hsub & Hdis & Hget2 & Hget3 & Heq);
          try eassumption.
        * admit.
        * eapply Included_trans; [ eassumption |]...
        * intros Hin'; eapply Hin. inv Hin'. constructor.
          eassumption. intros Hc1. eapply H3. inv Hc1; eauto.
          inv H4; eauto. exfalso; eauto.
        * inv Hnin; eauto. inv H0; eauto.
          inv H3; exfalso; eauto. 
        * destruct (alloc (Constr Size.Util.clo_tag [FunPtr B1'' g; Loc lenv']) H2)
            as [l2 H2'] eqn:Ha2.
          rewrite <- Union_assoc in Hnin. inv Hnin. inv H0; contradiction.

          do 4 eexists. split. eassumption.
          { split; [| split; [| split; [ eassumption | split; [ eassumption | ] ]]].
            - intros Hinr.
              eapply reach'_set_monotonic in Hinr. 
              eapply reach'_alloc_set in Hinr; [| | eassumption ].
              + inv Hinr.
                * inv H3. 
                  assert (Hdom : ~ l1' \in dom Hc).
                  { intros [v1 Hgetl1'].
                    erewrite alloc_fresh in Hgetl1'; eauto. congruence. }
                  destruct Heq as [_ Heq].
                  edestruct (get l1' Hc) as [v1 |] eqn:Hgetl1'; try contradiction.
                  eapply Hdom; eexists; eauto.
                * eapply Hsub. eassumption.
              + simpl. eapply Included_trans. eapply Hincl.
                eapply Included_trans; [| eapply reach'_heap_monotonic; eapply def_funs_subheap; eauto ].
                eapply reach'_set_monotonic.
                eapply Included_trans. eapply env_locs_monotonic. eassumption.
                unfold FV. rewrite !Setminus_Union_distr.
                rewrite !env_locs_Union. do 2 eapply Included_Union_preserv_l.
                eapply Included_trans; [| eapply def_closures_env_locs_subset; eassumption ].
                eapply env_locs_monotonic.
                rewrite !Setminus_Union. rewrite (Union_commut [set f1]), Union_assoc.
                rewrite <- Setminus_Union. eapply Included_Setminus.
                eapply Disjoint_Singleton_r. eassumption.
                eapply Included_Setminus_compat. reflexivity.
                now eauto with Ensembles_DB.
              + eapply env_locs_monotonic.
                eapply Included_trans. eapply Included_Setminus_compat.
                rewrite <- Union_assoc. eapply FV_Union2. reflexivity. 
                eapply Included_trans. eapply Included_Setminus_compat.
                rewrite (Union_commut [set v] (name_in_fundefs B1)).
                rewrite <- Setminus_Union. eapply Included_Union_compat.
                reflexivity. eapply FV_Setminus1. tci. reflexivity.
                rewrite !Setminus_Union_distr... 
            - admit.
            - rewrite Ha2.

              assert (Ha' : well_formed (reach' Hc (val_loc (Loc l1'))) Hc) by admit. 
              ...
              * eapply Singleton_Included.
                destruct Heq as [_ Heq].
                edestruct (get l1' Hc) as [v1 |] eqn:Hgetl1'; try contradiction.
                edestruct (get l2 H2') as [v2 |] eqn:Hgetl2; try contradiction.
                eexists; eauto. destruct v1 as [| [|]]; try contradiction.
              * eapply HL.alloc_subheap. eassumption. }
        * admit. (* add assumption *)
    - simpl.
      eapply Proper_Fun_inv_Scope. eapply Setminus_Empty_set_neut_r.
      reflexivity. reflexivity. reflexivity.
      eapply Proper_Fun_inv_Funs. eapply Union_Empty_set_neut_l.
      reflexivity. reflexivity.
      inv Hclo. inv Hccf1. simpl. 
      eapply Hfun. eassumption.
      eassumption. Setminus_Empty_set_neut_r. reflexivity. reflexivity. reflexivity.

      1. _l. rewrite
        clear f1 Hin Hnin Hwfc. Opaque def_funs. 
        
        setoid_rewrite env_locs_set_not_In.  
        ...
        Focus 6.
        eapply IHB1.
clear Hwfc n. revert f1 Hin Hnin.
