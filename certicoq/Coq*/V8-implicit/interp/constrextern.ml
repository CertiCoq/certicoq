...
(* $Id: constrextern.ml 9918 2007-06-30 11:01:05Z herbelin $ *)
...
let rec check_same_pattern p1 p2 =
  match p1, p2 with
    ...
    | CPatAtom(_,r1,_), CPatAtom(_,r2,_) when r1=r2 -> ()
...
let rec check_same_type ty1 ty2 =
  ...
  | CCoFix(_,(_,id1),fl1), CCoFix(_,(_,id2),fl2) when id1=id2 ->
      List.iter2 (fun (id1,bl1,a1,b1) (id2,bl2,a2,b2) ->
        ...
        check_same_type b1 b2)
        fl1 fl2
  | CArrow(_,imp1,a1,b1), CArrow(_,imp2,a2,b2) when imp1=imp2->
      check_same_type a1 a2;
      check_same_type b1 b2
  ...
  | CLambdaN(_,bl1,a1), CLambdaN(_,bl2,a2) ->
      List.iter2 check_same_binder bl1 bl2;
      check_same_type a1 a2
  | CLetIn(_,(_,na1),imp1,a1,b1), CLetIn(_,(_,na2),imp2,a2,b2)
      when na1=na2 && imp1=imp2 ->
      check_same_type a1 a2;
      check_same_type b1 b2
  | CAppExpl(_,r1,al1), CAppExpl(_,r2,al2) when r1=r2 ->
      let al1,imps1 = List.split al1 in
      let al2,imps2 = List.split al2 in
      if imps1<>imps2 then
        failwith "not same imps";
      List.iter2 check_same_type al1 al2
  | CApp(_,(_,e1),al1), CApp(_,(_,e2),al2) ->
      check_same_type e1 e2;
      List.iter2 (fun ((a1,imp1),e1) ((a2,imp2),e2) ->
                    if e1<>e2 || imp1<>imp2 then failwith "not same expl";
...
and check_same_binder (nal1,e1) (nal2,e2) =
  List.iter2 (fun ((_,na1),imp1) ((_,na2),imp2) ->
    if imp1<>imp2 then failwith "not same implicit status";
...
and check_same_fix_binder bl1 bl2 =
  List.iter2 (fun b1 b2 ->
    match b1,b2 with
      ...
      | LocalRawDef(na1,imp1,def1), LocalRawDef(na2,imp2,def2) ->
          check_same_binder ([(na1,imp1)],def1) ([(na2,imp2)],def2)          
...
let rec same_patt p1 p2 =
  match p1, p2 with
      PatVar(_,na1,_), PatVar(_,na2,_) -> if na1<>na2 then failwith "PatVar"
...
let rec same_raw c d =
  ...
  | RApp(_,f1,a1,imps1), RApp(_,f2,a2,imps2) when imps1=imps2 ->
      List.iter2 same_raw (f1::a1) (f2::a2)
  | RLambda(_,na1,imp1,t1,m1), RLambda(_,na2,imp2,t2,m2) when imp1=imp2 ->
      if na1 <> na2 then failwith "RLambda";
      same_raw t1 t2; same_raw m1 m2
  | RProd(_,na1,imp1,t1,m1), RProd(_,na2,imp2,t2,m2) when imp1=imp2 ->
      if na1 <> na2 then failwith "RProd";
      same_raw t1 t2; same_raw m1 m2
  | RLetIn(_,na1,imp1,t1,m1), RLetIn(_,na2,imp2,t2,m2) ->
      if na1 <> na2 || imp1<>imp2 then failwith "RLetIn";
      same_raw t1 t2; same_raw m1 m2
      ...
      List.iter2
        (fun (t1,(al1,oind1)) (t2,(al2,oind2)) ->
          ...
          option_iter2(fun (_,i1,_,nl1) (_,i2,_,nl2) ->
            if i1<>i2 || nl1 <> nl2
            then failwith "RCases") oind1 oind2) c1 c2;
  ...
  | RRec(_,fk1,na1,bl1,ty1,def1), RRec(_,fk2,na2,bl2,ty2,def2) ->
      ...
      array_iter2
        (List.iter2 (fun (na1,imps1,bd1,ty1) (na2,imps2,bd2,ty2) ->
          if imps1<>imps2 || na1<>na2 then failwith "RRec";
...
let rec match_cases_pattern metas sigma a1 a2 = match (a1,a2) with
  ...
  | PatVar (_,Anonymous,_), AHole _ -> sigma
  | a, AHole _ -> sigma
...
let rec extern_cases_pattern_in_scope (scopes:local_scopes) vars pat =
  ...
  with No_match ->
  match pat with
  | PatVar (loc,Name id,imp) -> CPatAtom (loc,Some (Ident (loc,id)),imp)
  | PatVar (loc,Anonymous,imp) -> CPatAtom (loc, None,imp) 
	...
	(* Try availability of interpretation ... *)
        match keyrule with
	      ...
	      let qid = shortest_qualid_of_syndef vars kn in
              CPatAtom (loc,Some (Qualid (loc, qid)),Expl)
      with
  ...
  let rec exprec q = function
    | a::args, imp::impl when is_status_implicit imp ->
	  ...
	  (!print_implicits_defensive &
	   is_significant_implicit (fst a) impl tail &
	   not (is_inferable_implicit inctx n imp))
	in
        if visible then 
	  (a,Some (dummy_loc, ExplByName (fst(name_of_implicit imp)))) :: tail 
	else
	  tail
    | a::args, _::impl -> (a,None) :: exprec (q+1) (args,impl)
    | args, [] ->  (*In case of polymorphism*)
        List.map (fun a -> (a,None)) args
    | [], _ -> [] in
...
let rec remove_coercions inctx = function
  | RApp (loc,RRef (_,r),args,imps) as c
	  ...
	  | Some n when n < nargs && (inctx or n+1 < nargs) ->
	      ...
	      let limps = list_skipn n imps in
              let (a,l,limps) =
                match l,limps with
                  | a::l,_::limps -> (a,l,limps)
                  | _ -> assert false in
              let (a,l,limps) =
                (* Recursively remove the head coercions *)
                match remove_coercions true a with
                  | RApp (_,a,l',imps') -> a,l'@l,imps'@limps
                  | a -> a,l,limps in
              if l = [] then a
              else
                (* Recursively remove coercions in arguments *)
                RApp (loc,a,List.map (remove_coercions true) l, limps)
	  | _ -> c
      with Not_found -> c)
  | c -> c
...
let rec share_fix_binders n rbl ty def =
  match ty,def with
      RProd(_,na0,imp0,t0,b), RLambda(_,na1,imp1,t1,m) when imp0=imp1->
            ...
            match na0, na1 with
              ...
              | Anonymous, Name id -> 
                  if not (occur_rawconstr id b) then (na1,b,m)
                    ...
                    failwith "share_fix_binders: capture"
              | _ -> (na1,b,m) in
          share_fix_binders (n-1) ((na,imp0,None,t0)::rbl) b m
...
let rec extern inctx scopes vars r =
	 ...
	 | RRef (rloc,ref) ->
	     ...
	     let args =
	       extern_args (extern true) (snd scopes) vars args subscopes in
             let args = List.combine args imps in
	     extern_app loc inctx (implicits_of_global ref)
               (Some ref,extern_reference rloc vars ref)
	       args
	 | _       -> 
             let args = List.combine args imps in
             explicitize loc inctx []
               (None,sub_extern false scopes vars f)
               (List.map 
                 (fun (a,impa) -> sub_extern true scopes vars a, impa)
                 args))

  | RProd (loc,Anonymous,imp,t,c) ->
      (* Anonymous product are never factorized *)
      CArrow (loc,imp,extern_typ scopes vars t, extern_typ scopes vars c)

  | RLetIn (loc,na,imp,t,c) ->
      CLetIn (loc,(loc,na),imp,sub_extern false scopes vars t,
              extern inctx scopes (add_vname vars na) c)

  | RProd (loc,na,imp,t,c) ->
      let t = extern_typ scopes vars (anonymize_if_reserved na t) in
      let (idl,c) = factorize_prod scopes (add_vname vars na) imp t c in
      CProdN (loc,[((dummy_loc,na),imp)::idl,t],c)

  | RLambda (loc,na,imp,t,c) ->
      let t = extern_typ scopes vars (anonymize_if_reserved na t) in
      let (idl,c) =
        factorize_lambda inctx scopes (add_vname vars na) imp t c in
      CLambdaN (loc,[((dummy_loc,na),imp)::idl,t],c)
	...
	(na',option_map (fun (loc,ind,n,nal) ->
	  let params = list_tabulate
	    (fun _ -> RHole (dummy_loc,Evd.InternalHole)) n in
          let pimps = List.map(fun _ -> Expl) params in
	  let args = List.map (function
	    | Anonymous, _ -> RHole (dummy_loc,Evd.InternalHole) 
	    | Name id, _ -> RVar (dummy_loc,id)) nal in
          let aimps = List.map (fun _ -> Expl) args in
	  let t = RApp (dummy_loc,RRef (dummy_loc,IndRef ind),
                        params@args,pimps@aimps) in
  ...
  | RLetTuple (loc,nal,(na,typopt),tm,b) ->
      let naimps = List.map(fun (na,imp)->((dummy_loc,na),imp)) nal in
      CLetTuple (loc,naimps,
        (option_map (fun _ -> na) typopt,
         option_map (extern_typ scopes (add_vname vars na)) typopt),
        sub_extern false scopes vars tm,
        extern false scopes
          (List.fold_left (fun a (na,_) -> add_vname a na) vars nal) b)
...
and factorize_prod scopes vars imp aty c =
  try 
    if !Options.raw_print or !print_no_symbol then raise No_match;
    ([],extern_symbol scopes vars c (uninterp_notations c))
  with No_match -> match c with
  | RProd (loc,(Name id as na),imp1,ty,c)
      when same aty (extern_typ scopes vars (anonymize_if_reserved na ty))
	& not (occur_var_constr_expr id aty) (* avoid na in ty escapes scope *)
        & imp=imp1 ->
      let (nal,c) = factorize_prod scopes (Idset.add id vars) imp aty c in
      (((loc,Name id),imp)::nal,c)
  | c -> ([],extern_typ scopes vars c)

and factorize_lambda inctx scopes vars imp aty c =
  try 
    if !Options.raw_print or !print_no_symbol then raise No_match;
    ([],extern_symbol scopes vars c (uninterp_notations c))
  with No_match -> match c with
  | RLambda (loc,na,imp1,ty,c)
      when same aty (extern_typ scopes vars (anonymize_if_reserved na ty))
	& not (occur_name na aty) (* To avoid na in ty' escapes scope *)
        & imp=imp1 ->
      let (nal,c) =
	factorize_lambda inctx scopes (add_vname vars na) imp aty c in
      (((loc,na),imp)::nal,c)
...
and extern_local_binder scopes vars = function
  ...
  | (na,imp,Some bd,ty)::l ->
      ...
      (na::ids,
       LocalRawDef((dummy_loc,na), imp, extern false scopes vars bd) :: l)
          ...
          (ids,LocalRawAssum(nal,ty')::l)
            when same ty ty' &
              match na with Name id -> not (occur_var_constr_expr id ty')
                | _ -> true ->
              (na::ids,
               LocalRawAssum(((dummy_loc,na),imp)::nal,ty')::l)
        | (ids,l) ->
            (na::ids,
             LocalRawAssum([((dummy_loc,na),imp)],ty) :: l))
...
and extern_symbol (tmp_scope,scopes as allscopes) vars t = function
  ...
  | (keyrule,pat,n as _rule)::rules ->
	...
	let (t,args) = match t,n with
	  | RApp (_,(RRef _ as f),args,imps), Some n
              when List.length args >= n ->
	      let args1, args2 = list_chop n args in
	      let imps1, imps2 = list_chop n imps in
	      (if n = 0 then f else RApp (dummy_loc,f,args1,imps1)),
              List.combine args2 imps2
	  | RApp (_,(RRef _ as f),args,imps), None ->
              f, List.combine args imps
	...
	(* Try availability of interpretation ... *)
 	...
 	if args = [] then e 
	  ...
	  (List.map
            (fun (a,imp) -> extern true allscopes vars a, imp) args)
      with
...
let it_destPLambda n c =
  let rec aux n nal c =
    if n=0 then (nal,c) else match c with
      | PLambda (na,_,_,c) -> aux (n-1) (na::nal) c
...
let rec raw_of_pat env = function
  ...
  | PApp (f,args) ->
      RApp (loc,raw_of_pat env f,array_map_to_list (raw_of_pat env) args,
            array_map_to_list(fun _ -> Expl) args)
  | PSoApp (n,args) ->
      RApp (loc,RPatVar (loc,(true,n)),
        List.map (raw_of_pat env) args,List.map(fun _->Expl) args)
  | PProd (na,imp,t,c) ->
      RProd (loc,na,imp,raw_of_pat env t,raw_of_pat (na::env) c)
  | PLetIn (na,imp,t,c) ->
      RLetIn (loc,na,imp,raw_of_pat env t, raw_of_pat (na::env) c)
  | PLambda (na,imp,t,c) ->
      RLambda (loc,na,imp,raw_of_pat env t, raw_of_pat (na::env) c)
  | PIf (c,b1,b2) ->
      RIf (loc, raw_of_pat env c, (Anonymous,None), 
           raw_of_pat env b1, raw_of_pat env b2)
  | PCase ((LetStyle,[|n|],ind,None),PMeta None,tm,[|b|]) ->
      let nal,b =
        it_destRLambda_or_LetIn_names n (raw_of_pat env b) in
...
and raw_of_eqn env constr construct_nargs branch =
  let make_pat x env b ids =
    ...
    PatVar (dummy_loc,Name id,Term.Expl),(Name id)::env,id::ids
  in
