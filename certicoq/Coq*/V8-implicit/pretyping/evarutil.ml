...
(* $Id: evarutil.ml 9868 2007-05-29 10:01:06Z herbelin $ *)
...
let whd_castappevar_stack sigma c = 
  let rec whrec (c, l, limps as s) =
    match kind_of_term c with
      | Evar (ev,args) when Evd.mem sigma ev & Evd.is_defined sigma ev -> 
	  whrec (existential_value sigma (ev,args), l, limps)
      | Cast (c,_,_) -> whrec (c, l, limps)
      | App (f,args,imps) ->
          whrec (f, Array.fold_right (fun a l -> a::l) args l,
                 Array.fold_right (fun a l -> a::l) imps limps)
      | _ -> s
  in 
  whrec (c, [], [])
...
let new_evar_instance sign evd imp
    typ ?(src=(dummy_loc,InternalHole)) instance =
...
let push_rel_context_to_named_context env typ =
  ...
  let vars = 
    fold_named_context (fun env (id,_,b,_) l -> mkVar id :: l) env ~init:[] in
  ...
  let (subst, _, _, inst, env) =
    Sign.fold_rel_context
      (fun (na,imp,c,t) (subst, n, avoid, inst, env) ->
	(*
	  match na with
	...
	*)
	let id = next_name_away na avoid in
	  ...
	  push_named (id,imp,option_map (substl subst) c,substl subst t) env))
      (rel_context env) ~init:([], n, avoid, vars, env) in
    (named_context_val env, substl subst typ, inst)
      
let new_evar evd env ?(src=(dummy_loc,InternalHole)) imp typ =
  let sign,typ',instance = push_rel_context_to_named_context env typ in
    assert (not (dependent dummy_var typ));
    new_evar_instance sign evd imp typ' ~src:src instance

  (* The same using side-effect *)
let e_new_evar evd env ?(src=(dummy_loc,InternalHole)) imp ty =
  let (evd',ev) = new_evar !evd env ~src:src imp ty in
    evd := evd';
    ev
          ...
          (try List.assoc t subst
           with Not_found ->
             if rigid then error()
               ...
               not strict_inverse &&
               List.exists (fun (id',_,_,_) -> id=id') (evar_context evi)
             then
               failwith "cannot solve pb yet"
             else t)
      | Evar (ev,args) ->
...
let do_restrict_hyps env k evd ev args =
  ...
  let nc = e_new_evar evd env'
    ~src:(evar_source ev !evd) Expl evi.evar_concl in
  ...
  in 
    match kind_of_term c with
	  ...
	  if Evd.is_defined_evar !evd (e,l) then
	    ...
	    let ev'=
              e_new_evar evd env ~src:(evar_source e !evd) Expl	nconcl in
...
let solve_pattern_eqn env l1 imps1 c =
  let c' = List.fold_right2 (fun a imp c ->
    let c' = subst_term (lift 1 a) (lift 1 c) in
    match kind_of_term a with
      (* Rem: if [a] links to a let-in, do as if it were an assumption *)
      | Rel n ->
          let (na,_,_,t) = lookup_rel n env in mkLambda (na,imp,lift n t,c')
      | Var id ->
          let (id,_,_,t) = lookup_named id env in mkNamedLambda id imp t c'
      | _ -> assert false) 
    l1 imps1 c in
  whd_eta c'
...
let solve_refl conv_algo env isevars ev argsv1 argsv2 =
  ...
  let (evd',newev) =
    ...
    new_evar isevars env
      ~src:(evar_source ev isevars) Expl evd.evar_concl in
...
let define_evar_as_abstraction abs evd imp (ev,args) =
  let evi = Evd.find (evars_of evd) ev in
  let evenv = evar_env evi in
  let (evd1,dom) = new_evar evd evenv Expl (new_Type()) in
  let nvar =
    next_ident_away (id_of_string "x")
      (ids_of_named_context (evar_context evi)) in
  let newenv = push_named (nvar, imp, None, dom) evenv in
  let (evd2,rng) =
    new_evar evd1 newenv
      ~src:(evar_source ev evd1) Expl (new_Type()) in
  let prod = abs (Name nvar, imp, dom, subst_var nvar rng) in
  ...
  let prod' = abs (Name nvar, imp, mkEvar evdom, mkEvar evrng) in
  (evd3,prod')

let define_evar_as_arrow evd imp (ev,args) =
  define_evar_as_abstraction (fun t -> mkProd t) evd imp (ev,args)

let define_evar_as_lambda evd imp (ev,args) =
  define_evar_as_abstraction (fun t -> mkLambda t) evd imp (ev,args)
...
let split_tycon loc env isevars tycon = 
  let rec real_split c = 
    ...
    let t = whd_betadeltaiota env sigma c in
      match kind_of_term t with
	| Prod (na,imp,dom,rng) -> isevars, (na, imp, dom, rng)
	| Evar ev when not (Evd.is_defined_evar isevars ev) ->
	    let (isevars',prod) = define_evar_as_arrow isevars Expl ev in
	    let (_,imp,dom,rng) = destProd prod in
	      isevars',(Anonymous, imp, dom, rng)
	| _ -> error_not_product_loc loc env sigma c
  in
    match tycon with
      | None -> isevars,(Anonymous,Expl,None,None)
      | Some (abs, c) ->
	  (match abs with
	       None -> 
		 let isevars', (n, imp, dom, rng) = real_split c in
		   isevars', (n, imp, mk_tycon dom, mk_tycon rng)
	     | Some (init, cur) ->
		 if cur = 0 then 
		   let isevars', (x, imp, dom, rng) = real_split c in
		     isevars, (Anonymous, imp,
			       Some (Some (init, 0), dom), 
			       Some (Some (init, 0), rng))
		 else
		   isevars,
                   (Anonymous, Expl, None, Some (Some (init, pred cur), c)))
