...
let int_to_real n =
   ...
   then get coq_R0
     ...
     (let s=ref (get coq_R1) in
      for i=1 to (nn-1) do
        s:=mkApp (get coq_Rplus,[|get coq_R1;!s|],[|Expl;Expl|])
      done;
      if n<0 then mkApp (get coq_Ropp, [|!s|],[|Expl|]) else !s)
;;
...
let rational_to_real x =
   ...
   mkApp (get coq_Rmult,
     [|int_to_real n;mkApp(get coq_Rinv,[|int_to_real d|],[|Expl|])|],
     [|Expl;Expl|])
;;
...
let my_cut c gl=
  ...
  apply_type (mkProd(Anonymous,Expl,c,concl)) [create_meta()] [Expl]
    gl
;;
...
let list_of_sign s = List.map (fun (x,_,_,z)->(x,z)) s;;
...
let rec fourier gl=
	...
	(*print_string "Fourier's method can prove the goal...";flush stdout;*)
	  ...
	  let t1=ref (mkApp(get coq_Rmult,
	                  [|rational_to_real c1; h1.hleft|],[|Expl;Expl|])) in
	  let t2=ref (mkApp(get coq_Rmult,
	                  [|rational_to_real c1; h1.hright|],[|Expl;Expl|])) in
	  List.iter (fun (h,c) ->
	       s:=(!s)||(h.hstrict);
	       t1:=(mkApp(get coq_Rplus,
	                  [| !t1;
                             mkApp(get coq_Rmult,
                                   [| rational_to_real c; h.hleft|],
                                   [| Expl;Expl|])|],
                          [|Expl;Expl|]));
	       t2:=(mkApp(get coq_Rplus,
	                  [| !t2;
                             mkApp(get coq_Rmult,
                                   [| rational_to_real c; h.hright|],
                                   [|Expl;Expl|]) |],
                          [|Expl;Expl|])))
               lutil;
          let ineq=mkApp((if (!s) then get coq_Rlt else get coq_Rle),
			 [| !t1; !t2 |],[|Expl;Expl|]) in
...
(* en attendant Field, ça peut aider Ring de remplacer 1/1 par 1 ... *)	
					  ...
					  (tclTHEN (apply (get coq_sym_eqT))
                       ...
                       !tac1]);
	   tac:=(tclTHENS (cut Expl (get coq_False))
