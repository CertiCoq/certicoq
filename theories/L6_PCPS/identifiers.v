Require Import List BinNat Relations Coq.MSets.MSetRBT List Ensembles.
Require Import cps cps_util set_util Ensembles_util.
...
Instance Decidable_name_in_fundefs (B : fundefs) : Decidable (name_in_fundefs B).
Proof.
  constructor.
...
(** [fun+in_fundefs B] is the set of functions defined in [B] *)
...
Lemma name_in_fundefs_find_def_None f B:
  ~ name_in_fundefs B f ->
  find_def f B = None.
Proof.
...
Proof.
  ...
  - inv Hspl.
    + edestruct IHB3 as [B1' [B2' [Heq Hspl]]]; eauto.
      exists (Fcons v t0 l e0 B1'), B2'. rewrite Heq; eauto. split; eauto.
      simpl. constructor; eauto.
    + exists Fnil, B3; eauto.
  - inv Hspl.
Qed.
  ...
  Same_set var (occurs_free_fundefs (Fcons f t xs e B))
           (Union var (Setminus var (occurs_free e)
                                (Union var (Singleton var f)
                                       (Union var (FromList xs) (name_in_fundefs B))))
...
Lemma split_fds_fun_in_fundefs B1 B2 B3 :
  split_fds B1 B2 B3 ->
  Same_set _ (fun_in_fundefs B3) (Union _ (fun_in_fundefs B1) (fun_in_fundefs B2)).
Proof.
...
Lemma occurs_free_fundefs_big_cup B :
  Same_set _ (occurs_free_fundefs B)
           (big_cup (fun_in_fundefs B)
                    (fun p =>
                       let '(f, _, xs, e) := p in
                       (Setminus _ (occurs_free e)
                                 (Union _ (FromList xs) (name_in_fundefs B))))).
Proof.
  induction B; simpl.
  - rewrite occurs_free_fundefs_Fcons. rewrite IHB.
    ...
    rewrite Setminus_Union, <- Union_assoc, (Union_sym (Singleton var v)).
    apply Same_set_refl.
...
Proof.
  ...
  apply Same_set_refl. apply Same_set_Union_compat.
  apply Same_set_refl. rewrite !name_in_fundefs_big_cup_fun_in_fundefs.
  apply Same_Set_big_cup_l; eauto.
Qed.
  
Lemma same_split_fds_closed_occurs_free_fundefs B1 B2 B3 B3' :
  split_fds B1 B2 B3 ->
  split_fds B1 B2 B3' ->
...
Proof.
  ...
  rewrite same_split_fds_closed_occurs_free_fundefs; eauto.
Qed.
...
Proof.
  ...
  constructor; eauto.
  inv H0. constructor; eauto.
  inv H. constructor 3; eauto. constructor 2; eauto.
Qed.
...
Proof.
  ...
  - inv Hun1. constructor; eauto.
    + intros Hc. apply H5. 
      ...
      inv HD. exfalso. eapply H0. constructor; eauto.
      constructor; eauto.
    + rewrite split_fds_bound_vars; eauto.
      constructor. intros x H. inv H. inv H0; eauto.
      eapply H7. constructor; eauto.
      eapply HD; constructor; eauto. constructor; eauto.
    + rewrite split_fds_bound_vars; eauto.
      constructor. intros x H. inv H. inv H1; eauto.
      eapply H8. constructor; eauto. 
      eapply HD; constructor; eauto. constructor 3; eauto.
...
Lemma find_def_not_name_in_fundefs f B1 :
  ~ name_in_fundefs B1 f ->
  find_def f B1 = None.
Proof.
  induction B1; eauto; intros H.
  simpl. destruct (M.elt_eq f v); subst.
  exfalso. apply H; left; eauto.
  apply IHB1. intros H'. apply H. constructor 2; eauto.
Qed.
...
Lemma fun_in_fundefs_name_in_fundefs f tau xs e B :
    ...
    name_in_fundefs B f.
Proof.
  induction B; intros H; eauto; try now inv H.
  ...
  - simpl in H.
    ...
    + inv H. inv H0. congruence. inv Hun; eauto.
      ...
      * simpl. constructor; eauto.
        ...
        rewrite (fundefs_append_bound_vars B1 (Fcons f tau xs e B2)
...
Proof.
  ...
  - edestruct fun_in_fundefs_unique_bindings_split
    ...
    + erewrite find_def_fundefs_append_r.
      ...
      apply find_def_not_name_in_fundefs.
...
Proof.
  ...
  - rewrite !name_in_fundefs_find_def_None; eauto.
    intros Hn. apply H.
...
Proof.
  ...
  intros [[[f tau] xs] e]; rewrite Setminus_Union; rewrite <- Union_assoc.
  rewrite split_fds_name_in_fundefs; eauto. apply Same_set_refl.
...
Proof.
  split; intros x H; inv H; eauto.
  apply Bound_Efun1; eauto. apply Bound_Efun2; eauto.
Qed.
...
Proof.
  ...
  { eapply split_fds_unique_bindings_fundefs_r; [ | | | apply Hsp2 ]; eauto.
    inv H2. rewrite bound_var_Efun in *. constructor; eauto. intros H. apply H7.
    apply Bound_Efun2; eauto.
    eapply Disjoint_Union_r; eauto.
    eapply Disjoint_Union_r; eauto.
    inv H13; eauto.
...
Proof.
  ...
  - simpl. destruct (M.elt_eq f v); subst.
    + edestruct split_fds_cons_l_append_fundefs as [B1' [B2' [Heq Hspl3]]]; eauto.
      ...
      eapply find_def_not_name_in_fundefs. intros Hc.
