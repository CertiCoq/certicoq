  ...
  intros; apply (H t); elim H1; clear H1; intros; split;
    [ apply Rle_trans with (Rmin b a); try assumption; right;
      ...
      | apply Rle_trans with (Rmax b a); try assumption; right;
        unfold Rmax in |- * ];
    (case (Rle_dec a b); case (Rle_dec b a); intros; apply impl_PI;
...
Proof.
  ...
  unfold Rmin in |- *; case (Rle_dec a b); intro; apply impl_PI;
    [ reflexivity | elim n0; assumption ].
  assert (H13 : Rmax a b = b).
  unfold Rmax in |- *; case (Rle_dec a b); intro; apply impl_PI;
    [ reflexivity | elim n0; assumption ].
...
Proof.
  ...
  apply RiemannInt_P2 with f un wn; apply impl_PI; assumption.
  assert (H1 : b <= a) by (apply impl_PI; auto with real).
    ...
    apply existT with (- x); unfold Un_cv in |- *; unfold Un_cv in p; 
      intros; elim (p _ H4); intros; exists x0; intros; 
        generalize (H5 _ H6); unfold R_dist, RiemannInt_SF in |- *;
          case (Rle_dec b a); case (Rle_dec a b); intros; apply impl_PI.
  elim n; assumption.
  unfold vn' in H7;
...
Proof.
  ...
  elim (phi_sequence_prop vn pr2 n); intros psi_vn H5;
    elim (phi_sequence_prop un pr1 n); intros psi_un H6;
      ...
      rewrite <- StepFun_P30.
  case (Rle_dec a b); intro; apply impl_PI.
  apply Rle_lt_trans with
    (RiemannInt_SF
      (mkStepFun
        (StepFun_P32
...
Proof.
  ...
  intro; assert (H2 := Nzorn H0 H1); elim H2; intros; exists x; elim p; intros;
    split.
  apply H3.
  destruct (total_order_or (a + INR (S x) * del) b) as [[a0|b0]|r].
...
Proof.
  ...
  case (Rle_dec x1 D); intro; apply impl_PI.
  assumption.
    ...
    forall eps:posreal,
      ...
      (fun delta:posreal =>
        forall x y:R,
          ...
          a <= y <= b -> Rabs (x - y) < delta -> Rabs (f x - f y) < eps).
Proof.
  intro f; intros; destruct (total_order_T a b) as [[a0|b0]|r].
  assert (H0 := Heine_cor1 (impl_PI a0) H eps); elim H0; intros; apply existT with x;
    elim p; intros; apply H2; assumption.
  apply existT with (mkposreal _ Rlt_0_1); intros; assert (H3 : x = y);
    [ elim H0; elim H1; intros; rewrite (impl_PI b0) in H3; rewrite (impl_PI b0) in H5;
  ...
  apply existT with (mkposreal _ Rlt_0_1); intros; elim H0; intros;
    elim (impl_PI (Rlt_irrefl _ (Rle_lt_trans _ _ _ (Rle_trans _ _ _ H3 H4) r))).
Qed.
...
Proof.
  ...
  rewrite SubEqui_P5; reflexivity.
  rewrite SubEqui_P6.
  case (Rle_dec (a + INR (S N) * del) t0); intro; apply impl_PI.
...
Proof.
  intros; case (total_order_T a b); intro;
    [ elim s; intro;
      [ apply RiemannInt_P6; apply impl_PI; assumption | rewrite (impl_PI b0); apply RiemannInt_P7 ]
      | elim (impl_PI (Rlt_irrefl _ (Rle_lt_trans _ _ _ H r))) ].
Qed.
...
Proof.
  ...
  elim (X eps); intros; split with x; elim p; intros; split with x0; elim p0;
    intros; split; try assumption; rewrite (impl_PI e); intros; 
  ...
  unfold Rdiv in |- *; apply Rmult_lt_0_compat;
    [ apply (cond_pos eps)
      | apply Rinv_0_lt_compat; apply Rmult_lt_0_compat;
        [ prove_sup0 | apply Rabs_pos_lt; apply impl_PI; assumption ] ].
  ...
  apply Rinv_0_lt_compat; apply Rabs_pos_lt; apply impl_PI; assumption.
  rewrite <- Rmult_assoc; rewrite <- Rinv_l_sym;
    [ rewrite Rmult_1_l;
      replace (/ Rabs l * (eps / 2)) with (eps / (2 * Rabs l));
      [ apply H2
        | unfold Rdiv in |- *; rewrite Rinv_mult_distr;
          [ ring | discrR | apply Rabs_no_R0; apply impl_PI; assumption ] ]
      | apply Rabs_no_R0; apply impl_PI; assumption ].
Qed.
...
Proof.
  ...
  case (Rle_dec a b); intro Hyp; apply impl_PI.
  assert (H4 : 0 < eps / 3).
  unfold Rdiv in |- *; apply Rmult_lt_0_compat;
    [ assumption | apply Rinv_0_lt_compat; prove_sup0 ].
...
Proof.
  intros; case (Rle_dec a b); intro; apply impl_PI;
...
Proof.
  intros; case (Rle_dec l1 l2); intro; apply impl_PI.
  assumption.
  assert (H2 : l2 < l1).
  auto with real.
...
Proof.
  ...
  intros; unfold phi2_m in |- *; simpl in |- *; unfold phi2_aux in |- *;
    case (Req_EM_T t a); case (Req_EM_T t b); intros; apply impl_PI.
  ...
  apply StepFun_P37; try assumption.
  intros; unfold phi2_m in |- *; simpl in |- *; unfold phi2_aux in |- *;
    case (Req_EM_T x1 a); case (Req_EM_T x1 b); intros; apply impl_PI.
  ...
  elim H7; clear H7; intros; unfold phi2_aux in |- *; case (Req_EM_T x1 a);
    case (Req_EM_T x1 b); intros; apply impl_PI.
  ...
  match Rle_dec a x with
    | left [r] =>
      match Rle_dec x b with
        | left [r0] => RiemannInt (pr x (impl_PI r) (impl_PI r0))
...
Proof.
  ...
  unfold primitive in |- *; case (Rle_dec a b); case (Rle_dec b b); intros;
    [ apply RiemannInt_P5
      | elim (impl_PI n); right; reflexivity
      | elim (impl_PI n); assumption
      | elim (impl_PI n0); assumption ].
  symmetry  in |- *; unfold primitive in |- *; case (Rle_dec a a);
    case (Rle_dec a b); intros; apply impl_PI;
      [ apply RiemannInt_P9
        | elim n; assumption
...
Proof.
  ...
  unfold Rmin in |- *; case (Rle_dec a c); intro; apply impl_PI;
  ...
  auto with real.
  unfold Rmin in |- *; case (Rle_dec b c); intro; apply impl_PI;
    [ reflexivity | elim n0; assumption ].
  unfold Rmax in |- *; case (Rle_dec a c); case (Rle_dec b c); intros; apply impl_PI;
  ...
  simpl in |- *; intros; unfold psi3 in |- *; elim H0; clear H0; intros;
    case (Rle_dec a x); case (Rle_dec x b); intros; apply impl_PI;
  ...
  case (Rle_dec a x); case (Rle_dec x b); intros;  apply impl_PI; reflexivity || elim n;
    assumption.
  ...
  unfold Rmin in |- *; case (Rle_dec a b); intro; apply impl_PI;
    [ reflexivity | elim n; assumption ].
  left; elim H7; intros; assumption.
  unfold phi3 in |- *; case (Rle_dec a x); case (Rle_dec x b); intros; apply impl_PI;
    reflexivity || elim n; assumption.
...
Proof.
  ...
  assumption.
  unfold Rmax in |- *; case (Rle_dec a c); intro; apply impl_PI;
    [ reflexivity | elim n; assumption ].
  unfold Rmax in |- *; case (Rle_dec a b); intro; apply impl_PI;
    [ reflexivity | elim n; apply Rle_trans with c; assumption ].
  unfold Rmin in |- *; case (Rle_dec a c); case (Rle_dec a b); intros; apply impl_PI;
    [ reflexivity
...
Proof.
  ...
  assumption.
  unfold Rmin in |- *; case (Rle_dec c b); intro; apply impl_PI;
    [ reflexivity | elim n; assumption ].
  unfold Rmin in |- *; case (Rle_dec a b); intro; apply impl_PI;
    [ reflexivity | elim n; apply Rle_trans with c; assumption ].
  unfold Rmax in |- *; case (Rle_dec c b); case (Rle_dec a b); intros; apply impl_PI;
    [ reflexivity
      | elim n; apply Rle_trans with c; assumption
      | elim n; assumption
      | elim n0; assumption ].
...
Proof.
  ...
  apply RiemannInt_P1; assumption.
  apply impl_PI; split; [ assumption | auto with real ].
  apply RiemannInt_P1; apply RiemannInt_P21 with b; try apply impl_PI;
...
Proof.
  ...
  left; assumption.
  unfold Rmax in |- *; case (Rle_dec b c); intro; apply impl_PI;
    [ reflexivity | elim n0; assumption ].
  do 2
    rewrite <-
      (Rplus_comm
...
Proof.
  intros; case (Rle_dec a b); case (Rle_dec b c); intros; apply impl_PI.
  apply RiemannInt_P25; assumption.
  case (Rle_dec a c); intro; apply impl_PI.
  assert (H : c <= b).
  auto with real.
...
Proof.
  ...
  apply continuity_implies_RiemannInt; try (apply impl_PI; assumption).
  ...
  apply RiemannInt_P1; apply continuity_implies_RiemannInt; try apply impl_PI; auto with real.
  intros; apply C0; elim H7; intros; split.
  apply Rle_trans with (x + h0).
  ...
  unfold Rdiv in |- *; rewrite Rabs_mult; case (Rle_dec x (x + h0)); intro; apply impl_PI.
  apply Rle_lt_trans with
    (RiemannInt
      (RiemannInt_P16
  ...
  case (Rle_dec x b); intros; apply impl_PI; try (elim n; assumption || left; assumption).
  rewrite <- (RiemannInt_P26 (FTC_P1 h C0 (impl_PI r0) (impl_PI r)) H7
                             (FTC_P1 h C0 (impl_PI r2) (impl_PI r1))); ring.
...
Proof.
  ...
  split with (mkposreal _ H10); intros; case (Rcase_abs h0); intro; apply impl_PI.
  assert (H14 : b + h0 < b).
  ...
  intros; unfold primitive in |- *; case (Rle_dec a (b + h0));
    ...
    intros; apply impl_PI; try (elim n; right; reflexivity) || (elim n; left; assumption).
  rewrite <- (RiemannInt_P26 (FTC_P1 h C0 (impl_PI r3) (impl_PI r2)) H13
                             (FTC_P1 h C0 (impl_PI r1) (impl_PI r0))); ring.
  elim n; assumption.
  left; assumption.
  apply Rplus_le_reg_l with (- a - h0).
  replace (- a - h0 + a) with (- h0); [ idtac | ring ].
  ...
  unfold primitive in |- *; case (Rle_dec a (b + h0));
    case (Rle_dec (b + h0) b); intros; apply impl_PI;
  ...
  unfold f_b in |- *; unfold Rminus in |- *; rewrite Rplus_opp_r;
    rewrite Rmult_0_r; rewrite Rplus_0_l; unfold primitive in |- *;
      case (Rle_dec a b); case (Rle_dec b b); intros; apply impl_PI;
        [ apply RiemannInt_P5
          | elim n; right; reflexivity
          | elim n; left; assumption
          | elim n; right; reflexivity ].
(*****)
  ...
  intros; case (Rcase_abs h0); intro; apply impl_PI.
  assert (H12 : a + h0 < a).
...
(*****)
  ...
  assert (H7 : 0 < del).
  unfold del in |- *; unfold Rmin in |- *; case (Rle_dec x0 x1); intro; apply impl_PI.
  apply (cond_pos x0).
  apply (cond_pos x1).
  split with (mkposreal _ H7); intros; case (Rcase_abs h0); intro; apply impl_PI.
  assert (H10 : a + h0 < a).
  pattern a at 2 in |- *; rewrite <- Rplus_0_r; apply Rplus_lt_compat_l;
    assumption.
  rewrite H1; unfold primitive in |- *; case (Rle_dec a (a + h0));
    case (Rle_dec (a + h0) b); case (Rle_dec a a); case (Rle_dec a b); 
    intros; apply impl_PI; try (elim n; right; assumption || reflexivity).
  ...
  replace (RiemannInt (FTC_P1 h C0 (impl_PI r1) (impl_PI r0))) with (f_b b).
  fold (f_b (b + h0)) in |- *.
...
Proof.
  intro f; intros; case (Rle_dec a b); intro;
    [ apply continuity_implies_RiemannInt; apply impl_PI; try assumption; intros;
      apply (cont1 f)
      | assert (H : b <= a);
        [ apply impl_PI; auto with real
...
Proof.
  intro f; intros; case (Rle_dec a b); intro; apply impl_PI;
