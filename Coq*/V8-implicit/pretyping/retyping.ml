...
let sort_of_atomic_type env sigma ft args =
  let rec concl_of_arity env ar =
    ...
    | Prod (na, imp, t, b) ->
        concl_of_arity (push_rel (na,imp,None,t) env) b
    | Sort s -> s
    | _ -> decomp_sort env sigma (subst_type env sigma ft (Array.to_list args))
  in concl_of_arity env ft

let type_of_var env id =
  try let (_,_,_,ty) = lookup_named id env in ty
  with Not_found ->
...
let typeur sigma metamap =
  let rec type_of env cstr=
    ...
    | Case (_,p,c,lf) ->
        let Inductiveops.IndType(_,realargs,rimps) =
          try Inductiveops.find_rectype env sigma (type_of env c)
          with Not_found -> anomaly "type_of: Bad recursive type" in
        let t = whd_beta (applist (p, realargs,rimps)) in
        (match kind_of_term (whd_betadeltaiota env sigma (type_of env t)) with
          | Prod _ -> whd_beta (applist (t, [c],[Expl]))
          | _ -> t)
    | Lambda (name,imp,c1,c2) ->
          mkProd (name, imp,c1, type_of (push_rel (name,imp,None,c1) env) c2)
    | LetIn (name,imp,b,c1,c2) ->
         subst1 b (type_of (push_rel (name,imp,Some b,c1) env) c2)
    | Fix ((_,i),(_,tys,_)) -> tys.(i)
    | CoFix (i,(_,tys,_)) -> tys.(i)
    | App(f,args,_) when isGlobalRef f ->
  ...
  and sort_of env t = 
    ...
    | Prod (name,imp,t,c2) ->
        (match (sort_of env t,
                sort_of (push_rel (name,imp,None,t) env) c2) with
	  | _, (Prop Null as s) -> s
    ...
    | App(f,args,_) -> sort_of_atomic_type env sigma (type_of env f) args
  ...
  and sort_family_of env t =
    ...
    | Sort (Type u) -> InType
    | Prod (name,imp,t,c2) ->
        sort_family_of (push_rel (name,imp,None,t) env) c2
    | App(f,args,_) -> 
