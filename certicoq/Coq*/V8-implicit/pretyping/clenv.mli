...
(*i $Id: clenv.mli 9850 2007-05-22 21:37:55Z herbelin $ i*)
...
(* returns the minimum expliciteness of the proof term. Expl means
   the proof of the clenv uses only computational vars *)
val clenv_imp       : clausenv -> impl
...
val clenv_environments :
 evar_defs -> int option -> types ->
   evar_defs * constr list * impl list * types
...
val clenv_environments_evars :
 env -> evar_defs -> int option -> types ->
   evar_defs * constr list * impl list * types

(* [clenv_conv_leq env sigma t c n] looks for c1...cn s.t. [t <= c c1...cn] *)
val clenv_conv_leq :
 env -> evar_map -> types -> constr -> int -> constr list * impl list
