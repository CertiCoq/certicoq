...
Definition print_inductive i : string :=
  match i with
  | mkInd str m => ("(ind: " ++ str ++ " " ++ nat_to_string m ++ ")")
  end.
Fixpoint print_exp e : string :=
  match e with
  | Var_e N => "(Var)"
  | Lam_e nm e => "(Lam)"
  | App_e fn arg => "(App)"
  | Con_e (i, m) _ => ( "(Con: " ++  print_inductive i ++ ")" )
  | Match_e _ _ _ => "(Match)"
  | Let_e _ _ _ => "(Let)"
  | Fix_e _ _ => "(Fix)"
  | Ax_e _ => "(Ax)"
...
with print_exps es: string :=
       match es with
       | enil => "(enil)"
       | econs _ _ => "(econs)"
       end
with print_efnlst efns: string :=
       match efns with
       | eflnil => "(eflnil)"
       | eflcons _ _ _ => "(eflcons)"
       end
with print_branches bs: string :=
       match bs with
       | brnil_e => "(brnil)"
       | brcons_e _ _ _ _ => "(brcons)"
       end.
...
Definition isLam (t:exp) : Prop := exists nm bod, t = Lam_e nm bod.
Lemma isLam_dec: forall t, {isLam t}+{~ isLam t}.
induction t;
  try (solve [right; intros h; unfold isLam in h;
              elim h; intros x h1; elim h1; intros x0 h2; discriminate]).
left. exists n, t. reflexivity.
Qed.

Definition isFix (t:exp) : Prop := exists ds n, t = Fix_e ds n.
Lemma isFix_dec: forall t, {isFix t}+{~ isFix t}.
induction t;
  try (solve [right; intros h; unfold isFix in h;
              elim h; intros x h1; elim h1; intros x0 h2; discriminate]).
left. exists e, n. reflexivity.
Qed.


(** Big-step evaluation for [exp]. *)
Inductive eval: exp -> exp -> Prop :=
| eval_Prf: eval Prf_e Prf_e
| eval_aPrf: forall fn arg, eval fn Prf_e -> eval (App_e fn arg) Prf_e
...
| eval_App_e: forall e1 na e1' e2 v2 v,
                 ...
                 eval (e1'{0 ::= v2}) v -> 
                 eval (App_e e1 e2) v
| eval_AppCong_e: forall fn efn arg arg1,
              eval fn efn ->
              ~ isLam efn -> ~ isFix efn -> Prf_e <> efn ->
              eval arg arg1 ->
              eval (App_e fn arg) (App_e efn arg1)
...
| eval_FixApp_e: forall e (es:efnlst) n e2 e' e'',
    eval e (Fix_e es n) ->
    enthopt (N.to_nat n) es = Some e' ->
    eval (App_e (sbst_fix es e') e2) e'' ->
    eval (App_e e e2) e''
| eval_Ax_e : forall s, eval (Ax_e s) (Ax_e s)
...
Proof.
  ...
  - apply H. inversion H0; subst; try reflexivity.
    + apply H. inversion e. specialize (H _ e).
  - inversion H2; subst.
...
Function eval_n (n:nat) (e:exp) {struct n}: exception exp := 
  match n with
    | 0%nat => raise "eval_n: out of time"
    | S n => match e with
               | Lam_e na d => Ret (Lam_e na d)
               | Fix_e es k => Ret (Fix_e es k)
               | Con_e d es => 
                 match evals_n n es with
                     | Exc s => raise ("eval_n, Con, args don't eval, " ++ s)
                     | Ret vs => Ret (Con_e d vs)
                 end
               | App_e e1 e2 =>
                   ...
                   | Ret (Fix_e es k) =>
                     match enthopt (N.to_nat k) es with
                       | Some e' =>
                         ...
                         eval_n n (App_e t' e2)
                       | None => raise ("Fix redex, enthopt fail")
                     end
                   | Ret Prf_e => Ret Prf_e  (* proof redex *)
                   | Ret T =>  (* App congruence *)
                     match eval_n n e2 with
                     | Exc s =>
                       raise "eval_n: TApp congruence, arg doesn't eval"
                     | Ret ea1 => Ret (App_e T ea1)
                     end
                   | Exc s => raise ("App_e, " ++ s)
                  end
               | Let_e _ e1 e2 =>
                 match eval_n n e1 with
                   | Exc s => raise s
                   | Ret v1 => eval_n n (e2{0::=v1})
                 end
               | Match_e e p bs =>
                 match eval_n n e with
                 | Ret (Con_e d vs) =>
                   match find_branch d (exps_length vs) bs with
                   | None => raise ("find_branch, "
                                      ++ (print_inductive (fst d)))
                     | Some e' => eval_n n (sbst_list e' vs)
                   end
                 | Ret x =>
                   raise ("Match_e: discriminee not constructor, "
                            ++ (print_exp x))
                 | Exc s => raise ("Match_e: discriminee doesn't eval, " ++ s)
                 end
               | Ax_e s => Ret (Ax_e s)
               | Prf_e => Ret Prf_e
               | Var_e e => raise "Var_e"
...
with evals_n n (es:exps) : exception exps :=
       match n with
         | 0%nat => raise "evals_n: out of time"
         | S n => 
           match es with
             | enil => Ret enil
             | econs e es => 
               match (eval_n n e, evals_n n es) with
                 | (Ret v, Ret vs) => Ret (econs v vs)
                 | (Ret _, Exc s) => raise ("econs, RE, " ++ s)
                 | (Exc s, Ret _) => raise ("econs, ER, " ++ s)
                 | (Exc s1, Exc s2) =>
                   raise ("econs, EE, " ++ s1 ++ ", " ++ s2)
               end
           end
       end.
...
Lemma eval_n_monotone:
  forall (n:nat) e f, eval_n n e = Ret f ->
                      forall (m:nat), eval_n (n + m) e = Ret f.
Proof.
Admitted.

Lemma evals_n_monotone:
  forall (n:nat) es fs, evals_n n es = Ret fs ->
                        forall (m:nat), evals_n (n + m) es = Ret fs.
Admitted.
...
Lemma eval_n_Ret_Succ:
  forall e (n:nat) v, eval_n n e = Ret v -> n = (S (n - 1))%nat.
Proof.
  induction n; intros.
  - simpl in H. discriminate.
  - omega.
Qed.
Lemma evals_n_Ret_Succ:
  forall es (n:nat) vs, evals_n n es = Ret vs -> n = (S (n - 1))%nat.
Proof.
  induction n; intros.
  - simpl in H. discriminate.
  - omega.
...
Lemma evaln_eval: 
  forall (n:nat),
  (forall (e:exp) v, eval_n n e = Ret v -> eval e v) /\
  (forall (es:exps) vs, evals_n n es = Ret vs -> evals es vs).
Proof.
  apply (eval_evals_n_ind (fun n e ov => forall v, ov = Ret v -> eval e v)
        (fun n es ovs => forall vs, ovs = Ret vs -> evals es vs));
 ...
 + subst. eapply eval_FixApp_e; try eassumption; intuition.
  + injection H0; intros; subst. specialize (H _ e4).
    constructor. assumption.
  + injection H1; intros; subst. specialize (H _ e4).
    constructor; intuition.
    * destruct H2 as [x0 [x1 jx]]. subst. contradiction.
    * destruct H2 as [x0 [x1 jx]]. subst. contradiction.
    * destruct H2 as [x0 [x1 jx]]. subst. contradiction.
  + econstructor.
    * specialize (H _ e4). eapply H.
  ...
  + econstructor; eauto. 
  + injection H; intros h0. subst. constructor.
  + injection H; intros h0. subst. econstructor.     
  + injection H; intros h0. subst. constructor.
...
Lemma eval_evaln:
  (forall (e v:exp), eval e v -> exists (n:nat), eval_n n e = Ret v) /\
  (forall (es vs:exps), evals es vs -> exists (n:nat), evals_n n es = Ret vs).
Proof.
  assert (jjj:forall (m:nat) (x:nat), (m > x -> m = S (m - 1))%nat).
  induction m; intuition.
  apply my_eval_ind; intros; try (solve [exists 1%nat; reflexivity]).
  - destruct H as [x h]. exists (S x). cbn. rewrite h. reflexivity.
  - destruct H as [x h]. destruct H0 as [x0 h0]. destruct H1 as [x1 h1].
    exists (x+x0+x1)%nat.
    assert (j:=eval_n_Ret_Succ _ _ _ h).
    assert (j0:=eval_n_Ret_Succ _ _ _ h0).
    assert (j1:=eval_n_Ret_Succ _ _ _ h1).
  ...
  - destruct H as [x h]. destruct H0 as [x0 h0]. exists (x + x0)%nat.
    assert (j:=eval_n_Ret_Succ _ _ _ h).
    assert (j0:=eval_n_Ret_Succ _ _ _ h0).
    assert (k:= eval_n_monotone _ _ _ h).
    assert (k0:= eval_n_monotone _ _ _ h0).
    rewrite j.
    replace (S (x - 1) + x0)%nat
    with (S (x + (x0 - 1)))%nat; try lia.
    simpl. rewrite k.
  ...
  - destruct H as [x h]. exists (x+1)%nat.
    assert (j:=evals_n_Ret_Succ _ _ _ h).
  ...
  - destruct H as [x h]. destruct H0 as [x0 h0].
    assert (j:=eval_n_Ret_Succ _ _ _ h).
    assert (j0:=evals_n_Ret_Succ _ _ _ h0).
    assert (k:= eval_n_monotone _ _ _ h).
...
Goal eval_n 100 (App_e SKKe (Con_e conex enil)) = Ret (Con_e conex enil).
Proof.
  reflexivity.
Qed.
...
Definition unRet (x:exception exp) : exp :=
  match x with 
    | Ret e => e
    | Exc _ => Var_e 9999
  end.
...
Example One := Eval compute in (unRet (eval_n 100 one)).
...
Example Five := Eval vm_compute in (unRet (eval_n 100 five)).
Example Six := Eval vm_compute in (unRet (eval_n 100 six)).

Goal eval (SS $ ZZ) One. repeat econstructor. Qed.
Goal eval_n 100 (add $ ZZ $ one) = Ret One. reflexivity. Qed.
Goal eval_n 100 (add $ one $ ZZ) = Ret One. reflexivity. Qed.
Goal (eval_n 1000 (add $ two $ one)) = Ret Three.
vm_compute. reflexivity. Qed.
Goal (eval_n 1000 (add $ two $ three)) = Ret Five.
vm_compute. reflexivity. Qed.
Goal (eval_n 1000 (mul $ two $ three)) = Ret Six.
vm_compute. reflexivity. Qed.
End ChurchNaturals.
...
Goal eval_n 100 (cdr $ (Cons $ TTT $ Nil)) = Ret Nil.
reflexivity.
Qed.

Goal eval_n 100 (Cons $ TTT $ Nil) =
     Ret (Con_e dCons (econs (Con_e TT enil) [|Con_e dNil enil|])).
  reflexivity.
Qed.
Goal eval_n 100 (cdr $ (Cons $ FFF $ (Cons $ TTT $ Nil))) =
     Ret (Con_e dCons (econs (Con_e TT enil) [|Con_e dNil enil|])).
reflexivity.
Qed.
...
Goal eval_n 100 (SSS $ one) = Ret Two.
reflexivity.
Qed.

Goal eval_n 100 (SSS $ three) = Ret Four.
reflexivity.
Qed.
...
Goal eval_n 100 (copy $ ZZZ) = Ret ZZZ.
reflexivity.
Qed.
...
Proof.
  ...
  - inversion_Clear H1. apply H0. unfold sbst_fix. cbn. admit.
    (************
   inversion H2; subst. 
...
| appCong_is_value:
    forall fn arg, is_value fn -> is_value arg ->
                   ~ isLam fn -> ~ isFix fn -> fn <> Prf_e ->
                   is_value (fn $ arg)
...
Fixpoint is_valueb (e:exp): bool :=
  match e with
    | Var_e _ => true
    | Lam_e _ _ => true
    | App_e fn arg => is_valueb fn && is_valueb arg &&
...
Proof.
  apply my_exp_ind; simpl; intros; split; intros; auto;
    try discriminate; try inversion_Clear H1.
  - 
...
Definition fnames (e:efnlst) : list (BasicAst.name) :=
