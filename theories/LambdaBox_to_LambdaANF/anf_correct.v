...
Proof.
  ...
  - destruct l as [| a l'].
    + reflexivity.
    + simpl nth_error. rewrite Nnat.Nat2N.inj_succ.
      change (nthN (a :: l') (N.succ (N.of_nat k')) = nth_error l' k').
      replace (nthN (a :: l') (N.succ (N.of_nat k')))
        with (nthN l' (N.succ (N.of_nat k') - 1)%N).
      2:{ destruct (N.succ (N.of_nat k')) eqn:Heq; [lia | reflexivity]. }
      replace (N.succ (N.of_nat k') - 1)%N with (N.of_nat k') by lia.
      apply IH.
Qed.
...
Section Correct.
  ...
  Lemma fold_left_KernameSet_In_defs (l : list (EAst.def EAst.term)) (init : KernameSet.t) (k : kername) :
    KernameSet.In k init \/ Exists (fun d => KernameSet.In k (term_global_deps (EAst.dbody d))) l ->
    KernameSet.In k (fold_left (fun acc d => KernameSet.union (term_global_deps (EAst.dbody d)) acc) l init).
  Proof.
    revert init. induction l as [| d ds IH]; intros init [Hinit | Hex].
    - exact Hinit.
    - inversion Hex.
    - apply IH. left. apply KernameSet.union_spec. right. exact Hinit.
    - inversion Hex; subst.
  ...
  Lemma kn_deps_mfix_subset_fix mfix idx k :
    kn_deps_mfix mfix k ->
    kn_deps (EAst.tFix mfix idx) k.
  Proof.
    intros Hk. unfold kn_deps_mfix in Hk. unfold kn_deps. simpl.
    apply fold_left_KernameSet_In_defs. right. exact Hk.
  Qed.
  ...
  Lemma anf_cvt_disjoint_occurs_free_ctx_app S e1 vn S2 C1 x1
    ...
    Disjoint _ (occurs_free (C2 |[ Eletapp r x1 func_tag [x2] e_k ]|))
               ((S \\ S2) \\ [set x1]).
  Proof.
    intros Hcvt_e1 Hcvt_e2 Hr Hdis_vn Hdis_cm Hdis_ek.
    (* Subset chains *)
    assert (HS3_S2 : S3 \subset S2) by (eapply anf_cvt_exp_subset; eassumption).
    assert (HS2_S : S2 \subset S) by (eapply anf_cvt_exp_subset; eassumption).
    (* Context free variables inclusion *)
    assert (Hctx : occurs_free_ctx C2 \subset
                     FromList vn :|: (S2 \\ S3) :|: cmap_vars cmap).
    { eapply anf_cvt_occurs_free_ctx_exp; [exact Hcvt_e2 | |].
      - eapply Disjoint_Included_r; [exact HS2_S | exact Hdis_vn].
      - eapply Disjoint_Included_r; [exact HS2_S | exact Hdis_cm]. }
    (* Result variable location *)
    assert (Hx2_loc : x2 \in FromList vn \/ x2 \in S2 \/ x2 \in cmap_vars cmap)
      by (eapply anf_cvt_result_in_consumed; eassumption).
    (* Main disjointness *)
    constructor. intros z Hc.
    inversion Hc as [? Hz_free Hz_tgt]; subst; clear Hc.
    unfold Ensembles.In in Hz_free, Hz_tgt.
    destruct Hz_tgt as [Hz_SS2 Hz_nx1]. destruct Hz_SS2 as [Hz_S Hz_nS2].
    apply occurs_free_ctx_app in Hz_free. inv Hz_free.
    - (* z ∈ occurs_free_ctx C2 *)
      match goal with H : Ensembles.In _ (occurs_free_ctx C2) _ |- _ =>
        apply Hctx in H; inv H end.
      + (* z ∈ FromList vn :|: (S2 \\ S3) *)
        match goal with H : Ensembles.In _ _ _ |- _ => inv H end.
        * match goal with H : Ensembles.In _ (FromList _) _ |- _ =>
            exact (Disjoint_In_l _ _ _ Hdis_vn H Hz_S) end.
        * match goal with H : Ensembles.In _ (Setminus _ _ _) _ |- _ =>
            unfold Ensembles.In in H; destruct H as [Hz_in _];
            exact (Hz_nS2 Hz_in) end.
      + (* z ∈ cmap_vars *)
        match goal with H : Ensembles.In _ (cmap_vars _) _ |- _ =>
          exact (Disjoint_In_l _ _ _ Hdis_cm H Hz_S) end.
    - (* z ∈ occurs_free (Eletapp ...) \ bound_stem_ctx C2 *)
      match goal with H : Ensembles.In _ (Setminus _ _ _) _ |- _ =>
        unfold Ensembles.In in H; destruct H as [Hz_body Hz_nb] end.
      apply (proj1 (occurs_free_Eletapp _ _ _ _ _)) in Hz_body. inv Hz_body.
      + (* z ∈ {x1} ∪ FromList [x2] *)
        match goal with H : Ensembles.In _ _ _ |- _ => inv H end.
        * (* z = x1 *)
          match goal with H : Ensembles.In _ (Singleton _ _) _ |- _ =>
            inv H; exact (Hz_nx1 (In_singleton _ _)) end.
        * (* z = x2 *)
          match goal with H : Ensembles.In _ (FromList _) _ |- _ =>
            unfold FromList, Ensembles.In in H; simpl in H;
            destruct H as [<- | []] end.
          destruct Hx2_loc as [Hin | [Hin | Hin]].
          -- exact (Disjoint_In_l _ _ _ Hdis_vn Hin Hz_S).
          -- exact (Hz_nS2 Hin).
          -- exact (Disjoint_In_l _ _ _ Hdis_cm Hin Hz_S).
      + (* z ∈ occurs_free e_k \ {r} *)
        match goal with H : Ensembles.In _ (Setminus _ _ _) _ |- _ =>
          unfold Ensembles.In in H; destruct H as [Hz_ek Hz_nr] end.
  ...
  Proof.
    ...
    - (* vars = a :: vars *)
      ...
      revert vs Hget Hset.
      intros vs. eapply rev_ind with (l := vs).
      + (* vs = [] *)
      ...
      + (* vs = l ++ [x] *)
        ...
        2:{ eapply preord_exp_trans; [tci | exact eq_fuel_idemp | | ].
            2:{ intros m. eapply preord_exp_Eproj_red. eassumption.
                ...
                replace (N.of_nat (Datatypes.length l - 0) - N.of_nat (Datatypes.length l))%N
                  with 0%N by lia.
                reflexivity.
                lia. }
    ...
    - intros e1 e2 body0 rho0 rho' rho'' idx0 na0 mfix0 v2 r0
      ...
      + intros v v' Heq Hrel'. subst r0.
        ...
        2:{ eapply preord_exp_trans; [tci | exact eq_fuel_idemp | | ].
            ...
            { (* x1 = x2: argument shadows closure *)
              subst x2.
              (* Step 1: get v_rho related to both source values *)
              assert (Hrel_rho_ex : exists v_rho,
                M.get x1 rho = Some v_rho /\
                anf_val_rel' (ClosFix_v rho' mfix0 idx0) v_rho /\
                anf_val_rel' v2 v_rho /\
                ClosFix_v rho' mfix0 idx0 = v2).
              { destruct (In_dec Pos.eq_dec x1 vnames) as [Hin_vn | Hni_vn].
                - apply In_nth_error in Hin_vn. destruct Hin_vn as [k0 Hk0].
                  assert (Heval1_k : nth_error rho0 k0 = Some (ClosFix_v rho' mfix0 idx0)).
                  { eapply anf_cvt_rel_var_lookup;
                      [exact Heval1 | exact Hcvt_e1 | exact Hdis | exact Hdis_cmap
                      | exact Hcons | exact Hcmap | exact Hk0]. }
                  assert (Heval2_k : nth_error rho0 k0 = Some v2).
                  ...
                  destruct (Forall2_nth_error_r _ _ _ _ _ Henv Hk0)
                    as [v_src [Hsrc [v_rho [Hget Hrel]]]].
                  assert (v_src = ClosFix_v rho' mfix0 idx0) by congruence. subst v_src.
                  assert (Heq_v2' : ClosFix_v rho' mfix0 idx0 = v2) by congruence.
                  exists v_rho. split; [exact Hget |]. split; [exact Hrel |].
                  split; [subst v2; exact Hrel | exact Heq_v2'].
                - destruct (anf_cvt_result_in_consumed _ _ _ _ _ _ Hcvt_e1)
                    as [Hin1 | [Hin1 | Hin1]].
                  ...
                  + destruct Hin1 as [k_c1 Hlk1].
                    ...
                    exists v_rho. split; [exact Hget_g |].
                    assert (Heval1_cmap : exists f_c t_c,
                      src_eval [] body_g (fuel_sem.Val (ClosFix_v rho' mfix0 idx0)) f_c t_c).
                    { eapply anf_cvt_cmap_eval;
                        [exact Heval1 | exact Hcvt_e1 | exact Hdis | exact Hdis_cmap
                        | exact Hcons | exact Hcmap | exact Hlk1 | exact Hdecl_g | exact Hbody_g]. }
                    destruct Heval1_cmap as [f_c1 [t_c1 Hev1]].
                    ...
                    eapply eval_val_det; [exact Hev1 | exact Hev2]. }
              destruct Hrel_rho_ex as [v_rho [Hget_rho [Hrel_rho [Hrel_rho_v2 Heq_v2]]]].
              (* Step 2: preord_val (Vfun rho1 Bs f0) v2' *)
              assert (Hpv_cv : forall j, preord_val cenv eq_fuel j
                        (Vfun rho1 Bs f0) v2').
              { intro j. eapply preord_val_trans; [tci | exact eq_fuel_idemp | | ].
                - eapply (@anf_cvt_val_alpha_equiv
              ...
              destruct v2' as [ | rho2_fc B2 f2_v | | ];
                try (simpl in Hpv_inst; contradiction).
              (* Step 4: get body correspondence *)
              assert (Hset_fc : Some rho_bc = set_lists [x_pc]
                [Vfun rho2_fc B2 f2_v] (def_funs Bs Bs rho1 rho1)).
              { unfold rho_bc. reflexivity. }
              edestruct Hpv_inst as (xs2_pc & e2_body & rho2_body &
                Hfind_v2 & Hset_v2 & Hbody_preord).
              { reflexivity. }
              { exact Hfind_fc. }
              { exact Hset_fc. }
              (* Step 5: transfer body via preord_exp' *)
              assert (Hbody_pe : preord_exp' cenv (preord_val cenv) eq_fuel eq_fuel
                        (cin_bc + i)%nat
              ...
              { simpl in Hres2_bc. contradiction. }
              simpl in Hres2_bc.
              (* Step 6: forall m', preord_val v_bc_val v2_bc via bstep determinism *)
              assert (Hpv_all : forall m', preord_val cenv eq_fuel m' v_bc_val v2_bc).
              { intro m'.
                pose proof (Hpv_cv (cin_bc + m' + 1)%nat) as Hpv_m.
                rewrite preord_val_eq in Hpv_m.
                ...
                replace e2_m with e2_body in * by congruence.
                replace xs2_m with xs2_pc in * by congruence.
                replace rho2_m with rho2_body in * by congruence.
                assert (Hba_m : preord_exp' cenv (preord_val cenv) eq_fuel eq_fuel
                          (cin_bc + m')%nat
                          (C_bc |[ Ehalt r_bc ]|, rho_bc) (e2_body, rho2_body)).
                { apply Hbp_m. lia.
              ...
              { intro Heq. subst x.
                assert (Hx1_in_S : x1 \in S).
                { eapply anf_cvt_exp_subset; [exact Hcvt_e1 |].
                  eapply anf_cvt_exp_subset; [exact Hcvt_e2 |]. exact H8. }
                destruct (anf_cvt_result_in_consumed _ _ _ _ _ _ Hcvt_e1)
                  as [Hin1 | [Hin1 | Hin1]].
                - eapply Hdis. constructor; eassumption.
                - assert (Hx1_not_S2 : ~ x1 \in S2)
                    by (eapply anf_cvt_result_not_in_output; eassumption).
                  apply Hx1_not_S2. eapply anf_cvt_exp_subset; [exact Hcvt_e2 | exact H8].
                - eapply Hdis_cmap. constructor; eassumption. }
              (* Step 9: env bridge *)
              assert (Hrefl : preord_exp cenv eq_fuel eq_fuel i
                        (e_k, M.set x v' rho)
              ...
              { eapply preord_exp_refl. now eapply eq_fuel_compat.
                ...
                - intros w Hget. rewrite M.gss in Hget. inv Hget.
                  eexists. split. rewrite M.gss. reflexivity.
                  exact Hpv_direct.
                - intros w Hget. rewrite M.gso in Hget by auto.
                  destruct (Pos.eq_dec y x1) as [-> | Hneq_x1].
                  + eexists. split.
                  ...
                  + eexists. split.
                    { rewrite M.gso by auto. rewrite M.gso by auto.
                      rewrite M.gso by auto. exact Hget. }
                    eapply preord_val_refl. tci. }
              edestruct Hrefl as (v_cont & cin_cont & cout_cont &
                Hbstep_cont & Heq_cont & Hres_cont).
              { exact Hle_ek. }
      ...
      + intros v v' Heq Hrel'. subst r0.
        ...
        2:{ (* Simplify context: comp_ctx_f C1 (Eletapp_c ...) |[e_k]| → C1 |[Eletapp ... e_k]| *)
            ...
            2:{ (* === Step 1b: IH_mch through C1 === *)
                intros m.
                (* Assert Disjoint for Eletapp continuation before edestruct *)
                assert (Hdis_eletapp : Disjoint var
                  (occurs_free (Eletapp x f0 func_tag [x1] e_k))
                  (S \\ [set f0] \\ [set y] \\ S2 \\ [set x1])).
                { constructor. intros z Hz. inversion Hz; subst; clear Hz.
                  destruct H0 as [[[[Hz_S Hz_nf0] Hz_ny] Hz_nS2] Hz_nx1].
                  ...
                  - (* z ∈ {f0} ∪ FromList [x1] *)
                    ...
                    + (* z ∈ FromList [x1] *)
                      unfold FromList, Ensembles.In in H. simpl in H.
                      destruct H as [<- | []]. apply Hz_nx1. constructor.
                  - (* z ∈ occurs_free(e_k) \\ {x} *)
                    destruct H0 as [Hz_ek Hz_neq_x].
                    inv Hdis_ek. eapply H.
            ...
            { (* Assert Disjoint for Ehalt r_br before edestruct to resolve evar *)
              assert (Hdis_ehalt : Disjoint var (occurs_free (Ehalt r_br))
                ...
                + eapply global_env_rel_set_fresh.
                  * eapply global_env_rel_set_fresh.
                    -- eapply global_env_rel_mono; [exact Hglob |].
                       ...
                       exact (find_branch_In _ _ _ _ _ Hbranch).
                    -- intros Hc. inv Hdis_cmap. eapply H.
                       constructor; [exact Hc | exact Hf0_in_S].
                  * intros Hc. inv Hdis_cmap. eapply H.
            ...
            { eapply preord_exp_refl. exact eq_fuel_compat.
              ...
              -- (* z = x1: alpha-equivalence case *)
                 rewrite M.gss.
                 eexists. split; [reflexivity |].
                 (* Both v_z and Vconstr c_tag vs_anf are anf_val_rel' to Con_v.
                    Get anf_val_rel' (Con_v ...) v_z, then alpha-equiv. *)
                 assert (Hdis_mch : Disjoint _ (FromList vnames) (S \\ [set f0] \\ [set y])).
                 { eapply Disjoint_Included_r; [| exact Hdis].
                   eapply Included_trans; apply Setminus_Included. }
                 assert (Hdis_cmap_mch : Disjoint _ (cmap_vars cmap) (S \\ [set f0] \\ [set y])).
                 { eapply Disjoint_Included_r; [| exact Hdis_cmap].
                   eapply Included_trans; apply Setminus_Included. }
                 assert (Hrel_vz : anf_val_rel' (Con_v (dcon_of_con ind c0) vs0) v_z).
                 { destruct (In_dec Pos.eq_dec x1 vnames) as [Hin_vn | Hni_vn].
                   (* x1 ∈ vnames: use anf_cvt_result_in_vnames_eval *)
                   - eapply anf_cvt_result_in_vnames_eval;
                       [ exact Henv | exact Hcons | exact Hcmap
                       | exact Hdis_mch | exact Hdis_cmap_mch
                       | exact Hcvt_mch | exact Hin_vn
                       | exact Heval_mch | exact Hget_z ].
                   (* x1 ∉ vnames: split S vs cmap *)
                   - destruct (anf_cvt_result_in_consumed _ _ _ _ _ _ Hcvt_mch)
                       as [Hin1 | [Hin1 | Hin1]].
                     + contradiction.
                     + (* x1 ∈ S \\ {f0} \\ {y}: contradiction with Hdis_ek *)
                       exfalso.
                       assert (Hx1_in_S : Ensembles.In var S x1).
                       { destruct Hin1 as [[Hc _] _]. exact Hc. }
                       assert (Hx1_notin_S2 : ~ x1 \in S2).
                       { eapply anf_cvt_result_not_in_output;
                           [exact Hcvt_mch | exact Hdis_mch | exact Hdis_cmap_mch]. }
                       assert (Hx1_notin_S3 : ~ x1 \in S3).
                       { intros Hc.
                         apply Hx1_notin_S2.
                         eapply anf_cvt_branches_subset; [exact Hcvt_brs | exact Hc]. }
                       inv Hdis_ek. eapply H.
                       ...
                       constructor.
                       ** constructor; [exact Hx1_in_S |].
                          intros [Hc1 _]. exact (Hx1_notin_S3 Hc1).
                       ** intros Hc. inv Hc. exact (Hneq_zx eq_refl).
                     + (* x1 ∈ cmap_vars: use global_env_rel' *)
                       destruct Hin1 as [k_c Hlk_c].
                       assert (Hkc_deps : kn_deps (tCase (ind, npars) mch brs) k_c).
                       { unfold kn_deps. simpl. apply KernameSet.union_spec. right.
                         assert (Hdep_mch : KernameSet.In k_c (term_global_deps mch)).
                         { eapply anf_cvt_cmap_result_in_deps;
                             [ exact Hcvt_mch | exact Hlk_c
                             | exact Hdis_mch | exact Hdis_cmap_mch
                             | exact Hni_vn ]. }
                         assert (Hfold_mono : forall (l : list (list name * term)) base,
                           KernameSet.In k_c base ->
                           KernameSet.In k_c (fold_left (fun acc x =>
                             KernameSet.union (term_global_deps (snd x)) acc) l base)).
                         { intros l. induction l as [| [? ?] l' IHl]; intros base Hbase;
                             simpl; [exact Hbase |].
                           apply IHl. apply KernameSet.union_spec. right. exact Hbase. }
                         exact (Hfold_mono _ _ Hdep_mch). }
                       destruct (Hglob k_c x1 Hkc_deps Hlk_c)
                         as (decl_g & body_g & anf_v & Hdecl_g & Hbody_g & Hget_g & Hrel_g).
                       assert (anf_v = v_z) by congruence. subst anf_v.
                       assert (Heval_cmap : exists f_c t_c,
                         src_eval [] body_g (fuel_sem.Val (Con_v (dcon_of_con ind c0) vs0)) f_c t_c).
                       { eapply anf_cvt_cmap_eval;
                           [ exact Heval_mch | exact Hcvt_mch
                           | exact Hdis_mch | exact Hdis_cmap_mch
                           | exact Hcons | exact Hcmap
                           | exact Hlk_c | exact Hdecl_g | exact Hbody_g ]. }
                       destruct Heval_cmap as [f_c [t_c Hev_c]].
                       exact (Hrel_g _ _ _ Hev_c). }
                 eapply (@anf_cvt_val_alpha_equiv
                   _ _ _ _ eq_fuel eq_fuel tgm cmap cenv
                   eq_fuel_compat (fun _ _ H0 => H0)
                   nat LambdaBox_resource_fuel LambdaBox_resource_trace
          ...
          * (* z = x1 *)
            ...
            -- (* x1 ∈ xs0: both y and the set_many value related to v0 *)
               ...
               { destruct (nth_error es0 k0) eqn:Heq; [eexists; reflexivity | exfalso].
                 ...
                 rewrite (anf_cvt_rel_args_length _ _ _ _ _ _ Hcvt_tail) in Hk0_lt. lia. }
               ...
               { destruct (nth_error vs0 k0) eqn:Heq; [eexists; reflexivity | exfalso].
                 apply nth_error_None in Heq.
                 pose proof (anf_cvt_rel_args_length _ _ _ _ _ _ Hcvt_tail).
                 pose proof (eval_fuel_many_length _ _ _ _ _ Heval_es). lia. }
                 ...
                 - (* x1 ∈ cmap_vars: case split on x1 ∈ FromList vnames *)
                   ...
                   assert (Hsub_k_S : S_k \subset S)
                     by (eapply Included_trans;
                           [exact Hsub_k | eapply anf_cvt_exp_subset; exact Hcvt_head]).
                   destruct (In_dec Pos.eq_dec x1 vnames) as [Hin_vn' | Hni_vn].
                   + (* x1 also in FromList vnames: use var_lookup *)
                     apply In_nth_error in Hin_vn'.
                     ...
                     assert (Hvk_i := anf_cvt_rel_var_lookup _ _ _ _ _
                       ...
                       Hcons Hcmap Hi0).
                     congruence.
                   + (* x1 not in FromList vnames: use global_env_rel' *)
                     assert (Hkn_deps : kn_deps_list (e0 :: es0) kn_x).
                     { unfold kn_deps_list. constructor.
                       eapply anf_cvt_cmap_result_in_deps;
                         [exact Hcvt_head | exact Hlk_x
                         | exact Hdis | exact Hdis_cmap | exact Hni_vn]. }
               ...
               (* First get es0[j] — exists because j < length xs0 = length es0 *)
               ...
               { destruct (nth_error es0 j) eqn:Heq; [eexists; reflexivity | exfalso].
                 apply nth_error_None in Heq.
                 rewrite (anf_cvt_rel_args_length _ _ _ _ _ _ Hcvt_tail) in Hj_lt. lia. }
               destruct Hj_es as [e_j He_j].
               assert (Hv_j_eq : v0 = v_j).
               { destruct (anf_cvt_rel_args_nth_cvt _ _ _ _ _ _ Hcvt_tail _ _ _ He_j Hj_xs)
                   as [S_j [S_j' [C_j [Hcvt_j Hsub_j]]]].
                 destruct (eval_fuel_many_nth _ _ _ _ _ _ _ _ Heval_es He_j Hv_j)
                   as [f_j [t_j Heval_j]].
                 ...
                 - (* x1 ∈ cmap_vars *)
                   destruct Hin_cm as [kn_x Hlk_x].
                   destruct (In_dec Pos.eq_dec x1 vnames) as [Hin_vn' | Hni_vn].
                   + (* x1 also in vnames: use var_lookup *)
                     apply In_nth_error in Hin_vn'.
                     destruct Hin_vn' as [i0 Hi0]. change positive with var in Hi0.
                     assert (Hv0_i := anf_cvt_rel_var_lookup _ _ _ _ _
                       Heval_e _ _ _ _ _ i0 Hcvt_head Hdis Hdis_cmap Hcons Hcmap Hi0).
