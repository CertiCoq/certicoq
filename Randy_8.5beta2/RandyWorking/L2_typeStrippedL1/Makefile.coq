...
# coq_makefile -R ../common Common -R ../L1_MalechaQuoted L1 -R . L2 L2.v stripEvalCommute.v wNorm.v wcbvEval.v wndEval.v program.v term.v -o Makefile.coq 
#
...
install:
	cd "." && for i in $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/L2/$$i`"; \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/L2/$$i; \
	done
...
uninstall_me.sh: Makefile.coq
	echo '#!/bin/sh' > $@ 
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/L2 && rm -f $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "L2" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
