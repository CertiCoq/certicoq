...
(*i $Id: Zcomplements.v 10028 2007-07-18 22:38:06Z letouzey $ i*)
...
Proof.
  ...
  elim (Zabs_dec p); intro eq;
    rewrite (impl_PI eq); elim H; auto with zarith.
  unfold Q in |- *; clear Q; intros.
  apply pair; apply HP.
  rewrite Zabs_eq; auto; intros.
  elim (H (Zabs m)); intros; auto with zarith.
  elim (Zabs_dec m); intro eq; rewrite (impl_PI eq); trivial.
...
Proof.
  ...
  elim (Zabs_dec p); intro eq; rewrite (impl_PI eq);
