...
module Default = struct
  ...
  let whd_app_evar sigma t =
    match kind_of_term t with
      | App (f,l,imps) -> mkApp (whd_evar sigma f,l,imps)
      | _ -> whd_evar sigma t
  ...
  let apply_coercion_args env argl funj =
    let rec apply_rec imps typ = function
      | [] -> { uj_val = applist (j_val funj,argl,List.rev imps);
		uj_type = typ }
      | h::restl ->
	  (* On devrait pouvoir s'arranger pour qu'on n'ait pas à faire hnf_constr *)
		...
		apply_rec (imp::imps) (subst1 h c2) restl
  ...
  let apply_pattern_coercion loc pat p =
    List.fold_left
      (fun pat (co,n) ->
	 let f i = if i<n then Rawterm.PatVar (loc, Anonymous,Expl)
         else pat in
  ...
  let apply_coercion env p hj typ_cl =
    try 
      fst (List.fold_left
		...
		let argl = (fst(class_args_of typ_cl))@[ja.uj_val] in
  ...
  let inh_app_fun env isevars j = 
	...
	| Prod _ -> (isevars,j)
	| Evar ev ->
	    let (isevars',t) = define_evar_as_arrow isevars Expl ev in
  ...
  let rec inh_conv_coerce_to_fail loc env isevars v t c1 =
    ...
    with
    | Prod (_,imp1,t1,t2), Prod (name,imp2,u1,u2) when imp1=imp2 -> 
	let v' = option_map (whd_betadeltaiota env (evars_of isevars)) v in
	let (evd',b) =
	  match v' with 
	  | Some v' ->
	      (match kind_of_term v' with
	       | Lambda (x,impl,v1,v2) when impl=imp1 ->
	...
	(match b with
	 | Some (x, v1, v2) ->
	     let env1 = push_rel (x,imp1,None,v1) env in
	     let (evd'', v2', t2') = inh_conv_coerce_to_fail loc env1 evd'
	       (Some v2) t2 u2 in
	     (evd'', option_map (fun v2' -> mkLambda (x, imp1, v1, v2')) v2', 
	     mkProd (x, imp1, v1, t2'))
	 | None ->
	     ...
	     let name = match name with 
	       ...
	       | _ -> name
	     in
	     let env1 = push_rel (name,imp1,None,u1) env in
	     let (evd', v1', t1') =
		 ...
		 match v with 
		 | Some v ->
                     option_map (fun x -> mkApp(lift 1 v,[|x|],[|imp1|])) v1'
		 | None -> None
	       and evd', t2 =
		 match v1' with 
		 ...
		 | None -> 
		     let evd', ev =
		       new_evar evd' env ~src:(loc, InternalHole) Expl t1' in
		     evd', subst_term ev t2
	       in
	       inh_conv_coerce_to_fail loc env1 evd' v2 t2 u2
	     in
	     (evd'',
             option_map (fun v2' -> mkLambda (name, imp1, u1, v2')) v2',
	     mkProd (name, imp1, u1, t2')))
    | _ -> raise NoCoercion
