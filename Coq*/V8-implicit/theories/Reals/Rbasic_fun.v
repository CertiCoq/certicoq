...
Proof.
  intros r1 r2 r; unfold Rmin in |- *; destruct (Rle_dec r1 r2); apply impl_PI.
  split.
  assumption.
...
Proof.
  intros; unfold Rmin in |- *; destruct (Rle_dec x y); apply impl_PI;
...
Proof.
  intros; unfold Rmin in |- *; destruct (Rle_dec a b); destruct (Rle_dec b a);
    apply impl_PI;
...
Proof.
  ...
  intro; unfold Rmax in |- *; destruct (Rle_dec r1 r2); destruct H;
    apply impl_PI; auto.
...
Proof.
  intros r1 r2; unfold Rmax in |- *; destruct (Rle_dec r1 r2);
...
Proof.
  intros p q; unfold Rmax in |- *; destruct (Rle_dec p q); destruct (Rle_dec q p);
    apply impl_PI; auto;
...
Proof.
  ...
  destruct (Rle_dec p q); destruct (Rle_dec (r * p) (r * q));
   apply impl_PI; auto.
  case H; intros E1.
...
Proof.
  unfold Rabs in |- *; destruct (Rcase_abs 0); apply impl_PI; auto.
  generalize (Rlt_irrefl 0); intro; elimtype False; auto.
Qed.

Lemma Rabs_R1 : Rabs 1 = 1.
Proof.
unfold Rabs in |- *; destruct (Rcase_abs 1); apply impl_PI; auto with real.
...
Proof.
  intros; unfold Rabs in |- *; destruct (Rcase_abs r); apply impl_PI; trivial;
    absurd (r >= 0).
...
Proof.
  intros; unfold Rabs in |- *; destruct (Rcase_abs r); apply impl_PI.
  absurd (r >= 0).
...
Proof.
  intros; unfold Rabs in |- *; destruct (Rcase_abs x); apply impl_PI.
...
Proof.
  intro; unfold Rabs in |- *; destruct (Rcase_abs x); apply impl_PI; fourier.
Qed.
...
Proof.
  intros; unfold Rabs in |- *; destruct (Rcase_abs (x - y));
    destruct (Rcase_abs (y - x)); apply impl_PI.
  generalize (Rminus_lt y x r0); generalize (Rminus_lt x y r); intros;
  ...
  generalize (Ropp_lt_gt_0_contravar (x - y) H0); rewrite (Ropp_minus_distr x y);
    intro; unfold Rgt in H0; generalize (Rlt_asym 0 (y - x) H); 
      intro; elimtype False; auto.
  rewrite (Rminus_diag_uniq y x H); trivial.
  rewrite (Rminus_diag_uniq x y H0); trivial.
  rewrite (Rminus_diag_uniq x y H0); trivial.
Qed.
...
Proof.
  intros; unfold Rabs in |- *; case (Rcase_abs (x * y)); case (Rcase_abs x);
    case (Rcase_abs y); intros; apply impl_PI; auto.
...
Proof.
  intro; unfold Rabs in |- *; case (Rcase_abs r); case (Rcase_abs (/ r));
    intros; apply impl_PI; auto.
...
Proof.
  ...
  intros; rewrite H0.
  ring.
  unfold Rabs in |- *; destruct (Rcase_abs (-1)); apply impl_PI.
  ring.
  generalize (Rge_le (-1) 0 r); intros.
  generalize (Ropp_le_ge_contravar 0 (-1) H0).
  rewrite Ropp_involutive; rewrite Ropp_0.
  intro; generalize (Rgt_not_le 1 0 Rlt_0_1); intro; generalize (Rge_le 0 1 H1);
    intro; elimtype False; auto.  
  ring.
Qed.
...
Proof.
  intros a b; unfold Rabs in |- *; case (Rcase_abs (a + b)); case (Rcase_abs a);
    case (Rcase_abs b); intros; apply impl_PI.
...
Proof.
  unfold Rabs in |- *; intro x; case (Rcase_abs x); intros; apply impl_PI.
