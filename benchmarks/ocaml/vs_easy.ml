...
module Coq_Pos =
 struct
  ...
  | XI p0 ->
    (match p0 with
     | XI p1 ->
       sqrtrem_step (fun x -> XI x)
         (fun x -> XI x) (sqrtrem p1)
     | XO p1 ->
       sqrtrem_step (fun x -> XO x)
         (fun x -> XI x) (sqrtrem p1)
     | XH ->
       Pair (XH, (IsPos (XO XH))))
  | XO p0 ->
...
let cnf = function
| Entailment (a0, a1) ->
  let Assertion (pureL, spaceL) = a0
  in
  let Assertion (pureR, spaceR) = a1
  in
...
module Superposition =
 struct
  ...
  let simplify_atoms l atms =
    fold_left (fun atms0 d0 ->
      map
        (rewrite_clause_in_space d0)
        atms0) l atms
...
let example_ent (_ : unit0) =
  Entailment ((Assertion ((Cons
    ((Nequ (c, e)), Nil)), (Cons
    ((Lseg (a, b)), (Cons ((Lseg (a,
    c)), (Cons ((Next (c, d)), (Cons
    ((Lseg (d, e)), Nil)))))))))),
