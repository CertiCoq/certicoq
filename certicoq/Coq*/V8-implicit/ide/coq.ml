...
(* $Id: coq.ml 9796 2007-04-26 08:54:28Z corbinea $ *)
...
type hyp = env * evar_map *
           ((identifier * string) * impl * constr option * constr) * 
           (string * string)
type concl = env * evar_map * constr * string
type meta = env * evar_map * string
type goal = hyp list * concl

let prepare_hyp sigma env ((i,imp,c,d) as a) =
  env, sigma,
  ((i,string_of_id i),imp,c,d), 
...
let compute_reset_info = function 
  ...
  | VernacAssumption (_,_, (_,(((_,id)::_,_),_))::_)
...
let hyp_menu (env, sigma, ((coqident,ident),_,_,ast),(s,pr_ast)) =
