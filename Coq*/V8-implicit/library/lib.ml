...
(* $Id: lib.ml 9477 2007-01-10 17:13:30Z lmamane $ *)
...
let rec extract_hyps = function
  | (id::idl,(id',_,_,_ as d)::hyps) when id=id' ->
      d :: extract_hyps (idl,hyps)
...
let add_section_replacement f g hyps =
  ...
  | (vars,exps,abs)::sl ->
    ...
    let (args,imps) = Sign.instance_from_named_context (List.rev sechyps) in
    sectab :=
    (vars,f (Array.map Term.destVar args,imps) exps,
     g sechyps abs)::sl
