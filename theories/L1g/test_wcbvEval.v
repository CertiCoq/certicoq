...
(***********
Function admitCopy (n:nat) {wf lt n} :=
  match n with 0 => 0 | S k => S (admitCopy k) end.
Proof.
Admitted.
Print Assumptions admitCopy.
Quote Recursively Definition aCopy := admitCopy. (* program *)
Print admitCopy_tcc.
(**********************
...
Print aCopy.
***********************)
Definition admitCopyx := admitCopy x.
Compute admitCopyx.  (** evals correctly in Coq **)
Quote Recursively Definition cbv_admitCopyx :=
  ltac:(let t:=(eval cbv in admitCopyx) in exact t).
Definition ans_admitCopyx :=
  Eval cbv in (main (program_Program cbv_admitCopyx)).
Quote Recursively Definition p_admitCopyx := admitCopyx. (* program *)
Definition P_admitCopyx :=                            (* Program *)
 Eval cbv in (program_Program p_admitCopyx).
Goal
  let env := (env P_admitCopyx) in
  let main := (main P_admitCopyx) in
  wcbvEval (env) 100 (main) = Ret ans_admitCopyx.
  vm_compute. reflexivity.  (**** HERE  *****)
Qed.
 ***************************)
...
Time Definition eval_myMain :=
  Eval vm_compute in (wcbvEval P_env_myMain 600 P_main_myMain).
Print eval_myMain.
...
Definition P_main_ce_example_myent :=
  Eval vm_compute in (AstCommon.main P_ce_example_myent).
Print P_main_ce_example_myent.
Print ce_example_myent.
Time Definition eval_ce_example_myent :=
  Eval vm_compute in
...
Definition P_main_myMain := AstCommon.main P_myMain.
Time Definition eval_myMain :=
  Eval vm_compute in (wcbvEval P_env_myMain 60 P_main_myMain).
Print eval_myMain.
************************)
...
Eval cbv in cbv_one.
(* [Term] of Coq's answer *)
Eval cbv in (program_Program cbv_one).
...
Print cbv_plus11.
(* [Term] of Coq's answer *)
Eval cbv delta in (program_Program cbv_plus11).
Definition ans_plus11 := Eval cbv in (main (program_Program cbv_plus11)).
Print ans_plus11.
(* [program] of the program *)
Quote Recursively Definition p_plus11 := plus11.
