...
# coq_makefile -R ../common Common -R . L1 L1.v malecha_L1.v wNorm.v wcbvEval.v awndEval.v wndEval.v program.v term.v -o Makefile.coq 
#
...
install:
	cd "." && for i in $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES); do \
	 install -d "`dirname "$(DSTROOT)"$(COQLIBINSTALL)/L1/$$i`"; \
	 install -m 0644 $$i "$(DSTROOT)"$(COQLIBINSTALL)/L1/$$i; \
	done
...
uninstall_me.sh: Makefile.coq
	echo '#!/bin/sh' > $@ 
	printf 'cd "$${DSTROOT}"$(COQLIBINSTALL)/L1 && rm -f $(VOFILES) $(VFILES) $(GLOBFILES) $(NATIVEFILES) $(CMOFILES) $(CMIFILES) $(CMAFILES) && find . -type d -and -empty -delete\ncd "$${DSTROOT}"$(COQLIBINSTALL) && find "L1" -maxdepth 0 -and -empty -exec rmdir -p \{\} \;\n' >> "$@"
