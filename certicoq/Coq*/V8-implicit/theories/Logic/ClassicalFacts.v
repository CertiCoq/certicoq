...
(*i $Id: ClassicalFacts.v 9708 2007-03-15 16:37:07Z herbelin $ i*)
...
Section Proof_irrelevance_Prop_Ext_CC.
  ...
  Definition BoolP_elim [C:Prop] (c1 c2:C) (b:BoolP) := b C c1 c2.
...
Section Proof_irrelevance_EM_CC.
  ...
  Hypothesis or_elim :
    forall [A B C:Prop], (A -> C) -> (B -> C) -> or A B -> C.
  Hypothesis
    or_elim_redl :
...
Section Proof_irrelevance_CCI.
  ...
  Definition or_elim_redl [A B C:Prop] (f:A -> C) (g:B -> C) 
    (a:A) : f a = or_ind f g (or_introl B a) :=
    refl_equal (f a).
  Definition or_elim_redr [A B C:Prop] (f:A -> C) (g:B -> C) 
    (b:B) : g b = or_ind f g (or_intror A b) :=
    refl_equal (g b).
