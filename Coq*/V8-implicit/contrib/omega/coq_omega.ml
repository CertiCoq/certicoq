...
(* $Id: coq_omega.ml 9962 2007-07-09 13:49:57Z letouzey $ *)
...
let coq_le_gt_dec = lazy (constant "le_gt_or")
...
let mk_plus t1 t2 = mkApp (Lazy.force coq_Zplus, [| t1; t2 |],[|Expl;Expl|])
let mk_times t1 t2 = mkApp (Lazy.force coq_Zmult, [| t1; t2 |],[|Expl;Expl|])
let mk_minus t1 t2 = mkApp (Lazy.force coq_Zminus, [| t1;t2 |],[|Expl;Expl|])
let mk_eq t1 t2 =
  mkApp (build_coq_eq (), [| Lazy.force coq_Z; t1; t2 |],[|Expl;Expl;Expl|])
let mk_le t1 t2 = mkApp (Lazy.force coq_Zle, [| t1; t2 |],[|Expl;Expl|])
let mk_gt t1 t2 = mkApp (Lazy.force coq_Zgt, [| t1; t2 |],[|Expl;Expl|])
let mk_inv t = mkApp (Lazy.force coq_Zopp, [| t |],[|Expl|])
let mk_and t1 t2 =  mkApp (build_coq_and (), [| t1; t2 |],[|Expl;Expl|])
let mk_or t1 t2 =  mkApp (build_coq_or (), [| t1; t2 |],[|Expl;Expl|])
let mk_not t = mkApp (build_coq_not (), [| t |],[|Expl|])
let mk_eq_rel t1 t2 = mkApp (build_coq_eq (),
	      [| Lazy.force coq_comparison; t1; t2 |],[|Expl;Expl;Expl|])
let mk_inj t = mkApp (Lazy.force coq_Z_of_nat, [| t |],[|Expl|])
...
let destructurate_prop t =
  let c, args,_ = decompose_app t in
  match kind_of_term c, args with
	...
	Kapp (And,[mkArrow Expl t1 t2;mkArrow Expl t2 t1])
    ...
    | Prod (Name _,_,_,_),[] -> error "Omega: Not a quantifier-free goal"
    | _ -> Kufo
...
let recognize_number t =
  let rec loop t =
    match decompose_app t with
      | f, [t],_ when f = Lazy.force coq_xI -> one + two * loop t
      | f, [t],_ when f = Lazy.force coq_xO -> two * loop t
      | f, [],_ when f = Lazy.force coq_xH -> one
      | _ -> failwith "not a number" 
  in
  match decompose_app t with 
    | f, [t],_ when f = Lazy.force coq_Zpos -> loop t
    | f, [t],_ when f = Lazy.force coq_Zneg -> neg (loop t)
    | f, [],_ when f = Lazy.force coq_Z0 -> zero
    | _ -> failwith "not a number"
...
let context operation path (t : constr) =
  let rec loop i p0 t = 
    match (p0,kind_of_term t) with 
      ...
      | ([], _) -> operation i t
      | ((P_APP n :: p),  App (f,v,imps)) ->
	  let v' = Array.copy v in
	  v'.(pred n) <- loop i p v'.(pred n); mkApp (f, v',imps)
      | ((P_BRANCH n :: p), Case (ci,q,c,v)) ->
	  (* avant, y avait mkApp... anyway, BRANCH seems nowhere used *)
	  let v' = Array.copy v in
	  v'.(n) <- loop i p v'.(n); (mkCase (ci,q,c,v'))
      | ((P_ARITY :: p),  App (f,l,imps)) ->
	  appvect (loop i p f,l,imps)
      | ((P_ARG :: p),  App (f,v,imps)) ->
	  let v' = Array.copy v in
	  v'.(0) <- loop i p v'.(0); mkApp (f,v',imps)
      | (p, Fix ((_,n as ln),(tys,lna,v))) ->
	  let l = Array.length v in
	  let v' = Array.copy v in
	  v'.(n)<- loop (Pervasives.(+) i l) p v.(n); (mkFix (ln,(tys,lna,v')))
      | ((P_BODY :: p), Prod (n,imp,t,c)) ->
	  (mkProd (n,imp,t,loop (succ i) p c))
      | ((P_BODY :: p), Lambda (n,imp,t,c)) ->
	  (mkLambda (n,imp,t,loop (succ i) p c))
      | ((P_BODY :: p), LetIn (n,imp,b,t,c)) ->
	  (mkLetIn (n,imp,b,t,loop (succ i) p c))
      | ((P_TYPE :: p), Prod (n,imp,t,c)) ->
	  (mkProd (n,imp,loop i p t,c))
      | ((P_TYPE :: p), Lambda (n,imp,t,c)) ->
	  (mkLambda (n,imp,loop i p t,c))
      | ((P_TYPE :: p), LetIn (n,imp,b,t,c)) ->
	  (mkLetIn (n,imp,b,loop i p t,c))
      | (p, _) -> 
...
let occurence path (t : constr) =
  let rec loop p0 t = match (p0,kind_of_term t) with
    ...
    | ([], _) -> t
    | ((P_APP n :: p),  App (f,v,_)) -> loop p v.(pred n)
    | ((P_BRANCH n :: p), Case (_,_,_,v)) -> loop p v.(n)
    | ((P_ARITY :: p),  App (f,_,_)) -> loop p f
    | ((P_ARG :: p),  App (f,v,_)) -> loop p v.(0)
...
let abstract_path typ path t =
  ...
  mkLambda (Name (id_of_string "x"), Expl, typ, abstract), !term_occur
...
let rec val_of = function
  | Oatom c -> mkVar c
  | Oz c -> mk_integer c
  | Oinv c -> mkApp (Lazy.force coq_Zopp, [| val_of c |],[|Expl|])
  | Otimes (t1,t2) ->
      mkApp (Lazy.force coq_Zmult, [| val_of t1; val_of t2 |],[|Expl;Expl|])
  | Oplus(t1,t2) ->
      mkApp (Lazy.force coq_Zplus, [| val_of t1; val_of t2 |],[|Expl;Expl|])
  | Oufo c -> c
...
let clever_rewrite_base_poly typ p result theorem gl = 
  ...
  let t = 
    applist
      (mkLambda
	 (Name (id_of_string "P"),Expl,
	  mkArrow Expl typ mkProp,
          mkLambda
	    (Name (id_of_string "H"),Expl,
	     applist (mkRel 1,[result],[Expl]),
	     mkApp (Lazy.force coq_eq_ind_r, 
		       [| typ; result; mkRel 2; mkRel 1; occ; theorem |],
                       [|Impl;Impl;Impl;Expl;Impl;Expl|]))),
       [abstracted],[Expl]) 
  in
  exact (applist(t,[mkNewMeta()],[Expl])) gl
...
let clever_rewrite_gen p result (t,args,imps) = 
  let theorem = applist(t, args,imps) in 
  clever_rewrite_base p result theorem

let clever_rewrite_gen_nat p result (t,args,imps) = 
  let theorem = applist(t, args,imps) in 
...
let clever_rewrite p vpath t gl = 
  ...
  let t' = applist(t, (vargs @ [abstracted]),
                   List.map(fun _->Expl)vargs@[Expl]) in
  exact (applist(t',[mkNewMeta()],[Expl])) gl
...
let rec scalar p n = function
  ...
  | Oufo c ->
      [], Oufo
        (mkApp (Lazy.force coq_Zmult, [| mk_integer n; c |],[|Expl;Expl|]))
...
let rec negate p = function
  ...
  | Oufo c -> [], Oufo (mkApp (Lazy.force coq_Zopp, [| c |],[|Expl|]))
	...
	let tac,t = 
	  transform p
	    (mkApp (Lazy.force coq_Zplus,
		     [| t1; mkApp (Lazy.force coq_Zopp, [| t2 |],[|Expl|]) |],
                     [|Expl;Expl|])) in
	unfold sp_Zminus :: tac,t
    | Kapp(Zsucc,[t1]) ->
	let tac,t = transform p (mkApp (Lazy.force coq_Zplus,
				 [| t1; mk_integer one |],[|Expl;Expl|])) in
  ...
  let rec loop t =
    match t with
	  ...
	  tclTHENLIST [
		...
		val_of eq2;
		mk_integer k; 
		mkVar id1; mkVar id2 |],Array.create 5 Expl)] [Expl]);
	    (mk_then tac);
	    (intros_using [aux]);
	    (resolve_id aux);
            reflexivity
	  ...
	  let theorem =
            mkApp (Lazy.force coq_OMEGA2, [| 
		      ...
		      mkVar (hyp_of_tag e2.id) |],
                      Array.create 4 Expl)
	  in
	  tclTHEN (tclTHEN (generalize_tac [theorem] [Expl])
            (mk_then tac)) (solve_le)
	  ...
	  tclTHENS 
	    ...
	    [ tclTHENS
		  ...
		  (clear [aux;id]);
		  (intros_using [id]);
		  (cut Expl (mk_gt kk dd)) ])
	        [ tclTHENS 
		    (cut Expl (mk_gt kk izero)) 
		    [ tclTHENLIST [
		        (intros_using [aux1; aux2]);
		        (generalize_tac 
			  [mkApp (Lazy.force coq_Zmult_le_approx,
			    [| kk;eq2;dd;mkVar aux1;mkVar aux2; mkVar id |],
                            Array.create 6  Expl)] [Expl]);
		        (clear [aux1;aux2;id]);
		        (intros_using [id]);
		        (loop l) ];
		      tclTHENLIST [
	  ...
	  tclTHENS 
	    (cut Expl (mk_gt dd izero)) 
	    [ tclTHENS (cut Expl (mk_gt kk dd)) 
		[tclTHENLIST [
		  (intros_using [aux2;aux1]);
		  (generalize_tac 
		    [mkApp (Lazy.force coq_OMEGA4, 
                      [| dd;kk;eq2;mkVar aux1; mkVar aux2 |],
                      Array.create 5 Expl)] [Expl]);
		  (clear [aux1;aux2]);
		  (unfold sp_not);
		  (intros_using [aux]);
		  (resolve_id aux);
	  ...
	  tclTHENS 
	    (simplest_elim
              (applist (Lazy.force coq_OMEGA19, [eq; mkVar id],[Expl;Expl])))
	  ...
	  if k1 =? one & e2.kind = EQUA then
	    ...
	    let tac= shuffle_mult p_initial k1 e1.body k2 e2.body in
            tclTHENS (cut Expl (mk_gt kk1 izero)) 
	      [tclTHENS 
		 (cut Expl (mk_gt kk2 izero)) 
		 [tclTHENLIST [
		   (intros_using [aux2;aux1]);
		   (generalize_tac
	  ...
	  tclTHEN (generalize_tac [mkVar (hyp_of_tag e)][Expl])
            Equality.discrConcl
      | CONSTANT_NUL(e) :: l -> 
	  tclTHEN (resolve_id (hyp_of_tag e)) reflexivity
      | CONSTANT_NEG(e,k) :: l ->
	  tclTHENLIST [
	    (generalize_tac [mkVar (hyp_of_tag e)] [Expl]);
            (unfold sp_Zle);
	    simpl_in_concl;
	    (unfold sp_not);
	    (intros_using [aux]);
...
let normalize_equation id flag theorem pos t t1 t2 (tactic,defs) =
  ...
  let shift_left = 
    tclTHEN 
      (generalize_tac
        [mkApp (theorem, [| t1; t2; mkVar id |],[|Expl;Expl;Expl|]) ][Expl])
      (tclTRY (clear [id]))
  in
  if tac <> [] then
    let id' = new_identifier () in 
  ...
  let prelude,sys = 
    List.fold_left 
	 ...
	 if b then
           ...
           tag_hypothesis id i;
           (tclTHENLIST [
	     (simplest_elim (applist (Lazy.force coq_intro_Z, [t],[Expl])));
	     (intros_using [v; id]);
	     (elim_id id);
	     (clear [id]);
	     (intros_using [th;id]);
	    ...
	    body = [{v=intern_id v; c=one}]; 
           ...
           (tclTHENLIST [
	     (simplest_elim (applist (Lazy.force coq_new_var, [t],[Expl])));
	     (intros_using [v;th]);
	     tac ]),
           sys)
    ...
    try match destructurate_term t with
      | Kapp(Plus,[t1;t2]) ->
	    ...
	    (clever_rewrite_gen p (mk_plus (mk_inj t1) (mk_inj t2))
              ((Lazy.force coq_inj_plus),[t1;t2],[Expl;Expl]));
      ...
      | Kapp(Mult,[t1;t2]) ->
	    ...
	    (clever_rewrite_gen p (mk_times (mk_inj t1) (mk_inj t2))
              ((Lazy.force coq_inj_mult),[t1;t2],[Expl;Expl]));
	    (explore (P_APP 1 :: p) t1);
	    (explore (P_APP 2 :: p) t2)
          ]
      | Kapp(Minus,[t1;t2]) ->
          ...
          tclTHENS
            (tclTHEN 
	       (simplest_elim
                 (applist (Lazy.force coq_le_gt_dec, [t2;t1],[Expl;Expl]))) 
	       (intros_using [id])) 
	    [
	      tclTHENLIST [
		  ...
		  (mk_minus (mk_inj t1) (mk_inj t2))
                  ((Lazy.force coq_inj_minus1),[t1;t2;mkVar id],
                   [Expl;Expl;Expl]));
		(loop [id,mkApp 
                          (Lazy.force coq_le, [| t2;t1 |],[|Expl;Expl|])]);
		(explore (P_APP 1 :: p) t1);
		(explore (P_APP 2 :: p) t2) ];
	      (tclTHEN 
		 (clever_rewrite_gen p (mk_integer zero)
                    ...
                    [Expl;Expl;Expl]))
		 (loop [id,mkApp
                           (Lazy.force coq_gt, [| t2;t1 |],[|Expl;Expl|])]))
	    ]
      | Kapp(S,[t']) ->
          let rec is_number t =
            try match destructurate_term t with 
	  ...
	  in
		...
		Kapp(S,[t]) ->
                  (tclTHEN 
                     (clever_rewrite_gen p 
			(mkApp (Lazy.force coq_Zsucc, [| mk_inj t |],[|Expl|]))
			((Lazy.force coq_inj_S),[t],[Expl])) 
		     (loop (P_APP 1 :: p) t))
              | _ -> explore p t 
      ...
      | Kapp(Pred,[t]) ->
	    ...
	    mkApp (Lazy.force coq_minus, [| t; 
		      mkApp (Lazy.force coq_S, 
                           [| Lazy.force coq_O |],[|Expl|]) |],
                   [|Expl;Expl|]) in
          tclTHEN
            (clever_rewrite_gen_nat (P_APP 1 :: p) t_minus_one 
               ((Lazy.force coq_pred_of_minus),[t],[Expl]))
...
let rec decidability gl t =
  match destructurate_prop t with
	...
	begin match destructurate_type (pf_nf gl typ) with
          | Kapp(Z,[]) -> 
              mkApp (Lazy.force coq_dec_eq, [| t1;t2 |],[|Expl;Expl|])
          | Kapp(Nat,[]) ->
              mkApp (Lazy.force coq_dec_eq_nat, [| t1;t2 |],[|Expl;Expl|])
          | _ -> errorlabstrm "decidability" 
		(str "Omega: Can't solve a goal with equality on " ++ 
		   Printer.pr_lconstr typ)
	end
    | Kapp(Zne,[t1;t2]) ->
        mkApp (Lazy.force coq_dec_Zne, [| t1;t2 |],[|Expl;Expl|])
    | Kapp(Zle,[t1;t2]) ->
        mkApp (Lazy.force coq_dec_Zle, [| t1;t2 |],[|Expl;Expl|])
    | Kapp(Zlt,[t1;t2]) ->
        mkApp (Lazy.force coq_dec_Zlt, [| t1;t2 |],[|Expl;Expl|])
    | Kapp(Zge,[t1;t2]) ->
        mkApp (Lazy.force coq_dec_Zge, [| t1;t2 |],[|Expl;Expl|])
    | Kapp(Zgt,[t1;t2]) ->
        mkApp (Lazy.force coq_dec_Zgt, [| t1;t2 |],[|Expl;Expl|])
    | Kapp(Le, [t1;t2]) ->
        mkApp (Lazy.force coq_dec_le, [| t1;t2 |],[|Expl;Expl|])
    | Kapp(Lt, [t1;t2]) ->
        mkApp (Lazy.force coq_dec_lt, [| t1;t2 |],[|Expl;Expl|])
    | Kapp(Ge, [t1;t2]) ->
        mkApp (Lazy.force coq_dec_ge, [| t1;t2 |],[|Expl;Expl|])
    | Kapp(Gt, [t1;t2]) ->
        mkApp (Lazy.force coq_dec_gt, [| t1;t2 |],[|Expl;Expl|])
	...
	begin try match destructurate_prop t with
	  | Kapp(False,[]) -> elim_id i
          ...
          | Kapp(Or,[t1;t2]) ->
              (tclTHENS 
                (elim_id i)
                [ onClearedName i (fun i -> (loop ((i,Expl,None,t1)::lit)));
                  onClearedName i (fun i -> (loop ((i,Expl,None,t2)::lit))) ])
          | Kapp(And,[t1;t2]) ->
              tclTHENLIST [
                (elim_id i);
		(tclTRY (clear [i]));
		(fun gl -> 
		  ...
		  tclTHENLIST [
		    (introduction i1);
		    (introduction i2);
		    (loop ((i1,Expl,None,t1)::(i2,Expl,None,t2)::lit)) ] gl)
	      ]
          | Kimp(t1,t2) ->
              if
		...
		tclTHENLIST [
		  (generalize_tac [mkApp (Lazy.force coq_imp_simp,
                    ...
                    Array.create 4 Expl)] [Expl]);
		  (onClearedName i (fun i ->
		    (loop ((i,Expl,None,mk_or (mk_not t1) t2)::lit))))
                ]
              else
		loop lit
          | Kapp(Not,[t]) -> 
		  ...
		  Kapp(Or,[t1;t2]) -> 
		      ...
		      (generalize_tac
                        ...
                        [|Expl;Expl;Expl|])] [Expl]);
		      (onClearedName i (fun i ->
                        (loop ((i,Expl,None,mk_and (mk_not t1) (mk_not t2)):: lit))))
                    ]
		| Kapp(And,[t1;t2]) -> 
                    tclTHENLIST [
		      (generalize_tac
			  ...
			  decidability gl t1;mkVar i|],
                        Array.create 4 Expl)] [Expl]);
		      (onClearedName i (fun i ->
                        (loop ((i,Expl,None,mk_or (mk_not t1) (mk_not t2))::lit))))
                    ]
		| Kimp(t1,t2) ->
                    tclTHENLIST [
		      (generalize_tac 
		        [mkApp (Lazy.force coq_not_imp, [| t1; t2; 
		          decidability gl t1;mkVar i |],
                        Array.create 4 Expl)] [Expl]);
		      (onClearedName i (fun i ->
                        (loop ((i,Expl,None,mk_and t1 (mk_not t2)) :: lit))))
                    ]
		| Kapp(Not,[t]) ->
                    tclTHENLIST [
		      (generalize_tac
			[mkApp (Lazy.force coq_not_not, [| t;
			  decidability gl t; mkVar i |],
                        Array.create 3 Expl)] [Expl]);
		      (onClearedName i (fun i -> (loop ((i,Expl,None,t)::lit))))
                    ]
		| Kapp(Zle, [t1;t2]) ->
                    tclTHENLIST [
		      (generalize_tac
