...
(* $Id: safe_typing.ml 9821 2007-05-11 17:00:58Z aspiwack $ *)
...
(* Same as push_named, but check that the variable is not already
   ...
   cost too much. *)
let safe_push_named (id,_,_,_ as d) env =
  let _ =
    try
...
let push_named_def (id,imp,b,topt) senv =
  let (c,typ,cst) = translate_local_def senv.env (b,topt) in
  let env' = add_constraints cst senv.env in
  let env'' = safe_push_named (id,imp,Some c,typ) env' in
  (cst, {senv with env=env''})

let push_named_assum (id,imp,t) senv =
  let (t,cst) = translate_local_assum senv.env t in
  let env' = add_constraints cst senv.env in
  let env'' = safe_push_named (id,imp,None,t) env' in
  (cst, {senv with env=env''})
