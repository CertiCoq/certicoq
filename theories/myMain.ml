...
module OrderedTypeFacts =
 ...
 struct
  ...
  let lt_dec x0 y =
    let c0 = compSpec2Type x0 y (O.compare x0 y) in
    (match c0 with
     | CompLtT -> Left
     | _ -> Right)
...
module Coq_Pos =
 struct
  ...
  let rec peano_rect a0 f p =
    let f2 =
      peano_rect (f XH a0) (fun p0 x0 -> f (succ (XO p0)) (f (XO p0) x0))
    in
    (match p with
     | XI q -> f (XO q) (f2 q)
     | XO q -> f2 q
  ...
  | PeanoSucc (p, q0) ->
    PeanoSucc ((succ (XO p)), (PeanoSucc ((XO p), (peanoView_xO p q0))))
  ...
  | PeanoSucc (p, q0) ->
    PeanoSucc ((succ (XI p)), (PeanoSucc ((XI p), (peanoView_xI p q0))))
  ...
  | PeanoOne -> a0
  | PeanoSucc (p, q0) -> f p (coq_PeanoView_iter a0 f p q0)
...
| Nil -> Nil
| Cons (x0, l') -> app (rev0 l') (Cons (x0, Nil))
...
| Cons (x0, tl) ->
  ...
  (match f x0 with
   | True -> Pair ((Cons (x0, g)), d0)
   | False -> Pair (g, (Cons (x0, d0))))
 ...
 struct
  module Raw =
   struct
    ...
    | Leaf -> acc
    | Node (_, l, x0, r) ->
      rev_elements_aux (Cons (x0, (rev_elements_aux acc l))) r
    ...
    let rec cons s e0 =
      match s with
      | Leaf -> e0
      | Node (_, l, x0, r) -> cons l (More (x0, r, e0))
    ...
    | Leaf -> Leaf
    | Node (_, a0, x0, b0) -> Node (Black, a0, x0, b0)

    (** val makeRed : tree -> tree **)

    let makeRed = function
    | Leaf -> Leaf
    | Node (_, a0, x0, b0) -> Node (Red, a0, x0, b0)
            ...
            | Leaf ->
              (match c0 with
               ...
               | Node (t1, b0, y, c1) ->
                 (match t1 with
                  | Red ->
                    Node (Red, (Node (Black, a0, x0, b0)), y, (Node (Black,
                      c1, k, r)))
                  | Black -> Node (Black, l, k, r)))
            | Node (t1, a1, x1, b0) ->
              (match t1 with
               | Red ->
                 Node (Red, (Node (Black, a1, x1, b0)), x0, (Node (Black, c0,
                   k, r)))
               | Black ->
                 (match c0 with
                  | Leaf -> Node (Black, l, k, r)
                  | Node (t2, b1, y, c1) ->
                    (match t2 with
                     | Red ->
                       Node (Red, (Node (Black, a0, x0, b1)), y, (Node
                         (Black, c1, k, r)))
                     | Black -> Node (Black, l, k, r)))))
         | Black -> Node (Black, l, k, r))
    ...
    let lbalS l k r =
      ...
      | Node (t0, a0, x0, b0) ->
        (match t0 with
         | Red -> Node (Red, (Node (Black, a0, x0, b0)), k, r)
         | Black ->
           (match r with
            | Leaf -> Node (Red, l, k, r)
            | Node (t1, a1, z0, c0) ->
            ...
            (match b0 with
             ...
             | Node (t1, b1, y, c0) ->
               (match t1 with
                | Red -> Node (Red, l, k, r)
                | Black ->
                  Node (Red, (lbal (makeRed a0) x0 b1), y, (Node (Black, c0,
                    k, r)))))
          | Black -> lbal (Node (Red, a0, x0, b0)) k r))
    | Node (t0, b0, y, c0) ->
      (match t0 with
               ...
               (match b1 with
                ...
                | Node (t2, b2, y0, c1) ->
                  (match t2 with
                   | Red -> Node (Red, l, k, r)
                   | Black ->
                     Node (Red, (lbal (makeRed a0) x0 b2), y0, (Node (Black,
                       c1, k, r)))))
             | Black -> lbal (Node (Red, a0, x0, b1)) k r)))
            ...
            | Red ->
              ...
              (match lrl with
               ...
               | Node (t0, lr', x0, rl') ->
                 (match t0 with
                  | Red ->
                    Node (Red, (Node (Red, ll, lx, lr')), x0, (Node (Red,
                      rl', rx, rr)))
            ...
            | Black ->
              ...
              (match lrl with
               ...
               | Node (t0, lr', x0, rl') ->
                 (match t0 with
                  | Red ->
                    Node (Red, (Node (Black, ll, lx, lr')), x0, (Node (Black,
                      rl', rx, rr)))
    ...
    | Node (_, a0, y, b0) ->
      (match X.compare x0 y with
       ...
       | Lt ->
         (match a0 with
          | Leaf -> Node (Red, (del x0 a0), y, b0)
          | Node (t1, _, _, _) ->
            (match t1 with
             | Red -> Node (Red, (del x0 a0), y, b0)
             | Black -> lbalS (del x0 a0) y b0))
       | Gt ->
         (match b0 with
          | Leaf -> Node (Red, a0, y, (del x0 b0))
          | Node (t1, _, _, _) ->
            (match t1 with
             | Red -> Node (Red, a0, y, (del x0 b0))
             | Black -> rbalS a0 y (del x0 b0))))
    ...
    | Leaf -> None
    | Node (_, l, x0, r) ->
      let Pair (k, t1) = delmin l x0 r in Some (Pair (k, (makeBlack t1)))
    ...
    let treeify_cont f g acc =
      ...
      (match l0 with
       | Nil -> bogus
       | Cons (x0, acc0) ->
         let Pair (r, acc1) = g acc0 in Pair ((Node (Black, l, x0, r)), acc1))
         ...
         (match a0 with
          ...
          | Node (t2, a1, x0, b0) ->
            (match t2 with
             | Red ->
               (match c0 with
                | Leaf -> f a1 x0 b0 y c0
                | Node (t3, b1, y0, c1) ->
                  (match t3 with
                   | Red -> f a0 y b1 y0 c1
                   | Black -> f a1 x0 b0 y c0))
             | Black ->
               (match c0 with
                | Leaf -> g t0
                | Node (t3, b1, y0, c1) ->
...
| Equ (x0, y) ->
  mkPureClause Nil (Cons ((order_eqv_pure_atom (Eqv (x0, y))), Nil))
| Nequ (x0, y) ->
  mkPureClause (Cons ((order_eqv_pure_atom (Eqv (x0, y))), Nil)) Nil
...
| Cons (p, l') ->
  (match p with
   | Equ (x0, y) ->
     ...
     Pair ((Cons ((order_eqv_pure_atom (Eqv (x0, y))), p0)), n0)
   | Nequ (x0, y) ->
     let Pair (p0, n0) = mk_pureR l' in
     Pair (p0, (Cons ((order_eqv_pure_atom (Eqv (x0, y))), n0))))
...
module OrderedClause =
 struct
  ...
  let eq_dec x0 y =
    ...
    (match j with
     | Eq ->
       let c0 = compSpec2Type x0 y (compare x0 y) in
       (match c0 with
        | CompEqT -> Left
        | _ -> assert false (* absurd case *))
     | Lt -> Right
     | Gt ->
       let c0 = compSpec2Type x0 y (compare_clause' x0 y) in
       (match c0 with
        | CompEqT -> Left
        | _ -> Right))
 end
 ...
 struct
  ...
  | Cons (s, sc') ->
    (match s with
     ...
     | Lseg (x1, y0) ->
       (match x1 with
        | Nil1 -> lseg_in_dom_atoms x0 sc'
        | Var x' ->
          (match Ident.eq_dec x0 x' with
           | Left ->
             Cons ((order_eqv_pure_atom (Eqv (x1, y0))),
       ...
       (match x0 with
        ...
        | Var x' ->
          (match next_in_dom1 x' z0 sigma1 with
           | True ->
             Cons ((Pair ((Eqv (x0, z0)),
               (insert (rev_cmp compare_space_atom) (Next (x0, z0))
                ...
                Cons
                  ((insert (rev_cmp compare_space_atom) (Lseg (x0, y))
                     (insert (rev_cmp compare_space_atom) (Lseg (y, Nil1))
                       (app (rev0 sigma0) sigma2'))),
                  (unfolding3' (Cons ((Lseg (x0, Nil1)), sigma0)) sigma1
                    sigma2'))
              | None ->
                unfolding3' (Cons ((Lseg (x0, Nil1)), sigma0)) sigma1 sigma2')
                 ...
                 | True ->
                   Cons
                     ((insert (rev_cmp compare_space_atom) (Lseg (x0, y))
                        (insert (rev_cmp compare_space_atom) (Lseg (y, z0))
                          (app (rev0 sigma0) sigma2'))),
                     (unfolding4NPR' (Cons ((Lseg (x0, z0)), sigma0)) sigma1
                       sigma2'))
                 | False ->
                   unfolding4NPR' (Cons ((Lseg (x0, z0)), sigma0)) sigma1
                     sigma2')
                ...
                (match lseg_in_dom2 x' z0 sigma1 with
                 | Some y ->
                   ...
                   app (map build_res atms)
                     (unfolding6NPR' (Cons ((Lseg (x0, z0)), sigma0)) sigma1
                       sigma2')
                 | None ->
                   unfolding6NPR' (Cons ((Lseg (x0, z0)), sigma0)) sigma1
                     sigma2')))))
...
let harder_ent =
  Entailment ((Assertion (Nil, (Cons ((Next ((x (XI (XO (XO XH)))),
    (x (XI XH)))), (Cons ((Lseg ((x (XI (XI XH))), (x (XO (XO (XO XH)))))),
    (Cons ((Lseg ((x XH), (x (XO (XI XH))))), (Cons ((Next
    ((x (XO (XI (XO XH)))), (x (XO XH)))), (Cons ((Next ((x (XI XH)),
    (x (XI (XI XH))))), (Cons ((Lseg ((x (XI (XO XH))),
    (x (XO (XI (XO XH)))))), (Cons ((Next ((x (XO (XO XH))),
    (x (XI (XO (XO XH)))))), (Cons ((Next ((x (XI (XO (XI XH)))),
    (x (XI (XO XH))))), (Cons ((Lseg ((x (XO (XO (XO XH)))),
    (x (XI (XO XH))))), (Cons ((Next ((x (XI (XI (XO XH)))),
    (x (XO (XO (XI XH)))))), (Cons ((Next ((x (XO (XO (XI XH)))),
    (x (XI (XI (XO XH)))))), (Cons ((Next ((x (XO XH)), (x (XO (XO XH))))),
    (Cons ((Lseg ((x (XO (XI XH))), (x (XI XH)))),
    Nil)))))))))))))))))))))))))))), (Assertion (Nil, (Cons ((Lseg
    ((x (XO XH)), (x (XO (XI (XO XH)))))), (Cons ((Lseg
    ((x (XO (XO (XI XH)))), (x (XI (XI (XO XH)))))), (Cons ((Lseg
    ((x (XI (XI (XO XH)))), (x (XO (XO (XI XH)))))), (Cons ((Lseg
    ((x (XO (XI XH))), (x (XI XH)))), (Cons ((Lseg ((x XH),
    (x (XO (XI XH))))), (Cons ((Lseg ((x (XI (XO (XI XH)))),
    (x (XI (XO XH))))), (Cons ((Lseg ((x (XO (XI (XO XH)))), (x (XO XH)))),
    Nil)))))))))))))))))

(** val harder_ent2 : entailment **)

let harder_ent2 =
  Entailment ((Assertion (Nil, (Cons ((Next ((x (XO XH)),
    (x (XI (XO (XI XH)))))), (Cons ((Lseg ((x (XI XH)), (x (XO (XI XH))))),
    (Cons ((Lseg ((x (XO (XI (XO XH)))), (x (XI (XO XH))))), (Cons ((Next
    ((x (XO (XO XH))), (x (XO (XO (XI (XO XH))))))), (Cons ((Next
    ((x (XO (XO (XO (XO XH))))), (x (XI (XI (XO XH)))))), (Cons ((Lseg
    ((x (XO (XI XH))), (x (XO (XI (XI XH)))))), (Cons ((Next
    ((x (XO (XO (XI (XO XH))))), (x (XI (XI (XO XH)))))), (Cons ((Lseg
    ((x (XI (XO (XO (XO XH))))), (x (XO (XI XH))))), (Cons ((Lseg
    ((x (XI (XI (XO (XO XH))))), (x (XI (XO (XI XH)))))), (Cons ((Lseg
    ((x (XI (XI (XO XH)))), (x XH))), (Cons ((Next ((x (XI (XI (XI XH)))),
    (x (XI (XO (XI XH)))))), (Cons ((Next ((x XH),
    (x (XI (XO (XO (XO XH))))))), (Cons ((Next ((x (XI (XO XH))),
    (x (XO (XI (XO (XO XH))))))), (Cons ((Lseg ((x (XO (XO (XO XH)))),
    (x (XI (XO XH))))), (Cons ((Next ((x (XI (XI XH))),
    (x (XI (XO (XI XH)))))), (Cons ((Next ((x (XO (XI (XI XH)))),
    (x (XO XH)))), (Cons ((Next ((x (XO (XI (XO (XO XH))))),
    (x (XO (XI XH))))), (Cons ((Next ((x (XO (XO (XI XH)))),
    (x (XI (XO (XO (XO XH))))))), (Cons ((Lseg ((x (XI (XO (XI XH)))),
    (x (XO (XO (XO XH)))))), (Cons ((Next ((x (XI (XO (XO XH)))),
    (x (XO (XI (XO XH)))))), Nil)))))))))))))))))))))))))))))))))))))))))),
    (Assertion (Nil, (Cons ((Lseg ((x (XI (XI (XI XH)))), (x (XO XH)))),
    (Cons ((Lseg ((x (XO (XO XH))), (x (XO (XI XH))))), (Cons ((Lseg
    ((x (XI (XI (XO (XO XH))))), (x (XI (XO (XI XH)))))), (Cons ((Lseg
    ((x (XO (XI (XO XH)))), (x (XI (XO XH))))), (Cons ((Lseg
    ((x (XI (XI XH))), (x (XI (XO (XI XH)))))), (Cons ((Lseg ((x (XO XH)),
    (x (XI (XO (XI XH)))))), (Cons ((Lseg ((x (XO (XO (XO (XO XH))))),
    (x (XI (XI (XO XH)))))), (Cons ((Lseg ((x (XI XH)), (x (XO (XI XH))))),
    (Cons ((Lseg ((x (XO (XO (XI XH)))), (x (XI (XO (XO (XO XH))))))), (Cons
    ((Lseg ((x (XI (XO (XO XH)))), (x (XO (XI (XO XH)))))),
    Nil)))))))))))))))))))))))

(** val harder_ent3 : entailment **)

let harder_ent3 =
  Entailment ((Assertion (Nil, (Cons ((Next ((x XH), (x (XO (XI (XI XH)))))),
    (Cons ((Next ((x (XO (XI (XI XH)))), (x (XI (XI (XO (XO XH))))))), (Cons
    ((Next ((x (XI (XO XH))), (x (XO (XO (XO XH)))))), (Cons ((Next
    ((x (XI (XO (XI XH)))), (x (XO (XI (XO (XO XH))))))), (Cons ((Lseg
    ((x (XO (XI (XO (XO XH))))), (x (XI (XO (XI XH)))))), (Cons ((Lseg
    ((x (XI (XI (XI XH)))), (x (XO XH)))), (Cons ((Next
    ((x (XI (XO (XO (XO XH))))), (x (XI (XI (XI XH)))))), (Cons ((Next
    ((x (XO (XO (XI (XO XH))))), (x (XO (XI XH))))), (Cons ((Next
    ((x (XO (XI XH))), (x (XI (XO (XI XH)))))), (Cons ((Lseg
    ((x (XI (XI (XO XH)))), (x (XI (XI (XO (XO XH))))))), (Cons ((Next
    ((x (XI (XI XH))), (x (XO (XO (XI XH)))))), (Cons ((Next
    ((x (XO (XI (XO XH)))), (x (XI (XO (XO (XO XH))))))), (Cons ((Lseg
    ((x (XO (XO (XO (XO XH))))), (x (XO (XO (XI (XO XH))))))), (Cons ((Next
    ((x (XO XH)), (x (XO (XO (XI (XO XH))))))), (Cons ((Next
    ((x (XI (XI (XO (XO XH))))), (x (XO (XO XH))))), (Cons ((Lseg
    ((x (XI (XO (XO XH)))), (x (XI XH)))), (Cons ((Lseg
    ((x (XO (XO (XO XH)))), (x (XO (XO (XI XH)))))), (Cons ((Lseg
    ((x (XO (XO (XI XH)))), (x (XO (XO (XI (XO XH))))))), (Cons ((Next
    ((x (XO (XO XH))), (x (XO (XI XH))))), (Cons ((Next ((x (XI XH)),
    (x (XO (XO (XI (XO XH))))))),
    Nil)))))))))))))))))))))))))))))))))))))))))), (Assertion (Nil, (Cons
    ((Lseg ((x (XO (XI XH))), (x (XO (XI (XO (XO XH))))))), (Cons ((Lseg
    ((x (XO (XO XH))), (x (XO (XI XH))))), (Cons ((Lseg ((x (XI (XI XH))),
    (x (XO (XI XH))))), (Cons ((Lseg ((x XH), (x (XO (XO XH))))), (Cons
    ((Lseg ((x (XI (XI (XO XH)))), (x (XI (XI (XO (XO XH))))))), (Cons ((Lseg
    ((x (XO (XI (XO (XO XH))))), (x (XI (XO (XI XH)))))), (Cons ((Lseg
    ((x (XO (XO (XO (XO XH))))), (x (XO (XO (XI (XO XH))))))), (Cons ((Lseg
    ((x (XO (XI (XO XH)))), (x (XO (XO (XI (XO XH))))))), (Cons ((Lseg
    ((x (XI (XO XH))), (x (XO (XO (XI XH)))))), (Cons ((Lseg ((x (XI XH)),
    (x (XO (XO (XI (XO XH))))))), (Cons ((Lseg ((x (XI (XO (XO XH)))),
    (x (XI XH)))), Nil)))))))))))))))))))))))))

(** val example_myent : entailment **)

let example_myent =
  Entailment ((Assertion (Nil, Nil)), (Assertion ((Cons ((Equ (a, a)), Nil)),
    Nil)))

(** val example1_myent : entailment **)

let example1_myent =
  Entailment ((Assertion ((Cons ((Equ (a, b)), Nil)), Nil)), (Assertion
    ((Cons ((Equ (b, a)), Nil)), Nil)))

(** val example2_myent : entailment **)

let example2_myent =
  Entailment ((Assertion ((Cons ((Equ (a, b)), (Cons ((Equ (b, c)), Nil)))),
    Nil)), (Assertion ((Cons ((Equ (c, a)), Nil)), Nil)))

(** val example1_myfail : entailment **)

let example1_myfail =
  Entailment ((Assertion (Nil, Nil)), (Assertion ((Cons ((Equ (a, b)), Nil)),
    Nil)))

(** val myMain : VeriStar.veristar_result list **)

let myMain =
  map VeriStar.check_entailment (Cons (example_myent, (Cons (example1_myent,
    (Cons (example2_myent, (Cons (example1_myfail, (Cons (example1_myfail,
    (Cons (example_ent, (Cons (harder_ent, (Cons (harder_ent2, (Cons
    (harder_ent3, Nil))))))))))))))))))
