...
(* $Id: tacmach.ml 9952 2007-07-06 15:31:37Z letouzey $ *)
...
let pf_hyps_types gls  =
  ...
  List.map (fun (id,_,_,x) -> (id, x)) sign

let pf_nth_hyp_id gls n =
  let (id,_,c,t) = List.nth (pf_hyps gls) (n-1) in id
...
let pf_get_hyp_typ gls id =
  let (_,_,_,ty)= (pf_get_hyp gls id) in
  ty
...
let internal_cut_no_check id imp t gl = 
  refiner (Prim (Cut (true,id,imp,t))) gl

let internal_cut_rev_no_check id imp t gl = 
  refiner (Prim (Cut (false,id,imp,t))) gl
...
let internal_cut d imp t     = with_check (internal_cut_no_check d imp t)
let internal_cut_rev d imp t = with_check (internal_cut_rev_no_check d imp t)
