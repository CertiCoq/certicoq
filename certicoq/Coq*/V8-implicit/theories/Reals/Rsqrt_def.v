...
(*i $Id: Rsqrt_def.v 9454 2006-12-15 15:30:59Z bgregoir $ i*)
...
Proof.
  ...
  unfold cv_infty in |- *.
  intro.
  destruct (total_order_or 0 M) as [[a|b]|r].
  set (N := up M).
  cut (0 <= N)%Z.
  intro.
...
Proof.
  ...
  destruct (total_order_or x y) as [[a|b]|r].
...
Proof.
  ...
  assumption.
  simpl in |- *.
  case_eq (P ((Dichotomy_lb x y P n + Dichotomy_ub x y P n) / 2)); trivial.
Qed.
...
Proof.
  ...
  case (Rle_dec 0 z); intro; apply impl_PI.
  split.
  intro; assumption.
  intro; reflexivity.
  split.
  ...
  destruct (total_order_or 0 (f x0)) as [[a|b]|r].
  left; assumption.
...
Proof.
  intros.
  destruct (total_order_T 0 (f x)) as [s|r].
  destruct (total_order_T 0 (f y)) as [s0|r].
  destruct s as [a|b].
  destruct s0 as [a0|b].

  cut (0 < f x * f y);
    [ intro; elim (Rlt_irrefl _ (Rle_lt_trans _ _ _ H1 H2))
      | apply impl_PI; apply Rmult_lt_0_compat; assumption ].

  exists y.
  split.
  split; [ assumption | right; reflexivity ].
  apply impl_PI; symmetry  in |- *; exact b.
  ...
  apply impl_PI; unfold opp_fct in |- *; apply Ropp_0_gt_lt_contravar; assumption.
  unfold opp_fct in |- *.
  apply impl_PI; apply Rplus_lt_reg_r with (f x); rewrite Rplus_opp_r; rewrite Rplus_0_r;
    assumption.

  inversion H0.
  assumption.
  rewrite H2 in a.
  elim (impl_PI (Rlt_irrefl _ (Rlt_trans _ _ _ r a))).
  exists x; apply impl_PI.
  split.
  split; [ right; reflexivity | assumption ].

  symmetry  in |- *; assumption.

  destruct (total_order_T 0 (f y)) as [[a|b]|r0].
  cut [x < y].
  intro.
  apply IVT; apply impl_PI; assumption.
  inversion H0.
  assumption.
  rewrite H2 in r.
  elim (Rlt_irrefl _ (Rlt_trans _ _ _ r a)).
  exists y; apply impl_PI.
  split.
  split; [ assumption | right; reflexivity ].
  symmetry  in |- *; assumption.
  cut (0 < f x * f y).
  ...
  apply impl_PI; rewrite <- Rmult_opp_opp; apply Rmult_lt_0_compat;
    apply Ropp_0_gt_lt_contravar; assumption.
Qed.
...
Proof.
  ...
  cut (continuity f).
  intro.
  destruct (total_order_T y 1) as [[a|b]|r].
  cut (0 <= f 1).
  intro.
  cut (f 0 * f 1 <= 0).
  intro.
  ...
  apply impl_PI; rewrite Rplus_0_r; rewrite Rplus_comm; unfold Rminus in |- *;
    rewrite Rplus_assoc; rewrite Rplus_opp_l; rewrite Rplus_0_r; 
      left; assumption.
  apply existT with 1.
  split.
  left; apply Rlt_0_1.
  rewrite (impl_PI b); symmetry  in |- *; apply Rsqr_1.
  cut [0 <= f y].
  intro.
  cut (f 0 * f y <= 0).
  intro.
  assert (X := IVT_cor f 0 y H1 H (impl_PI H3)).
  elim X; intros t H4.
  exists t; apply impl_PI.
  elim H4; intros.
  split.
  elim H5; intros; assumption.
  unfold f in H6.
  ...
  apply impl_PI; apply Rmult_le_compat_l; assumption.
  unfold f in |- *.
  apply Rplus_le_reg_l with y.
