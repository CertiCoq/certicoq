...
(*i $Id: SeqProp.v 9454 2006-12-15 15:30:59Z bgregoir $ i*)
...
Proof.
  ...
  rewrite H8; apply H4.
  intro; case (Rle_dec (Un n) x0); intro; apply impl_PI.
  assumption.
  ...
  case (Rlt_le_dec x0 (Un n)); intro; apply impl_PI.
  assumption.
  elim n0; assumption.
...
Proof.
  intros; case (total_order_T (Un n) l); intro; apply impl_PI.
  elim s; intro; apply impl_PI.
  left; assumption.
  right; assumption.
  cut (0 < Un n - l).
...
Proof.
  ...
  assumption.
  unfold cv_infty in |- *; intro; case (total_order_T M0 0); intro; apply impl_PI.
  elim s; intro; apply impl_PI.
  exists 0%nat; intros.
