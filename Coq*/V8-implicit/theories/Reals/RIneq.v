...
(*i $Id: RIneq.v 9454 2006-12-15 15:30:59Z bgregoir $ i*)
...
Proof.
  intros; generalize (total_order_or r1 r2) Rlt_dichotomy_converse.
  intuition eauto 3. 
Qed.
...
Proof.
  intros; generalize (total_order_or r1 r2); tauto.
Qed.
...
Qed.
Definition Rle_or r1 r2 := sumbool_or (Rle_dec r1 r2).
...
Proof.
  intros n m; destruct (Rlt_le_dec m n); apply impl_PI; auto with real.
Qed.
...
Proof.
  intro; destruct (Rlt_le_dec r 0); apply impl_PI; unfold Rsqr in |- *.
...
Proof.
  ...
  intros [H| H]; apply impl_PI; simpl in |- *.
  ...
  intro H; apply impl_PI; simpl in |- *.
...
Definition cond_pos (r:posreal) : 0 < r :=
  match r return 0<r with
    mkposreal r [a] => impl_PI a
  end.
