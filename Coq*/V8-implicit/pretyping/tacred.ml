...
(* $Id: tacred.ml 9843 2007-05-21 06:23:28Z herbelin $ *)
...
let is_evaluable env ref =
  match ref with
    ...
    | EvalVarRef id ->
        ...
        Idpred.mem id ids &
        let (_,_,value,_) = Environ.lookup_named id env in
        value <> None
...
let reference_opt_value sigma env = function
  ...
  | EvalVar id ->
      let (_,_,v,_) = lookup_named id env in
      v
  | EvalRel n ->
      let (_,_,v,_) = lookup_rel n env in
      option_map (lift n) v
	...
	match refi with
	  ...
	  | Some ref ->
		...
		| Some c -> 
		    ...
		    let _, l',_ = whd_betalet_stack ccl in
		    let labs' = List.map (fun(_,_,t)->t) labs' in
...
let compute_consteval_direct sigma env ref =
  let rec srec env n labs c =
    let c',l,_ = whd_betadelta_stack env sigma c in
    match kind_of_term c' with
      | Lambda (id,imp,t,g) when l=[] ->
	  srec (push_rel (id,imp,None,t) env) (n+1) (t::labs) g
      | Fix fix ->
...
let compute_consteval_mutual_fix sigma env ref =
  let rec srec env minarg labs ref c =
    let c',l,_ = whd_betalet_stack c in
    let nargs = List.length l in
    match kind_of_term c' with
      | Lambda (na,imp,t,g) when l=[] ->
	  srec (push_rel (na,imp,None,t) env) (minarg+1) (t::labs) ref g
      | Fix ((lv,i),(names,_,_)) ->
...
let make_elim_fun (names,(nbfix,lv,n)) largs =
  let (args,imps) = list_of_stack largs in
  let lu = list_firstn n args in
  let limps = list_firstn n imps in
  ...
  fun i ->
    match names.(i) with
      | None -> None
      | Some ref ->
          let body = applistc (mkEvalRef ref) la limps in
          let g = 
	      ...
	      mkLambda (x,List.nth limps (n-ij),tij',c)) 1 body (List.rev lv)
          in Some g
...
let reduce_mind_case_use_function func env mia =
  match kind_of_term mia.mconstr with 
	...
	mkCase (mia.mci, mia.mP, applist(cofix_def,mia.mcargs,mia.mimps),
                mia.mlf)
    | _ -> assert false
  ...
  let rec redrec s = 
    ...
    if isEvalRef env constr then
      ...
      match reference_opt_value sigma env ref with
	    ...
	    if reducible_mind_case gvalue then
	      ...
	      reduce_mind_case_use_function constr env
	        {mP=p; mconstr=gvalue; mcargs=args;
                 mimps=imps;mci=ci; mlf=lf}
	    else
	      redrec (gvalue, cargs)
    else
      if reducible_mind_case constr then
        let (args,imps) = list_of_stack cargs in
        reduce_mind_case
	  {mP=p; mconstr=constr; mcargs=args;
	   mimps=imps;mci=ci; mlf=lf}
      else 
    ...
    match kind_of_term x with
      | Lambda (na,_,t,c) ->
          (match decomp_stack stack with 
             | None -> s
             | Some (a,_,rest) -> stacklam redrec [a] c rest)
      | LetIn (n,_,b,t,c) -> stacklam redrec [b] c stack
      | App (f,cl,imps) -> redrec (f, append_stack cl imps stack)
    ...
    match kind_of_term x with
      | App (f,l,imps) -> 
          (match kind_of_term f with
             | Fix fix ->
                 let stack = append_stack l imps empty_stack in
                 (match fix_recarg fix stack with
                    | None -> raise Redelimination
                    | Some (recargnum,recarg) ->
             ...
             | _ -> simpfun (appvect (redrec env f, l, imps)))
      | Cast (c,_,_) -> redrec env c
      | Prod (x,imp,a,b) ->
          mkProd (x, imp, a, redrec (push_rel (x,imp,None,a) env) b)
      | LetIn (_,_,a,_,t) -> redrec env (subst1 a t)
  ...
  then error "mixing of positive and negative occurences"
   ...
   let rec traverse (env,c as envc) t =
	...
	(* find other occurrences of c in t; TODO: ensure left-to-right *)
        let (f,l,imps) = destApp t in
	mkApp (f, array_map_left (traverse envc) l,imps)
      else
	t
    else
...
let rec substlin env name n ol c =
  match kind_of_term c with
    ...
    | App (c1,cl,imps) ->
        array_fold_left2 
	  (fun (n1,ol1,c1') c2 imp ->
	     (match ol1 with 
                | [] -> (n1,[],applist(c1',[c2],[imp]))
                | _  ->
                    let (n2,ol2,c2') = substlin env name n1 ol1 c2 in
                    (n2,ol2,applist(c1',[c2'],[imp]))))
          (substlin env name n ol c1) cl imps

    | Lambda (na,imp,c1,c2) ->
        let (n1,ol1,c1') = substlin env name n ol c1 in
        (match ol1 with 
           | [] -> (n1,[],mkLambda (na,imp,c1',c2))
           | _  ->
               let (n2,ol2,c2') = substlin env name n1 ol1 c2 in
               (n2,ol2,mkLambda (na,imp,c1',c2')))

    | LetIn (na,imp1,c1,t,c2) ->
        let (n1,ol1,c1') = substlin env name n ol c1 in
        (match ol1 with 
           | [] -> (n1,[],mkLetIn (na,imp1,c1',t,c2))
           | _  ->
               let (n2,ol2,c2') = substlin env name n1 ol1 c2 in
               (n2,ol2,mkLetIn (na,imp1,c1',t,c2')))

    | Prod (na,imp,c1,c2) ->
        let (n1,ol1,c1') = substlin env name n ol c1 in
        (match ol1 with 
           | [] -> (n1,[],mkProd (na,imp,c1',c2))
           | _  ->
               let (n2,ol2,c2') = substlin env name n1 ol1 c2 in
               (n2,ol2,mkProd (na,imp,c1',c2')))
...
let abstract_scheme env sigma (locc,a) c =
  ...
  if occur_meta a then 
    mkLambda (na,Expl,ta,c)
  else 
    mkLambda (na,Expl,ta,subst_term_occ locc a c)
...
let reduce_to_ind_gen allow_product env sigma t = 
  let rec elimrec env t l =
    ...
    let (f,_,_) = decompose_app t in
    match kind_of_term f with
      | Ind ind-> (ind, it_mkProd_or_LetIn t l)
      | Prod (n,imp,ty,t') ->
	  if allow_product then
	    elimrec (push_rel (n,imp,None,ty) env) t' ((n,imp,None,ty)::l)
	  else
    ...
    match kind_of_term x with
      | Lambda (n,_,t,c)  ->
          (match decomp_stack stack with
             ...
             | Some (a,_,rest) -> (subst1 a c, rest))
      | App (f,cl,imps) -> redrec (f, append_stack cl imps stack)
      | LetIn (_,_,f,_,cl) -> (subst1 f cl,stack)
      | Cast (c,_,_) -> redrec (c,stack)
      | Case (ci,p,c,lf) ->
          (try
...
let reduce_to_ref_gen allow_product env sigma ref t =
  ...
  let rec elimrec env t l = 
    let c, _, _ = Reductionops.whd_stack t in
    match kind_of_term c with
      | Prod (n,imp,ty,t') ->
	  if allow_product then
	    elimrec (push_rel (n,imp,None,t) env) t' ((n,imp,None,ty)::l)
	  else 
	     errorlabstrm "" 
