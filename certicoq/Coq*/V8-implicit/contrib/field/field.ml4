...
let constr_of_opt a opt =
  ...
  let ac3 = mkArrow Expl ac (mkArrow Expl ac ac) in
  match opt with
  | None -> mkApp (init_constant "None",[|ac3|],[|Impl|])
  | Some f -> mkApp (init_constant "Some",[|ac3;constr_of f|],[|Impl;Expl|])
...
let add_field a aplus amult aone azero aopp aeq ainv aminus_o adiv_o rth
  ...
  begin
    ...
    let th = mkApp ((constant ["LegacyField_Theory"] "Build_Field_Theory"),
      [|a;aplus;amult;aone;azero;aopp;aeq;ainv;aminus_o;adiv_o;rth;ainv_l|],
      Array.create 12 Expl) in
    begin
...
let field g =
  ...
  let typ = 
    match Hipattern.match_with_equation (pf_concl g) with
      | Some (eq,t::args,_)
          when eq = (Coqlib.build_coq_eq_data()).Coqlib.eq -> t
