  ...
  | S n =>
    ...
    | TApp fn a1 args =>
      ...
      | Ret (TLambda _ _ bod) =>      (* beta redex *)
        ...
        | Exc s =>
          raise ("wcbvEval TApp: arg doesn't eval: " ++ print_term topt ++ s)
      ...
      | Ret ((TInd _) as tc) =>            (* applied inductive *)
          match wcbvEvals n (tcons a1 args) with
          | Exc s =>
            raise ("wcbvEval TApp:args:" ++ print_term topt
                                         ++ ":" ++ s)
    ...
    | TWrong str => raise ("wcbvEval: TWrong: " ++ str)
    end
  end
