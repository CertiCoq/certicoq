...
(* $Id: pretyping.ml 9961 2007-07-07 14:53:20Z letouzey $ *)
...
sig
  ...
  *)
   ...
   *)
  val pretype : 
    impl -> type_constraint -> env -> evar_defs ref -> 
    var_map * (identifier * identifier option) list ->
    rawconstr -> unsafe_judgment
    
  val pretype_type : 
    impl -> val_constraint -> env -> evar_defs ref ->
    var_map * (identifier * identifier option) list ->
  ...
  val pretype_gen :
    evar_defs ref -> env -> 
    var_map * (identifier * identifier option) list ->
    typing_constraint -> impl -> rawconstr -> constr
...
module Pretyping_F (Coercion : Coercion.S) = struct
  ...
  *)
  ...
  let pretype_id loc env (lvar,unbndltacvars) id =
	...
	with Not_found ->
	  try
	    let (_,_,_,typ) = lookup_named id env in
  ...
  let make_dep_of_undep env (IndType (indf,realargs,raimps)) pj =
    let n = List.length realargs in
    let rec decomp p = function
        [] -> p
      | imp::imps ->
	  (match kind_of_term p with
	    | Lambda (_,_,_,c) -> decomp c imps
	    | _ -> decomp (applist (lift 1 p, [mkRel 1],[imp])) imps)
    in
    let sign,s = decompose_prod_n n pj.uj_type in
    let ind = build_dependent_inductive env indf in
    let s' = mkProd (Anonymous, Expl, ind, s) in
    let ccl = lift 1 (decomp pj.uj_val (List.rev raimps)) in
    let ccl' = mkLambda (Anonymous, Expl,ind, ccl) in
  ...
  let rec pretype imp (tycon : type_constraint) env isevars lvar = function
    | RRef (loc,ref) ->
	...
	let ty =
          match tycon with 
            | Some (None, ty) -> ty
            | None | Some _ ->
		e_new_evar isevars env
                  ~src:(loc,InternalHole) Impl (new_Type ()) in
	{ uj_val = e_new_evar isevars env ~src:(loc,k) imp ty; uj_type = ty }

    | RRec (loc,fixkind,names,bl,lar,vdef) ->
	let rec type_bl env ctxt = function
            [] -> ctxt
          | (na,imp,None,ty)::bl ->
              let ty' = pretype_type Impl empty_valcon env isevars lvar ty in
              let dcl = (na,imp,None,ty'.utj_val) in
		type_bl (push_rel dcl env) (add_rel_decl dcl ctxt) bl
          | (na,imp,Some bd,ty)::bl ->
              let ty' = pretype_type Impl empty_valcon env isevars lvar ty in
              let bd' =
                pretype imp (mk_tycon ty'.utj_val) env isevars lvar ty in
              let dcl = (na,imp,Some bd'.uj_val,ty'.utj_val) in
	...
	let larj =
          array_map2
            (fun e ar ->
               pretype_type Impl
                 empty_valcon (push_rel_context e env) isevars lvar ar)
            ctxtv lar in
	let lara = Array.map (fun a -> a.utj_val) larj in
	let ftys =
          array_map2
            (fun e a -> it_mkProd_or_LetIn a e) ctxtv lara in
	  ...
	  array_map2_i 
		...
		* the nbfix variables pushed to newenv *)
		 ...
		 decompose_prod_n_assum (rel_context_length ctxt)
               ...
               let j = pretype imp (mk_tycon ty) nenv isevars lvar def in
    ...
    | RApp (loc,f,args,imps) -> 
	let fj = pretype imp empty_tycon env isevars lvar f in
	...
	let rec apply_rec env n resj = function
	  ...
	  | c::rest ->
		  ...
		  | Prod (na,imp',c1,c2) ->
                      let imp = impl_merge[imp;imp'] in
		      let hj = pretype imp (mk_tycon c1) env isevars lvar c in
		      let value, typ =
                        applist (j_val resj, [j_val hj],[imp']),
			...
			rest
		  | _ ->
		      let hj = pretype Impl empty_tycon env isevars lvar c in
	...
	let resj =
	  ...
	  | App (f,args,imps) ->
              ...
              | Ind _ (* | Const _ *) ->
	          let sigma = evars_of !isevars in
		  let c = mkApp (f,Array.map (whd_evar sigma) args,imps) in
    ...
    | RLambda(loc,name,imp,c1,c2)      ->
	let (name',_,dom,rng) =
          evd_comb1 (split_tycon loc env) isevars tycon in
	let dom_valcon = valcon_of_tycon dom in
	let j = pretype_type Impl dom_valcon env isevars lvar c1 in
	let var = (name,imp,None,j.utj_val) in
	let j' = pretype imp rng (push_rel var env) isevars lvar c2 in 
	  judge_of_abstraction env (orelse_name name name') imp j j'

    | RProd(loc,name,imp,c1,c2)        ->
	let j = pretype_type imp empty_valcon env isevars lvar c1 in
	let var = (name,Expl,j.utj_val) in
	let env' = push_rel_assum var env in
	let j' = pretype_type imp empty_valcon env' isevars lvar c2 in
	let resj =
	  try judge_of_product env name imp j j'
	  with TypeError _ as e -> Stdpp.raise_with_loc loc e in
	inh_conv_coerce_to_tycon loc env isevars resj tycon
	    
    | RLetIn(loc,name,imp',c1,c2)      ->
	let j = pretype imp' empty_tycon env isevars lvar c1 in
	let t = refresh_universes j.uj_type in
	let var = (name,imp',Some j.uj_val,t) in
        let tycon = lift_tycon 1 tycon in
	let j' = pretype imp tycon (push_rel var env) isevars lvar c2 in
	  { uj_val = mkLetIn (name, imp', j.uj_val, t, j'.uj_val) ;
	    uj_type = subst1 j.uj_val j'.uj_type }

    | RLetTuple (loc,nal,(na,po),c,d) ->
	let cj = pretype imp empty_tycon env isevars lvar c in
	let (IndType (indf,realargs,raimps)) = 
	...
	let cstrs = get_constructors env indf in
	  ...
	  let cs = cstrs.(0) in
	    ...
	    let fsign = List.map2
              (fun (na,imp1) (_,imp2,c,t) ->
                if imp1<>imp2 then user_err_loc(loc,"",str"bad implicits");
                (na,imp2,c,t))
		...
		if not !allow_anonymous_refs then
		  List.map (fun (_,imp,b,t) -> (Anonymous,imp,b,t)) arsgn
		else arsgn
	    in
	    let psign =
              (na,Expl,None,build_dependent_inductive env indf)::arsgn in
	    let nar = List.length arsgn in
	      (match po with
		 | Some p ->
		     let env_p = push_rels psign env in
		     let pj =
                       pretype_type Impl empty_valcon env_p isevars lvar p in
		     let ccl = nf_isevar !isevars pj.utj_val in
		     let psign =
                       make_arity_signature env true indf in (* with names *)
		     ...
		     let fj =
                       pretype imp (mk_tycon fty) env_f isevars lvar d in
		     let f = it_mkLambda_or_LetIn fj.uj_val fsign in
		     let v =
		       let mis,_,_ = dest_ind_family indf in
		 ...
		 | None -> 
		     ...
		     let fj = pretype imp tycon env_f isevars lvar d in
		     let f = it_mkLambda_or_LetIn fj.uj_val fsign in
			 ...
			 error_cant_find_case_type_loc loc env
                           (evars_of !isevars) cj.uj_val in
		     let ccl = refresh_universes ccl in
		     let p =
                       it_mkLambda_or_LetIn (lift (nar+1) ccl) psign in
		     let v =
		       let mis,_,_ = dest_ind_family indf in
		       let ci = make_case_info env mis LetStyle in
	...
	let cstrs = get_constructors env indf in 
	  ...
	  let pred,p = match po with
	    | Some p ->
		let env_p = push_rels psign env in
		let pj = pretype_type Impl empty_valcon env_p isevars lvar p in
		let ccl = nf_evar (evars_of !isevars) pj.utj_val in
		let pred = it_mkLambda_or_LetIn ccl psign in
		let typ = lift (- nar)
                  (beta_applist (pred,[cj.uj_val],[Expl])) in
		let jtyp = inh_conv_coerce_to_tycon loc env isevars
                  {uj_val = pred;
		   uj_type = typ} tycon 
		in
		...
		let p = match tycon with
		  | Some (None, ty) -> ty
		  | None | Some _ ->
                      e_new_evar isevars env ~src:(loc,InternalHole)
                        Impl (new_Type ()) in
	 ...
	 (*   msgnl (str "Pred is: " ++ Termops.print_constr_env env pred);*)
	  let f cs b =
	    ...
	    let pi =
              beta_applist (pi, [build_dependent_constructor cs],[Expl]) in
	    let csgn = 
	      if not !allow_anonymous_refs then
		List.map (fun (_,imp,b,t) -> (Anonymous,imp,b,t)) cs.cs_args 
	      else 
		List.map 
		  (fun (n, imp, b, t) ->
		     match n with
                         Name _ -> (n, imp, b, t)
                       | Anonymous -> (Name (id_of_string "H"), imp, b, t))
		cs.cs_args
	    in
	    let env_c = push_rels csgn env in 
(* 	      msgnl (str "Pi is: " ++ Termops.print_constr_env env_c pi); *)
	    let bj = pretype imp (mk_tycon pi) env_c isevars lvar b in
	...
	Cases.compile_cases loc
	  ((fun imp vtyc env -> pretype imp vtyc env isevars lvar),isevars)
	  imp tycon env (* loc *) (po,tml,eqns)
	...
	let cj =
	  match k with
	    ...
	    | CastConv (k,t) ->
		let tj = pretype_type Impl empty_valcon env isevars lvar t in
		let cj =
                  pretype imp (mk_tycon tj.utj_val) env isevars lvar c in
  ...
  and pretype_type imp valcon env isevars lvar = function
    | RHole loc ->
	(match valcon with
	   | Some v ->
               let s =
	   ...
	   | None ->
		 ...
		 { utj_val = e_new_evar isevars env ~src:loc imp (mkSort s);
		   utj_type = s})
    | c ->
	let j = pretype imp empty_tycon env isevars lvar c in
  ...
  let pretype_gen isevars env lvar kind imp c =
    let c' = match kind with
      | OfType exptyp ->
	  let tycon = match exptyp with None -> empty_tycon | Some t -> mk_tycon t in
	    (pretype imp tycon env isevars lvar c).uj_val
      | IsType ->
	  (pretype_type imp empty_valcon env isevars lvar c).utj_val in
  ...
  *)
  ...
  let understand_judgment sigma env c =
    ...
    let j = pretype Expl empty_tycon env isevars ([],[]) c in
  ...
  let ise_pretype_gen fail_evar sigma env lvar kind imp c =
    let isevars = ref (Evd.create_evar_defs sigma) in
    let c = pretype_gen isevars env lvar kind imp c in
    let isevars,_ = consider_remaining_unif_problems env !isevars in
  ...
  let understand_gen kind sigma env c =
    snd (ise_pretype_gen true sigma env ([],[]) kind Expl c)

  let understand sigma env ?expected_type:exptyp c =
    snd (ise_pretype_gen true sigma env ([],[]) (OfType exptyp) Expl c)

  let understand_type sigma env c =
    snd (ise_pretype_gen true sigma env ([],[]) IsType Expl c)

  let understand_ltac sigma env lvar kind c =
    ise_pretype_gen false sigma env lvar kind Expl c
      
  let understand_tcc_evars isevars env kind c =
    pretype_gen isevars env ([],[]) kind Expl c

  let understand_tcc sigma env ?expected_type:exptyp c =
    let ev, t =
      ise_pretype_gen false sigma env ([],[]) (OfType exptyp) Expl c in
      Evd.evars_of ev, t
end
