...
(************************************************************************)
(* $Id: subtac_coercion.ml 9973 2007-07-12 11:04:41Z msozeau $ *)
...
(*        with _ -> ()); *)
    ...
    and coerce' env x y : (Term.constr -> Term.constr) option =
      ...
      let rec coerce_application typ c c' l limps l' =
	let len = Array.length l in
	let rec aux tele typ i co = 
	  if i < len then
	    let hdx = l.(i) and hdy = l'.(i) in
	      try isevars := the_conv_x_leq env hdx hdy !isevars;
		let (n, imp, eqT, restT) = destProd typ in
		aux ((hdx,imp) :: tele) (subst1 hdy restT) (succ i) co
	      with Reduction.NotConvertible ->
		let (n, imp,eqT, restT) = destProd typ in
		let restargs = lift_args 1 
		  (List.rev (Array.to_list (Array.sub l (succ i) (len - (succ i)))))
		in 
                let restimps = List.rev (Array.to_list (Array.sub limps (succ i) (len - (succ i)))) in 
		let args = List.rev (restargs @ mkRel 1 :: lift_args 1
                  (List.map fst tele)) in
                let imps = List.rev (restimps @ Expl :: List.map snd tele) in
		let pred =
                  mkLambda (n, Expl, eqT, applistc (lift 1 c) args imps) in
		let eq =
                  mkApp(Lazy.force eq_ind,
                        [| eqT; hdx; hdy |],[|Expl;Expl;Expl|]) in
(* 		let jmeq = mkApp (Lazy.force jmeq_ind, [| eqT; hdx; eqT; hdy |]) in *)
		let evar = make_existential dummy_loc env isevars Expl eq in
		let eq_app x = mkApp (Lazy.force eq_rect,
				      [| eqT; hdx; pred; x; hdy; evar|],
                                      [|Impl;Impl;Impl;Expl;Impl;Expl|]) in
		  trace (str"Inserting coercion at application");
		  aux ((hdy,imp) :: tele) (subst1 hdy restT) (succ i)  (fun x -> eq_app (co x))
	  else co
		   ...
		   | _, _ ->
			 ...
			 (fun f -> 
			    mkLambda (name', imp', a',
				      app_opt c2
					(mkApp (Term.lift 1 f, 
						[| app_opt c1 (mkRel 1) |],[|imp'|])))))
		  
	  | App (c, l,imps), App (c', l',imps') ->
			 ...
			 then
			   begin 
			     ...
			     let rec remove_head a c = 
			       match kind_of_term c with 
				 | Lambda (n, imp,t, t') -> c, t'
				     (*| Prod (n, t, t') -> t'*)
				 | Evar (k, args) ->
				     let (evs, t) =
                                       Evarutil.define_evar_as_lambda !isevars Expl (k,args) in
				       isevars := evs;
				       let (n, imp, dom, rng) = destLambda t in
			     ...
			     let env' = push_rel (make_name "x", Expl, None, a) env in
			     let c2 = coerce_unify env' b b' in
			       match c1, c2 with
				   None, None -> 
				     None
				     ...
				     Some 
					  ...
					  let x, y = 
					    app_opt c1 (mkApp (existS.proj1,
							       [| a; pb; x |],[|Impl;Impl;Expl|])),
					    app_opt c2 (mkApp (existS.proj2, 
							       [| a; pb; x |],[|Impl;Impl;Expl|]))
					  in
					    mkApp (existS.intro, [| a'; pb'; x ; y |],[|Impl;Impl;Expl;Expl|]))
			   end
			 else 
			   begin 
			     let (a, b), (a', b') = 
				     ...
				     Some 
					  ...
					  let x, y = 
					    app_opt c1 (mkApp (prod.proj1,
							       [| a; b; x |],[|Impl;Impl;Expl|])),
					    app_opt c2 (mkApp (prod.proj2, 
							       [| a; b; x |],[|Impl;Impl;Expl|]))
					  in
					    mkApp (prod.intro, [| a'; b'; x ; y |],[|Impl;Impl;Expl;Expl|]))
			   end
		       else
...
(* 			     if not (is_arity env evm typ) then *)
			       Some (coerce_application typ c c' l imps l'))
(* 			     else subco () *)
			 else
			   subco ()
		 | x, y when x = y ->
...
(* 			 if not (is_arity env evm lam_type) then ( *)
			   Some (coerce_application lam_type c c' l imps l')
(* 			 ) else subco () *)
		     else subco ()
		 | _ -> subco ())
	  | _, _ ->  subco ()
		...
		Some (u, p) ->
		  let c = coerce_unify env x u in
		    Some 
			 ...
			 let evar = make_existential dummy_loc env
                           isevars Impl (mkApp (p, [| cx |],[|Expl|]))
			 in
			   (mkApp 
			      ((Lazy.force sig_).intro, 
			       [| u; p; cx; evar |],[|Impl;Impl;Expl;Impl|])))
	      | None -> 
  ...
  let apply_coercion_args env argl funj =
    let rec apply_rec acc accimps typ = function
      | [] -> { uj_val = applist (j_val funj,argl,List.rev accimps);
		uj_type = typ }
      | h::restl ->
	  (* On devrait pouvoir s'arranger pour qu'on n'ait pas à faire hnf_constr *)
		...
		apply_rec (h::acc) (imp::accimps) (subst1 h c2) restl
