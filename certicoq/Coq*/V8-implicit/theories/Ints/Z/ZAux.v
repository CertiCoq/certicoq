...
intros p Hp Hp1.
case (prime_dec_aux p p); intros H1; apply impl_PI.
...
intros x1 ((H2, H3), H4); case (prime_dec x1); intros H5; apply impl_PI.
...
apply Zmult_le_compat_r; auto with zarith.
case (prime_dec x2); intros H7; apply impl_PI.
...
case (prime_dec p); intros H3; apply impl_PI.
...
intros q Rec H2.
case (Zdivide_dec p q); intros H3; apply impl_PI.
...
assert (Hp0: 0 < p).
apply Zlt_le_trans with 2; auto with zarith; apply prime_le_2; auto.
case (Zdivide_dec p k); intros H1; apply impl_PI.
...
case (Z_eq_dec p p1); intros Hpp1; apply impl_PI; subst.
...
intros m Rec Hm H2.
case (prime_dec m); intros Hm1; apply impl_PI.
...
case (prime_dec x); intros H2; apply impl_PI.
