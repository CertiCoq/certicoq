...
(* $Id: tacinterp.ml 10021 2007-07-18 15:01:02Z herbelin $ *)
...
let make_hyps = List.map (fun (id,imp,_,typ) -> (id,typ))
...
let rec intern_atomic lf ist x =
  ...
  | TacCut (imp,c) -> TacCut (imp,intern_type ist c)
  | TacAssert (otac,ipat,c) ->
      TacAssert (option_map (intern_tactic ist) otac,
                 intern_intro_pattern lf ist ipat,
                 intern_constr_gen (otac<>None) ist c)
  | TacGeneralize (imps,cl) ->
      TacGeneralize (imps,List.map (intern_constr ist) cl)
  | TacGeneralizeDep (imp,c) -> TacGeneralizeDep (imp,intern_constr ist c)
  | TacLetTac (na,imp,c,cls) ->
      let na = intern_name lf ist na in
      TacLetTac (na,imp,intern_constr ist c,
  ...
  | TacLApply (imp,c) -> TacLApply (imp,intern_constr ist c)
...
let pf_interp_imp_constr_list_as_list ist gl imp (c,_ as x) =
  match c with
    | RVar (_,id) ->
        (try
          List.map (fun c -> (imp,c))
            (constr_list_of_VList (pf_env gl) (List.assoc id ist.lfun))
        with Not_found -> [imp,interp_constr ist (project gl) (pf_env gl) x])
    | _ -> [imp, interp_constr ist (project gl) (pf_env gl) x]

let pf_interp_imp_constr_list ist gl imps l =
  List.split
    (List.flatten
      (List.map2 (pf_interp_imp_constr_list_as_list ist gl) imps l))
...
and interp_atomic ist gl = function
  ...
  | TacCut (imp,c) -> h_cut imp (pf_interp_type ist gl c)
  | TacAssert (t,ipat,c) ->
  ...
  | TacGeneralize (imps,cl) ->
      let (imps,cl) = pf_interp_imp_constr_list ist gl imps cl in
      h_generalize imps cl
  | TacGeneralizeDep (imp,c) ->
      h_generalize_dep imp (pf_interp_constr ist gl c)
  | TacLetTac (na,imp,c,clp) ->
      let clp = interp_clause ist gl clp in
      h_let_tac (interp_fresh_name ist gl na) imp (pf_interp_constr ist gl c) clp
  ...
  | TacLApply (imp,c) -> h_lapply imp (pf_interp_constr ist gl c)
...
let rec subst_atomic subst (t:glob_atomic_tactic_expr) = match t with
  ...
  | TacCut (imp,c) -> TacCut (imp,subst_rawconstr subst c)
  | TacAssert (b,na,c) -> TacAssert (b,na,subst_rawconstr subst c)
  | TacGeneralize (imps,cl) ->
      TacGeneralize (imps,List.map (subst_rawconstr subst) cl)
  | TacGeneralizeDep (imp,c) ->
      TacGeneralizeDep (imp,subst_rawconstr subst c)
  | TacLetTac (id,imp,c,clp) -> TacLetTac (id,imp,subst_rawconstr subst c,clp)
  ...
  | TacLApply (imp,c) -> TacLApply (imp,subst_rawconstr subst c)
